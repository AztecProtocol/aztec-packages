...
export PREFUNDED_MNEMONIC_INDICES=$(echo "$DEFAULT_ACCOUNTS_INDICES $VALIDATOR_KEY_INDICES $EXTRA_ACCOUNTS_INDICES $PROVER_KEY_INDICES $BOT_KEY_INDICES $SLASHER_KEY_INDICES" | tr ' ' '\n' | grep -v '^$' | sort -u | tr '\n' ',' | sed 's/,$//')
