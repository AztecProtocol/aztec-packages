...
export NUMBER_OF_SLASHER_KEYS=$VALIDATOR_REPLICAS
export SLASHER_KEY_START_INDEX=$(./read_value.sh "aztec.slasherKeyIndexStart" $value_yamls)

export PUBLISHERS_PER_VALIDATOR_KEY=$(./read_value.sh "validator.publishersPerValidatorKey" $value_yamls)
export NUMBER_OF_VALIDATOR_PUBLISHER_KEYS=$((NUMBER_OF_VALIDATOR_KEYS * PUBLISHERS_PER_VALIDATOR_KEY))
export VALIDATOR_PUBLISHER_KEY_START_INDEX=$(./read_value.sh "aztec.validatorPublisherIndexStart" $value_yamls)

export PUBLISHERS_PER_PROVER_KEY=$(./read_value.sh "proverNode.publishersPerProver" $value_yamls)
export NUMBER_OF_PROVER_PUBLISHER_KEYS=$((NUMBER_OF_PROVER_KEYS * PUBLISHERS_PER_PROVER_KEY))
export PROVER_PUBLISHER_KEY_START_INDEX=$(./read_value.sh "aztec.proverPublisherIndexStart" $value_yamls)
...
SLASHER_KEY_INDICES=$(seq $SLASHER_KEY_START_INDEX $((SLASHER_KEY_START_INDEX + NUMBER_OF_SLASHER_KEYS - 1)))
VALIDATOR_PUBLISHER_KEY_INDICES=$(seq $VALIDATOR_PUBLISHER_KEY_START_INDEX $((VALIDATOR_PUBLISHER_KEY_START_INDEX + NUMBER_OF_VALIDATOR_PUBLISHER_KEYS - 1)))
PROVER_PUBLISHER_KEY_INDICES=$(seq $PROVER_PUBLISHER_KEY_START_INDEX $((PROVER_PUBLISHER_KEY_START_INDEX + NUMBER_OF_PROVER_PUBLISHER_KEYS - 1)))

# Combine all indices, filter out empty items, sort, and format
export PREFUNDED_MNEMONIC_INDICES=$(echo "$DEFAULT_ACCOUNTS_INDICES $VALIDATOR_KEY_INDICES $EXTRA_ACCOUNTS_INDICES $PROVER_KEY_INDICES $BOT_KEY_INDICES $SLASHER_KEY_INDICES $VALIDATOR_PUBLISHER_KEY_INDICES $PROVER_PUBLISHER_KEY_INDICES" | tr ' ' '\n' | grep -v '^$' | sort -u | tr '\n' ',' | sed 's/,$//')
...
echo "PUBLISHERS_PER_PROVER_KEY: $PUBLISHERS_PER_PROVER_KEY"
echo "TOTAL_VALIDATOR_KEYS: $NUMBER_OF_VALIDATOR_KEYS"
echo "TOTAL_SLASHER_KEYS: $NUMBER_OF_SLASHER_KEYS"
