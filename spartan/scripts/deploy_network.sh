...
if [[ "$PROVER_ENABLED" == "true" && "$PROVER_AGENT_KEDA_ENABLED" == "true" && -z "$PROVER_AGENT_KEDA_PROMETHEUS_SERVER_ADDRESS" ]]; then
  die "PROVER_AGENT_KEDA_ENABLED=true requires PROVER_AGENT_KEDA_PROMETHEUS_SERVER_ADDRESS. Use network_deploy.sh to resolve it from deploy-telemetry, or set it explicitly."
fi
...
RPC_INGRESS_LOG_SAMPLE_RATE=${RPC_INGRESS_LOG_SAMPLE_RATE:-null}
