...
VALIDATOR_PRIMARY_REPLICA_COUNT=${VALIDATOR_PRIMARY_REPLICA_COUNT:-}
...
EFFECTIVE_PRIMARY_COUNT=${VALIDATOR_PRIMARY_REPLICA_COUNT:-$VALIDATOR_REPLICAS}
EFFECTIVE_HA_COUNT=${VALIDATOR_HA_REPLICA_COUNT:-$VALIDATOR_REPLICAS}
if [[ $VALIDATOR_HA_REPLICAS -gt 0 ]]; then
  MAX_VALIDATOR_NODES=$(( EFFECTIVE_PRIMARY_COUNT > EFFECTIVE_HA_COUNT ? EFFECTIVE_PRIMARY_COUNT : EFFECTIVE_HA_COUNT ))
else
  MAX_VALIDATOR_NODES=$EFFECTIVE_PRIMARY_COUNT
fi
...
# Primary has EFFECTIVE_PRIMARY_COUNT pods, each HA release has EFFECTIVE_HA_COUNT pods
if [[ $VALIDATOR_HA_REPLICAS -gt 0 ]]; then
  TOTAL_VALIDATOR_PUBLISHERS=$(( (EFFECTIVE_PRIMARY_COUNT + VALIDATOR_HA_REPLICAS * EFFECTIVE_HA_COUNT) * VALIDATOR_PUBLISHERS_PER_REPLICA ))
else
  TOTAL_VALIDATOR_PUBLISHERS=$(( EFFECTIVE_PRIMARY_COUNT * VALIDATOR_PUBLISHERS_PER_REPLICA ))
fi
...
VALIDATOR_PRIMARY_REPLICA_COUNT = ${VALIDATOR_PRIMARY_REPLICA_COUNT:-null}
