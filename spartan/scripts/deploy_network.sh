...
# All infra defaults (CLUSTER, GCP_*, ETHEREUM_*, LABS_INFRA_*, VALIDATOR_*,
# PROVER_*, BOT_*, etc.) live in common.env — sourced by every .env file.
...
########################
# COMPUTED VALUES & TERRAFORM SENTINELS
########################
# Terraform-specific `null` sentinels (kept here, not in common.env, because
# the literal string "null" would leak into generated TS presets via codegen)
...
# Snapshot/blob/tx file store URLs need conditional null sentinel for tfvars
...
AZTEC_SLASHING_VETOER = $(tf_str "${AZTEC_SLASHING_VETOER:-}")
