  ...
  if [[ -n "${env_file:-}" ]]; then
    if [[ -f "$env_file" ]]; then
      ...
      if grep -q "REPLACE_WITH_GCP_SECRET" "$env_file" && command -v gcloud &> /dev/null; then
        ...
        # Ensure we have a docker image specified
        if [[ -z "${AZTEC_DOCKER_IMAGE:-}" ]]; then
          echo "Error: AZTEC_DOCKER_IMAGE must be set when using GCP secrets" >&2
          echo "Set it with: export AZTEC_DOCKER_IMAGE=aztecprotocol/aztec:X.Y.Z" >&2
          exit 1
        fi
        ...
        # Process GCP secrets
        ./scripts/setup_gcp_secrets.sh "$temp_env_file" "$AZTEC_DOCKER_IMAGE"

        # Source the processed file
        set -a
        # shellcheck disable=SC1090
        cp "$temp_env_file" ~/processed.env
