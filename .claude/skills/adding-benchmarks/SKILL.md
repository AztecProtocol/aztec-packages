...
Example:
- File location: `yarn-project/stdlib/bench-out/tx.bench.json`
- JSON content: `{"name": "Tx/private/getTxHash/avg", ...}`
- **Final name**: `yarn-project/stdlib/Tx/private/getTxHash/avg`
...
- PR has label: `ci-merge-queue`, `ci-full`, or `ci-full-no-test-cache`
- Running on merge queue
...
| Problem | Check |
|---------|-------|
| Not in dashboard | Hasn't run in last 10 commits? Missing CI label? |
| Not in merged JSON | File ends with `.bench.json`? In `bench-out/` dir? |
| Invalid format | Is it an array `[...]`? Is `value` numeric? |
| Disappeared | `bench_cmds` entry removed? Test disabled? |
