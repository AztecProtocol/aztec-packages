...
5. Repin the standard contracts: `noir-projects/noir-contracts/bootstrap.sh pin-standard-build`, then commit the regenerated `noir-projects/noir-contracts/pinned-standard-contracts.tar.gz`. `noir-contracts/bootstrap.sh build` extracts this archive and excludes `standard/` contracts from recompilation, so their precomputed VKs (e.g. `MultiCallEntrypoint`) do NOT refresh on a bb change unless this pin is rebuilt — a stale one surfaces as a `MultiCallEntrypoint:entrypoint` "Computed VK differs from precomputed VK" mismatch during chonk capture verification. `noir-contracts/bootstrap.sh` defaults `BB` to the non-AVM `bb`, so set it explicitly: `BB=$(realpath ../../barretenberg/cpp/build/bin/bb-avm) ./bootstrap.sh pin-standard-build`.
6. Recapture and upload Chonk flows with `barretenberg/cpp/scripts/chonk_inputs.sh update`.
