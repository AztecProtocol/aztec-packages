...
ipc_server_t *ipc_server_create_mpsc_shm(const char *base_name,
  ...
  return wrap_server(ipc::IpcServer::create_mpsc_shm(
      base_name, max_clients, request_ring_size, response_ring_size));
}

void ipc_server_destroy(ipc_server_t *server) { delete server; }
...
bool ipc_server_send(ipc_server_t *server, int client_id, const uint8_t *data,
                     size_t len) {
  return server && server->impl ? server->impl->send(client_id, data, len)
                                : false;
}
...
void ipc_server_run(ipc_server_t *server, ipc_server_handler_fn handler,
  ...
  server->impl->run(
      [handler, ctx](int client_id,
                     std::span<const uint8_t> raw) -> std::vector<uint8_t> {
...
void ipc_client_destroy(ipc_client_t *client) { delete client; }
...
bool ipc_client_send(ipc_client_t *client, const uint8_t *data, size_t len,
                     uint64_t timeout_ns) {
  return client && client->impl ? client->impl->send(data, len, timeout_ns)
                                : false;
}
