  ...
  virtual void run(const Handler &handler) {
    while (!shutdown_requested_.load(std::memory_order_acquire)) {
      ...
      try {
        ...
        release(client_id, request.size());
      } catch (const ShutdownRequested &shutdown_req) {
        // Release message before shutting down. Catch variable is
        // shutdown_req (not just shutdown) to avoid shadowing the
        // POSIX shutdown(2) declaration under wasi-sdk, which would
        // make `shutdown.response()` ambiguous to parse.
        release(client_id, request.size());

        // Send final response before shutting down
        if (!shutdown_req.response().empty()) {
          send(client_id, shutdown_req.response().data(),
               shutdown_req.response().size());
        }
