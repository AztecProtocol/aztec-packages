 ...
 * @brief Tagged-union with msgpack [name, payload] wire format.
 *
 * Each type in the union must declare:
 *   static constexpr const char MSGPACK_SCHEMA_NAME[] = "...";
 *
 * This is the standalone, namespace-`ipc::` version intended for use by
 * codegen-emitted reflection in services that don't link barretenberg.
 * Barretenberg's own bb::NamedUnion (common/named_union.hpp) is a sibling
 * implementation kept for in-tree consumers; the two are wire-compatible
 * (both serialise as [type_name, payload]) but live in distinct namespaces.
 */
#include <concepts>
#include <cstdlib>
#include <msgpack.hpp>
...
// Local THROW shim: under -fno-exceptions (WASM), `throw` is a compile error
// and unwinding wouldn't work anyway. Substitute abort() so the same source
// compiles for both targets. ipc-runtime is standalone; we don't pull in
// barretenberg/common's try_catch_shim.
#ifdef BB_NO_EXCEPTIONS
#define IPC_RUNTIME_THROW(MsgType, msg)                                        \
  do {                                                                         \
    std::abort();                                                              \
  } while (0)
#else
#define IPC_RUNTIME_THROW(MsgType, msg) throw MsgType(msg)
#endif
#include <type_traits>
#include <variant>
...
private:
  ...
  static VariantType construct_by_index(size_t index, auto &o) {
    ...
    IPC_RUNTIME_THROW(std::runtime_error,
                      "ipc::NamedUnion: invalid variant index");
  }
...
public:
  ...
  void msgpack_unpack(msgpack::object const &o) {
    if (o.type != msgpack::type::ARRAY || o.via.array.size != 2) {
      IPC_RUNTIME_THROW(std::runtime_error,
                        "ipc::NamedUnion: expected array of size 2");
    }
    ...
    if (!index_opt.has_value()) {
      IPC_RUNTIME_THROW(std::runtime_error,
                        "ipc::NamedUnion: unknown type name " +
