...
TEST(ShmTest, SingleClientSmallRingHighVolume) {
  ...
  std::thread server_thread([&]() {
    ...
    while (server_running.load(std::memory_order_acquire)) {
      ...
      if (request.size() > 0) {
        ...
        for (size_t i = 0; i < std::min(request.size(), size_t(16)); i++) {
          ...
          if (request[i] != expected) {
            ...
            std::cerr << "Pattern mismatch at offset " << i
      ...
      while (!server->send(client_id, request.data(), request.size())) {
        ...
        std::cerr << iter << " Server send size " << request.size()
                  << " timeout, retrying..." << '\n';
  ...
  std::thread sender_thread([&]() {
    ...
    for (size_t iter = 0; iter < NUM_ITERATIONS; iter++) {
      ...
      while (!client->send(send_buffer.data(), size, 100000000)) {
        ...
        std::cerr << iter << " Client send size " << size
                  << " timeout, retrying..." << '\n';
  ...
  std::thread receiver_thread([&]() {
    for (size_t iter = 0; iter < NUM_ITERATIONS; iter++) {
      ...
      uint8_t iter_byte = static_cast<uint8_t>(iter & 0xFF);
      if (response.size() > 0) {
        ASSERT_EQ(response[0], iter_byte)
...
/**
 * Timeouts ≥ ~4.295s used to be truncated uint64→uint32 at the ring API and
 * silently wrapped (e.g. 4.5s → ~205ms). Verify a 4.5s wait_for_data survives
 * past the wrap point and sees data published at the 2s mark.
 */
TEST(ShmTest, TimeoutDoesNotWrapAbove4Seconds) {
  ...
  int client_id = server->wait_for_data(
      4'500'000'000ULL); // 4.5s — would wrap to ~205ms as uint32
  auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::steady_clock::now() - start);

  EXPECT_EQ(client_id, 0)
      << "wait_for_data should see the data sent at the 2s mark";
  EXPECT_GE(elapsed.count(), 1500)
      << "returned before the 2s publish — timeout wrapped";
...
TEST(ShmTest, RingRejectsCorruptLengthPrefix) {
  ...
  EXPECT_THROW((void)ring_receive_msg(consumer, 10'000'000ULL),
               std::runtime_error);
...
TEST(ShmTest, MpscEchoTwoClients) {
  ...
  auto server =
      IpcServer::create_mpsc_shm(base_name, NUM_CLIENTS, RING_SIZE, RING_SIZE);
  ...
  auto run_client = [&](size_t client_id) {
    ...
    ASSERT_TRUE(client->connect())
        << "Client " << client_id << " failed to connect";
    ...
    for (size_t iter = 0; iter < NUM_MESSAGES; iter++) {
      ...
      ASSERT_EQ(response.size(), MSG_SIZE)
      ...
      ASSERT_EQ(response[0], static_cast<uint8_t>(client_id))
          << "client " << client_id << " got cross-client response at iter "
          << iter;
      for (size_t i = 1; i < MSG_SIZE; i++) {
        uint8_t expected = static_cast<uint8_t>((client_id ^ iter ^ i) & 0xFF);
        ASSERT_EQ(response[i], expected)
            << "client " << client_id << " iter " << iter << " offset " << i;
      }
      client->release(response.size());
    }
    client->close();
