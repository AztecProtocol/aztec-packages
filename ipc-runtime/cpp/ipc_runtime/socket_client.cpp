...
bool SocketClient::apply_timeout(int option, uint64_t& applied_ns, uint64_t timeout_ns)
{
...
int SocketClient::send_exact(const void* buf, size_t len, bool& partial)
{
    ...
    while (total_sent < len) {
        ssize_t n = ::send(fd_, static_cast<const uint8_t*>(buf) + total_sent, len - total_sent, 0);
        if (n < 0) {
            if (errno == EINTR) {
...
int SocketClient::recv_exact(void* buf, size_t len, bool& partial)
{
    ...
    while (total_read < len) {
        ssize_t n = ::recv(fd_, static_cast<uint8_t*>(buf) + total_read, len - total_read, 0);
        if (n < 0) {
            if (errno == EINTR) {
                continue; // Interrupted, retry
            }
