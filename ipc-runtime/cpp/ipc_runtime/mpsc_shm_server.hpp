...
/**
 * @brief IPC server implementation using shared memory with multi-client
 * support
 *
...
public:
  ...
  bool listen() override {
    ...
    try {
      ...
      request_consumer_ = MpscConsumer::create(
          base_name_ + "_req", max_clients_, request_ring_size_);
      ...
      for (size_t i = 0; i < max_clients_; i++) {
        ...
        response_rings_.push_back(
            SpscShm::create(resp_name, response_ring_size_));
      }
  ...
  std::span<const uint8_t> receive(int client_id) override {
    if (!request_consumer_.has_value() || client_id < 0 ||
        static_cast<size_t>(client_id) >= max_clients_) {
      return {};
    }
    // Peek on the specific client's request ring via MpscConsumer
    void *len_ptr = request_consumer_->peek(static_cast<size_t>(client_id),
    ...
    if (msg_len > MAX_FRAME_SIZE ||
        msg_len > request_ring_size_ / 2 - sizeof(uint32_t)) {
      throw std::runtime_error(
          "MpscShmServer::receive: corrupt length prefix (" +
          std::to_string(msg_len) + " bytes exceeds ring/frame limits)");
    }

    void *msg_ptr = request_consumer_->peek(
        static_cast<size_t>(client_id), sizeof(uint32_t) + msg_len, 100000000);
    if (msg_ptr == nullptr) {
      return {};
    }
    return std::span<const uint8_t>(
        static_cast<const uint8_t *>(msg_ptr) + sizeof(uint32_t), msg_len);
  }
  ...
  void release(int client_id, size_t message_size) override {
    ...
    request_consumer_->release(static_cast<size_t>(client_id),
                               sizeof(uint32_t) + message_size);
  }

  bool send(int client_id, const void *data, size_t len) override {
    if (client_id < 0 ||
        static_cast<size_t>(client_id) >= response_rings_.size()) {
      return false;
    }
    return ring_send_msg(response_rings_[static_cast<size_t>(client_id)], data,
                         len, 100000000);
  }
  ...
  CleanupPaths cleanup_paths() const override {
    ...
    for (size_t i = 0; i < max_clients_; i++) {
      paths.shm_unlink_names.push_back(base_name_ + "_req_ring_" +
                                       std::to_string(i));
      paths.shm_unlink_names.push_back(base_name_ + "_resp_" +
                                       std::to_string(i));
    }
    return paths;
  }
