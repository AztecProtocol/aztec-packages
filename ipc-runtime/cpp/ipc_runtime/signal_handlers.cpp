...
void cache_cleanup_paths(const IpcServer &server) {
  ...
  for (const auto &p : paths.unlink_paths) {
    if (g_num_unlink_paths >= MAX_CLEANUP_PATHS ||
        p.size() >= MAX_CLEANUP_PATH_LEN) {
      continue;
    }
    std::strncpy(g_unlink_paths[g_num_unlink_paths], p.c_str(),
                 MAX_CLEANUP_PATH_LEN - 1);
  ...
  for (const auto &p : paths.shm_unlink_names) {
    if (g_num_shm_unlink_names >= MAX_CLEANUP_PATHS ||
        p.size() >= MAX_CLEANUP_PATH_LEN) {
      continue;
    }
    std::strncpy(g_shm_unlink_names[g_num_shm_unlink_names], p.c_str(),
                 MAX_CLEANUP_PATH_LEN - 1);
...
void setup_parent_death_monitoring() {
  ...
  std::thread([parent_pid]() {
    ...
    if (auto *s = g_signal_server.load(std::memory_order_acquire);
        s != nullptr) {
      s->request_shutdown_from_signal();
    }
  }).detach();
