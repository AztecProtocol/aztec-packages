...
int MpscConsumer::wait_for_data(uint64_t timeout_ns) {
  ...
  // Block in bounded slices. The producer's doorbell wake is gated on
  // consumer_blocked and can be missed under a tight publish/block race;
  // once asleep, FUTEX_WAIT does not re-read seq, so an infinite wait that
  // misses a wake would hang forever. Capping each sleep makes the next
  // slice re-issue the wait (re-reading seq) and re-poll the rings, so a
  // missed wake self-heals within one slice. A real wake returns in the
  // first slice.
