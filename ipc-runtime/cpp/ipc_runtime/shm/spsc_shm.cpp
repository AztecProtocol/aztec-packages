...
bool SpscShm::wait_for_data(size_t need, uint64_t timeout_ns) {
  ...
  if (previous_had_data_) {
    ...
    // Preserve the infinite sentinel: subtracting from UINT64_MAX yields a
    // near-max value no longer recognized as infinite, which overflows the
    // slice-loop deadline below.
    remaining_timeout = (timeout_ns == TIMEOUT_INFINITE_NS)
                            ? TIMEOUT_INFINITE_NS
                            : (timeout_ns > SPIN_NS) ? (timeout_ns - SPIN_NS) : 0;
  } else {
  ...
  // Block in bounded slices. A producer's wake is gated on consumer_blocked
  // and can be missed under a tight publish/block race; once asleep,
  // FUTEX_WAIT does not re-read head, so an infinite wait that misses a wake
  // would hang forever. Capping each sleep makes the next slice re-issue the
  // wait (re-reading head) and re-check availability, so a missed wake
  // self-heals within one slice. A real wake returns within the first slice.
  const bool infinite = (remaining_timeout == TIMEOUT_INFINITE_NS);
  // Saturating add: a near-UINT64_MAX finite timeout must not overflow into
  // a past deadline (which would return immediately).
  uint64_t deadline = 0;
  if (!infinite) {
    uint64_t start_now = mono_ns_now();
    deadline = (remaining_timeout > UINT64_MAX - start_now)
                   ? UINT64_MAX
                   : start_now + remaining_timeout;
  }
  while (true) {
    uint64_t slice = MAX_FUTEX_SLICE_NS;
    if (!infinite) {
      uint64_t now = mono_ns_now();
      if (now >= deadline) {
        break;
      }
...
bool SpscShm::wait_for_space(size_t need, uint64_t timeout_ns) {
  ...
  // Block in bounded slices — see SpscShm::wait_for_data for why a missed
  // cross-process wake (gated on producer_blocked) requires the slice cap to
  // self-heal an otherwise-infinite wait.
  const bool infinite = (remaining_timeout == TIMEOUT_INFINITE_NS);
  // Saturating add: a near-UINT64_MAX finite timeout must not overflow into
  // a past deadline (which would return immediately).
  uint64_t deadline = 0;
  ...
  while (true) {
    ...
    ctrl_->producer_blocked.store(false, std::memory_order_relaxed);
    if (check_space()) {
      previous_had_space_ = true; // Found space after waking
      return true;
    }
  }
