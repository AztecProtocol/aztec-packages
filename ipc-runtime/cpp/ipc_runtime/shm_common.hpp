...
inline bool ring_send_msg(SpscShm &ring, const void *data, size_t len,
                          uint64_t timeout_ns) {
...
inline std::span<const uint8_t> ring_receive_msg(SpscShm &ring,
                                                 uint64_t timeout_ns) {
  ...
  if (msg_len > MAX_FRAME_SIZE || msg_len > ring.capacity() / 2 - 4) {
    throw std::runtime_error("ring_receive_msg: corrupt length prefix (" +
  ...
  return std::span<const uint8_t>(static_cast<const uint8_t *>(msg_ptr) + 4,
                                  msg_len);
}
