...
MsgpackClientAsync::~MsgpackClientAsync() { close_internal(); }

Napi::Value
MsgpackClientAsync::setResponseCallback(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
...
void MsgpackClientAsync::poll_responses() {
  ...
  while (!shutdown_.load(std::memory_order_acquire)) {
    ...
    auto status = tsfn_.NonBlockingCall(
        response_data, [](Napi::Env env, Napi::Function js_callback,
...
Napi::Value MsgpackClientAsync::call(const Napi::CallbackInfo &info) {
  ...
  if (!client_->send(input_data, input_len, 1)) {
    throw Napi::Error::New(
        env, "Failed to send request, ring buffer full. Make it bigger?");
  }
...
Napi::Value MsgpackClientAsync::acquire(const Napi::CallbackInfo &info) {
  ...
  if (ref_count_ == 0) {
    ...
    tsfn_ = Napi::ThreadSafeFunction::New(env, js_callback_.Value(),
...
Napi::Function MsgpackClientAsync::get_class(Napi::Env env) {
  return DefineClass(
      env, "MsgpackClientAsync",
      {
          MsgpackClientAsync::InstanceMethod(
              "setResponseCallback", &MsgpackClientAsync::setResponseCallback),
          MsgpackClientAsync::InstanceMethod("call", &MsgpackClientAsync::call),
          MsgpackClientAsync::InstanceMethod("acquire",
                                             &MsgpackClientAsync::acquire),
          MsgpackClientAsync::InstanceMethod("release",
                                             &MsgpackClientAsync::release),
          MsgpackClientAsync::InstanceMethod("close",
                                             &MsgpackClientAsync::close),
      });
}
