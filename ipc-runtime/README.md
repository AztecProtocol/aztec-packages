...
fire-and-forget commands. Two ways to exit the loop cleanly: call
`request_shutdown()` (signal-safe — `install_default_signal_handlers` wires
SIGINT/SIGTERM to it), or throw `ipc::ShutdownRequested(response_bytes)`
from inside the handler to flush a final response before exit. Either way,
RAII destructors run normally.
