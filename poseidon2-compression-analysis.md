...
## Appendix: Standalone Poseidon2 hash proving (Ultra vs Mega)

Isolated benchmark of a single Poseidon2 hash over a vector of `N` field elements, proven directly with `UltraProver` / `MegaProver` (no IVC, no client circuits). The goal is to measure how Mega's specialised `q_poseidon2_external` / `q_poseidon2_internal` selectors translate into wall-time savings as the dyadic domain grows. Bench code: `barretenberg/cpp/src/barretenberg/benchmark/ultra_bench/{ultra_honk,mega_honk}.bench.cpp::construct_proof_*_poseidon2_hash`. Remote AMD EPYC 7R13, `HARDWARE_CONCURRENCY=16`. **Prove-only** — circuit construction and VK creation are paused out of the timed region.

### Circuit sizes

| `N` (inputs) | Ultra gates | Ultra dyadic | Mega gates | Mega dyadic |
|-------------:|------------:|-------------:|-----------:|------------:|
| 1,500 | 38,016 | 2^16 | 17,524 | 2^15 |
| 3,000 | 76,016 | 2^17 | 35,024 | 2^16 |
| 6,000 | 152,016 | 2^18 | 70,024 | 2^17 |
| 12,000 | 304,016 | 2^19 | 140,024 | 2^18 |
| 50,000 | 1,266,707 | 2^21 | 583,368 | 2^20 |

Mega uses ~11.7 gates/input vs Ultra's ~25.3 → **~2.17× fewer gates per hash**, which lands Mega exactly one dyadic tier below Ultra at every input count.

### Total prove time

| `N` | Ultra (ms) | Mega (ms) | Mega speedup |
|----:|-----------:|----------:|:------------:|
| 1,500 | 302 | 202 | 1.50× |
| 3,000 | 498 | 316 | 1.58× |
| 6,000 | 883 | 523 | 1.69× |
| 12,000 | 1,607 | 933 | 1.72× |
| 50,000 | 5,753 | 3,285 | **1.75×** |

The per-hash speedup grows with `N` (1.50× → 1.75×) because Ultra crosses each dyadic boundary one input-count earlier than Mega, and Pippenger constants improve marginally with size.

### Why isn't the speedup 2×?

Half the dyadic at the same hash count would naïvely yield 2×. The gap to 2× breaks down as follows, based on per-stage profiling (3,000 inputs):

| Stage | Ultra (ms) | Mega (ms) | Mega/Ultra |
|-------|-----------:|----------:|:----------:|
| commit_to_wires (MSM) | 132 | 71 | 0.54× |
| commit_to_z_perm (MSM) | 54 | 30 | 0.56× |
| commit_to_lookup_counts_and_w4 (MSM) | 47 | 24 | 0.51× |
| commit_to_logderiv_inverses (Mega only) | — | 3 | — |
| `CommitmentKey::commit` (sumcheck round + Gemini MSMs) | 181 | 108 | 0.60× |
| **`sumcheck.prove`** | **39** | **51** | **1.31×** |
| Total prove | 498 | 317 | 0.64× (=1/1.58) |
...
| `N` | Ultra ms (dyadic) | Mega ms (dyadic) | Mega/Ultra | Ultra μs/row | Mega μs/row |
|----:|:-----------------:|:----------------:|:----------:|:------------:|:-----------:|
| 3,000 | 39.4 (2^17) | 51.1 (2^16) | 1.30× | 0.30 | 0.78 |
| 6,000 | 71.5 (2^18) | 90.1 (2^17) | 1.26× | 0.27 | 0.69 |
| 12,000 | 137.3 (2^19) | 169.6 (2^18) | 1.24× | 0.26 | 0.65 |
| 50,000 | 541.6 (2^21) | 661.3 (2^20) | 1.22× | 0.26 | 0.63 |

Mega's sumcheck is 1.22–1.30× slower than Ultra's despite half the domain — extra `poseidon2_external` / `poseidon2_internal` subrelations cost more per active row than the half-domain saves. The ratio shrinks with `N` because Mega has more per-round fixed overhead (subrelation accumulators, skip checks) to amortize over `log₂(N)` rounds: per-row cost falls −19% on Mega vs −13% on Ultra from `N`=3k to 50k.

### Sumcheck share of total prove time

| `N` | Ultra share | Mega share |
|----:|:-----------:|:----------:|
| 3,000 | 7.9% | 16.3% |
| 6,000 | 8.1% | 17.1% |
| 12,000 | 8.6% | 18.2% |
...
Both flavors see sumcheck become a larger slice as `N` grows (PCS gets relatively cheaper at large N). Mega's sumcheck share is consistently ~2× Ultra's — by `N`=50,000 it accounts for a fifth of Mega's prove time. **At this point sumcheck, not MSM, is the lever for further per-hash gains** — reducing sumcheck cost on Mega's Poseidon2 relations would push the per-hash speedup past 1.75×.
