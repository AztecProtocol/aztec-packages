...
case "$cmd" in
  ...
  "ci-grind-p2p")
    ...
    for test in "${e2e_tests[@]}"; do
      echo_header "Grinding: $test"
      full_cmd="${hash}:ISOLATE=1:MAKEFILE_TARGET=yarn-project:NAME=${test} DEBUG_P2P_DISABLE_ABORT_ON_PING_FAILURE=true LOG_LEVEL=\"verbose; debug:p2p\" yarn-project/end-to-end/scripts/run_test.sh simple src/${test}"
      grind_test "$full_cmd" "$e2e_timeout" 50 || { echo "FAILED: $test"; failed=1; }
    done
