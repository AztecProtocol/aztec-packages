...
function bench_publish {
  ...
  mkdir -p bench-out
  bench_merge
  ci3_client artifact_upload bench-$(git rev-parse HEAD^{tree}).tar.gz bench-out/bench.json
}
...
function release {
  ...
  if [ "${DRY_RUN:-0}" != 1 ]; then
    CACHE_FORCE_UPLOAD=1 ci3_client artifact_upload "npm-release-$REF_NAME.tar.gz" npm-release
  fi
}
