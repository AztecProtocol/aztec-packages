...
# Set A: pre-create user-installed sentinel shims that print known strings,
# then assert bare names resolve to them after the aztec install (i.e. NOT
# shadowed). The shim directory comes AFTER ~/.aztec/current/bin on PATH,
# mirroring a realistic user setup where the shell PATH update was the last
# step. Names cover every bare bin the installer used to expose.
...
# Set B.1: every entry in $aztec_bin must be `aztec` or `aztec-*`. Catches any
# future regression that adds a new bare-named bin.
...
# Set B.2: bins known to support --version cleanly must run.
...
echo "PASS: aztec installer does not shadow user binaries; aztec-* surface works"
