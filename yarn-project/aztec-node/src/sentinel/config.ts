...
export const sentinelConfigMappings: ConfigMappingsType<SentinelConfig> = {
  sentinelHistoryLengthInEpochs: {
    description: 'The number of L2 epochs kept of history for each sequencer for computing their stats.',
    env: 'SENTINEL_HISTORY_LENGTH_IN_EPOCHS',
    ...numberConfigHelper(24),
  },
  /**
   * The number of L2 epochs kept of proven performance history for each sequencer.
   * This value must be large enough so that we have proven performance for every sequencer
   * for at least slashInactivityConsecutiveEpochThreshold. Assuming this value is 3,
   * and the committee size is 48, and we have 10k sequencers, then we pick 48 out of 10k each draw.
  ...
  sentinelHistoricProvenPerformanceLengthInEpochs: {
    description: 'The number of L2 epochs kept of proven performance history for each sequencer.',
