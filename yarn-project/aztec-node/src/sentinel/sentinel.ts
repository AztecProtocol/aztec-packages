...
const HISTORY_LENGTH_IN_EPOCHS = 20n; // How many epochs to keep in the store

type ValidatorStatusInSlot =
  | 'block-mined'
  | 'block-proposed'
  | 'block-missed'
  | 'attestation-sent'
...
export class Sentinel implements L2BlockStreamEventHandler {
  ...
  private store: SentinelStore = new SentinelStore();
  ...
  public async handleBlockStreamEvent(event: L2BlockStreamEvent): Promise<void> {
    ...
    if (event.type === 'blocks-added') {
      for (const block of event.blocks) {
        this.slotNumberToArchive.set(block.block.header.getSlot(), block.block.archive.toString());
      }
    }
  }
  ...
  private async getSlotActivity(slot: bigint, epoch: bigint, proposer: EthAddress, committee: EthAddress[]) {
    this.logger.verbose(`Computing stats for slot ${slot} at epoch ${epoch}`, { slot, epoch, proposer, committee });
    ...
    this.logger.verbose(`Block for slot ${slot} was ${blockStatus}`, { archive, slot });
    ...
    this.logger.verbose(`Found ${attestors.size}/${committee.length} attestors for slot ${slot}`, {
      archive,
      slot,
      attestors: [...attestors],
  ...
  private async updateValidators(slot: bigint, stats: Record<`0x${string}`, ValidatorStatusInSlot | undefined>) {
    for (const [who, status] of Object.entries(stats)) {
      if (status) {
        await this.store.pushValidatorStatusForSlot(who as `0x${string}`, slot, status);
      }
    }
  }
...
type ValidatorStatusHistory = [{ slot: bigint; status: ValidatorStatusInSlot }];

class SentinelStore {
  public async pushValidatorStatusForSlot(
    who: `0x${string}`,
    slot: bigint,
    status: 'block-mined' | 'block-proposed' | 'block-missed' | 'attestation-sent' | 'attestation-missed',
  ) {}

  public async getStats(): Record<`0x${string}`, ValidatorStatusHistory> {}
}
