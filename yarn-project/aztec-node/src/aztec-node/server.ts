...
export class AztecNodeService implements AztecNode, AztecNodeAdmin, Traceable {
  ...
  public static async createAndSync(
    ...
    if (useRealVerifiers) {
      peerProofVerifier = await BatchChonkVerifier.new(config, config.bbChonkVerifyBatchSize, 'peer');
  ...
  public async setConfig(config: Partial<AztecNodeAdminConfig>): Promise<void> {
    ...
    if (newConfig.realProofs !== this.config.realProofs) {
      ...
      if (newConfig.realProofs) {
        this.peerProofVerifier = await BatchChonkVerifier.new(newConfig, newConfig.bbChonkVerifyBatchSize, 'peer');
