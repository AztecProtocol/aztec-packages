 ...
 *   `getPredictedMinFees` returns the current min fee followed by one entry per slot of the fee oracle's lag
 *   window, starting at the slot of the next L1 block. We index into it by how far the requested slot is past
 *   that starting slot, clamping to the last entry for slots beyond the window. Within the window the
 *   prediction assumes target mana usage per checkpoint, so an unusually full or empty chain drifts from the
 *   exact value; past the window the fee is a floor rather than an estimate.
...
export class UpstreamGlobalVariableBuilder implements GlobalVariableBuilder {
  ...
  /**
   * Picks the entry of the upstream's fee prediction window that best matches the given slot. Entry 0 is the
   * current min fee and entries 1..n are the predictions for consecutive slots starting at the slot of the
   * next L1 block, so a slot `d` slots past that start maps to entry `d + 1`, clamped to the window.
   */
  private async getMinFeesAtSlot(slotNumber: SlotNumber): Promise<GasFees> {
    ...
    const firstPredictedSlot = getSlotAtNextL1Block(BigInt(this.dateProvider.nowInSeconds()), this.config);
    const slotsAhead = Math.max(slotNumber - firstPredictedSlot, 0);
    const index = Math.min(slotsAhead + 1, fees.length - 1);
