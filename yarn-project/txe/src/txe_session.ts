...
export class TXESession implements TXESessionStateHandler {
  ...
  static async init(contractStore: ContractStore) {
    ...
    // The auth registry was previously a protocol contract and thus implicitly deployed. Since its demotion it is an
    // ordinary deterministic-address contract that must be published in each environment. Aztec.nr hardcodes its
    // canonical address (e.g. public authwit calls the auth registry). Its artifact and instance are preregistered in
    // the shared contract store (see buildSharedContractStore), so here we only emit its deployment nullifier to make it
    // available to every test without explicit setup.
    //
    // That nullifier is folded into the single baseline block that TXE mines on startup rather than mined in an extra
    // block of its own, so the test-visible block counter is unchanged: `next_block_number()` after init is still 2, as
    // block-number-hardcoded tests (e.g. deployment_proofs) require.
