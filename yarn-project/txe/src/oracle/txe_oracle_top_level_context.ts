...
export class TXEOracleTopLevelContext implements IMiscOracle, ITxeExecutionOracle {
   ...
   * Used at session init to publish the standard auth registry — whose artifact and instance are already registered in
   * the shared contract store every worker clones — into the same baseline block TXE mines on startup, so it is
   * available to every test without advancing the test-visible block counter.
   */
