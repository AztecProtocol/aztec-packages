 ...
 * `#[auto_serialization_test]` from the same seed/scenario convention. Oracles with struct or ephemeral-array
 * inputs/outputs (or collections of structs) are skipped (no fixture, so no test).
 */
 ...
 * (`noir-projects/aztec-nr/.../macros/oracle_testing.nr`). The seed is the parameter position so same-typed params get
 * distinct values, which makes a Noir/TS parameter-ordering mismatch fail rather than pass silently. A value change on
 * one side without the other is exactly the serialization drift these tests catch.
 */
const TEST_VALUE_IMPLS: TestValueImpl[] = [
  scalar(FIELD, seed => new Fr(10 + seed)),
  scalar(U32, seed => 10 + seed),
  scalar(BLOCK_NUMBER, seed => BlockNumber(10 + seed)),
  scalar(BIGINT, seed => BigInt(10 + seed)),
  scalar(BYTE, seed => 10 + seed),
  scalar(BOOL, seed => seed % 2 !== 0),
  scalar(AZTEC_ADDRESS, seed => AztecAddress.fromNumber(10 + seed)),
  scalar(FUNCTION_SELECTOR, seed => FunctionSelector.fromField(new Fr(10 + seed))),
  scalar(NOTE_SELECTOR, seed => NoteSelector.fromField(new Fr(10 + seed))),
  scalar(BLOCK_HASH, seed => new BlockHash(new Fr(10 + seed))),
...
function synthesizeScenariosOrThrow(entry: OracleRegistryEntry): OracleTestScenario[] {
  const params = entry.params.map((param, index) => ({
    name: param.name,
    scenarios: scenariosForType(param.type, index),
  }));
  const returnScenarios = entry.returnType === undefined ? undefined : scenariosForType(entry.returnType, 0);
