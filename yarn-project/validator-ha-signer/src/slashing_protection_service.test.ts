...
describe('SlashingProtectionService', () => {
  ...
  describe('stuck-duty cleanup with in-flight locks', () => {
    it('should not clean up an in-flight duty whose lock is still held, but reclaims untracked stuck duties', async () => {
      const shortAgeConfig = { ...config, maxStuckDutiesAgeMs: 1 };
      const svc = new SlashingProtectionService(db, shortAgeConfig, {
        metrics: new HASignerMetrics(telemetryClient, shortAgeConfig.nodeId),
        dateProvider,
      });

      // In-flight duty held by this service: its lock token is tracked and must be excluded.
      const heldParams: CheckAndRecordParams = {
        rollupAddress: ROLLUP_ADDRESS,
        validatorAddress: VALIDATOR_ADDRESS,
        slot: SlotNumber(100),
        blockNumber: BLOCK_NUMBER,
      ...
      await svc.checkAndRecord(heldParams);

      // A stuck duty for the same node that this service never tracked (e.g. from a crashed run).
      const untrackedParams: CheckAndRecordParams = {
        ...heldParams,
        slot: SlotNumber(200),
        blockNumber: BlockNumber(60),
      };
      await db.tryInsertOrGetExisting(untrackedParams);

      // Both rows become older than maxStuckDutiesAgeMs (1ms) after a brief real delay.
      await sleep(20);

      // Run cleanup cycles: the held token is excluded, the untracked one is not.
      await svc.start();
      await sleep(100);
      await svc.stop();

      const held = await db.tryInsertOrGetExisting(heldParams);
      expect(held.isNew).toBe(false);
      expect(held.record.status).toBe(DutyStatus.SIGNING);

      const untracked = await db.tryInsertOrGetExisting(untrackedParams);
      expect(untracked.isNew).toBe(true);
    });

    it('reclaims a duty once its held lock is released via recordSuccess', async () => {
      const shortAgeConfig = { ...config, maxStuckDutiesAgeMs: 1 };
      const svc = new SlashingProtectionService(db, shortAgeConfig, {
        metrics: new HASignerMetrics(telemetryClient, shortAgeConfig.nodeId),
        dateProvider,
      ...
      const lockToken = await svc.checkAndRecord(params);

      // Releasing the lock (recordSuccess) removes it from the active set so cleanup no longer
      // shields the row. It becomes SIGNED, which stuck cleanup ignores; deleteDuty instead removes
      // the row outright. Here we assert the token is no longer excluded by cleaning up a fresh
      // SIGNING row inserted under the same token after release.
      await svc.deleteDuty({
        rollupAddress: ROLLUP_ADDRESS,
        validatorAddress: VALIDATOR_ADDRESS,
        slot: SlotNumber(300),
        blockIndexWithinCheckpoint: BLOCK_INDEX_WITHIN_CHECKPOINT,
      ...
      // Re-insert directly (untracked) and let it become stuck; cleanup must reclaim it.
      await db.tryInsertOrGetExisting(params);
      await sleep(20);
