...
export async function foreignCallHandler(name: string, args: ForeignCallInput[]): Promise<ForeignCallOutput[]> {
  ...
  if (name === 'debugLog') {
    ...
    // flatten all inputs and use constants.
    ...
    const startBlobAccumulatorFields = flattenedArgs.slice(offset, (offset += BLOB_ACCUMULATOR_PUBLIC_INPUTS + 1));
    const [x, y] = [
    ...
    const startBlobAccumulator = new BlobAccumulatorPublicInputs(
      ...
      new BLS12Point(x, y, x.isZero() && y.isZero()), // startBlobAccumulatorFields[13] is the is_inf flag - TODO(MW): FIX
      Fr.fromString(startBlobAccumulatorFields[14]),
      BLS12Fr.fromNoirBigNum({ limbs: startBlobAccumulatorFields.slice(15, 18) }),
    );
