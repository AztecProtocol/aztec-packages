...
/**
 * Creates a foreign call handler for server-side protocol circuits.
...
export function makeForeignCallHandler(log: Logger): ForeignCallHandler {
  return async function foreignCallHandler(name: string, args: ForeignCallInput[]): Promise<ForeignCallOutput[]> {
