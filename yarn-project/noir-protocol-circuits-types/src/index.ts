...
import {
  ...
  PrivateKernelReset as executePrivateKernelResetWithACVM,
...
export const ClientCircuitArtifacts: Record<ClientProtocolArtifact, NoirCompiledCircuit> = {
  ...
  PrivateKernelResetArtifact: PrivateKernelResetArtifact,
...
export async function executeReset(
  ...
  const returnType = await executePrivateKernelResetWithACVM(
    mapPrivateKernelResetCircuitPrivateInputsToNoir(privateKernelResetCircuitPrivateInputs),
    PrivateKernelResetSimulatedJson as CompiledCircuit,
    foreignCallHandler,
  );
...
export function convertPrivateKernelResetInputsToWitnessMap(
  ...
  const initialWitnessMap = abiEncode(PrivateKernelResetArtifact.abi as Abi, { input: mapped as any });
  return initialWitnessMap;
}
 ...
 * @returns The witness map
 */
export function convertSimulatedPublicSetupRollupInputsToWitnessMap(
 ...
 * @returns The witness map
 */
export function convertSimulatedPublicInnerRollupInputsToWitnessMap(
  inputs: PublicKernelCircuitPrivateInputs,
): WitnessMap {
  const mapped = mapPublicKernelCircuitPrivateInputsToNoir(inputs);
 ...
 * @returns The witness map
 */
export function convertSimulatedPublicTeardownRollupInputsToWitnessMap(
  inputs: PublicKernelCircuitPrivateInputs,
): WitnessMap {
  const mapped = mapPublicKernelCircuitPrivateInputsToNoir(inputs);
 ...
 * @returns The public inputs.
 */
export function convertSimulatedPublicSetupRollupOutputFromWitnessMap(
  outputs: WitnessMap,
 ...
 * @returns The public inputs.
 */
export function convertSimulatedPublicInnerRollupOutputFromWitnessMap(
  outputs: WitnessMap,
): PublicKernelCircuitPublicInputs {
  // Decode the witness map into two fields, the return values and the inputs
 ...
 * @returns The public inputs.
 */
export function convertSimulatedPublicTeardownRollupOutputFromWitnessMap(
  outputs: WitnessMap,
): PublicKernelCircuitPublicInputs {
  // Decode the witness map into two fields, the return values and the inputs
