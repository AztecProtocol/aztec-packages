...
export class EncryptedEventFunctionL2Logs extends FunctionL2Logs<EncryptedL2EventLog> {
  /**
   * Creates an empty L2Logs object with no logs.
   * @returns A new FunctionL2Logs object with no logs.
   */
  public static empty(): EncryptedEventFunctionL2Logs {
    return new EncryptedEventFunctionL2Logs([]);
  }
   ...
   * @returns Deserialized instance of `FunctionL2Logs`.
   */
  public static fromBuffer(buf: Buffer, isLengthPrefixed = true): EncryptedEventFunctionL2Logs {
    const reader = new BufferReader(buf, 0);
    ...
    return new EncryptedEventFunctionL2Logs(logs.map(EncryptedL2EventLog.fromBuffer));
  }

  /**
   * Creates a new L2Logs object with `numLogs` logs.
   * @param numLogs - The number of logs to create.
   * @returns A new EncryptedEventFunctionL2Logs object.
   */
  public static random(numLogs: number): EncryptedEventFunctionL2Logs {
    ...
    const logs: EncryptedL2EventLog[] = [];
    for (let i = 0; i < numLogs; i++) {
      logs.push(EncryptedL2EventLog.random());
    }
