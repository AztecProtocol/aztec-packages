...
describe('L1 Note Payload', () => {
  ...
  it.only('encrypted tagged log of finalized partial note matches Noir', () => {
    ...
    const storageSlot = new Fr(0x0fe46be583b71f4ab5b70c2657ff1d05cccf1d292a9369628d1a194f944e6599n);
    const noteValue = new Fr(0x301640ceea758391b2e161c92c0513f129020f4125256afdae2646ce31099f5cn);
    const notePublicValue1 = new Fr(0x14172339287e8d281545c177313f02b6aa2fedfd628cfd8b7f11a136fd0d6557n);
    const notePublicValue2 = new Fr(0x0834d81e3f73c7e2809b08ae38600ffc76a2554473eeab6de7bff4b33a84feacn);
    const noteTypeId = new NoteSelector(4135); // note type id of mock_note.nr
    ...
    // We manually update the log because modifying encrypt func to support public values would just polute the code
    // because it is never used in prod --> the encrypt functionality is only evner used without the public values.
    const encryptedBuf = taggedLog.encrypt(ephSk, recipientAddress, ivpk, ovKeys);

    // We update the encrypted buffer with the public values
    encryptedBuf[64] = numPublicValues;
    const encryptedBufWithPublicValues = Buffer.concat([
      encryptedBuf,
      notePublicValue1.toBuffer(),
      notePublicValue2.toBuffer(),
    ]);

    const encrypted = encryptedBufWithPublicValues.toString('hex');

    expect(encrypted).toMatchInlineSnapshot(
      `""`,
    );
    ...
    updateInlineTestData(
      'noir-projects/aztec-nr/aztec/src/encrypted_logs/payload.nr',
      'encrypted_note_log_of_finalized_partial_from_typescript',
      byteArrayString,
    );
  });
