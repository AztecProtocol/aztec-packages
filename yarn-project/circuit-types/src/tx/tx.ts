...
export class Tx extends Gossipable {
  ...
  async getStats(): Promise<TxStats> {
    return {
      ...
      classRegisteredCount: this.data.getNonEmptyContractClassLogs().length,
  ...
  static clone(tx: Tx): Tx {
    ...
    const clonedTx = new Tx(publicInputs, clientIvcProof, enqueuedPublicFunctionCalls, publicTeardownFunctionCall);
    if (tx.txHash) {
