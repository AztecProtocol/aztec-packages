...
export function injectCommands(
  ...
  const createAccountCommand = program
    ...
    .description(
      'Creates an aztec account that can be used for sending transactions. Registers the account on the PXE and deploys an account contract. Uses a Schnorr account which uses a separate key for encryption and authentication.',
    )
