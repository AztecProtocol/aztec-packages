...
export class WalletDB {
  ...
  async pushBridgedFeeJuice(recipient: AztecAddress, secret: Fr, amount: bigint, leafIndex: bigint, log: LogFn) {
    let stackPointer = (await this.#bridgedFeeJuice.get(`${recipient.toString()}:stackPointer`))?.readInt8() || 0;
    stackPointer++;
  ...
  async popBridgedFeeJuice(recipient: AztecAddress, log: LogFn) {
    let stackPointer = (await this.#bridgedFeeJuice.get(`${recipient.toString()}:stackPointer`))?.readInt8() || 0;
    const result = await this.#bridgedFeeJuice.get(`${recipient.toString()}:${stackPointer}`);
    if (!result) {
      throw new Error(
  ...
  async retrieveTxData(txHash: TxHash) {
    const nonceBuffer = await this.#transactions.get(`${txHash.toString()}:nonce`);
    if (!nonceBuffer) {
      throw new Error(
    ...
    const cancellable = (await this.#transactions.get(`${txHash.toString()}:cancellable`))!.toString() === 'true';
    const gasBuffer = (await this.#transactions.get(`${txHash.toString()}:gasSettings`))!;
  ...
  async retrieveAccountMetadata(aliasOrAddress: AztecAddress | string, metadataKey: string) {
    const { address } = await this.retrieveAccount(aliasOrAddress);
    const result = await this.#accounts.get(`${address.toString()}:${metadataKey}`);
    if (!result) {
  ...
  async retrieveAccount(address: AztecAddress | string) {
    const secretKeyBuffer = await this.#accounts.get(`${address.toString()}:sk`);
    if (!secretKeyBuffer) {
    ...
    const salt = Fr.fromBuffer((await this.#accounts.get(`${address.toString()}:salt`))!);
    const type = (await this.#accounts.get(`${address.toString()}:type`))!.toString('utf8') as AccountType;
