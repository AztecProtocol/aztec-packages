...
aztec-wallet deploy counter_contract@Counter --init initialize --args 0 accounts:main accounts:main -ac bootstrap -a counter
aztec-wallet send increment -ca counter --args accounts:main accounts:main -ac bootstrap
aztec-wallet send increment -ca counter --args accounts:main accounts:main -ac bootstrap

echo
echo "Deploy main account claiming the fee juice, use it later"
echo
aztec-wallet deploy-account -ac main --payment method=native,claim
# These should use --payment method=native
aztec-wallet send increment -ca counter --args accounts:main accounts:main -ac main 
aztec-wallet send increment -ca counter --args accounts:main accounts:main -ac main

RESULT=$(aztec-wallet simulate get_counter -ca counter --args accounts:main -ac main | grep "Simulation result:" | awk '{print $3}')
