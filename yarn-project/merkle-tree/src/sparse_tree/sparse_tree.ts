 ...
 */
export class SparseMerkleTree implements MerkleTree {
  /**
  ...
  constructor(
    ...
    root?: Buffer,
    initialLeafValue = SparseMerkleTree.ZERO_ELEMENT,
  ) {
  ...
  static async new(
    ...
    const tree = new SparseMerkleTree(db, hasher, name, depth, 0n, undefined, initialLeafValue);
   ...
   * @returns The newly created tree.
   */
  static async fromName(db: LevelUp, hasher: Hasher, name: string, initialLeafValue = SparseMerkleTree.ZERO_ELEMENT) {
    const meta: Buffer = await db.get(name);
    const { root, depth, size } = decodeMeta(meta);
    return new SparseMerkleTree(db, hasher, name, depth, size, root, initialLeafValue);
  }
