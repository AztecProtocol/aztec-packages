...
export const standardBasedTreeTestSuite = (
  ...
  describe(testName, () => {
    ...
    it('should have correct empty tree root for depth 32', async () => {
      ...
      const root = await tree.getRoot(false);
    ...
    it('should have correct root and sibling paths', async () => {
      ...
      expect(tree.getRoot(false)).toEqual(await pedersen.hash(level1ZeroHash, level1ZeroHash));
      ...
      expect(tree.getRoot(true)).toEqual(
        await pedersen.hash(await pedersen.hash(values[0], INITIAL_LEAF), level1ZeroHash),
      );
      ...
      expect(tree.getRoot(true)).toEqual(
        await pedersen.hash(await pedersen.hash(values[0], values[1]), level1ZeroHash),
      );
      ...
      expect(tree.getRoot(true)).toEqual(
        pedersen.hash(await pedersen.hash(values[0], values[1]), await pedersen.hash(values[2], INITIAL_LEAF)),
      );
      expect(await tree.getSiblingPath(2n, true)).toEqual(
        new SiblingPath(TEST_TREE_DEPTH, [INITIAL_LEAF, await pedersen.hash(values[0], values[1])]),
      );
      expect(tree.getNumLeaves(false)).toEqual(0n);
      expect(tree.getRoot(false)).toEqual(await pedersen.hash(level1ZeroHash, level1ZeroHash));
      ...
      expect(tree.getRoot(true)).toEqual(
        pedersen.hash(await pedersen.hash(values[0], values[1]), await pedersen.hash(values[2], values[3])),
      );
      expect(await tree.getSiblingPath(3n, true)).toEqual(
        new SiblingPath(TEST_TREE_DEPTH, [values[2], await pedersen.hash(values[0], values[1])]),
      );
      expect(tree.getNumLeaves(false)).toEqual(0n);
      expect(tree.getRoot(false)).toEqual(await pedersen.hash(level1ZeroHash, level1ZeroHash));
      ...
      expect(await tree.getSiblingPath(0n, true)).toEqual(
        new SiblingPath(TEST_TREE_DEPTH, [values[1], await pedersen.hash(values[2], values[3])]),
      );
      expect(await tree.getSiblingPath(1n, true)).toEqual(
        new SiblingPath(TEST_TREE_DEPTH, [values[0], await pedersen.hash(values[2], values[3])]),
      );
      expect(await tree.getSiblingPath(2n, true)).toEqual(
        new SiblingPath(TEST_TREE_DEPTH, [values[3], await pedersen.hash(values[0], values[1])]),
      );
      expect(await tree.getSiblingPath(3n, true)).toEqual(
        new SiblingPath(TEST_TREE_DEPTH, [values[2], await pedersen.hash(values[0], values[1])]),
      );
