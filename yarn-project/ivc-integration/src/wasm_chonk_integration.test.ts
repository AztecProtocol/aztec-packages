...
import { proveChonk as proveChonkWasm, proveThenVerifyAztecClient } from './prove_wasm.js';
import {
...
describe('chonk Integration', () => {
  ...
  it('Should generate a verifiable chonk proof from a simple mock tx via bb.js, verified by bb', async () => {
    ...
    const proofPath = path.join(chonkWorkingDirectory, 'proof');
    await writeChonkProofToPath(wasmProof, proofPath);
  ...
  it('Should generate an array of gate numbers for the stack of programs being proved by Chonk', async () => {
