...
export class BlobAccumulatorPublicInputs {
  ...
  toBuffer() {
    return serializeToBuffer(this.v, this.z, this.y, this.c, this.gamma, this.gammaPow);
  }
  ...
  async accumulateBlobs(blobs: Blob[], finalBlobChallenges: FinalBlobBatchingChallenges) {
    ...
    return new BlobAccumulatorPublicInputs(acc.vAcc, acc.zAcc, acc.yAcc, acc.cAcc, acc.gammaAcc, acc.gammaPow);
  }
...
export class FinalBlobAccumulatorPublicInputs {
  constructor(public v: Fr, public z: Fr, public y: BLS12Fr, public c: BLS12Point) {}
  ...
  toBuffer() {
    return serializeToBuffer(this.v, this.z, this.y, this.c);
  }

  static fromBatchedBlob(blob: BatchedBlob) {
    return new FinalBlobAccumulatorPublicInputs(blob.v, blob.z, blob.y, blob.commitment);
  }
...
export class BlockBlobPublicInputs {
  ...
  static async fromBlobs(startBlobAccumulator: BatchedBlobAccumulator, blobs: Blob[]): Promise<BlockBlobPublicInputs> {
    let endBlobAccumulator = await startBlobAccumulator.accumulateBlobs(blobs);
    return new BlockBlobPublicInputs(
