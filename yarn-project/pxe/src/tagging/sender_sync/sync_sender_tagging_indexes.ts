 ...
 *
 * @param secret - The sender-side tagging key. May be an `ExtendedDirectionalAppTaggingSecret` (unconstrained
 * delivery, unique for a `(sender, recipient, app)` tuple) or a `ConstrainedAppTaggingSecret` (constrained delivery,
 * keyed on the app-siloed shared secret).
