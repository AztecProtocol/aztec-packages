...
describe('loadPrivateLogsForSenderRecipientPair', () => {
  ...
  it('returns empty array when no logs found', async () => {
    ...
    expect(await taggingStore.getHighestAgedIndex(secret, TEST_JOB_ID)).toBeUndefined();
    expect(await taggingStore.getHighestFinalizedIndex(secret, TEST_JOB_ID)).toBeUndefined();
  });
  ...
  it('loads log and updates highest finalized index but not highest aged index', async () => {
    ...
    expect(await taggingStore.getHighestFinalizedIndex(secret, TEST_JOB_ID)).toBe(logIndex);
    expect(await taggingStore.getHighestAgedIndex(secret, TEST_JOB_ID)).toBeUndefined();
  });
  ...
  it('loads log and updates both highest aged and highest finalized indexes', async () => {
    ...
    expect(logs).toHaveLength(1);
    expect(await taggingStore.getHighestAgedIndex(secret, TEST_JOB_ID)).toBe(logIndex);
    expect(await taggingStore.getHighestFinalizedIndex(secret, TEST_JOB_ID)).toBe(logIndex);
  });
  ...
  it('logs at boundaries are properly loaded, window and highest indexes advance as expected', async () => {
    ...
    await taggingStore.updateHighestAgedIndex(secret, highestAgedIndex, TEST_JOB_ID);
    await taggingStore.updateHighestFinalizedIndex(secret, highestFinalizedIndex, TEST_JOB_ID);
    ...
    expect(await taggingStore.getHighestFinalizedIndex(secret, TEST_JOB_ID)).toBe(log2Index);
    expect(await taggingStore.getHighestAgedIndex(secret, TEST_JOB_ID)).toBe(log1Index);
