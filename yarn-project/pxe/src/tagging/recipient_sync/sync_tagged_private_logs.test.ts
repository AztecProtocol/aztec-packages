...
import { AppTaggingSecretKind, type AppTaggingSecret, SiloedTag } from '@aztec/stdlib/logs';
...
function makeSecrets(count: number): Promise<AppTaggingSecret[]> {
  return Promise.all(
    Array.from({ length: count }, () => randomAppTaggingSecret(AppTaggingSecretKind.UNCONSTRAINED)),
  );
}
