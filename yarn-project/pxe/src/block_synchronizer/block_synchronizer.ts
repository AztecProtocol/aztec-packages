...
import type { CanonicalChainStore } from '../storage/canonical_chain_store/canonical_chain_store.js';
...
export class BlockSynchronizer implements L2BlockStreamEventHandler {
  ...
  private async doHandleBlockStreamEvent(event: L2BlockStreamEvent): Promise<void> {
    ...
    switch (event.type) {
      case 'blocks-added': {
        ...
        await this.canonicalChainStore.setMany(origins);
      ...
      case 'chain-pruned': {
        const currentAnchorBlockHeader = await this.canonicalChainStore.getBlockHeader();
  ...
  private async updateAnchorBlockHeader(blockHeader: BlockHeader) {
    ...
    await this.canonicalChainStore.setHeader(blockHeader);
  }
  ...
  private async doSync() {
    ...
    try {
      currentHeader = await this.canonicalChainStore.getBlockHeader();
    } catch {
    ...
    // Genesis (block 0) is covered by `setHeader` above; hydration is for the open chain (blocks >= 1).
    const fromHeight = Math.max(tips.finalized.block.number, tipHeight + 1, INITIAL_L2_BLOCK_NUM);
