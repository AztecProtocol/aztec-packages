 ...
 * {@link Option.empty} for an absent one. The type guards {@link isSome} and {@link isNone} narrow
 * `value` in conditional branches.
 */
export class Option<T> {
  private constructor(
    public readonly value: T | undefined,
    public readonly emptyTemplate: T | undefined,
  ) {}
   ...
   * @example
   * ```ts
   * return Option.some(BoundedVec.from({ data: plaintext, maxLength: ciphertext.maxLength }));
   * ```
   */
   ...
   * For types whose wire size varies per call site (`BoundedVec`, `FixedArray`), pass a `emptyTemplate` so the
   * serializer knows how many zero fields to emit. Omit the emptyTemplate when the Option will not be
   * re-serialized (e.g. deserialized input params).
   *
   * @param emptyTemplate - A representative empty `T` whose serialization determines the zero-filled wire format.
   *
   * @example None for a fixed-size type:
   * ```ts
   * return Option.empty(AztecAddress.ZERO);
   * ```
   *
   * @example None for a dynamic-size type:
   * ```ts
   * return Option.empty(BoundedVec.empty<number>({ maxLength: ciphertext.maxLength }));
   * ```
   */
  static empty<T>(emptyTemplate?: T): Option<T> {
    return new Option<T>(undefined, emptyTemplate);
  }
