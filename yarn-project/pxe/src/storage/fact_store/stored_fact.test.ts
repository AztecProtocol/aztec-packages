...
describe('StoredFact', () => {
  ...
  it('round-trips a retractable fact through buffer serialization', () => {
    const fact = new StoredFact(contract, scope, entityType, correlation, factType, [new Fr(9n), new Fr(10n)], {
      blockNumber: 12,
  ...
  it('round-trips a non-retractable fact (no anchor)', () => {
    const fact = new StoredFact(contract, scope, entityType, correlation, factType, [new Fr(9n)], undefined);
    const back = StoredFact.fromBuffer(fact.toBuffer());
    expect(back).toEqual(fact);
  ...
  it('derives stable composite keys', () => {
    ...
    expect(entityKeyOf(fact)).toBe(`${contract}:${scope}:${entityType}:${correlation}`);
  ...
  it('gives distinct payload hashes for distinct payloads and equal for equal', () => {
    const a = new StoredFact(contract, scope, entityType, correlation, factType, [new Fr(1n)], undefined);
    const b = new StoredFact(contract, scope, entityType, correlation, factType, [new Fr(2n)], undefined);
