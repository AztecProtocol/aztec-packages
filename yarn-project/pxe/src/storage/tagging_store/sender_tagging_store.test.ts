...
describe('SenderTaggingStore', () => {
  ...
  describe('storePendingIndexes', () => {
    it('stores a single pending index', async () => {
      ...
      const preTag: PreTag = { extendedSecret: secret1, index: 5 };
    ...
    it('stores multiple pending indexes for different secrets', async () => {
      ...
      const preTags: PreTag[] = [
        { extendedSecret: secret1, index: 3 },
        { extendedSecret: secret2, index: 7 },
      ];
    ...
    it('stores multiple pending indexes for the same secret from different txs', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash1, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash2, 'test');
    ...
    it('throws when storing duplicate secrets in the same call', async () => {
      ...
      const preTags: PreTag[] = [
        { extendedSecret: secret1, index: 3 },
        { extendedSecret: secret1, index: 7 },
      ];
    ...
    it('throws when storing a different index for an existing secret + txHash pair', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 5 }], txHash, 'test');

      // Try to store a different index for the same secret + txHash pair
      await expect(
        taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash, 'test'),
    ...
    it('throws when storing a pending index lower than the last finalized index', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 10 }], txHash1, 'test');
      await taggingStore.finalizePendingIndexes([txHash1], 'test');

      // Try to store a pending index lower than the finalized index
      await expect(
        taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 5 }], txHash2, 'test'),
    ...
    it('throws when storing a pending index equal to the last finalized index', async () => {
      ...
      await expect(
        taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 10 }], txHash2, 'test'),
    ...
    it('allows storing a pending index higher than the last finalized index', async () => {
      ...
      await expect(
        taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 15 }], txHash2, 'test'),
    ...
    describe('window length validation', () => {
      it('throws when storing an index beyond window length from finalized index', async () => {
        ...
        // First store and finalize an index
        await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: finalizedIndex }], txHash1, 'test');
        await taggingStore.finalizePendingIndexes([txHash1], 'test');

        // Try to store an index beyond the window
        await expect(
          taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: indexBeyondWindow }], txHash2, 'test'),
        ).rejects.toThrow(
      ...
      it('allows storing an index at the window boundary from finalized index', async () => {
        ...
        await expect(
          taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: indexAtBoundary }], txHash2, 'test'),
        ).resolves.not.toThrow();
  ...
  describe('getTxHashesOfPendingIndexes', () => {
    ...
    it('returns tx hashes for indexes within the specified range', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash1, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 5 }], txHash2, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 8 }], txHash3, 'test');
    ...
    it('includes startIndex and excludes endIndex (range is [startIndex, endIndex))', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 5 }], txHash1, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 10 }], txHash2, 'test');
    ...
    it('handles parallel pending indexes for the same secret from different txs', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret2, index: 7 }], txHash1, 'test');
      // Store "parallel" index for secret1 with a different tx (can happen when sending logs from multiple PXEs)
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash3, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash4, 'test');
  ...
  describe('getLastUsedIndex', () => {
    ...
    it('returns the highest of multiple pending indexes', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash1, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash2, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 5 }], txHash3, 'test');
  ...
  describe('dropPendingIndexes', () => {
    it('removes all pending indexes for a given tx hash', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash1, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret2, index: 5 }], txHash1, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash2, 'test');
  ...
  describe('finalizePendingIndexes', () => {
    ...
    it('does not update finalized index when newly finalized index is lower', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash1, 'test');
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash2, 'test');
    ...
    it('does nothing when tx hash does not exist', async () => {
      const txHash = TxHash.random();
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash, 'test');
  ...
  describe('complex scenarios', () => {
    ...
    it('handles multiple secrets with different lifecycles', async () => {
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret2, index: 5 }], txHash2, 'test');
  ...
  describe('staged writes', () => {
    it('writes of uncommitted jobs are not visible outside the job that makes them', async () => {
      const committedTxHash = TxHash.random();
      {
        ...
        await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], committedTxHash, commitJobId);
        await taggingStore.commit(commitJobId);
      }

      const stagedTxHash = TxHash.random();
      const stagingJobId: string = 'staging-job';
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 5 }], stagedTxHash, stagingJobId);
    ...
    it('job staged data is correctly isolated when storing and finalizing pending indexes', async () => {
      ...
      {
        const commitJobId: string = 'commit-job';
        await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash1, commitJobId);
      ...
      await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash2, stagingJobId);
    ...
    it('discardStaged removes staged data without affecting persistent storage', async () => {
      {
        ...
        const txHash2 = TxHash.random();
        const commitJobId: string = 'commit-job';
        await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 2 }], txHash1, commitJobId);
        await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 3 }], txHash2, commitJobId);
        await taggingStore.finalizePendingIndexes([txHash1], commitJobId);
        await taggingStore.commit(commitJobId);
      }
      ...
      {
        const txHash3 = TxHash.random();
        await taggingStore.storePendingIndexes([{ extendedSecret: secret1, index: 7 }], txHash3, stagingJobId);
