...
export const SCHEMA_TESTS: readonly SchemaTest[] = [
...

  {
    ...
    writeToStore: async kvStore => {
      ...
      await taggingSecretSourcesStore.addSender(AztecAddress.fromBigInt(2n));
      await taggingSecretSourcesStore.addSender(AztecAddress.fromBigInt(3n));
      await taggingSecretSourcesStore.addSender(AztecAddress.fromBigInt(5n));

      await taggingSecretSourcesStore.addSharedSecret(AztecAddress.fromBigInt(7n), new Point(new Fr(2n), new Fr(3n)));
      await taggingSecretSourcesStore.addSharedSecret(AztecAddress.fromBigInt(7n), new Point(new Fr(5n), new Fr(7n)));
