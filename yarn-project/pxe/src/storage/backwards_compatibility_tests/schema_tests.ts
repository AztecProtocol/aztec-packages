...
export const SCHEMA_TESTS: readonly SchemaTest[] = [
  ...
  {
    name: 'CanonicalChainStore',
    writeToStore: async kvStore => {
      const canonicalChainStore = new CanonicalChainStore(kvStore);
      ...
      await canonicalChainStore.setHeader(
        new BlockHeader(
      ...
      await canonicalChainStore.setMany([
        { blockNumber: 83, blockHash: new Fr(89n).toString() },
        { blockNumber: 97, blockHash: new Fr(101n).toString() },
      ]);
    },
    snapshotStore: async kvStore => ({
