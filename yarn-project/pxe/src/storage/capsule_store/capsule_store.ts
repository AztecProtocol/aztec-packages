...
export class CapsuleStore implements StagedStore {
   ...
   * @param jobId - Optional jobId for staged writes.
   * @remarks A capsule is a "blob" of data that is passed to the contract through an oracle. It works similarly
   * to public contract storage in that it's indexed by the contract address and storage slot but instead of the global
   * network state it's backed by local PXE db.
   */
  async storeCapsule(contractAddress: AztecAddress, slot: Fr, capsule: Fr[], jobId?: string): Promise<void> {
   ...
   * @param jobId - Optional jobId for staged writes.
   */
  async deleteCapsule(contractAddress: AztecAddress, slot: Fr, jobId?: string): Promise<void> {
    const key = dbSlotToKey(contractAddress, slot);
  ...
  async copyCapsule(
    ...
    if (jobId) {
      ...
      for (const i of indexes) {
        ...
        await this.storeCapsule(contractAddress, currentDstSlot, toCopy, jobId);
      }
    } else {
  ...
  appendToCapsuleArray(contractAddress: AztecAddress, baseSlot: Fr, content: Fr[][], jobId?: string): Promise<void> {
    const doAppend = async () => {
      ...
      for (let i = 0; i < content.length; i++) {
        ...
        await this.storeCapsule(contractAddress, nextSlot, content[i], jobId);
      }

      // Update length to include all new capsules
      const newLength = currentLength + content.length;
      await this.storeCapsule(contractAddress, baseSlot, [new Fr(newLength)], jobId);
    };
  ...
  setCapsuleArray(contractAddress: AztecAddress, baseSlot: Fr, content: Fr[][], jobId?: string) {
    const doSet = async () => {
      ...
      // Set the new length
      await this.storeCapsule(contractAddress, baseSlot, [new Fr(content.length)], jobId);

      // Store the new content, possibly overwriting existing values
      for (let i = 0; i < content.length; i++) {
        await this.storeCapsule(contractAddress, arraySlot(baseSlot, i), content[i], jobId);
      }

      // Clear any stragglers
      for (let i = content.length; i < originalLength; i++) {
        await this.deleteCapsule(contractAddress, arraySlot(baseSlot, i), jobId);
      }
    };
