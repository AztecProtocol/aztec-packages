...
This might be because the note was nullified in the same tx which created it or it was a partial note.
In that case, everything is fine. To check whether this note was nullified in the same transaction, look back through
the logs for a notification
'important: chopped commitment for siloed inner hash note
${siloedNoteHash.toString()}'.
If you can see that notification or this note is meant to be completed at a later date
(it implements CompletableNoteInterface in the contract) then everything's fine.
