...
export async function createPXEService(
  ...
  const keyStore = new KeyStore(
    await createStore('pxe_key_store', config, createDebugLogger('aztec:pxe:keystore:lmdb')),
  );
  const db = new KVPxeDatabase(await createStore('pxe_data', config, createDebugLogger('aztec:pxe:data:lmdb')));
