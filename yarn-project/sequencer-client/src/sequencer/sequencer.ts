import { type BlockAttestation, type EpochProofQuote, type L1ToL2MessageSource, type L2Block, type L2BlockSource, type ProcessedTx, Tx, type TxHash, type TxValidator, type WorldStateSynchronizer } from '@aztec/circuit-types';
import { type AllowedElement, BlockProofError, type WorldStateSynchronizerStatus } from '@aztec/circuit-types/interfaces';
import { type L2BlockBuiltStats } from '@aztec/circuit-types/stats';
import { AppendOnlyTreeSnapshot, ContentCommitment, GENESIS_ARCHIVE_ROOT, Header, StateReference } from '@aztec/circuits.js';
...
export class Sequencer {
  ...
  constructor(
    ...
    log.verbose(`Initialized sequencer with ${this.minTxsPerBLock}-${this.maxTxsPerBlock} txs per block.`);
  }
  ...
  public async stop(): Promise<void> {
    log.debug(`Stopping sequencer`);
    await this.runningPromise?.stop();
    this.publisher.interrupt();
    this.state = SequencerState.STOPPED;
    log.info('Stopped sequencer');
  }
  ...
  protected async work() {
    ...
    if (!prevBlockSynced) {
      log.debug('Previous block has not been mined and processed yet');
      return;
    }

    if (prevBlockSynced && this.state === SequencerState.PUBLISHING_BLOCK) {
      log.debug(`Block has been synced`);
      this.state = SequencerState.IDLE;
    }
    ...
    try {
      ...
      log.debug(`Cannot propose for block ${newBlockNumber}`);
      return;
    }
    ...
    log.debug(`Retrieved ${pendingTxs.length} txs from P2P pool`);
    ...
    try {
      ...
      if (BlockProofError.isBlockProofError(err)) {
        ...
        log.warn(`Proving block failed, removing ${txHashes.length} txs from pool`);
        await this.p2pClient.deleteTxs(txHashes);
      }
      log.error(`Error assembling block`, (err as any).stack);
    }
  }
  ...
  async mayProposeBlock(tipArchive: Buffer, proposalBlockNumber: bigint): Promise<bigint> {
    ...
    try {
      ...
      log.debug(`Can propose block ${proposalBlockNumber} at slot ${slot}`);
      return slot;
    } catch (err) {
      prettyLogViemError(err, log);
      throw err;
    }
  }
  ...
  shouldProposeBlock(historicalHeader: Header | undefined, args: ShouldProposeArgs): boolean {
    if (this.isFlushing) {
      logChecks.verbose(`Flushing all pending txs in new block`);
      return true;
    }
    ...
    if (args.processedTxsCount != undefined) {
      if (args.processedTxsCount === 0 && !skipCheck && this.minTxsPerBLock > 0) {
        logChecks.verbose('No txs processed correctly to build block. Exiting');
        return false;
      }
    }
  ...
  private async buildBlockAndAttemptToPublish(
    ...
    log.info(`Building block ${newGlobalVariables.blockNumber.toNumber()} with ${validTxs.length} transactions`);

    // Get l1 to l2 messages from the contract
    log.debug('Requesting L1 to L2 messages from contract');
    ...
    try {
      ...
      if (failedTxs.length > 0) {
        ...
        log.debug(`Dropping failed txs ${Tx.getHashes(failedTxData).join(', ')}`);
      ...
      if (this.isFlushing) {
        log.verbose(`Flushing completed`);
      }

      const txHashes = validTxs.map(tx => tx.getTxHash());

      this.isFlushing = false;
      log.verbose('Collecting attestations');
      const attestations = await this.collectAttestations(block, txHashes);
      log.verbose('Attestations collected');

      log.verbose('Collecting proof quotes');
      const proofQuote = await this.createProofClaimForPreviousEpoch(newGlobalVariables.slotNumber.toBigInt());
      log.verbose(proofQuote ? `Using proof quote ${inspect(proofQuote.payload)}` : 'No proof quote available');
  ...
  protected async collectAttestations(block: L2Block, txHashes: TxHash[]): Promise<Signature[] | undefined> {
    ...
    log.debug(`Attesting committee length ${committee.length}`);

    if (committee.length === 0) {
      log.debug(`Attesting committee length is 0, skipping`);
      return undefined;
    }
    ...
    log.verbose('Creating block proposal');
    const proposal = await this.validatorClient.createBlockProposal(block.header, block.archive.root, txHashes);

    this.state = SequencerState.PUBLISHING_BLOCK_TO_PEERS;
    log.verbose('Broadcasting block proposal to validators');
    ...
    log.verbose(`Collected attestations from validators, number of attestations: ${attestations.length}`);
  ...
  protected async createProofClaimForPreviousEpoch(slotNumber: bigint): Promise<EpochProofQuote | undefined> {
    try {
      ...
      if (epochForBlock < 1n) {
        ...
        log.verbose(`First epoch has no claim`);
        return undefined;
      }
      ...
      if (canClaim != epochToProve) {
        ...
        log.verbose(`Unable to claim previous epoch (${canClaim} != ${epochToProve})`);
        return undefined;
      }
      // Get quotes for the epoch to be proven
      const quotes = await this.p2pClient.getEpochProofQuotes(epochToProve);
      log.verbose(`Retrieved ${quotes.length} quotes, slot: ${slotNumber}, epoch to prove: ${epochToProve}`);
      for (const quote of quotes) {
        log.verbose(inspect(quote.payload));
      }
      ...
      if (!validQuotes.length) {
        log.verbose(`Failed to find any valid proof quotes`);
        return undefined;
      }
      ...
      log.error(`Failed to create proof claim for previous epoch: ${err}`);
      return undefined;
    }
  }
  ...
  protected async takeValidTxs<T extends Tx | ProcessedTx>(txs: T[], validator: TxValidator<T>): Promise<T[]> {
    ...
    if (invalid.length > 0) {
      log.debug(`Dropping invalid txs from the p2p pool ${Tx.getHashes(invalid).join(', ')}`);
  ...
  protected async isBlockSynced() {
    ...
    logChecks.debug(`Sync check to last published block ${this.lastPublishedBlock} ${result ? 'succeeded' : 'failed'}`, {
      worldState,
      p2p,
      l2BlockSource,
      l1ToL2MessageSource,
