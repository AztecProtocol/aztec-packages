...
export class CheckpointProposalJob {
   ...
   */
  async execute(): Promise<Checkpoint | undefined> {
    ...
    using fork = await this.worldState.fork(this.syncedToBlockNumber);
  ...
  private async buildBlocksForCheckpoint(
    ...
    while (true) {
      ...
      const buildDeadline = new Date((this.getSlotStartBuildTimestamp() + timingInfo.deadline) * 1000);
  ...
  private async buildSingleBlock(
    ...
    try {
      ...
      if (!forceCreate && numTxs < minTxs) {
        this.log.warn(
          `Block ${blockNumber} at index ${indexWithinCheckpoint} on slot ${this.slot} has too few valid txs to be proposed (got ${numTxs} but required ${minTxs})`,
          { slot: this.slot, blockNumber, numTxs, indexWithinCheckpoint },
        );
        this.eventEmitter.emit('block-tx-count-check-failed', { minTxs, availableTxs: numTxs, slot: this.slot });
  ...
  private async waitForMinTxs(opts: {
    ...
    const startDeadline = new Date(buildDeadline.getTime() - MIN_BLOCK_BUILD_TIME_MS);
    ...
    while (!forceCreate && availableTxs < minTxs) {
      ...
      // If we're past deadline, give up
