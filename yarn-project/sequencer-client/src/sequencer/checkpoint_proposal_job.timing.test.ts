...
/**
 * Extended MockCheckpointBuilder that simulates execution time by advancing a TestDateProvider.
...
class TimingAwareMockCheckpointBuilder extends MockCheckpointBuilder {
  ...
  constructor(
    ...
    private readonly dateProvider: TestDateProvider,
 ...
 * 2. Tracks state transitions and block build times for assertions
 */
class TimingTestCheckpointProposalJob extends CheckpointProposalJob {
  ...
  constructor(
    private readonly testDateProvider: TestDateProvider,
...
describe('CheckpointProposalJob Timing Tests', () => {
  ...
  beforeEach(async () => {
    ...
    // Initialize test date provider at slot start
    dateProvider = new TestDateProvider();
    dateProvider.setTime(getSlotStartTime(slotNumber) * 1000);
    ...
    const checkpointConstants: CheckpointGlobalVariables & { timestamp: bigint } = {
  ...
  describe('Block Building with Simulated Time', () => {
    ...
    it('skips sub-slots when starting late', async () => {
      ...
      expect(checkpoint).toBeDefined();
      // Should have built 1-2 blocks in remaining sub-slots
      expect(checkpointBuilder.buildBlockCalls.length).toBeGreaterThanOrEqual(1);
    });
    ...
    it('waits between blocks to maintain sub-slot boundaries', async () => {
      ...
      // Track time at second block start
      let timeAtSecondBlockStart = 0;
      const originalBuildBlock = checkpointBuilder.buildBlock.bind(checkpointBuilder);
      let callCount = 0;
      jest.spyOn(checkpointBuilder, 'buildBlock').mockImplementation(async (...args) => {
        callCount++;
        if (callCount === 2) {
          timeAtSecondBlockStart = getSecondsIntoSlot();
        }
        return await originalBuildBlock(...args);
      });
      ...
      expect(timeAtSecondBlockStart).toBeGreaterThanOrEqual(9);
    });
    ...
    it('verifies deadlines are passed to block builder', async () => {
      ...
      expect(checkpointBuilder.buildBlockCalls.length).toBeGreaterThanOrEqual(2);

      // Verify each block was given the correct deadline
      const block1Deadline = checkpointBuilder.buildBlockCalls[0].opts.deadline;
      const block2Deadline = checkpointBuilder.buildBlockCalls[1].opts.deadline;

      expect(block1Deadline).toBeDefined();
      expect(block2Deadline).toBeDefined();

      // Deadlines should be at sub-slot boundaries (9s and 17s from slot start)
      const slotStart = getSlotStartTime(slotNumber);
      const block1DeadlineSeconds = block1Deadline!.getTime() / 1000 - slotStart;
      const block2DeadlineSeconds = block2Deadline!.getTime() / 1000 - slotStart;

      expect(block1DeadlineSeconds).toBeCloseTo(9, 0);
      expect(block2DeadlineSeconds).toBeCloseTo(17, 0);
    });
  });
  ...
  describe('Block Execution Overflow Handling', () => {
    it('handles block that finishes after its deadline by skipping next sub-slot', async () => {
      ...
      checkpointBuilder.setExecutionDurations([10, 5, 5]);
      ...
      // First block finished at 11s, past deadline of 9s
      // Second block should start at sub-slot 2 boundary (17s would be deadline, need minExec=2s before)
      // Since we're at 11s and deadline for sub-slot 2 is 17s, we have 6s which is >= minExecutionTime
      const buildTimes = checkpointBuilder.recordedBuildTimes;
      expect(buildTimes.length).toBeGreaterThanOrEqual(2);
      ...
      // Second block should start at 11s (immediately after first finishes) or at sub-slot boundary
      // The timetable finds the next available sub-slot where we have minExecutionTime
      expect(buildTimes[1].startTime).toBeGreaterThanOrEqual(11);
    });
