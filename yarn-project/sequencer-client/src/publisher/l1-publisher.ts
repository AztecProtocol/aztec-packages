...
export interface L1PublisherTxSender {
  ...
  sendEmitContractDeploymentTx(l2BlockNum: number, unverifiedData: UnverifiedData): Promise<string | undefined>;
...
export class L1Publisher implements L2BlockReceiver {
  ...
  private async sendYeetTx(l2BlockNum: number, unverifiedData: UnverifiedData): Promise<string | undefined> {
    while (!this.interrupted) {
      try {
        return await this.txSender.sendEmitContractDeploymentTx(l2BlockNum, unverifiedData);
      } catch (err) {
        this.log(`Error sending tx to L1`, err);
        await this.sleepOrInterrupted();
      }
