...
cleanup() {
    ...
    # # Terminate the second yarn serve process (port 8081)
    # if kill "$PID2" 2>/dev/null; then
    #     echo "Killed yarn serve on port 8081 (PID $PID2)."
    # else
...
# PID2=$!
# echo "Started yarn serve on port 8081 with PID $PID2."
...
monitor_processes() {
    while true; do
        ...
        #     echo "yarn serve on port 8081 (PID $PID2) has exited."
        #     cleanup
        # fi
