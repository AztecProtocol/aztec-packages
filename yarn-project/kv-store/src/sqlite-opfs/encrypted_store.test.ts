...
describe('AztecSQLiteOPFSStore with encryption', () => {
  ...
  it('round-trips values with the same key (persistent)', async () => {
    ...
    const a = await AztecSQLiteOPFSStore.open(mockLogger, name, false, dir, key);
    const mapA = a.openMap<string, string>('m');
    await mapA.set('k1', 'v1');
    await a.close();

    const b = await AztecSQLiteOPFSStore.open(mockLogger, name, false, dir, key);
  ...
  it('fails to open with the wrong key', async () => {
    ...
    await expect(async () => {
      const b = await AztecSQLiteOPFSStore.open(mockLogger, name, false, dir, wrongKey);
      await b.openMap<string, string>('m').getAsync('k1');
      await b.close();
    }).rejects.toThrow();

    const c = await AztecSQLiteOPFSStore.open(mockLogger, name, false, dir, key);
    await c.delete();
  });
