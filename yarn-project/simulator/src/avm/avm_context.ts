...
export class AvmContext {
   ...
   * @returns new AvmContext instance
   */
  public createNestedContractCallContext(address: AztecAddress, calldata: Fr[], temporaryFunctionSelector: FunctionSelector): AvmContext {
    const newExecutionEnvironment = this.environment.deriveEnvironmentForNestedCall(address, calldata, temporaryFunctionSelector);
   ...
   * @returns new AvmContext instance
   */
  public createNestedContractStaticCallContext(address: AztecAddress, calldata: Fr[], temporaryFunctionSelector: FunctionSelector): AvmContext {
    const newExecutionEnvironment = this.environment.deriveEnvironmentForNestedStaticCall(address, calldata, temporaryFunctionSelector);
    const forkedWorldState = this.persistableState.fork();
    const machineState = AvmMachineState.fromState(this.machineState);
    return new AvmContext(forkedWorldState, newExecutionEnvironment, machineState);
  }
