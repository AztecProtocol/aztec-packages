...
import { AztecAddress, Fr, SerializableContractInstance } from '@aztec/circuits.js';
  ...
  describe('GETCONTRACTINSTANCE', () => {
    ...
    describe.each([
      ...
      it(`Should read '${ContractInstanceMember[memberEnum]}' correctly`, async () => {
        ...
        await new GetContractInstance(/*indirect=*/ 0, memberEnum, /*addressOffset=*/ 0, /*dstOffset=*/ 1, /*existsOffset=*/ 2).execute(context);
    ...
    ])('GETCONTRACTINSTANCE member instruction works when contract does not exist', (memberEnum: ContractInstanceMember) => {
      it(`'${ContractInstanceMember[memberEnum]}' should be 0 when contract does not exist `, async () => {
        const emptyContractInstance = SerializableContractInstance.empty();
        ...
        expect(trace.traceGetContractInstance).toHaveBeenCalledTimes(1);
        expect(trace.traceGetContractInstance).toHaveBeenCalledWith(address, /*exists=*/ false, emptyContractInstance);
      });
    });
  });
});
