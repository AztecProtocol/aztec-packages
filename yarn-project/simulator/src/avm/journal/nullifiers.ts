...
export class NullifierManager {
   ...
   */
  private checkExistsHereOrParent(address: Fr, nullifier: Fr): boolean {
    // First check this cache
    let existsAsPending = this.cache.exists(address, nullifier);
    ...
    if (!existsAsPending && this.parent) {
      ...
      existsAsPending = this.parent.checkExistsHereOrParent(address, nullifier);
    }
  ...
  public async checkExists(
    ...
    const existsAsPending = this.checkExistsHereOrParent(address, nullifier);
    ...
    if (!existsAsPending) {
      ...
      leafIndex = await this.hostNullifiers.getNullifierIndex(siloNullifier(address, nullifier));
    }
   ...
   *
   * @param address - the address of the contract that the nullifier is associated with
   * @param nullifier - the nullifier to stage
   */
  public async append(address: Fr, nullifier: Fr) {
    const [exists, ,] = await this.checkExists(address, nullifier);
    if (exists) {
      throw new NullifierCollisionError(
        `Nullifier ${nullifier} at contract ${address} already exists in parent cache or host.`,
      );
    }
    this.cache.append(address, nullifier);
  }
...
export class NullifierCache {
   ...
   * One inner-set per contract address,
   ...
   */
  public exists(address: Fr, nullifier: Fr): boolean {
    const exists =
      this.cachePerContract.get(address.toBigInt())?.has(nullifier.toBigInt()) ||
      this.siloedNullifiers.has(siloNullifier(AztecAddress.fromField(address), nullifier).toBigInt());
    return !!exists;
  }
   ...
   * @param nullifier - the nullifier to stage
   */
  public append(address: Fr, nullifier: Fr) {
    if (this.exists(address, nullifier)) {
      throw new NullifierCollisionError(`Nullifier ${nullifier} at contract ${address} already exists in cache.`);
    }

    let nullifiersForContract = this.cachePerContract.get(address.toBigInt());
    ...
    if (!nullifiersForContract) {
      ...
      this.cachePerContract.set(address.toBigInt(), nullifiersForContract);
    }
