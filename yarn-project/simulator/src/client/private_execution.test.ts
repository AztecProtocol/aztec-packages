...
describe('Private Execution test suite', () => {
  ...
  describe('nested calls', () => {
    ...
    it('child function should be callable', async () => {
      ...
      expect(result.callStackItem.publicInputs.returnValues[0]).toEqual(
        computeVarArgsHash([new Fr(initialValue + privateIncrement)]),
      );
    });
    ...
    it('parent should call child', async () => {
      ...
      expect(result.callStackItem.publicInputs.returnValues[0]).toEqual(computeVarArgsHash([new Fr(privateIncrement)]));
      ...
      expect(result.nestedExecutions[0].callStackItem.publicInputs.returnValues[0]).toEqual(
        computeVarArgsHash([new Fr(privateIncrement)]),
      );
  ...
  describe('pending note hashes contract', () => {
    ...
    it('should be able to insert, read, and nullify pending note hashes in one call', async () => {
      ...
      expect(gotNoteValue).toEqual(computeVarArgsHash([new Fr(amountToTransfer)]));
    ...
    it('should be able to insert, read, and nullify pending note hashes in nested calls', async () => {
      ...
      const gotNoteValue = execGetThenNullify.callStackItem.publicInputs.returnValues[0];
      expect(gotNoteValue).toEqual(computeVarArgsHash([new Fr(amountToTransfer)]));
