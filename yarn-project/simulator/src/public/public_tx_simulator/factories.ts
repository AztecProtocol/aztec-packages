...
export function createPublicTxSimulatorForBlockBuilding(
  ...
  if (dumpDir) {
    ...
    return new DumpingPublicTxSimulator(avmSimulator, globalVariables, dumpingConfig, dumpDir, bindings, wsdbForkId);
  }
  return new TelemetryPublicTxSimulator(avmSimulator, globalVariables, telemetryClient, config, bindings, wsdbForkId);
}
