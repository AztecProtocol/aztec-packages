...
describe('Public TX simulator apps tests: benchmarks', () => {
  ...
  it('AVM simulator bulk test', async () => {
    ...
    const result = await bulkTest(tester, logger, AvmTestContractArtifact, (b: boolean) => expect(b).toBe(true));
    expect(result.revertCode.isOK()).toBe(true);
  });

  it('AVM simulator MEGA bulk test', async () => {
    tester.setMetricsPrefix('AvmTest contract tests');
    const result = await megaBulkTest(tester, logger, AvmTestContractArtifact, (b: boolean) => expect(b).toBe(true));
    expect(result.revertCode.isOK()).toBe(true);
  });
