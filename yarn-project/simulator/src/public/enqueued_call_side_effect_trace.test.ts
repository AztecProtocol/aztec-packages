  ...
  describe('Maximum accesses', () => {
    it('Should enforce maximum number of public storage reads', () => {
      for (let i = 0; i < MAX_PUBLIC_DATA_READS_PER_TX; i++) {
        const leafPreimage = new PublicDataTreeLeafPreimage(AztecAddress.fromNumber(i), new Fr(i), Fr.ZERO, 0n);
        trace.tracePublicStorageRead(address, slot, leafPreimage, Fr.ZERO, []);
      }
      const leafPreimage = new PublicDataTreeLeafPreimage(AztecAddress.fromNumber(42), new Fr(42), Fr.ZERO, 0n);
    ...
    it('Should enforce maximum number of public storage writes', () => {
      for (let i = 0; i < MAX_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX; i++) {
        const lowLeafPreimage = new PublicDataTreeLeafPreimage(AztecAddress.fromNumber(i), new Fr(i), Fr.ZERO, 0n);
        const newLeafPreimage = new PublicDataTreeLeafPreimage(AztecAddress.fromNumber(i + 1), new Fr(i + 1), Fr.ZERO, 0n);
        trace.tracePublicStorageWrite(address, slot, lowLeafPreimage, Fr.ZERO, [], newLeafPreimage, []);
      }
      const leafPreimage = new PublicDataTreeLeafPreimage(new Fr(42), new Fr(42), Fr.ZERO, 0n);
      expect(() =>
        trace.tracePublicStorageWrite(AztecAddress.fromNumber(42), new Fr(42), leafPreimage, Fr.ZERO, [], leafPreimage, []),
