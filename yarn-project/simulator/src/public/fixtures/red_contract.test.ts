...
function createFDivSpamContract() {
  const preambleInstructions = [
    // SET at address 1 an 1_u32 constant.
    new Set(/*indirect*/ 0, /*dstOffset*/ 1, TypeTag.UINT32, /* value */ 1).as(Opcode.SET_8, Set.wireFormat8),
    // COPY from calldata starting at offset 1, size 1, to address 2.
    ...
    // SET the starting value of the iterator at address 3
    new Set(/*indirect*/ 0, /*dstOffset*/ 3, TypeTag.UINT32, /*value*/ 0).as(Opcode.SET_8, Set.wireFormat8),
    // SET a FF at address 42
    new Set(/*indirect*/ 0, /*dstOffset*/ 42, TypeTag.FIELD, /*value*/ Fr.random().toBigInt()).as(
      Opcode.SET_FF,
      Set.wireFormatFF,
    ),
    // SET a FF at address 43
    new Set(/*indirect*/ 0, /*dstOffset*/ 43, TypeTag.FIELD, /*value*/ Fr.random().toBigInt()).as(
      Opcode.SET_FF,
      Set.wireFormatFF,
    ),
  ];
  ...
  const loopBodyInstructions = new Array(2600).fill(
    new FieldDiv(/*indirect*/ 0, /*aOffset*/ 43, /*bOffset*/ 42, /*dstOffset*/ 201).as(
      Opcode.FDIV_8,
...
it('generates fdiv spam with loop', async () => {
  const base64 = createFDivSpamContract().toString('base64');
  await writeFile('fdiv_spam.bytecode', base64);
});

it('executes fdiv spam with loop', async () => {
  const deployer = AztecAddress.fromNumber(42);

  const simTester = await PublicTxSimulationTester.create();
  const spamBytecode = createFDivSpamContract();
