...
function avm_check_circuit_cmds {
  ...
  # Specify timeout and resources. Most e2e-dumped txs are small and check-circuit comfortably within
  # the default 30s / 2-CPU ISOLATE budget. Heavier scenarios need more and set their own prefix in the
  # per-input override below; add a case there for any new dump directory that starts timing out.
  ...
  for input_file in "$default_avm_inputs_dump_dir"/*/*.bin "$default_avm_inputs_dump_dir"/*/*/*.bin "$default_avm_inputs_dump_dir"/*/*/*/*.bin; do
    ...
    # Block-building scenarios pack many txs / max blob data into a single block, producing much larger
    # AVM circuits (multiple_blobs is ~700k rows) that overflow the default 30s / 2-CPU budget and flake
    # right at the boundary. Give them more cores (check-circuit parallelizes across rows) plus headroom.
    local input_prefix="$prefix"
    case "$test_dir" in
      single-node/block-building/*)
        input_prefix="$hash:ISOLATE=1:TIMEOUT=5m:CPUS=8:MEM=16g"
        ;;
    esac

    # Use full path from repo root for the command (parallelize runs from there)
    local input_path="$dump_dir_from_top/$rel_path"
    echo "$input_prefix:NAME=$name $bb_avm avm_check_circuit -v --avm-inputs $input_path"
  done
}
