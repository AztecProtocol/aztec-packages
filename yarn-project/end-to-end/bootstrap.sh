...
function avm_check_circuit_cmds {
  ...
  # Larger transactions (e.g. e2e_multiple_blobs) produce AVM circuits with hundreds of thousands of
  # rows whose check-circuit pass does not fit in the default 2-CPU / 30s budget. bb-avm parallelises
  # the check across threads, so give these commands more CPUs and a generous timeout. MEM defaults to
  # CPUS*4 GiB, and parallelize's --memsuspend keeps concurrent jobs from overcommitting host memory.
  local prefix="$hash:ISOLATE=1:CPUS=16:TIMEOUT=120s"
