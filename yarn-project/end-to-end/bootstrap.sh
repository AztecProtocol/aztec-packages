...
function avm_check_circuit_cmds {
  ...
  # Each check runs with the default per-test resources (CPUS=2) and 16 run in parallel, so the
  # heaviest transactions (the multiple_blobs scenarios produce the largest AVM circuits) sit right
  # at ~30s under contention and intermittently trip a tighter timeout, killing the whole run with
  # exit 124. Give generous headroom so only a genuinely stuck check trips the timeout.
