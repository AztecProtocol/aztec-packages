...
function avm_check_circuit_cmds {
  ...
  # Most e2e test txs produce small circuits that check in a few seconds on the default 2 CPUs.
  local default_prefix="$hash:ISOLATE=1:TIMEOUT=30s"
  # Blob-heavy txs (e.g. e2e_multiple_blobs) produce much larger circuits (~700k rows). On only 2
  # CPUs their trace generation alone exceeds the 30s default and check-circuit gets killed
  # mid-run (exit 124). Give these more CPUs (trace generation is the CPU-bound bottleneck) and a
  # generous timeout so they complete comfortably.
  local heavy_prefix="$hash:ISOLATE=1:TIMEOUT=180s:CPUS=8"
  ...
  for input_file in "$default_avm_inputs_dump_dir"/*/*.bin "$default_avm_inputs_dump_dir"/*/*/*.bin; do
    ...
    # Pick a resource profile based on the originating e2e test. Blob-heavy txs need more.
    local prefix=$default_prefix
    case "$test_dir" in
    e2e_multiple_blobs*) prefix=$heavy_prefix ;;
    esac
