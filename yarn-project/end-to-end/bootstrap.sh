...
function avm_check_circuit_cmds {
  ...
  # Most dumped e2e transactions are small, but block-building cases such as multiple_blobs generate
  # larger AVM circuits and can legitimately need more than the generic 30s smoke-test budget.
  local check_circuit_timeout=${AVM_CHECK_CIRCUIT_TIMEOUT:-120s}
  local prefix="$hash:ISOLATE=1:TIMEOUT=$check_circuit_timeout"
