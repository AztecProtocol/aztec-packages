...
function test_cmds {
  ...
  if ../../barretenberg/cpp/bootstrap.sh hash | grep -qE no-avm; then
    if [ "$CI_FULL" -eq 1 ]; then
      echo "$prefix:TIMEOUT=25m:CPUS=16:MEM=96g:NAME=e2e_prover_client_real $run_test_script simple e2e_prover/client"
    else
      echo "$prefix:NAME=e2e_prover_client_fake FAKE_PROOFS=1 $run_test_script simple e2e_prover/client"
    fi
  else
    if [ "$CI_FULL" -eq 1 ]; then
      echo "$prefix:TIMEOUT=25m:CPUS=16:MEM=96g:NAME=e2e_prover_full_real $run_test_script simple e2e_prover/full"
    else
