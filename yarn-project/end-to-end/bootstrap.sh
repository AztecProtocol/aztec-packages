...
function avm_check_circuit_cmds {
  ...
  # Specify timeout and resources. Some dumped e2e transactions now produce large
  # AVM circuits, so keep a bounded but overrideable timeout for check-circuit.
  local check_circuit_timeout="${AVM_CHECK_CIRCUIT_TIMEOUT:-2m}"
