...
function avm_check_circuit_cmds {
  ...
  # Multiple-blob transactions can produce larger AVM circuits, so leave enough headroom for the
  # circuit check while keeping the per-input timeout bounded.
  local prefix="$hash:ISOLATE=1:TIMEOUT=2m"
