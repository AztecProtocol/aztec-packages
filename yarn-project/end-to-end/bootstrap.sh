...
function avm_check_circuit_cmds {
  ...
  # In that case, they might start timing out. Most e2e test txs are small and run check-circuit in
  # a few seconds, but heavier txs (e.g. e2e_multiple_blobs) produce much larger circuits whose trace
  # generation plus check can take well over 30s on the default 2 CPUs, so the timeout is set with
  # ample headroom. The fast checks are unaffected by the larger timeout since they still exit early.
