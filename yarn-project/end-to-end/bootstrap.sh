...
function avm_check_circuit_cmds {
  ...
  # Specify timeout and resources. Some dumped inputs produce large AVM traces, so give each
  # check-circuit run enough CPU quota and wall time while keeping 16-way parallelism within CI capacity.
  local prefix="$hash:ISOLATE=1:CPUS=4:MEM=8g:TIMEOUT=60s"
