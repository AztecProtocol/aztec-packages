...
function avm_check_circuit_cmds {
  ...
  for input_file in "$default_avm_inputs_dump_dir"/*/*.bin "$default_avm_inputs_dump_dir"/*/*/*.bin; do
    ...
    # Multiple-blobs fixtures currently produce larger AVM traces than the default budget covers.
    if [[ "$test_dir" == "e2e_multiple_blobs" || "$test_dir" == e2e_multiple_blobs/* ]]; then
      timeout="2m"
    fi
