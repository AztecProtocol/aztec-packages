...
function avm_check_circuit_cmds {
  ...
  # Most e2e test txs check-circuit in well under 10s on the default (limited) resources, but
  # blob-heavy txs (e.g. e2e_multiple_blobs) build a much larger AVM trace and can exceed 30s.
  # This is a safety-net timeout, not a performance gate, so keep enough headroom for CI variance.
