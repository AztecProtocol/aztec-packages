...
function avm_check_circuit_cmds {
  ...
  # Larger dumped e2e transactions can spend more than 30s in trace generation/checking under
  # the default isolated 2 CPU budget, so keep a larger wall-clock cap while preserving resources.
  local avm_check_circuit_timeout="${AVM_CHECK_CIRCUIT_TIMEOUT:-90s}"
  local prefix="$hash:ISOLATE=1:TIMEOUT=$avm_check_circuit_timeout"
