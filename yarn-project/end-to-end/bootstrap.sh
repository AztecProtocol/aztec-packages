...
function avm_check_circuit_cmds {
  ...
  # Most e2e test txs are small (a few seconds of tracegen) but the occasional heavier tx can push
  # past a tight budget, so we give some headroom. Timeouts are additionally retried-and-flaked via
  # the "bb-avm avm_check_circuit" entry in .test_patterns.yml rather than hard-failing the build.
