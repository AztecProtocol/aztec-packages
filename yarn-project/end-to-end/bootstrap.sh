...
function avm_check_circuit_cmds {
  ...
  # Keep 16-way parallelism within the 64-core check-circuit runner while allowing larger dumped e2e
  # inputs enough wall time to complete under the docker CPU quota.
  local prefix="$hash:ISOLATE=1:CPUS=4:MEM=8g:TIMEOUT=90s"
