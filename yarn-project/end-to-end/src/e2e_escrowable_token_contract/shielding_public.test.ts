...
describe('e2e_escrowable_token_contract shield + redeem_shield', () => {
  ...
  it('on behalf of self', async () => {
    ...
    const receipt = await asset.methods
      .shield_from_public(wallets[0].getAddress(), amount, secretHash, 0)
      .send()
      .wait();
    ...
    await asset.methods
      .redeem_shield(wallets[0].getAddress(), amount, secret, toAddressOption(), toAddressOption())
      .send()
      .wait();
  ...
  it('on behalf of other', async () => {
    ...
    const action = asset
      ...
      .methods.shield_from_public(wallets[0].getAddress(), amount, secretHash, nonce);
  ...
  describe('failure cases', () => {
    it('on behalf of self (more than balance)', async () => {
      ...
      await expect(
        asset.methods.shield_from_public(wallets[0].getAddress(), amount, secretHash, 0).prove(),
    ...
    it('on behalf of self (invalid nonce)', async () => {
      ...
      await expect(
        asset.methods.shield_from_public(wallets[0].getAddress(), amount, secretHash, 1).prove(),
    ...
    it('shielding from blacklisted account', async () => {
      await expect(
        asset.withWallet(blacklisted).methods.shield_from_public(blacklisted.getAddress(), 1n, secretHash, 0).prove(),
