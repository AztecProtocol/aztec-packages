...
import { EscrowTokenContractTest } from './escrowable_token_contract_test.js';

describe('e2e_escrowable_token_contract transfer public', () => {
  const t = new EscrowTokenContractTest('transfer_public');
  ...
  describe('failure cases', () => {
    ...
    it('transfer from a blacklisted account', async () => {
      await expect(
        asset.methods.transfer_public(t.blacklisted.getAddress(), wallets[0].getAddress(), 1n, 0n).prove(),
      ).rejects.toThrow("Assertion failed: Blacklisted: Sender '!from_roles.is_blacklisted'");
    });

    it('transfer to a blacklisted account', async () => {
      await expect(
        asset.methods.transfer_public(wallets[0].getAddress(), t.blacklisted.getAddress(), 1n, 0n).prove(),
