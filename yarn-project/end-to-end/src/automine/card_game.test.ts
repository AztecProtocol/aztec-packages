...
describe('automine/card_game', () => {
  ...
  beforeAll(async () => {
    ...
    const context = (await AutomineTestContext.setup({ numberOfAccounts: 0, additionallyFundedAccounts: players }))
      .context;
  ...
  describe('game play', () => {
    ...
    async function joinGame(playerWallet: Wallet, playerAddress: AztecAddress, cards: Card[], id = GAME_ID) {
      await contract
    ...
    it('should play a game, claim the winned cards and play another match with winned cards', async () => {
      ...
      await Promise.all([
        joinGame(wallet, winner, winnerGameDeck, GAME_ID + 1),
