 ...
 */
export class FeesTest extends BaseEndToEndTest {
  ...
  private setupOptions: Partial<SetupOptions & DeployL1ContractsArgs>;
  ...
  constructor(
    ...
    super(testName, createLogger(`e2e:e2e_fees:${testName}`));
    if (!numberOfAccounts) {
  ...
  override async setup() {
    await super.setup(this.numberOfAccounts, this.setupOptions);
  ...
  public async setupBasicContracts() {
    ...
    await this.deployBananaToken();
  }

  async initializeAccounts() {
    // Accounts are already deployed by setup(), just initialize the addresses
  ...
  async publishAccountContracts() {
    await ensureAccountContractsPublished(this.wallet, this.accounts);
  }
  ...
  async setupFeeJuice() {
    ...
    this.feeJuiceBridgeTestHarness = await FeeJuicePortalTestingHarnessFactory.create({
      ...
      aztecNodeAdmin: this.aztecNodeAdmin!,
  ...
  public async setupSponsoredFPC() {
    const feeJuiceContract = this.feeJuiceBridgeTestHarness.feeJuice;
    expect((await this.wallet.getContractMetadata(feeJuiceContract.address)).isContractPublished).toBe(true);
  ...
  public async fundAliceWithPrivateBananas() {
    await this.mintPrivateBananas(this.ALICE_INITIAL_BANANAS, this.aliceAddress);
  }
}
