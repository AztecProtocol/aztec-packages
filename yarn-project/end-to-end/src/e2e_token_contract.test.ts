...
import { publicDeployAccounts, setup, setupFromState } from './fixtures/setup.js';
...
describe('e2e_token_contract', () => {
  ...
  async function thisSetup(statePath: string) {
    const setupResult = await setup(3, statePath, 'e2e_token_contract');
    ({ teardown, logger, wallets, accounts } = setupResult);
    const { customData, snapshot } = setupResult;

    logger(`Public deploy accounts...`);
    await publicDeployAccounts(wallets[0], accounts.slice(0, 2));

    logger(`Deploying TokenContract...`);
    ...
    customData.tokenContractAddress = asset.address;

    await snapshot();
  }

  async function thisSetupFromState(statePath: string) {
    const setupData = await setupFromState(statePath, 'e2e_token_contract');
    ({ teardown, logger, wallets, accounts } = setupData);
    asset = await TokenContract.at(setupData.customData.tokenContractAddress, wallets[0]);
  }

  beforeAll(async () => {
    const { STATE_PATH: statePath = './e2e_token_contract_state' } = process.env;
    existsSync(statePath) ? await thisSetupFromState(statePath) : await thisSetup(statePath);
  ...
  describe('Reading constants', () => {
    ...
    describe('name', () => {
      it('check name private', async () => {
        ...
        await reader.methods.check_name_private(asset.address, TOKEN_NAME).send().wait();
        await expect(reader.methods.check_name_private(asset.address, 'WRONG_NAME').simulate()).rejects.toThrow(
          'name.is_eq(_what)',
        );
      });
      ...
      it('check name public', async () => {
        ...
        await reader.methods.check_name_public(asset.address, TOKEN_NAME).send().wait();
        await expect(reader.methods.check_name_public(asset.address, 'WRONG_NAME').simulate()).rejects.toThrow(
          'name.is_eq(_what)',
        );
      });
    });
