...
describe('SharedMutablePrivateGetter', () => {
  ...
  describe('key registration flow', () => {
    let accountAddedToRegistry: AztecAddress;
    let masterNullifierPublicKey: Point;
    let masterIncomingViewingPublicKey: Point;
    let masterOutgoingViewingPublicKey: Point;
    let masterTaggingPublicKey: Point;
    let partialAddress: PartialAddress;

    it('should generate and register with original keys', async () => {
      partialAddress = new Fr(69);

      masterNullifierPublicKey = new Point(new Fr(1), new Fr(2));
      masterIncomingViewingPublicKey = new Point(new Fr(3), new Fr(4));
      masterOutgoingViewingPublicKey = new Point(new Fr(5), new Fr(6));
      masterTaggingPublicKey = new Point(new Fr(7), new Fr(8));
    ...
    it('checks our registry contract from test contract and fails because the address has not been registered yet', async () => {
      ...
      // Checks freshness of newly added keys, but the change hasn't been affected yet
      await expect(testContract.methods.test_nullifier_key_freshness(accountAddedToRegistry, masterNullifierPublicKey, keyRegistry.address).send().wait()).rejects.toThrow();
    });
    ...
    it('', async () => {
      ...
      await pxe.registerRecipient(CompleteAddress.create(accountAddedToRegistry, Point.ZERO, partialAddress), [
        ...
        masterOutgoingViewingPublicKey,
        masterTaggingPublicKey]);
      await testContract.methods.test_nullifier_key_freshness(accountAddedToRegistry, masterNullifierPublicKey, keyRegistry.address).send().wait();
    })
