...
import { type SetupOptions, createSchnorrAccount, setupSponsoredFPC } from '../../fixtures/utils.js';
...
export class ClientFlowsBenchmark {
  ...
  async createBenchmarkingAccountManager(wallet: TestWallet, type: 'ecdsar1' | 'schnorr') {
    ...
    if (type === 'schnorr') {
      ...
      return createSchnorrAccount(wallet, benchysSecret, salt, benchysPrivateSigningKey);
    } else if (type === 'ecdsar1') {
      benchysPrivateSigningKey = randomBytes(32);
      const accountData = {
        secret: benchysSecret,
        salt,
        contract: new EcdsaRAccountContract(benchysPrivateSigningKey),
      };
  ...
  public async getBridgedFeeJuicePaymentMethodForWallet(_wallet: Wallet, sender: AztecAddress) {
    const claim = await this.feeJuiceBridgeTestHarness.prepareTokensOnL1(FEE_FUNDING_FOR_TESTER_ACCOUNT, sender);
