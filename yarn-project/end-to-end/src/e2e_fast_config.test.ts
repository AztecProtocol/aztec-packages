 ...
 *
 * The config gives us:
 *   - aztecSlotDuration = 12s, ethereumSlotDuration = 4s, aztecEpochDuration = 32 (default)
 *   - one block per slot (blockDurationMs unset → single-block mode in the timetable)
 *   - pipelining on (build slot N-1, commit to slot N)
 *   - EpochTestSettler wired (testOnlyAutoProveAfterPublish = true), but with the default
 *     32-slot epoch the settler does not fire within this test's window
 ...
 *
 * Proven-tip advancement is intentionally not asserted here: it requires a shorter epoch, which
 * interacts badly with the cheat-code `markAsProven` path under pipelined publishing (see
 * `FAST_E2E_SETUP_OPTS` doc for details).
 */
