import { type AztecNode, type PXE, createAztecNodeClient } from '@aztec/aztec.js';
...
describe('upgrade via cli', () => {
  ...
  let MNEMONIC: string;
  beforeAll(async () => {
    MNEMONIC = config.L1_ACCOUNT_MNEMONIC ?? 'test test test test test test test test test test test junk';

    const { process: nodeRpcProcess, port: nodeRpcPort } = await startPortForwardForRPC(config.NAMESPACE);
    const { process: execProcess, port: execPort } = await startPortForwardForEthereum(config.NAMESPACE);

    forwardProcesses.push(nodeRpcProcess);
    forwardProcesses.push(execProcess);

    const PXE_URL = `http://127.0.0.1:${nodeRpcPort}`;
    ETHEREUM_HOSTS = `http://127.0.0.1:${execPort}`;

    aztecNode = createAztecNodeClient(PXE_URL);
    ({ pxe: _pxe, cleanup } = await startCompatiblePXE(PXE_URL, config.REAL_VERIFIER, debugLogger));
  ...
  it(
    'should be able to upgrade via cli',
    async () => {
      ...
      const isOwner = owner.toString().toLowerCase() === l1Client.account.address.toLowerCase();
      debugLogger.info(`Registry owner: ${owner.toString()}, caller: ${l1Client.account.address}, isOwner=${isOwner}`);
