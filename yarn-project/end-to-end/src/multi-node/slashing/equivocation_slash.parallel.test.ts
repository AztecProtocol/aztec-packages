...
// Equivocation slashing suites, merged from the former duplicate_proposal_slash and
// duplicate_attestation_slash files. Both run 4 validators on the in-memory mock-gossip bus via
// ValidatorRegistrationHarness with two malicious nodes that share a validator key (different
// coinbases), and assert the equivocating validator is detected for slashing. The setups are
// identical except the attestation case adds slashDuplicateAttestationPenalty and the malicious
// nodes attest to both proposals. `.parallel` so CI runs each as an independent job.
 ...
 */
describe('multi-node/slashing/equivocation_slash/duplicate_proposal', () => {
 ...
 * (v5 always enforces the timetable).
 */
describe('multi-node/slashing/equivocation_slash/duplicate_attestation', () => {
  let harness: ValidatorRegistrationHarness;
  let nodes: AztecNodeService[];
