...
import { AZTEC_SLOT_DURATION, aztecEpochDuration, baseHarnessOpts, slashingRoundSize } from './setup.js';
 ...
 */
describe('multi-node/slashing/equivocation/duplicate_proposal', () => {
  let harness: ValidatorRegistrationHarness;
  let nodes: AztecNodeService[];

  beforeEach(async () => {
    harness = await ValidatorRegistrationHarness.create({ ...baseHarnessOpts });
  });
  ...
  it('slashes validator who sends duplicate proposals', async () => {
    ...
    const maliciousValidatorAddress = harness.addressAt(maliciousValidatorIndex);

    harness.logger.warn(`Malicious proposer address: ${maliciousValidatorAddress.toString()}`);
    ...
    harness.logger.warn(`Creating malicious node 1 with coinbase ${coinbase1.toString()}`);
    const maliciousNode1 = await harness.createValidatorNode(maliciousValidatorIndex, {
      coinbase: coinbase1,
    ...
    harness.logger.warn(`Creating malicious node 2 with coinbase ${coinbase2.toString()}`);
    const maliciousNode2 = await harness.createValidatorNode(maliciousValidatorIndex, {
      coinbase: coinbase2,
      broadcastEquivocatedProposals: true,
      dontStartSequencer: true,
      // Prevent HA peer proposals from being added to the archiver, so both
    ...
    harness.logger.warn('Waiting for duplicate proposal offense to be detected...');
    ...
    harness.logger.warn(`Collected duplicate proposal offenses`, { proposalOffenses });
    ...
    for (const offense of proposalOffenses) {
      ...
      harness.logger.info(`Offense slot ${offenseSlot}: proposer is ${proposerForSlot?.toString()}`);
      expect(proposerForSlot?.toString()).toEqual(maliciousValidatorAddress.toString());
    }

    harness.logger.warn('Duplicate proposal offense correctly detected and recorded');
  });
});
