...
describe('e2e_state_vars', () => {
  ...
  describe('SharedImmutable', () => {
    ...
    //   const tx = contract.methods.test_dynamic_array().send();
    ...
    //   await contract.methods.test_dynamic_array_2().send().wait();

    //   const numResponse = await contract.methods.test_dynamic_array_3().view();
    //   const lengthResponse = await contract.methods.test_dynamic_array_4().view();

    //   expect([numResponse, lengthResponse]).toStrictEqual([420n, 1n]);
    // });

    it('private read of uninitialized SharedImmutable', async () => {
      const tx = contract.methods.test_dynamic_array_priv().send();
      const extendedLogs = await tx.getUnencryptedLogs();
      
      const cardNote = await contract.methods.test_dynamic_array_priv_view().view();
    });

    // it('private read of uninitialized SharedImmutable', async () => {
    //   await contract.methods.test_fixed_array().send().wait();
    //   const tx = await contract.methods.test_fixed_array_2().send();
