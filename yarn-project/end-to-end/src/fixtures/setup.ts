...
export async function setup(
  ...
  try {
    ...
    // mined a few slots later. Observed worst case in CI: fee evolved ~20x between PXE snapshot and
    // inclusion, exceeding even LARGE_MIN_FEE_PADDING (15x). Use a wider 30x default under pipelining.
    const PIPELINED_FEE_PADDING = 30;
    const effectiveWalletMinFeePadding =
      opts.walletMinFeePadding ?? (config.enableProposerPipelining ? PIPELINED_FEE_PADDING : undefined);
