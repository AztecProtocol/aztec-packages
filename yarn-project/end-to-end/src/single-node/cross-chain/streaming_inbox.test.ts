...
describe('single-node/cross-chain/streaming_inbox', () => {
  ...
  // succeed. The block builder appends the block's bundle to its fork before executing txs, matching the prover
  // and the block-root circuit (which pins each tx's L1-to-L2 tree snapshot to the post-bundle root); if it did
  // not, this tx would revert at proposal time and succeed at proving time, making the epoch unprovable. The node
  // simulates public calls against the predicted next-block bundle, so a consume tx sent as soon as the message's
  // bucket becomes lag-eligible passes simulation and lands in the pool before the inserting block is built. The
  // send is timed to that instant and retried with fresh messages when a block slips in between.
  it('consumes a message in the same block that inserts it', async () => {
    ...
    for (let attempt = 0; attempt < maxAttempts && sameBlockReceipt === undefined; attempt++) {
      ...
      // Same block reached: the consume must have succeeded against the block's own bundle.
