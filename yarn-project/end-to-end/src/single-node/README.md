...
| `follower/` | Follower mode: a second, read-only node that replicates the chain from the first over HTTP JSON-RPC with no L1 connection and no p2p, and forwards the transactions it receives to it. `follower_node` (replication of blocks/checkpoints/proven tip/world state, a PXE attached to the follower deploying and sending through it, upstream rejection propagation, following an upstream `rollbackTo`, and the startup guards). `setup.ts` holds the upstream HTTP-RPC server and the follower-node factory. |
