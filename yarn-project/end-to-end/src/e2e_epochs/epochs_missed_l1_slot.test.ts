...
//                → checkSync passes
//          ─→ PROPOSER_CHECK(slot=N+1)
//          ─→ INITIALIZING_CHECKPOINT(target=N+2)  ← TEST WAITS
//          ─→ ... ─→ PUBLISHING_CHECKPOINT(target=N+2)
//                ✗ gated on parent CP_N+1 landing — only after RESUME
//
// Test signal: state-changed with newState=INITIALIZING_CHECKPOINT && slot=N+2.
//   - targetSlot=N+2 is unique to the bug-fix cycle (each cycle has a distinct target).
//   - INITIALIZING_CHECKPOINT is set in the foreground job before the pipeline parent-on-L1
//     gate, so it is reachable during the L1 pause (PUBLISHING_CHECKPOINT for target=N+2 is
//     not — it waits for parent CP_N+1, whose own L1 submission is blocked by the pause).
describe('e2e_epochs/epochs_missed_l1_slot', () => {
  ...
  it('builds a block after missed L1 slots when previous checkpoint is synced', async () => {
    ...
    // (targeting slot N+2 under pipelining) to pass `checkSync(slot=N+1)` and enter the
    // checkpoint proposal job. We wait for `state-changed` with
    // `newState=INITIALIZING_CHECKPOINT && slot=N+2`: the cycle's `targetSlot` is unique to it,
    // and INITIALIZING_CHECKPOINT is set in the foreground job before the pipeline parent-on-L1
    // gate, so it is reachable during the pause (PUBLISHING_CHECKPOINT for `slot=N+2` is not —
    // it's gated on parent CP_N+1 landing on L1, which is blocked by the pause).
