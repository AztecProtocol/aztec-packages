...
describe('e2e_epochs/epochs_optimistic_proving', () => {
  ...
  /**
   * Background sampler: at each tick, reads the wall-clock epoch and looks up the
   * in-flight job for *that exact* epoch. If the job has at least one checkpoint
   * registered, records the epoch as observed mid-epoch. Returns a stop function
   * that resolves the set of epochs observed.
   *
   * Catches the per-epoch claim "the prover-node started processing this epoch
   * before it ended on L1" without race conditions: both observations happen at
   * the same moment in the same callback.
   */
  const startMidEpochCheckpointSampler = (proverNode: TestProverNode) => {
  ...
  describe('happy path', () => {
    ...
    it('proves an epoch via checkpoint-driven flow', async () => {
      ...
      const stopSampler = startMidEpochCheckpointSampler(proverNode);
      ...
      // The prover-node started processing epoch 0 before it ended on L1.
      const observed = await stopSampler();
      logger.info(`Mid-epoch checkpoint observations: [${[...observed].sort().join(', ')}]`);
    ...
    it('proves multiple epochs via checkpoint-driven flow', async () => {
      ...
      // Every epoch the prover-node successfully proved should have had at least one
      // checkpoint registered with its in-flight job while the wall clock was still
      // inside the epoch — i.e. proving started mid-epoch, not after.
      const observed = await stopSampler();
      logger.info(`Mid-epoch checkpoint observations: [${[...observed].sort().join(', ')}]`);
      for (let epoch = 0; epoch < numEpochs; epoch++) {
        expect(observed.has(epoch)).toBe(true);
      }
    });
  });
