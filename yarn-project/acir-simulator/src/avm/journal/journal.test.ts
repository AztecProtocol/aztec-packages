...
describe('journal', () => {
  ...
  it('Should merge two successful journals together', async () => {
    ...
    const nestedJournal = new AvmWorldStateJournal(journal.hostStorage, journal);
    nestedJournal.writeStorage(contractAddress, key, valueT1);
    await nestedJournal.readStorage(contractAddress, key);
    nestedJournal.writeNoteHash(commitmentT1);
    nestedJournal.writeLog(logsT1);
    nestedJournal.writeL1Message(logsT1);
    nestedJournal.writeNullifier(commitmentT1);

    journal.acceptNestedWorldState(nestedJournal);
  ...
  it('Should merge failed journals together', async () => {
    ...
    journal.rejectNestedWorldState(nestedJournal);
  ...
  it('Can merge child journal', () => {
    const parentJournal = AvmWorldStateJournal.rootJournal(journal.hostStorage);
    const childJournal = AvmWorldStateJournal.branchParent(parentJournal);

    expect(() => parentJournal.acceptNestedWorldState(childJournal));
    expect(() => parentJournal.rejectNestedWorldState(childJournal));
  });
});
