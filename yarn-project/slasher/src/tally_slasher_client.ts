 ...
 * - Votes are encoded as bytes where the i-th nibble (4 bits) represents the slash amount (0-15 slash units)
 * for the i-th validator in the committee being slashed.
...
export class TallySlasherClient implements ProposerSlashActionProvider, SlasherClientInterface {
  ...
  protected async getExecuteSlashAction(slotNumber: bigint): Promise<ProposerSlashAction | undefined> {
    ...
    const slashingOffsetInRounds = BigInt(this.settings.slashingOffsetInRounds);
    const roundToCheck = currentRound - slashingExecutionDelayInRounds;
    ...
    try {
      ...
      if (slashActions.length === 0) {
        ...
        const committees = await this.collectCommitteesActiveDuringRound(roundToCheck - slashingOffsetInRounds);
