...
import { BLOBS_PER_CHECKPOINT, DA_GAS_PER_FIELD, FIELDS_PER_BLOB, MAX_PROCESSABLE_DA_GAS_PER_CHECKPOINT } from '@aztec/constants';
...
describe('CheckpointBuilder', () => {
  ...
  function setupBuilder(overrides: Partial<FullNodeBlockBuilderConfig> = {}) {
    config = {
      l1GenesisTime: 0n,
      slotDuration: 24,
      l1ChainId: 1,
  ...
  function createMockBlock(opts: {
    manaUsed: number;
    txBlobFields: number[];
    blockBlobFieldCount: number;
  }): L2Block {
    const { manaUsed, txBlobFields, blockBlobFieldCount } = opts;
    const block = {
      header: {
        totalManaUsed: { toNumber: () => manaUsed },
      },
      body: {
        txEffects: txBlobFields.map(() => ({})),
      },
      computeDAGasUsed: () => txBlobFields.reduce((acc, f) => acc + f * DA_GAS_PER_FIELD, 0),
      toBlobFields: () => new Array(blockBlobFieldCount).fill(Fr.ZERO),
