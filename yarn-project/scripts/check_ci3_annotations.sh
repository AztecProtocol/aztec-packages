...
# and that all existing annotations use valid syntax.
#
...
# ──────────────────────────────────────────────────────────────────────────────
# 1. Packages whose tests MUST have "// <ci3 isolate ... />" on line 1.
...
for test in !(end-to-end|kv-store|aztec)/src/**/*.test.ts; do
  ...
  # Check 1: Files in isolate-required packages must have "isolate" in annotation.
  if [[ "$test" =~ $isolate_regex ]]; then
    if [[ ! "$first_line" =~ ^//\ \<ci3\ .*isolate ]]; then
      echo "ERROR: $test — requires '// <ci3 isolate ... />' (package needs docker isolation)"
      ((errors++))
    fi
  fi

  # Check 2: Any file with a ci3 annotation must use valid syntax.
  if [[ "$first_line" =~ ^//\ \<ci3\  ]]; then
    if [[ ! "$first_line" =~ ^//\ \<ci3\ .+\ /\>$ ]]; then
      echo "ERROR: $test — malformed annotation (must end with ' />'): $first_line"
      ((errors++))
    fi
  fi
done
