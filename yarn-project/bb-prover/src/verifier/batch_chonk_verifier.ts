...
export class BatchChonkVerifier implements ClientProtocolCircuitVerifier {
   ...
   */
  static async new(config: BBConfig, batchSize = 8, label = 'verifier'): Promise<BatchChonkVerifier> {
    const verifier = new BatchChonkVerifier(config, batchSize, label);
  ...
  public verifyProof(tx: Tx): Promise<IVCProofVerificationResult> {
    ...
    // Catch send errors to avoid dangling promises if the bb process dies.
    void this.sendQueue
      .put(async () => {
