...
const logger = createLogger('ethereum:test:fees-per-gas-ladder');
...
describe('L1TxUtils fees-per-gas ladder capture (integration)', () => {
  ...
  it('captures the escalating fees-per-gas ladder and surfaces it on timeout', async () => {
    ...
    expect(txState.feesPerGasHistory).toBeDefined();
    expect(txState.feesPerGasHistory!.length).toBeGreaterThanOrEqual(2);

    // Priority fees strictly escalate across the ladder.
    const priorityFees = txState.feesPerGasHistory!.map(g => g.maxPriorityFeePerGas);
    ...
    expect(txState.attempts).toBe(txState.feesPerGasHistory!.length);
    expect(txState.finalFeesPerGas.maxPriorityFeePerGas).toBe(priorityFees[priorityFees.length - 1]);
