...
export const deployL1Contracts = async (
  ...
  if (args.initialValidators && args.initialValidators.length > 0) {
    ...
    if (newValidatorsAddresses.length > 0) {
      // Mint tokens and approve them for the rollup contract
      const stakeNeeded = args.minimumStake * BigInt(newValidatorsAddresses.length);
      await Promise.all(
        [
          await stakingAsset.write.mint([governanceAddress.toString(), stakeNeeded], {} as any),
          await stakingAsset.write.approve([rollupAddress.toString(), stakeNeeded], {} as any),
        ].map(txHash => publicClient.waitForTransactionReceipt({ hash: txHash })),
      );

      // Регистрируем каждого валидатора через стандартный процесс депозита
      for (const validatorAddress of newValidatorsAddresses) {
        const forwarderAddress = getExpectedAddress(ForwarderAbi, ForwarderBytecode, [validatorAddress], validatorAddress).address;
        await rollup.write.deposit([
          validatorAddress,
          forwarderAddress,
          validatorAddress, // withdrawer = validator
          args.minimumStake
        ]);
      }
    }
