...
export interface DeployL1ContractsArgs extends Omit<L1ContractsConfig, keyof L1TxUtilsConfig> {
  ...
  /** Whether to only add validators on the first rollup deployment */
...
export const deploySharedContracts = async (
  ...
  const { address: gseAddress } = await deployer.deploy(GSEArtifact, [
    l1Client.account.address,
...
export const deployRollup = async (
  ...
  logger.verbose(`Deployed Rollup at ${rollupAddress}, existed ${rollupExisted}`, rollupConfigArgs);
  ...
  let shouldAddValidators = true;
  if (rollupExisted && args.onlyAddValidatorsOnFirstDeploy) {
    logger.info('Skipping adding initial validators, rollup existed and onlyAddValidatorsOnFirstDeploy is set');
