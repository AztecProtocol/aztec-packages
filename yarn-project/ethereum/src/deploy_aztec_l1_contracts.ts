...
export async function deployAztecL1Contracts(
  ...
  // Use --slow for anvil to avoid race condition in forge where PendingTransactionBuilder
  // watchers miss already-mined transactions, causing forge to hang waiting for receipts.
  const forgeArgs = [
    'script',
    FORGE_SCRIPT,
    '--sig',
