...
export class GovernanceContract {
  private readonly publicGovernance: GetContractReturnType<typeof GovernanceAbi, L1Clients['publicClient']>;
  private readonly walletGovernance: GetContractReturnType<typeof GovernanceAbi, L1Clients['walletClient']> | undefined;
  ...
  public async proposeWithLock({ payloadAddress, withdrawAddress }: { payloadAddress: Hex; withdrawAddress: Hex }) {
    const walletGovernance = this.assertWalletGovernance();
    const proposeTx = await walletGovernance.write.proposeWithLock([payloadAddress, withdrawAddress]);
    return this.publicClient.waitForTransactionReceipt({ hash: proposeTx });
  }
