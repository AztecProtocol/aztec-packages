...
export class L2BlockStream {
  ...
  protected async work() {
    try {
      ...
      //
      // Seed the cache with ONLY the proposed tip. The tier tips (checkpointed/proven/finalized) must NOT be seeded:
      // they are all <= proposed, i.e. at heights the walk-back visits, and a snapshot tier entry that went stale (the
      // source reorged between this getL2Tips and the walk) equals the local OLD-fork hash at a reorged height, faking
      // agreement and stopping the walk ABOVE the true divergence — an under-deep prune that no later pass re-detects.
      // The proposed seed's staleness mode is benign: a stale proposed entry can only mask divergence at the tip height
      // for one pass (equivalent to the pass having run a moment earlier). Fresh entries appended after the
      // post-divergence re-read are safe.
      ...
      // Whether the download plan completed and the tier cursors may advance from this snapshot. This is meant to
      // catch a prune while we're downloading blocks: if that happens, the source will not be able to serve the blocks
      // we expected to get and won't reach the proposed chain tip. If that happened, we cannot emit chain checkpointed
      // and proven events with the stale tips, since those may have been pruned as well. And that's bad, because we may
      // be emitting chain checkpointed or proven events for blocks that the consumer never saw. In tips-only mode there
      // is no download plan: the snapshot is a single (atomic) getL2Tips read, so it always reconciles.
