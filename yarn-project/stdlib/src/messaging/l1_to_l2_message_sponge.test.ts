...
describe('L1ToL2MessageSponge', () => {
  ...
  // The sponge absorbs leaves and bucket-start flags only, so the timestamps carried alongside are irrelevant here.
  const bucket = (leaves: Fr[], timestamp = 1000n) => ({ timestamp, leaves });
  ...
  it('reaches the same state when a bundle is split at a bucket boundary', async () => {
    const whole: InboxMessageBundle = [bucket(leaves.slice(0, 3)), bucket(leaves.slice(3))];
  ...
  it('diverges when a bundle is split inside a bucket', async () => {
    ...
    const whole: InboxMessageBundle = [bucket(leaves)];
  ...
  it('commits to the bucket grouping, not just the leaf order', async () => {
    const oneBucket = await absorbed([bucket(leaves)]);
    const twoBuckets = await absorbed([bucket(leaves.slice(0, 2)), bucket(leaves.slice(2))]);
  ...
  it('absorbs one field per leaf with the flag packed above it', async () => {
    ...
    const bundle: InboxMessageBundle = [bucket(leaves.slice(0, 2)), bucket(leaves.slice(2, 3))];
