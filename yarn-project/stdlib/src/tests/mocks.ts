...
export const mockTx = async (
  ...
  data.constants.txContext.gasSettings = GasSettings.withMaxLimits({ gasLimits, maxFeesPerGas, maxPriorityFeesPerGas });
...
export async function mockProcessedTx({
  ...
  chainId = Fr.ZERO,
  version = Fr.ZERO,
  gasSettings = GasSettings.withMaxLimits({ maxFeesPerGas: new GasFees(10, 10) }),
  vkTreeRoot = Fr.ZERO,
