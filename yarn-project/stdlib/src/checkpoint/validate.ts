...
export function validateCheckpointStructure(
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (i > 0) {
      // Only `block_root_empty_tx_first_rollup` can produce a zero-tx block, and it is pinned to index 0 of a
      // checkpoint: it starts a fresh sponge blob and carries a non-zero in_hash, both of which the block-merge
      // continuity checks forbid at any later index. A zero-tx block here would therefore be accepted by every
      // node-side check and then be unprovable by any approved circuit, stalling the epoch until it is pruned.
