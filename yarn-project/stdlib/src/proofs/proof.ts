...
export class Proof {
  ...
  public withoutPublicInputs(): Buffer {
    ...
    assert(this.numPublicInputs >= PAIRING_POINT_ACCUMULATOR_SIZE, 'Proof does not contain an aggregation object');
    const proofStart = Fr.SIZE_IN_BYTES * (this.numPublicInputs - PAIRING_POINT_ACCUMULATOR_SIZE);
  ...
  public extractPublicInputs(): Fr[] {
    ...
    assert(this.numPublicInputs >= PAIRING_POINT_ACCUMULATOR_SIZE, 'Proof does not contain an aggregation object');
    const numInnerPublicInputs = this.numPublicInputs - PAIRING_POINT_ACCUMULATOR_SIZE;
