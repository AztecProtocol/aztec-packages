...
export class VerificationKey {
  ...
  /**
   * Builds a zero-filled MegaHonk verification key buffer of the correct size for the current
   * Mega flavor. Use when tests need a placeholder VK that passes size validation but don't
   * depend on the VK being semantically valid.
   */
  static makeFakeMegaHonk(): Buffer {
    return Buffer.alloc(MEGA_VK_LENGTH_IN_FIELDS * Fr.SIZE_IN_BYTES);
  }
}
