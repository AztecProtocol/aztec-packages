...
export class ConsensusPayload implements Signable {
  ...
  static get schema() {
    return z
      ...
      .transform(obj => new ConsensusPayload(obj.header, obj.stateReference));
  }

  static getFields(fields: FieldsOf<ConsensusPayload>) {
    return [fields.header, fields.stateReference] as const;
  }
  ...
  getPayloadToSign(domainSeparator: SignatureDomainSeparator): Buffer {
    ...
    const encodedData = encodeAbiParameters(abi, [
      domainSeparator,
      [stateReference, [0n] /* @todo See #9963 */, headerHash],
    ] as const);
  ...
  toBuffer(): Buffer {
    const buffer = serializeToBuffer([this.header, this.stateReference]);
  ...
  static fromBuffer(buf: Buffer | BufferReader): ConsensusPayload {
    ...
    return new ConsensusPayload(reader.readObject(ProposedBlockHeader), reader.readObject(StateReference));
  }

  static fromFields(fields: FieldsOf<ConsensusPayload>): ConsensusPayload {
    return new ConsensusPayload(fields.header, fields.stateReference);
  }

  static fromBlock(block: L2Block): ConsensusPayload {
    return new ConsensusPayload(block.header.toPropose(), block.header.state);
  }

  static empty(): ConsensusPayload {
    return new ConsensusPayload(ProposedBlockHeader.empty(), StateReference.empty());
  }
  ...
  toString() {
    return `header: ${this.header.toString()}, stateReference: ${this.stateReference.l1ToL2MessageTree.root.toString()}`;
  }
}
