...
class BackwardsCompatibleBlockProposal extends BlockProposal {
  constructor(blockNumber: Fr, payload: ConsensusPayload, signature: Signature) {
    super(blockNumber, payload, signature, undefined);
  }
...
describe('Block Proposal serialization / deserialization', () => {
  ...
  it('Should serialize / deserialize with or without included txs', async () => {
    ...
    const oldProposal = new BackwardsCompatibleBlockProposal(
      proposalWithTxs.blockNumber,
      proposalWithTxs.payload,
      proposalWithTxs.signature,
    );
