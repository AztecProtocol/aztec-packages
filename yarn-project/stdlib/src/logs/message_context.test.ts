...
describe('MessageContext', () => {
  it('serialization matches snapshots and output of Noir serialization', () => {
    ...
    // The serialization should match the MessageTxContext format (same fields, same order):
    // txHash, BoundedVec(uniqueNoteHashesInTx), firstNullifierInTx
    expect(serialized.length).toBe(1 + 64 + 1 + 1); // txHash + MAX_NOTE_HASHES_PER_TX storage + len + firstNullifier
    expect(serialized[0]).toEqual(new Fr(123n)); // txHash
    expect(serialized[serialized.length - 1]).toEqual(new Fr(6n)); // firstNullifier
