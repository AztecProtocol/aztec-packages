...
export class AccountContract implements AccountImplementation {
  ...
  async authenticateTx(executions: ExecutionRequest[], txContext: TxContext): Promise<TxExecutionRequest> {
    ...
    const functionPayload = await this.authProvider.authenticateTx(payload, hash, this.address);
    const abi = this.getEntrypointAbi();

    const selector = generateFunctionSelector(abi.name, abi.parameters);
    const txRequest = TxExecutionRequest.from({
      args: functionPayload.toFields(),
      origin: this.address,
