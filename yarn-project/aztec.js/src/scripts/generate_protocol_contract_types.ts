...
async function generateProtocolContractInterface(
  ...
  // Generate a logger name from the contract name (e.g., AuthRegistry -> auth_registry)
  const loggerName = input.name.replace(/([a-z])([A-Z])/g, '$1_$2').toLowerCase();
...
export class ${contractName} extends ContractBase {
  private static log = createLogger('aztecjs:${loggerName}');

  private constructor(wallet: Wallet) {
    super(ProtocolContractAddress.${protocolContractName}, ${contractName}Artifact, wallet, ${contractName}.log);
  }
