...
export type WorldStateQueueMetadata = {
  queueWaitMs: number;
  blockedByInFlight: number;
  blockedByInFlightMutating: number;
  blockedByOps: string[];
};
...
export class WorldStateOpsQueue {
  ...
  public async execute(
  ...
  ): Promise<{ result: any; queueMeta: WorldStateQueueMetadata }> {
    ...
    const result = await op.promise.promise;
    return {
      result,
      queueMeta: {
        queueWaitMs: op.queueWaitMs,
        blockedByInFlight: op.queuedInFlight ?? 0,
        blockedByInFlightMutating: op.queuedInFlightMutating ?? 0,
        blockedByOps: op.queuedBlockedByOps ?? [],
      },
    };
  }

  private getInFlightOpNames(): string[] {
    return [...this.ops.values()]
      .filter(o => o.requestId !== undefined && !this.requests.includes(o))
      .map(o => WorldStateMessageType[o.messageType]);
  }
  ...
  private executeMutating(op: WorldStateOp) {
    ...
    if (this.inFlightCount === 0) {
      ...
      op.queuedBlockedByOps = this.getInFlightOpNames();
  ...
  private sendEnqueuedRequest(op: WorldStateOp) {
    if (op.queuedInFlight !== undefined) {
      op.queueWaitMs = Date.now() - op.enqueuedAt;
      this.log?.debug(`Sending op ${WorldStateMessageType[op.messageType]} after ${op.queueWaitMs}ms in queue`, {
        waitMs: op.queueWaitMs,
