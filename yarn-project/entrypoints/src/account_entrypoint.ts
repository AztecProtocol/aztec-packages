...
import { type EntrypointInterface, EntrypointPayload, type FeeOptions } from '@aztec/aztec.js/entrypoint';
...
export class DefaultAccountEntrypoint implements EntrypointInterface {
  ...
  async createTxExecutionRequest(executions: FunctionCall[], feeOpts?: FeeOptions): Promise<TxExecutionRequest> {
    const appPayload = EntrypointPayload.fromAppExecution(executions);
    const feePayload = await EntrypointPayload.fromFeeOptions(feeOpts);
