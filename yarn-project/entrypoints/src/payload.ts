...
export abstract class ExecutionPayload {
  ...
  static empty(): ExecutionPayload {
    return { calls: [], authWitnesses: [], capsules: [] };
  }

  public static async encodeCalls(
    calls: FunctionCall[],
  ): Promise<Pick<EncodedExecutionPayload, 'encodedFunctionCalls' | 'hashedArguments'>> {
  ...
  static async fromFunctionCalls(functionCalls: FunctionCall[]) {
    const { encodedFunctionCalls, hashedArguments } = await this.encodeCalls(functionCalls);
  ...
  static async fromAppExecution(functionCalls: FunctionCall[] | Tuple<FunctionCall, 4>, nonce = Fr.random()) {
    ...
    const { encodedFunctionCalls, hashedArguments } = await this.encodeCalls(paddedCalls);
...
export abstract class HashableExecutionPayload implements EncodedExecutionPayload {
  constructor(
...
export class AppEntrypointPayload extends HashableExecutionPayload {
  constructor(
    encodedFunctionCalls: EncodedFunctionCall[],
    hashedArguments: HashedValues[],
...
export class FeeEntrypointPayload extends HashableExecutionPayload {
  #isFeePayer: boolean;
