const { z } = require('zod');

// Simulate the ProvingRequestType enum
const ProvingRequestType = {
  PUBLIC_VM: 0,
  PRIVATE_BASE_ROLLUP: 1,
  PUBLIC_BASE_ROLLUP: 2,
  MERGE_ROLLUP: 3,
  EMPTY_BLOCK_ROOT_ROLLUP: 4,
  BLOCK_ROOT_ROLLUP: 5,
  SINGLE_TX_BLOCK_ROOT_ROLLUP: 6,
  BLOCK_MERGE_ROLLUP: 7,
  ROOT_ROLLUP: 8,
  BASE_PARITY: 9,
  ROOT_PARITY: 10,
  TUBE_PROOF: 11,
};

// Create the schema like in the code
const ProvingJob = z.object({
  id: z.string(),
  type: z.nativeEnum(ProvingRequestType),
  epochNumber: z.number(),
  inputsUri: z.string(),
});

// Create a test job
const testJob = {
  id: 'test-job-id',
  type: ProvingRequestType.BASE_PARITY, // This is 9
  epochNumber: 1,
  inputsUri: 'test-uri',
};

console.log('Original job:', testJob);

// Serialize to JSON
const jsonStr = JSON.stringify(testJob);
console.log('Serialized JSON:', jsonStr);

// Parse back to object
const parsedObj = JSON.parse(jsonStr);
console.log('Parsed object:', parsedObj);

// Try to validate with schema
try {
  const result = ProvingJob.parse(parsedObj);
  console.log('Schema validation successful:', result);
} catch (error) {
  console.log('Schema validation failed:', error.message);
  console.log('Parsed type value:', parsedObj.type, 'typeof:', typeof parsedObj.type);
  console.log('Expected enum values:', Object.values(ProvingRequestType));
}
