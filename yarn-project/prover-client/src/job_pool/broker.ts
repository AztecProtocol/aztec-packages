...
export class ProofRequestBroker implements ProofRequestProducer, ProofRequestConsumer {
  ...
  reportProgress<T extends ProofType[]>(
    ...
    if (metadata) {
      ...
      this.logger.warn(`Proof request id=${id} not found in the in-progress pool. Sending new one`);
      return this.getProofRequest(filter);
    } else {
      this.logger.warn(`Proof request id=${id} not found in the in-progress pool`);
      return Promise.resolve(undefined);
    }
  }
