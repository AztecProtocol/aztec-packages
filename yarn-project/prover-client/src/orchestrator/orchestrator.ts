...
export class ProvingOrchestrator {
  ...
  private async runBaseRollup(provingState: ProvingState | undefined, index: bigint, tx: TxProvingState) {
    if (!tx.baseRollupInputs.kernelData.publicInputs.end.encryptedLogsHash.toBuffer().equals(tx.processedTx.encryptedLogs.hash())) {
      throw new Error(
        `Encrypted logs hash mismatch: ${tx.baseRollupInputs.kernelData.publicInputs.end.encryptedLogsHash} === ${Fr.fromBuffer(
          tx.processedTx.encryptedLogs.hash(),
        )}`,
      );
    }
    if (!tx.baseRollupInputs.kernelData.publicInputs.end.unencryptedLogsHash.toBuffer().equals(tx.processedTx.unencryptedLogs.hash())) {
      throw new Error(
        `Unencrypted logs hash mismatch: ${tx.baseRollupInputs.kernelData.publicInputs.end.unencryptedLogsHash} === ${Fr.fromBuffer(
