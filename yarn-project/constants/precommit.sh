...
if git diff --cached --name-only | grep -Fxq "$FILE_TO_WATCH"; then
    ...
    echo ""
    echo "Depending on the constants you've changed, these might include: ConstantsGen.sol, constants_gen.pil, aztec_constants.hpp."
    echo "(constants.gen.ts is generated at build time and needs no manual step.)"
    echo ""
