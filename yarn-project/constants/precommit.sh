...
if git diff --cached --name-only | grep -Fxq "$FILE_TO_WATCH"; then
    ...
    echo ""
    echo "Depending on the constants you've changed, these might include: constants_gen.pil, aztec_constants.hpp."
    echo "(constants.gen.ts and ConstantsGen.sol are generated at build time and need no manual step.)"
    echo ""
