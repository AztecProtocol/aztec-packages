...
if git diff --cached --name-only | grep -Fxq "$FILE_TO_WATCH"; then
    ...
    echo ""
    echo "Depending on the constants you've changed, this might include: aztec_constants.hpp."
    echo "(constants.gen.ts and ConstantsGen.sol regenerate at build time; constants_gen.pil regenerates when"
    echo "running barretenberg/cpp/scripts/avm2_gen.sh. None of those need a manual step.)"
    echo ""
    echo "Regenerate the remaining owned output with:"
