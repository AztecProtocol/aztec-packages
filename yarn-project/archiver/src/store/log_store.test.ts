...
async function addProposedBlocks(blockStore: BlockStore, blocks: L2Block[]): Promise<void> {
...
describe('LogStore', () => {
  ...
  describe('getPrivateLogsByTags', () => {
    ...
    it('returns logs in canonical (block, txIndex, logIndex) order for a known tag', async () => {
      ...
      for (let b = 1; b <= 3; b++) {
        const ckpt = await makeCheckpointWithLogs(b, {
          numTxsPerBlock: 2,
        ...
        blocks.push(ckpt.checkpoint.blocks[0]);
      }
      await addProposedBlocks(blockStore, blocks);
    ...
    it('respects fromBlock / toBlock', async () => {
      ...
      const blocks: L2Block[] = [];
      for (let b = 1; b <= 5; b++) {
        const ckpt = await makeCheckpointWithLogs(b, { numTxsPerBlock: 1, privateLogs: { numLogsPerTx: 1 } });
    ...
    it('returns empty when the range is before the first log', async () => {
      ...
      const ckpt = await makeCheckpointWithLogs(5, { numTxsPerBlock: 1, privateLogs: { numLogsPerTx: 1 } });
      ckpt.checkpoint.blocks[0].body.txEffects[0].privateLogs[0].fields[0] = sharedTag.value;
    ...
    it('returns empty when the range is past the last log', async () => {
      ...
      const ckpt = await makeCheckpointWithLogs(3, { numTxsPerBlock: 1, privateLogs: { numLogsPerTx: 1 } });
      ckpt.checkpoint.blocks[0].body.txEffects[0].privateLogs[0].fields[0] = sharedTag.value;
      await blockStore.addProposedBlock(ckpt.checkpoint.blocks[0]);
      await logStore.addLogs([ckpt.checkpoint.blocks[0]]);
    ...
    it('paginates correctly across a page boundary via afterLog', async () => {
      ...
      const blocks: L2Block[] = [];
      for (let b = 1; b <= 6; b++) {
        const ckpt = await makeCheckpointWithLogs(b, { numTxsPerBlock: 2, privateLogs: { numLogsPerTx: 2 } });
      ...
      const [page2] = await logStore.getPrivateLogsByTags({ tags: [sharedTag, undefined as never].slice(0, 1) as any });
      // Actually pass afterLog properly.
      ...
      // No overlap between page1 and page2b.
      const lastP1 = page1[page1.length - 1];
      const firstP2 = page2b[0];
      expect(
        firstP2.blockNumber > lastP1.blockNumber ||
          (firstP2.blockNumber === lastP1.blockNumber && firstP2.logIndexWithinTx > lastP1.logIndexWithinTx),
      ).toBe(true);
      // Sanity to silence unused var.
      void page2;
    });
    ...
    it('handles overlapping ranges across multiple tags', async () => {
      ...
      const blocks: L2Block[] = [];
      for (let b = 1; b <= 3; b++) {
        const ckpt = await makeCheckpointWithLogs(b, { numTxsPerBlock: 1, privateLogs: { numLogsPerTx: 2 } });
    ...
    it('rejects txHash combined with fromBlock/toBlock', async () => {
    ...
    it('referenceBlock caps a txHash query past the anchor', async () => {
      ...
      const ckpts: PublishedCheckpoint[] = [];
      for (let b = 1; b <= 3; b++) {
        const ckpt = await makeCheckpointWithLogs(b, { numTxsPerBlock: 1, privateLogs: { numLogsPerTx: 1 } });
