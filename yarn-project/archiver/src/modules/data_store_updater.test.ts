...
describe('ArchiverDataStoreUpdater', () => {
  ...
  describe('contract data', () => {
    it('stores contract class and instance data when blocks are added via addProposedBlock', async () => {
      ...
      const retrievedClass = await store.contractClassStore.getContractClass(contractClassId);
      ...
      const retrievedInstance = await store.contractInstanceStore.getContractInstance(instanceAddress, timestamp);
    ...
    it('removes contract class and instance data when blocks are pruned via setCheckpointData', async () => {
      ...
      expect(await store.contractClassStore.getContractClass(contractClassId)).toBeDefined();
      expect(await store.contractInstanceStore.getContractInstance(instanceAddress, timestamp)).toBeDefined();
      ...
      expect(await store.contractClassStore.getContractClass(contractClassId)).toBeUndefined();
      expect(await store.contractInstanceStore.getContractInstance(instanceAddress, timestamp)).toBeUndefined();
    });
  ...
  describe('logs handling', () => {
    it('does not duplicate logs when checkpoint contains same block as provisional', async () => {
      ...
      const publicLogs = await store.logStore.getPublicLogs({ fromBlock: block.number, toBlock: block.number + 1 });
    ...
    it('replaces logs when checkpoint conflicts with provisional block', async () => {
      ...
      await updater.addProposedBlock(localBlock);
      const publicLogsBefore = await store.logStore.getPublicLogs({});
      ...
      const storedBlock = await store.blockStore.getBlock(BlockNumber(1));
      expect(storedBlock?.archive.root.equals(checkpointBlock.archive.root)).toBe(true);
      const publicLogsAfter = await store.logStore.getPublicLogs({});
