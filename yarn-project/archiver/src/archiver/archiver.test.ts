...
describe('Archiver', () => {
  ...
  it('ignores blocks because of invalid attestations', async () => {
    ...
    const _rollupTxs = await Promise.all(blocks.map(b => makeRollupTx(b, signers)));
    const blobHashes = await Promise.all(blocks.map(makeVersionedBlobHashes));
    const _blobsFromBlocks = await Promise.all(blocks.map(b => makeBlobsFromBlock(b)));
  ...
  it('initial sync does not complete until archiver catches up with latest L1 block', async () => {
    const rollupTxs = await Promise.all(blocks.slice(0, 1).map(b => makeRollupTx(b)));
    const blobHashes = await Promise.all(blocks.slice(0, 1).map(makeVersionedBlobHashes));
    const blobsFromBlocks = await Promise.all(blocks.slice(0, 1).map(b => makeBlobsFromBlock(b)));
  ...
  it('handles a block gap due to a spurious L2 prune', async () => {
    ...
    const _rollupTxs = await Promise.all(blocks.map(b => makeRollupTx(b)));
    const blobHashes = await Promise.all(blocks.map(makeVersionedBlobHashes));
    const _blobsFromBlocks = await Promise.all(blocks.map(b => makeBlobsFromBlock(b)));
   ...
   */
  const makeRollupTx = async (l2Block: L2Block, signers: Secp256k1Signer[] = []) => {
   ...
   */
  const makeVersionedBlobHashes = async (l2Block: L2Block): Promise<`0x${string}`[]> => {
    const blobHashes = (await Blob.getBlobsPerBlock(l2Block.body.toBlobFields()))
      .map(b => b.getEthVersionedBlobHash())
      .map(bufferToHex);
   ...
   * @returns The blobs.
   */
  const makeBlobsFromBlock = async (block: L2Block) => {
    const blobs = await Blob.getBlobsPerBlock(block.body.toBlobFields());
    const blobsWithIndex = blobs.map((blob, index) => new BlobWithIndex(blob, index));
    allBlobs.set(block.archive.root.toString(), blobsWithIndex);
    return blobsWithIndex;
  };
});
