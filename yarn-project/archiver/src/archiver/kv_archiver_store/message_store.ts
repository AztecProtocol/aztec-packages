...
export class MessageStore {
  ...
  public rollbackL1ToL2MessagesToL2Block(targetBlockNumber: bigint): Promise<void> {
    ...
    const startIndex =
      InboxLeaf.smallestIndexFromL2Block(targetBlockNumber) + BigInt(NUMBER_OF_L1_L2_MESSAGES_PER_ROLLUP);
    return this.removeL1ToL2Messages(startIndex);
  }

  private indexToKey(index: bigint): number {
    return Number(index + 1n); // Using zero as a key in the map breaks the kvstore.
  }
