...
function test_cmds {
  ...
  # Deploy framework tests: deploy.test.ts spins up anvil + an in-process node (network usage ⇒ ISOLATE).
  echo "$hash:ISOLATE=1:NAME=aztec/deploy yarn-project/scripts/run_test.sh aztec/src/deploy"
}
