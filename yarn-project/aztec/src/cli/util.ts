...
export async function shutdown(logFn: LogFn, exitCode: ExitCode, cb?: Array<() => Promise<void>>): Promise<never> {
  if (shutdownPromise) {
...
export const installSignalHandlers = (logFn: LogFn, cb?: Array<() => Promise<void>>) => {
  ...
  process.once('SIGINT', () => shutdown(logFn, ExitCode.SUCCESS, cb));
  // eslint-disable-next-line @typescript-eslint/no-misused-promises
  process.once('SIGTERM', () => shutdown(logFn, ExitCode.SUCCESS, cb));
};
