...
import type { LocalNetworkConfig } from './config.js';
...
export { type LocalNetworkConfig, localNetworkConfigMappings, localNetworkCliConfigMappings } from './config.js';

/**
 * Create and start a new Aztec Node and PXE. Deploys L1 contracts.
 * Does not start any HTTP services nor populate any initial accounts.
 * @param config - Fully-resolved local network settings (callers build this via the config resolver).
 */
export async function createLocalNetwork(config: LocalNetworkConfig, userLog: LogFn) {
  ...
  const hdAccount = mnemonicToAccount(config.l1Mnemonic.getValue());
  if (
  ...
  const node = await AztecNodeService.createAndSync(aztecNodeConfig, { telemetry, dateProvider }, { genesis });
