...
export class WorkPoller {
  ...
  private async pollCycle(handler: WorkPollerHandler): Promise<void> {
    try {
      ...
      const provenEpoch = Math.floor(Number(provenBlock) / constants.epochDuration);
      const currentEpoch = Math.floor(Number(checkpointedBlock) / constants.epochDuration);
      ...
      for (let epoch = provenEpoch; epoch <= currentEpoch; epoch++) {
        const epochNumber = EpochNumber(epoch);

        // Get whatever checkpoints have been posted for this epoch so far.
        // This returns checkpoints even if the epoch is not yet complete.
        ...
        if (isComplete && allSubTreesComplete) {
          claimableItems.push({
