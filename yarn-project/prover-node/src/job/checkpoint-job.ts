...
export class CheckpointJob {
  ...
  public async whenDone(): Promise<void> {
    ...
    if (this.cancelPromise) {
      await this.cancelPromise.catch(() => {});
    }
  }
  ...
  public async provideTxs(txs: Map<string, Tx>): Promise<void> {
    ...
    this.provideTxsPromise = this.runProvideTxs(txs);
    await this.provideTxsPromise;
  }

  private async runProvideTxs(txs: Map<string, Tx>): Promise<void> {
    ...
    try {
      ...
      this.deps.log.verbose(`Starting processing checkpoint ${this.checkpoint.number}`, {
        number: this.checkpoint.number,
      ...
      void this.subTree.getSubTreeResult().then(
        result => this.blockProofs.resolve(result.blockProofOutputs),
  ...
  public cancel(): void {
    ...
    // Mark observed; the parent awaits via whenDone, but a fire-and-forget cancel
    // path should not surface unhandled rejections.
