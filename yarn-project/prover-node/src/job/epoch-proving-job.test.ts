...
describe('epoch-proving-job', () => {
  ...
  describe('removeCheckpointsAfter', () => {
    it('aborts a pending checkpoint above the threshold and clears the entry', () => {
      const job = createJob();
      ...
      const removed = job.removeCheckpointsAfter(CheckpointNumber(checkpoints[0].number - 1));
    ...
    it('removes only the suffix above the threshold, leaving earlier checkpoints intact', async () => {
      const job = createJob();
      const txsMap = new Map<string, Tx>(txs.map(tx => [tx.getTxHash().toString(), tx]));
      await addCheckpoint(job, checkpoints[0], txsMap, [], initialHeader);
      ...
      const removed = job.removeCheckpointsAfter(checkpoints[0].number);

      expect(removed).toBe(2);
      expect(subTrees[1].cancel).toHaveBeenCalled();
      expect(subTrees[2].cancel).toHaveBeenCalled();
      expect(subTrees[0].cancel).not.toHaveBeenCalled();
      expect(job.getCheckpointNumbers()).toEqual([checkpoints[0].number]);
    });

    it('returns 0 when no checkpoints sit above the threshold', () => {
      const job = createJob();
      registerPending(job, checkpoints[0], indexOf(checkpoints[0]), []);
      expect(job.removeCheckpointsAfter(CheckpointNumber(checkpoints.at(-1)!.number + 10))).toBe(0);
    });

    it('returns 0 when the job is in a terminal state', async () => {
      const job = createJob();
      await job.cancel();
      expect(job.removeCheckpointsAfter(CheckpointNumber(0))).toBe(0);
    });
    ...
    it('coexists a remove + re-register of the same checkpoint number via slot identity', async () => {
      // (number, slot) identity means a fresh registration at the same number after a
      // remove gets a separate job — there is no need to await any teardown.
      const v1 = checkpoints[0];
      const v2 = await Checkpoint.random(v1.number, {
        numBlocks: BLOCKS_PER_CHECKPOINT,
        startBlockNumber: 1,
        txsPerBlock: TXS_PER_BLOCK,
        slotNumber: SlotNumber(Number(v1.header.slotNumber) + 1),
      });
      expect(v2.header.slotNumber).not.toEqual(v1.header.slotNumber);

      const job = createJob();
      registerPending(job, v1, 0, []);
      expect(job.removeCheckpointsAfter(CheckpointNumber(v1.number - 1))).toBe(1);

      // No await, no release: v2 registers immediately with the same number, different slot.
      registerPending(job, v2, 0, []);
      expect(job.getCheckpointCount()).toBe(1);
      expect(job.hasCheckpoint(v2.number)).toBe(true);
    });

    it('finalizes the epoch with the re-registered checkpoint version, not the removed one', async () => {
      // Replace checkpoints[0] with a v2 (same number, different slot) before any txs are
      // provided, then run the rest of the epoch normally. The top-tree's snapshot should
      // contain v2, not v1.
      const v1 = checkpoints[0];
      const v2 = await Checkpoint.random(v1.number, {
        numBlocks: BLOCKS_PER_CHECKPOINT,
        startBlockNumber: v1.blocks[0].number,
        txsPerBlock: TXS_PER_BLOCK,
        slotNumber: SlotNumber(Number(v1.header.slotNumber) + 1),
      });
      const v2Txs = v2.blocks.flatMap(b =>
        b.body.txEffects.map(
          txEffect =>
            ({
              txHash: txEffect.txHash,
              getTxHash: () => txEffect.txHash,
              data: { forPublic: false },
            }) as unknown as Tx,
        ),
      );
      ...
      // v2 takes the slot-0 spot; the remaining checkpoints register and run unchanged.
      await addCheckpoint(job, v2, v2TxsMap, [], initialHeader);
      const txsMap = new Map<string, Tx>(txs.map(tx => [tx.getTxHash().toString(), tx]));
      for (let i = 1; i < checkpoints.length; i++) {
        const previousBlockHeader = checkpoints[i - 1].blocks.at(-1)!.header;
        const isLast = i === checkpoints.length - 1;
      ...
      const checkpointData = topTree.prove.mock.calls[0][3];
      expect(checkpointData).toHaveLength(NUM_CHECKPOINTS);
      // The slot-0 entry carries v2's blob fields, not v1's.
      expect(checkpointData[0].blobFields).toEqual(v2.toBlobFields());
      expect(checkpointData[0].blobFields).not.toEqual(v1.toBlobFields());
    });
  });
  ...
  describe('completeEpoch / whenComplete', () => {
    ...
    it('honors finalizationDelayMs and still allows removeCheckpointsAfter during the delay', async () => {
      ...
      const removed = job.removeCheckpointsAfter(CheckpointNumber(checkpoints[checkpoints.length - 2].number));
      expect(removed).toBe(1);
  ...
  describe('reorg-after-finalize', () => {
    ...
    it('prune that drops the last checkpoint cancels the top tree and restarts with survivors', async () => {
      ...
      // Drop the last checkpoint via a suffix prune. The in-flight top tree is cancelled.
      const removed = job.removeCheckpointsAfter(CheckpointNumber(checkpoints.at(-2)!.number));
      expect(removed).toBe(1);
    ...
    it('fails the epoch if all checkpoints are removed mid-finalize', async () => {
      // The first top-tree prove hangs until cancelled. A suffix-prune that wipes the
      // entire epoch cancels it; the restart loop sees zero survivors and throws.
      const firstTopTree = mock<TopTreeOrchestrator>();
      let firstReject: (err: Error) => void = () => {};
      const firstProvePromise = new Promise<{
        publicInputs: RootRollupPublicInputs;
      ...
      firstProvePromise.catch(() => {});
      firstTopTree.prove.mockReturnValue(firstProvePromise);
      firstTopTree.cancel.mockImplementation(() => firstReject(new TopTreeCancelledError()));
      firstTopTree.stop.mockResolvedValue(undefined);
      firstTopTree.getProverId.mockReturnValue(proverId);
      ...
      // Suffix prune that drops every checkpoint in one call.
      const removed = job.removeCheckpointsAfter(CheckpointNumber(checkpoints[0].number - 1));
      expect(removed).toBe(checkpoints.length);
      expect(firstTopTree.cancel).toHaveBeenCalledWith({ abortJobs: true });
