...
describe('epoch-proving-job', () => {
  ...
  beforeEach(async () => {
    ...
    l2BlockSource.getBlockHeadersForEpoch.mockImplementation(
      async () => await Promise.all(blocks.map(b => b.getBlockHeader())),
    );
  ...
  it('halts if a new block for the epoch is found', async () => {
    ...
    const newHeaders = await Promise.all(newBlocks.map(b => b.getBlockHeader()));
