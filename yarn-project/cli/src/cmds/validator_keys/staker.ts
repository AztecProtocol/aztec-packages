...
export async function generateStakerJson(options: StakerOptions, log: LogFn): Promise<void> {
  ...
  for (let i = 0; i < keystore.validators.length; i++) {
    ...
    for (let j = 0; j < outputs.length; j++) {
      ...
      const stakerOutputPath = join(outputDir, `${keystoreBaseName}_attester${attesterIndex}_staker_output.json`);
      writeFileSync(stakerOutputPath, prettyPrintJSON(outputs[j]), 'utf-8');
      log(`Wrote staker output to ${stakerOutputPath}`);
