...
import type { DeployL1Contracts, L1ContractsConfig, RollupContract } from '@aztec/ethereum';
...
export async function deployNewRollupContracts(
  ...
  const { createEthereumChain, deployRollupAndPeriphery } = await import('@aztec/ethereum');
  const { mnemonicToAccount, privateKeyToAccount } = await import('viem/accounts');
  const { getVKTreeRoot } = await import('@aztec/noir-protocol-circuits-types/vks');
