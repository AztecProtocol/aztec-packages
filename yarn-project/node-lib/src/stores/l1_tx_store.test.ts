...
describe('L1TxStore', () => {
  ...
  beforeEach(async () => {
    kvStore = await openTmpStore('l1-tx-store-test', true);
  ...
  describe('saveState and loadStates', () => {
    ...
    it('should handle states with receipts', async () => {
      ...
      expect(loaded[0].receipt!.transactionHash).toBe('0xabc');
    });
  ...
  describe('clearStates', () => {
    ...
    it('should reset ID counter when clearing states', async () => {
      ...
      const newState = await store.saveState(account, createMockState(3));
  ...
  describe('serialize/deserialize roundtrip', () => {
    it('should preserve all state fields through serialization', async () => {
      ...
      expect(roundtrippedState).toEqual(originalState);
    });
  });
});
