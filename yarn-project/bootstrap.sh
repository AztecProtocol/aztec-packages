...
function hash {
  # Pinned dependencies are hashed via yarn.lock (covered by the yarn-project patterns), so nothing
  # under noir-projects/fnd is an input here: everything it produces arrives as a pinned package.
