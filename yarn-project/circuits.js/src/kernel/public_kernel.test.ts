import { CircuitError, KERNEL_PUBLIC_DATA_READS_LENGTH, makeTuple, simulatePublicKernelCircuit, simulatePublicKernelCircuitNoPreviousKernel } from '../index.js';
import { makePublicDataRead, makePublicKernelInputsNoKernelInput, makePublicKernelInputsWithEmptyOutput } from '../tests/factories.js';
...
describe('kernel/public_kernel', () => {
  ...
  it("simulating public kernel circuit fails when aggregating proofs will overflow", async function () {
    ...
    await expect(simulatePublicKernelCircuit(input)).rejects.toThrow(new CircuitError(7009, "array_push cannot push to a full array"));
  })
});
