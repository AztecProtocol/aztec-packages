...
export function makeContractClassPublic(seed = 0, publicDispatchFunction?: PublicFunction): ContractClassPublic {
  ...
  const packedBytecode = publicDispatchFunction!.bytecode;
  const publicBytecodeCommitment = computePublicBytecodeCommitment(publicDispatchFunction!.bytecode);
...
export function makeContractInstanceFromClassId(classId: Fr, seed = 0): ContractInstanceWithAddress {
  ...
  const address = computeAddress(partialAddress, publicKeys.hash());
