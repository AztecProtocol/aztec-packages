...
export async function getContractClassFromArtifact(
  ...
  const artifactPublicFunctions: ContractClass['publicFunctions'] = (
    await Promise.all(
      artifact.functions
  ...
  const privateFunctions: ContractClass['privateFunctions'] = (
    await Promise.all(
      artifact.functions
...
export async function computeVerificationKeyHash(f: FunctionArtifact) {
  ...
  return await hashVK(await vkAsFieldsMegaHonk(Buffer.from(f.verificationKey, 'base64')));
}
