...
function computePrivateInputsHash(input: PrivateCircuitPublicInputs) {
  const toHash = [
    ...
    // TODO: use inputs.blockHeader.toBuffer() instead of the following
    input.blockHeader.state.partial.noteHashTree.root.toBuffer(),
    input.blockHeader.state.partial.nullifierTree.root.toBuffer(),
    input.blockHeader.state.partial.contractTree.root.toBuffer(),
    input.blockHeader.state.l1ToL2MessageTree.root.toBuffer(),
    input.blockHeader.lastArchive.root.toBuffer(),
    input.blockHeader.state.partial.publicDataTree.root.toBuffer(),
    computeGlobalsHash(input.blockHeader.globalVariables).toBuffer(),
...
export function computePublicInputsHash(input: PublicCircuitPublicInputs) {
  const toHash = [
    ...
    input.blockHeader.state.partial.contractTree.root.toBuffer(),
    input.blockHeader.state.l1ToL2MessageTree.root.toBuffer(),
    input.blockHeader.lastArchive.toBuffer(),
    input.blockHeader.state.partial.publicDataTree.root.toBuffer(),
    computeGlobalsHash(input.blockHeader.globalVariables).toBuffer(),
