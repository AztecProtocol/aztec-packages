...
describe('PublicLog', () => {
  ...
  it('number of emitted fields is correct', () => {
    ...
    const smallLog = new ContractClassLog(
      smallLogFields.concat(new Array(CONTRACT_CLASS_LOG_SIZE_IN_FIELDS - smallLogFields.length).fill(Fr.ZERO)),
    );
    ...
    const largeLog = new ContractClassLog(largeLogFields.concat(new Array(smallLogFields.length).fill(Fr.ZERO)));
