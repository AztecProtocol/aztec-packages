...
import { AuthRegistryArtifact, getAuthRegistryAddress } from '@aztec/protocol-contracts/auth-registry';
...
export async function getDefaultAllowedSetupFunctions(): Promise<AllowedElement[]> {
  if (defaultAllowedSetupFunctions === undefined) {
    const authRegistryAddress = getAuthRegistryAddress();
    defaultAllowedSetupFunctions = await Promise.all([
      ...
      buildAllowedElement(AuthRegistryArtifact, { address: authRegistryAddress }, '_set_authorized', {
        onlySelf: true,
      ...
      buildAllowedElement(AuthRegistryArtifact, { address: authRegistryAddress }, 'set_authorized', {
        rejectNullMsgSender: true,
      }),
