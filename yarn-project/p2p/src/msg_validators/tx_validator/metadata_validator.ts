...
export class MetadataTxValidator<T extends AnyTx> implements TxValidator<T> {
  ...
  #isValidForTimestamp(tx: T): boolean {
    ...
    if (!buildingBlock1 && includeByTimestamp.isSome && includeByTimestamp.value < this.values.timestamp) {
      ...
      this.#log.verbose(
        `Rejecting tx ${'txHash' in tx ? tx.txHash : tx.hash} for low expiration timestamp. Tx expiration timestamp: ${includeByTimestamp.value}, timestamp: ${this.values.timestamp}.`,
      );
      return false;
    } else {
      return true;
