...
export class PoolInstrumentation<PoolObject extends Gossipable> {
  ...
  private txAddedTimestamp: Map<TxHash, number> = new Map<TxHash, number>();
  ...
  public transactionsAdded(transactions: Tx[]) {
    ...
    for (const transaction of transactions) {
      this.txAddedTimestamp.set(transaction.txHash, timestamp);
    }
  }
  ...
  public transactionsRemoved(hashes: TxHash[]) {
    ...
    for (const hash of hashes) {
      const addedAt = this.txAddedTimestamp.get(hash);
      if (addedAt) {
        this.txAddedTimestamp.delete(hash);
