    ...
    describe('BLOCK_MINED events', () => {
      ...
      beforeEach(() => {
        ...
        context = {
          ...
          block: blockHeader,
          newNullifiers: new Set(newNullifiers.map(f => f.toString())),
        };
      });

      it('returns error for invalid context without block', async () => {
        const invalidContext: EvictionContext = {
          event: EvictionEvent.BLOCK_MINED,
          block: undefined as any,
          newNullifiers: new Set(),
        };

        const result = await rule.evict(invalidContext, txPool);

        expect(result.success).toBe(false);
        expect(result.error?.message).toContain('Invalid block mined context');
      });

      it('returns error for invalid context without nullifiers', async () => {
        const invalidContext: EvictionContext = {
          event: EvictionEvent.BLOCK_MINED,
          block: blockHeader,
          newNullifiers: undefined as any,
        };
      ...
      it('respects non-evictable transactions', async () => {
        ...
        txPool.getTxByHash.mockImplementation(txHash => {
          if (txHash.equals(evictableTx)) {
            return mockEvictableTx;
          }
          if (txHash.equals(nonEvictableTx)) {
            return mockNonEvictableTx;
          }
          return Promise.resolve(undefined);
        });
