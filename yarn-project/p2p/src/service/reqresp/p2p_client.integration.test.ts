...
import { createLibP2PPeerId, type P2PService } from '../index.js';
...
describe('Req Resp p2p client integration', () => {
  ...
  let logger = createDebugLogger('p2p-client-integration-test');
        ...
        const createClients = async (numberOfPeers: number) => {
          ...
          for (let i = 0; i < numberOfPeers; i++) {
            ...
            kvStore = openTmpStore();
              const client = await createP2PClient(config, kvStore, txPool as unknown as TxPool, attestationPool as unknown as AttestationPool, blockSource);
    ...
    it.skip("Returns undefined if unable to find a transaction from another peer", async () => {
