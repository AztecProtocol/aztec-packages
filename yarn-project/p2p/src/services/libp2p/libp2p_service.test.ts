...
import type { Tx, TxValidator } from '@aztec/stdlib/tx';
...
describe('LibP2PService', () => {
  ...
  describe('validateRequestedBlockTxs', () => {
    function makeRequest(archiveRoot: Fr, length: number, indices: number[]): BlockTxsRequest {
      const request = Object.create(BlockTxsRequest.prototype) as BlockTxsRequest;
      (request as { archiveRoot: Fr }).archiveRoot = archiveRoot;
      (request as { txIndices: BitVector }).txIndices = BitVector.init(length, indices);
      return request;
    }
    ...
    function makeResponse(archiveRoot: Fr, length: number, indices: number[], txHashes: string[]): BlockTxsResponse {
      ...
      const response = Object.create(BlockTxsResponse.prototype) as BlockTxsResponse & { txs: MockTx[] };
      (response as { archiveRoot: Fr }).archiveRoot = archiveRoot;
      (response as { txs: MockTx[] }).txs = txs;
      (response as { txIndices: BitVector }).txIndices = BitVector.init(length, indices);
      return response;
    }
