...
describe('EIP-2335 Keystore', () => {
  ...
  beforeAll(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'eip2335-test-'));
  });
  ...
  describe('createEip2335Keystore', () => {
    it('creates a valid EIP-2335 keystore structure', () => {
      const keystore = createEip2335Keystore(testPassword, testPrivateKey, testPublicKey, testPath);
    ...
    it('produces different ciphertexts for the same key with different passwords', () => {
      const keystore1 = createEip2335Keystore('password1', testPrivateKey, testPublicKey, testPath);
      const keystore2 = createEip2335Keystore('password2', testPrivateKey, testPublicKey, testPath);
    ...
    it('produces different ciphertexts on each call due to random salt and IV', () => {
      const keystore1 = createEip2335Keystore(testPassword, testPrivateKey, testPublicKey, testPath);
      const keystore2 = createEip2335Keystore(testPassword, testPrivateKey, testPublicKey, testPath);
    ...
    it('accepts private keys with or without 0x prefix', () => {
      const withPrefix = createEip2335Keystore(testPassword, '0x' + '42'.repeat(32), testPublicKey, testPath);
      const withoutPrefix = createEip2335Keystore(testPassword, '42'.repeat(32), testPublicKey, testPath);
    ...
    it('stores public key without 0x prefix in description field', () => {
      ...
      const keystore = createEip2335Keystore(testPassword, testPrivateKey, pubkeyWithPrefix, testPath);
    ...
    it('throws error for invalid private key length', () => {
      ...
      expect(() => createEip2335Keystore(testPassword, shortKey, testPublicKey, testPath)).toThrow(
        'BLS private key must be 32-byte hex',
      );

      const longKey = '0x' + '42'.repeat(33);
      expect(() => createEip2335Keystore(testPassword, longKey, testPublicKey, testPath)).toThrow(
        'BLS private key must be 32-byte hex',
      );
    });
    ...
    it('throws error for non-hex private key', () => {
      ...
      expect(() => createEip2335Keystore(testPassword, invalidKey, testPublicKey, testPath)).toThrow(
        'BLS private key must be 32-byte hex',
      );
    });
    ...
    it('normalizes password using NFKD', () => {
      ...
      const keystore = createEip2335Keystore(password, testPrivateKey, testPublicKey, testPath);
    ...
    it('handles empty password', () => {
      const keystore = createEip2335Keystore('', testPrivateKey, testPublicKey, testPath);
    ...
    it('preserves derivation path exactly as provided', () => {
      ...
      const keystore = createEip2335Keystore(testPassword, testPrivateKey, testPublicKey, customPath);

      expect(keystore.path).toBe(customPath);
    });
  });

  describe('loadEip2335Keystore', () => {
    it('loads and validates a valid EIP-2335 keystore file', () => {
      const keystore = createEip2335Keystore(testPassword, testPrivateKey, testPublicKey, testPath);
      const filePath = join(tempDir, 'valid-keystore.json');
      writeFileSync(filePath, JSON.stringify(keystore));

      const loaded = loadEip2335Keystore(filePath);
    ...
    it('throws on invalid JSON', () => {
      ...
      expect(() => loadEip2335Keystore(filePath)).toThrow(Eip2335Error);
      expect(() => loadEip2335Keystore(filePath)).toThrow(/Invalid JSON/);
    });
    ...
    it('throws on invalid keystore structure', () => {
      ...
      expect(() => loadEip2335Keystore(filePath)).toThrow(Eip2335Error);
      expect(() => loadEip2335Keystore(filePath)).toThrow(/Invalid EIP-2335 keystore format/);
    });

    it('throws on missing file', () => {
      const nonExistentPath = join(tempDir, 'does-not-exist.json');

      expect(() => loadEip2335Keystore(nonExistentPath)).toThrow(Eip2335Error);
    });
  });

  describe('decryptEip2335Keystore', () => {
    it('successfully decrypts a keystore with correct password', () => {
      const keystore = createEip2335Keystore(testPassword, testPrivateKey, testPublicKey, testPath);
      const filePath = join(tempDir, 'decrypt-test.json');
      writeFileSync(filePath, JSON.stringify(keystore));

      const decrypted = decryptEip2335Keystore(filePath, testPassword);
    ...
    it('throws on incorrect password', () => {
      ...
      expect(() => decryptEip2335Keystore(filePath, 'wrong-password')).toThrow(Eip2335Error);
      expect(() => decryptEip2335Keystore(filePath, 'wrong-password')).toThrow(/Checksum verification failed/);
    });

    it('works with empty password if keystore was created with empty password', () => {
      const emptyPassword = '';
      const keystore = createEip2335Keystore(emptyPassword, testPrivateKey, testPublicKey, testPath);
      const filePath = join(tempDir, 'empty-password-test.json');
      writeFileSync(filePath, JSON.stringify(keystore));

      const decrypted = decryptEip2335Keystore(filePath, emptyPassword);

      expect(decrypted).toBe(testPrivateKey);
    });
  });

  describe('decryptEip2335KeystoreFromObject', () => {
    it('decrypts from an in-memory keystore object', () => {
      const keystore = createEip2335Keystore(testPassword, testPrivateKey, testPublicKey, testPath);

      const decrypted = decryptEip2335KeystoreFromObject(keystore, testPassword);
    ...
    it('throws on unsupported KDF function', () => {
      ...
      expect(() => decryptEip2335KeystoreFromObject(keystore, testPassword)).toThrow(Eip2335Error);
      expect(() => decryptEip2335KeystoreFromObject(keystore, testPassword)).toThrow(/Unsupported KDF function/);
    });
    ...
    it('throws on unsupported cipher function', () => {
      ...
      expect(() => decryptEip2335KeystoreFromObject(keystore, testPassword)).toThrow(Eip2335Error);
      expect(() => decryptEip2335KeystoreFromObject(keystore, testPassword)).toThrow(/Unsupported cipher function/);
    });
  });
  ...
  describe('round-trip encryption and decryption', () => {
    it('encrypts and then decrypts to get original key', () => {
      ...
      const encrypted = createEip2335Keystore(password, originalKey, pubkey, path);

      // Decrypt it
      const decrypted = decryptEip2335KeystoreFromObject(encrypted, password);
    ...
    it('round-trips multiple different keys', () => {
      ...
      for (const { key, password } of testCases) {
        const encrypted = createEip2335Keystore(password, key, testPublicKey, testPath);
        const decrypted = decryptEip2335KeystoreFromObject(encrypted, password);
