...
import { hashMessage, recoverPublicKey as viemRecoverPublicKey, recoverAddress as viemRecoverAddress } from "viem";
...
describe('Secp256k1Signer', () => {
    ...
    it('Compare implementation against viem', async () => {
        ...
        const viemAddress = EthAddress.fromString(await viemRecoverAddress({hash: ethHashedMessage, signature: viemSignature.to0xString()}));
        const lightAddress = lightRecoverAddress(Buffer32.fromBuffer(Buffer.from(ethHashedMessage.slice(2), "hex")), lightSignature);
