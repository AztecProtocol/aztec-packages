...
describe('insertIntoSortedArray', () => {
  ...
  it('detects duplicates at beginning of array', () => {
    const arr = [1, 1, 1, 2, 3];
    const result = insertIntoSortedArray(arr, 1, cmp, false);
    expect(result).toBe(false);
    expect(arr).toEqual([1, 1, 1, 2, 3]);
  });

  it('detects duplicates at end of array', () => {
    const arr = [1, 2, 3, 3, 3];
    const result = insertIntoSortedArray(arr, 3, cmp, false);
    expect(result).toBe(false);
    expect(arr).toEqual([1, 2, 3, 3, 3]);
  });

  it('detects duplicates in middle of array with many duplicates', () => {
    const arr = [1, 2, 2, 2, 2, 3];
    const result = insertIntoSortedArray(arr, 2, cmp, false);
    expect(result).toBe(false);
    expect(arr).toEqual([1, 2, 2, 2, 2, 3]);
  });

  it('detects single duplicate in array', () => {
    const arr = [2];
    const result = insertIntoSortedArray(arr, 2, cmp, false);
    expect(result).toBe(false);
  ...
  it('allows inserting non-duplicate into array with duplicates', () => {
    const arr = [1, 2, 2, 2, 4];
    const result = insertIntoSortedArray(arr, 3, cmp, false);
    expect(result).toBe(true);
    expect(arr).toEqual([1, 2, 2, 2, 3, 4]);
  });
