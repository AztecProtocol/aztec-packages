export type EnvVar =
  ...
  | 'API_KEY' // Used for gated deployments such as devnet. API Key for startPXE and addPXE from CLI. Suspect it's not being used and that we're using AZTEC_NODE_URL instead. Confirmed with Esau that they're not using it.
  ...
  | 'AZTEC_NODE_URL' // The address of an Aztec Node URL that the PXE will connect to.
  | 'AZTEC_PORT' // Port to run the Aztec Services on
  ...
  | 'BOT_NO_WAIT_FOR_TRANSFERS' // Bot's signing key
  | 'BOT_L1_MNEMONIC' // Mnemonic for the bot
  | 'BOT_L1_PRIVATE_KEY' // private key for the bot
  | 'BOT_PRIVATE_KEY' //
  ...
  | 'COINBASE' // L1 address that would be eligible for block rewards
  ...
  | 'ETHEREUM_HOSTS' // List of URLs of ethereum execution clients
  ...
  | 'P2P_GOSSIPSUB_MCACHE_LENGTH' // The number of gossipsub interval message cache windows to keep
  | 'P2P_GOSSIPSUB_TX_INVALID_MESSAGE_DELIVERIES_DECAY' // Determines how quickly the penalty for invalid message deliveries decays over time. Between 0 and 1.
  | 'P2P_GOSSIPSUB_TX_INVALID_MESSAGE_DELIVERIES_WEIGHT' //
  | 'P2P_GOSSIPSUB_TX_TOPIC_WEIGHT' // The weight of the tx invalid message deliveries for the gossipsub protocol.
  | 'P2P_L2_QUEUE_SIZE' //
