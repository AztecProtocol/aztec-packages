...
describe('Private kernel', () => {
  ...
  it('Executes private kernel ordering after a deployment', async () => {
    ...
    const combinedAccumulatedData = new CombinedAccumulatedData(
      ...
      newNullifiers,
      nullifiedCommitments,
      makeTuple(MAX_PRIVATE_CALL_STACK_LENGTH_PER_TX, () => CallStackItem.empty()),
      makeTuple(MAX_PUBLIC_CALL_STACK_LENGTH_PER_TX, () => CallStackItem.empty()),
  ...
  it('Executes private kernel inner for a nested call', async () => {
    ...
    const privateCallStack = makeTuple(MAX_PRIVATE_CALL_STACK_LENGTH_PER_TX, () => CallStackItem.empty());
