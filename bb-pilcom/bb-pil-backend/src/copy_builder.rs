...
pub trait CopyBuilder {
    fn create_copy_files<F: FieldElement>(&self, name: &str, analyzed: &Analyzed<F>) -> Copies;
}

impl CopyBuilder for BBFiles {
    fn create_copy_files<F: FieldElement>(&self, name: &str, analyzed: &Analyzed<F>) -> Copies {
        let copies: Vec<&Identity<AlgebraicExpression<F>>> = analyzed
            .identities
            .iter()
            .filter(|identity| matches!(identity.kind, IdentityKind::Connect))
            .collect();

        let copy_pairs: Vec<Copy> = copies
            .iter()
            .map(|copy| {
                let pair = Copy {
...
pub fn get_grand_products_from_copies(copies: &Copies) -> Vec<String> {
    copies
        ...
        .map(|copy| {
            copy.attribute
                .clone()
                .unwrap_or_else(|| "copy_inverse".to_string())
        })
        .collect()
}
...
fn create_relation_exporter(copy_name: &str) -> String {
    ...
    let relation_export =
        format!("template <typename FF_> using {copy_name} = GenericCopy<{settings_name}, FF_>;");
...
fn create_copy_settings_file(copy: &Copy) -> String {
    ...
    let mut copy_entities: Vec<String> = vec![
        copy_name.clone(),
        copy_shift,
        "lagrange_first".to_owned(),
        "lagrange_last".to_owned(),
    ];
