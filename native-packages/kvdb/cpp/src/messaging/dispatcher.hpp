...
public:
  ...
  bool on_new_data(msgpack::object &obj, msgpack::sbuffer &buffer) const {
    ...
    if (iter == message_handlers.end()) {
      throw std::runtime_error("No registered handler for message of type " +
                               std::to_string(header.msgType));
    }

    // If the msg type has been marked as 'unique' then we need to give it
  ...
  void register_target(uint32_t msgType, const message_handler &handler,
                       bool unique = false) {
