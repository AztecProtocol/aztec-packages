...
// They are pulled in here via the `using namespace azteclabs::wsdb::merkle_tree` above and re-exported for
// the (many) existing world_state:: references.

const uint64_t CANONICAL_FORK_ID = 0;
const uint64_t NUM_TREES = 5;

using TreeStateReference = std::pair<azteclabs::wsdb::fr, azteclabs::wsdb::merkle_tree::index_t>;
