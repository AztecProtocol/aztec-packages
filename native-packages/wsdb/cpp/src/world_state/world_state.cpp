                                       ...
                                       bool ephemeral)
{
    ...
    }
    {
        ...
        std::vector<azteclabs::wsdb::fr> initial_values{ compute_initial_block_header_hash(
...
void WorldState::update_archive(const StateReference& block_state_ref,
                                const azteclabs::wsdb::fr& block_header_hash,
                                Fork::Id fork_id)
{
...
WorldStateStatusFull WorldState::sync_block(const StateReference& block_state_ref,
                                            const azteclabs::wsdb::fr& block_header_hash,
                                            const std::vector<azteclabs::wsdb::fr>& notes,
                                            const std::vector<azteclabs::wsdb::fr>& l1_to_l2_messages,
                                            const std::vector<merkle_tree::NullifierLeafValue>& nullifiers,
                                            const std::vector<merkle_tree::PublicDataLeafValue>& public_writes,
                                            const std::optional<azteclabs::wsdb::fr>& expected_archive_root,
                                            const std::optional<azteclabs::wsdb::fr>& expected_previous_archive_root)
{
    ...
    if (expected_previous_archive_root.has_value()) {
        const azteclabs::wsdb::fr actual_previous_archive_root =
    ...
    try {
        ...
        if (expected_archive_root.has_value()) {
            const azteclabs::wsdb::fr actual_archive_root =
...
GetLowIndexedLeafResponse WorldState::find_low_leaf_index(const WorldStateRevision& revision,
                                                          MerkleTreeId tree_id,
                                                          const azteclabs::wsdb::fr& leaf_key) const
{
    Fork::SharedPtr fork = retrieve_fork(revision.forkId);
    Signal signal;
...
azteclabs::wsdb::fr WorldState::compute_initial_block_header_hash(const StateReference& initial_state_ref,
                                                                  ...
                                                                  uint64_t genesis_timestamp)
{
    ...
    return AppendOnlyHashPolicy::hash(
        { generator_point,
          ...
          0,                                      // block_number
          0,                                      // slot_number
          azteclabs::wsdb::fr(genesis_timestamp), // timestamp
          0,                                      // coinbase
          0,                                      // fee_recipient
...
bool WorldState::is_archive_tip(const WorldStateRevision& revision, const azteclabs::wsdb::fr& block_header_hash) const
{
