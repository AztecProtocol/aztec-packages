...
#include "merkle_tree/signal.hpp"
...
#include "world_state/world_state_stores.hpp"
#include <array>
#include <atomic>
#include <cstddef>
#include <cstdint>
...
WorldState::WorldState(uint64_t thread_pool_size,
                       ...
                       const std::vector<fr>& prefilled_nullifiers,
                                       ...
                                       bool ephemeral)
{
    ...
    }
    {
        ...
        std::vector<fr> initial_values{ compute_initial_block_header_hash(
            get_state_reference(committed_revision(), fork, true),
...
uint64_t WorldState::create_fork(const std::optional<block_number_t>& blockNumber)
{
    ...
    if (!blockNumber.has_value()) {
        ...
        WorldStateRevision revision{ .forkId = CANONICAL_FORK_ID,
...
{
    return get_state_reference(
        WorldStateRevision{ .forkId = CANONICAL_FORK_ID, .blockNumber = LATEST_BLOCK, .includeUncommitted = false },
...
void WorldState::update_archive(const StateReference& block_state_ref, const fr& block_header_hash, Fork::Id fork_id)
{
    if (is_same_state_reference(
...
WorldStateStatusFull WorldState::sync_block(const StateReference& block_state_ref,
                                            const fr& block_header_hash,
                                            const std::vector<fr>& notes,
                                            const std::vector<fr>& l1_to_l2_messages,
                                            const std::vector<merkle_tree::NullifierLeafValue>& nullifiers,
                                            const std::vector<merkle_tree::PublicDataLeafValue>& public_writes,
                                            const std::optional<fr>& expected_archive_root,
                                            const std::optional<fr>& expected_previous_archive_root)
{
    ...
    if (expected_previous_archive_root.has_value()) {
        const fr actual_previous_archive_root = get_tree_info(committed_revision(), MerkleTreeId::ARCHIVE).meta.root;
    ...
    try {
        ...
        if (!is_archive_tip(uncommitted_revision(), block_header_hash)) {
            throw std::runtime_error("Can't synch block: block header hash is not the tip of the archive tree");
        }

        if (!is_same_state_reference(uncommitted_revision(), block_state_ref)) {
        ...
        if (expected_archive_root.has_value()) {
            const fr actual_archive_root = get_tree_info(uncommitted_revision(), MerkleTreeId::ARCHIVE).meta.root;
...
fr WorldState::compute_initial_block_header_hash(const StateReference& initial_state_ref,
                                                 ...
                                                 uint64_t genesis_timestamp)
{
    ...
    return AppendOnlyHashPolicy::hash(
        { generator_point,
          ...
          0,                     // block_number
          0,                     // slot_number
          fr(genesis_timestamp), // timestamp
          0,                     // coinbase
          0,                     // fee_recipient
...
bool WorldState::is_archive_tip(const WorldStateRevision& revision, const fr& block_header_hash) const
{
