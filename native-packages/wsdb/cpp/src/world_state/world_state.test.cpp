...
TEST_F(WorldStateTest, GetInitialTreeInfoForAllTrees)
{
...

    {
        ...
        EXPECT_EQ(info.meta.root, azteclabs::wsdb::fr(GENESIS_NULLIFIER_TREE_ROOT));
    }

    {
        ...
        EXPECT_EQ(info.meta.root,
                  azteclabs::wsdb::fr("0x2590f2aab19dd791700b4a43d3f52bb88ef2409a3731da8e848663559202e4c6"));
    }

    {
        ...
        EXPECT_EQ(info.meta.root,
                  azteclabs::wsdb::fr("0x1bef38b621017d3c7416663d0cd81369424560710526a3fbaaec13e356b9d084"));
    }

    {
        ...
        EXPECT_EQ(info.meta.root,
                  azteclabs::wsdb::fr("0x0fef6d80d31109ddb56d6b3f607cbc9c0af0bff3ea0d43e8f278983c64c11f7a"));
    }

    {
        ...
        EXPECT_EQ(info.meta.root, azteclabs::wsdb::fr(GENESIS_ARCHIVE_ROOT));
        ...
        assert_leaf_value(ws,
                          ...
                          MerkleTreeId::ARCHIVE,
                          0,
                          azteclabs::wsdb::fr(GENESIS_BLOCK_HEADER_HASH));
    }
}
...

    {
        auto state_ref = ws.get_state_reference(WorldStateRevision::committed());
        {
            ...
            EXPECT_EQ(
                snapshot,
                std::make_pair(
                    azteclabs::wsdb::fr("0x18935581a8ed73d08ffd00386fba55ba6c89f3ab848a76b8fedfa9034cee0454"), 128UL));
        }

        {
            ...
            EXPECT_EQ(
                snapshot,
                std::make_pair(
                    azteclabs::wsdb::fr("0x2590f2aab19dd791700b4a43d3f52bb88ef2409a3731da8e848663559202e4c6"), 0UL));
        }

        {
            ...
            EXPECT_EQ(
                snapshot,
                std::make_pair(
                    azteclabs::wsdb::fr("0x1bef38b621017d3c7416663d0cd81369424560710526a3fbaaec13e356b9d084"), 128UL));
        }

        {
            ...
            EXPECT_EQ(
                snapshot,
                std::make_pair(
                    azteclabs::wsdb::fr("0x0fef6d80d31109ddb56d6b3f607cbc9c0af0bff3ea0d43e8f278983c64c11f7a"), 0UL));
        }
    }

    {
        ws.append_leaves<azteclabs::wsdb::fr>(MerkleTreeId::NOTE_HASH_TREE, { 1 });
...

        {
            ...
            EXPECT_EQ(
                snapshot,
                std::make_pair(
                    azteclabs::wsdb::fr("0x278449ce779093eaaa1bd9fc608f7ed7b595417710e2baf359e1488650967f47"), 1UL));
        }
...
TEST_F(WorldStateTest, SyncBlockRejectsDivergentArchiveRoot)
{
    ...
    azteclabs::wsdb::fr previous_root;
    azteclabs::wsdb::fr resulting_root;
    {
        WorldState scratch(
...
{
    WorldState ws(thread_pool_size, data_dir, map_size, tree_heights, tree_prefill, initial_header_generator_point);
    azteclabs::wsdb::fr block_hash(1);
    StateReference block_state_ref = {
        { MerkleTreeId::NULLIFIER_TREE,
          { fr("0x2e2e2d8b72294a440c728a646f01476624063f0b50dcfe293cc0fc26bef9e311"), 129 } },
        { MerkleTreeId::NOTE_HASH_TREE,
...
TEST_F(WorldStateTest, ForkingAtBlock0AndAdvancingFork)
{
    ...
    ws.append_leaves<azteclabs::wsdb::fr>(MerkleTreeId::ARCHIVE, { fr(1) }, fork_id);
...
TEST_F(WorldStateTest, ForkingAtBlock0AndAdvancingCanonicalState)
{
    ...
    // fork the state
    ws.append_leaves<azteclabs::wsdb::fr>(MerkleTreeId::ARCHIVE, { fr(1) });
    ws.append_leaves<azteclabs::wsdb::fr>(MerkleTreeId::ARCHIVE, { fr(2) }, fork_id);
    ...
    assert_leaf_value<azteclabs::wsdb::fr>(
        ws, WorldStateRevision{ .includeUncommitted = false }, MerkleTreeId::ARCHIVE, 1, 1);
...
TEST_F(WorldStateTest, BuildsABlockInAFork)
{
    ...
    ws.append_leaves<azteclabs::wsdb::fr>(MerkleTreeId::NOTE_HASH_TREE, { 42 }, fork_id);
    ws.append_leaves<azteclabs::wsdb::fr>(MerkleTreeId::L1_TO_L2_MESSAGE_TREE, { 43 }, fork_id);
...
TEST_F(WorldStateTest, SyncEmptyBlockAcceptsBogusHashWithoutArchiveCheck)
{
    ...
    azteclabs::wsdb::fr previous_archive_root;
    azteclabs::wsdb::fr canonical_archive_root;
    {
        WorldState scratch(
            thread_pool_size, data_dir, map_size, tree_heights, tree_prefill, initial_header_generator_point);
    ...
    azteclabs::wsdb::fr actual_archive_root =
        ws.get_tree_info(WorldStateRevision::committed(), MerkleTreeId::ARCHIVE).meta.root;
