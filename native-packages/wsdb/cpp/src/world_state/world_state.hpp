...
class WorldState {
  public:
     ...
     *
     * @tparam T the type of the leaf. Either azteclabs::wsdb::fr, NullifierLeafValue, PublicDataLeafValue
     * @param revision The revision to query
     * @param tree_id The ID of the tree
     * @param leaf_index The index of the leaf
    ...
    WorldStateStatusFull sync_block(
        ...
        const std::optional<azteclabs::wsdb::fr>& expected_archive_root = std::nullopt,
        const std::optional<azteclabs::wsdb::fr>& expected_previous_archive_root = std::nullopt);
  ...
  private:
    ...
    bool is_archive_tip(const WorldStateRevision& revision, const azteclabs::wsdb::fr& block_header_hash) const;

    bool is_same_state_reference(const WorldStateRevision& revision, const StateReference& state_ref) const;
    static azteclabs::wsdb::fr compute_initial_block_header_hash(const StateReference& initial_state_ref,
                                      ...
                                      index_t leaf_index) const
{
    ...
    Signal signal;
    if constexpr (std::is_same_v<azteclabs::wsdb::fr, T>) {
