...
#include "field/merkle_hash_policy_for.hpp"
...
using merkle_tree::BatchInsertionResult;
using merkle_tree::SequentialInsertionResult;
...
class WorldState {
  public:
     ...
     * @return TreeInfo
     */
    merkle_tree::TreeMetaResponse get_tree_info(const WorldStateRevision& revision, MerkleTreeId tree_id) const;
     ...
     * @return merkle_tree::fr_sibling_path
     */
    merkle_tree::fr_sibling_path get_sibling_path(const WorldStateRevision& revision,
     ...
     */
    template <typename T>
    std::optional<merkle_tree::IndexedLeaf<T>> get_indexed_leaf(const WorldStateRevision& revision,
                                                                MerkleTreeId tree_id,
                                                                index_t leaf_index) const;
     ...
     *
     * @tparam T the type of the leaf. Either fr, NullifierLeafValue, PublicDataLeafValue
     * @param revision The revision to query
     * @param tree_id The ID of the tree
     * @param leaf_index The index of the leaf
     ...
     */
    merkle_tree::GetLowIndexedLeafResponse find_low_leaf_index(const WorldStateRevision& revision,
                                                               MerkleTreeId tree_id,
     ...
     */
    void update_public_data(const merkle_tree::PublicDataLeafValue& new_value, Fork::Id fork_id = CANONICAL_FORK_ID);
    ...
    WorldStateStatusFull sync_block(const StateReference& block_state_ref,
                                    ...
                                    const std::optional<fr>& expected_archive_root = std::nullopt,
                                    const std::optional<fr>& expected_previous_archive_root = std::nullopt);
  ...
  private:
    std::shared_ptr<azteclabs::wsdb::ThreadPool> _workers;
    ...
    bool is_archive_tip(const WorldStateRevision& revision, const fr& block_header_hash) const;

    bool is_same_state_reference(const WorldStateRevision& revision, const StateReference& state_ref) const;
    static fr compute_initial_block_header_hash(const StateReference& initial_state_ref,
...
template <typename T>
std::optional<merkle_tree::IndexedLeaf<T>> WorldState::get_indexed_leaf(const WorldStateRevision& rev,
                                                                        MerkleTreeId id,
                                                                        index_t leaf) const
{
                                      ...
                                      index_t leaf_index) const
{
    ...
    Signal signal;
    if constexpr (std::is_same_v<fr, T>) {
                                                                ...
                                                                Fork::Id fork_id)
{
    ...
    wrapper.tree->add_or_update_values(
        leaves, subtree_depth, [&](const TypedResponse<AddIndexedDataResponse<T>>& response) {
            if (response.success) {
                result.lowLeafWitnessData = *response.inner.lowLeafWitnessData;
                result.sortedLeaves = *response.inner.sortedLeaves;
                result.subtreePath = response.inner.subtreePath;
            } else {
                success = false;
                error_msg = response.message;
            }
                                                               ...
                                                               Fork::Id fork_id)
{
    ...
    wrapper.tree->add_or_update_values_sequentially(
        leaves, [&](const TypedResponse<AddIndexedDataSequentiallyResponse<T>>& response) {
            if (response.success) {
                result.lowLeafWitnessData = *response.inner.lowLeafWitnessData;
                result.insertionWitnessData = *response.inner.insertionWitnessData;
            } else {
                success = false;
                error_msg = response.message;
            }
