...
class ReferenceConformanceTest : public ::testing::Test {
  protected:
    ...
    static WorldStateRevision revision() { return uncommitted_revision(); }
...
TEST_F(ReferenceConformanceTest, InsertNullifiers)
{
    ...
    for (const fr& nullifier : { fr(1000), fr(500), fr(1000000), fr(750) }) {
        ...
        ASSERT_FALSE(ws_result.lowLeafWitnessData.empty());
        EXPECT_EQ(low.index, ws_result.lowLeafWitnessData[0].index);
        ASSERT_FALSE(ws_result.insertionWitnessData.empty());
        EXPECT_EQ(new_index, ws_result.insertionWitnessData[0].index);
