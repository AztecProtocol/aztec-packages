...
inline std::vector<wire::TreeStateReference> state_reference_to_wire(const world_state::StateReference& d)
{
    ...
    for (const auto& [tree_id, tree_ref] : d) {
        r.push_back({ .treeId = tree_id_to_wire(tree_id), .root = tree_ref.first, .size = tree_ref.second });
    }
    return r;
}
...
inline world_state::StateReference state_reference_from_wire(const std::vector<wire::TreeStateReference>& w)
{
    ...
    for (const auto& entry : w) {
        r.emplace(tree_id_from_wire(entry.treeId), world_state::TreeStateReference{ entry.root, entry.size });
    }
    return r;
}

inline wire::DBStats db_stats_to_wire(const azteclabs::lmdblib::DBStats& d)
{
    return { .name = d.name, .numDataItems = d.numDataItems, .totalUsedSize = d.totalUsedSize };
}

inline azteclabs::lmdblib::DBStats db_stats_from_wire(const wire::DBStats& w)
{
    return azteclabs::lmdblib::DBStats(w.name, w.numDataItems, w.totalUsedSize);
}

inline wire::TreeDBStats tree_db_stats_to_wire(const azteclabs::wsdb::merkle_tree::TreeDBStats& d)
{
...
inline azteclabs::wsdb::merkle_tree::TreeDBStats tree_db_stats_from_wire(const wire::TreeDBStats& w)
{
    return { w.mapSize,
             w.physicalFileSize,
...
inline wire::TreeMeta tree_meta_to_wire(const azteclabs::wsdb::merkle_tree::TreeMeta& d)
{
    return { .name = d.name,
             .depth = d.depth,
             .size = d.size,
...
inline azteclabs::wsdb::merkle_tree::TreeMeta tree_meta_from_wire(const wire::TreeMeta& w)
{
    return { w.name,
             w.depth,
             w.size,
...
inline wire::WorldStateDBStats world_state_db_stats_to_wire(const azteclabs::wsdb::world_state::WorldStateDBStats& d)
{
...
inline azteclabs::wsdb::world_state::WorldStateDBStats world_state_db_stats_from_wire(const wire::WorldStateDBStats& w)
{
...
inline wire::WorldStateMeta world_state_meta_to_wire(const azteclabs::wsdb::world_state::WorldStateMeta& d)
{
...
inline azteclabs::wsdb::world_state::WorldStateMeta world_state_meta_from_wire(const wire::WorldStateMeta& w)
{
...
inline wire::WorldStateStatusSummary world_state_status_summary_to_wire(
    const azteclabs::wsdb::world_state::WorldStateStatusSummary& d)
{
...
inline azteclabs::wsdb::world_state::WorldStateStatusSummary world_state_status_summary_from_wire(
...
inline wire::WorldStateStatusFull world_state_status_full_to_wire(
    const azteclabs::wsdb::world_state::WorldStateStatusFull& d)
{
...
inline azteclabs::wsdb::world_state::WorldStateStatusFull world_state_status_full_from_wire(
    const wire::WorldStateStatusFull& w)
{
