...
struct indexed_nullifier_leaf {
    ...
    MSGPACK_DEFINE_MAP(value, nextIndex, nextValue);
...
template <typename HashingPolicy> class WrappedNullifierLeaf {
  ...
  public:
     ...
     *
     * @return azteclabs::wsdb::fr
     */
    azteclabs::wsdb::fr hash() const
    {
        return data.has_value() ? HashingPolicy::hash(data.value().get_hash_inputs()) : azteclabs::wsdb::fr::zero();
    }
