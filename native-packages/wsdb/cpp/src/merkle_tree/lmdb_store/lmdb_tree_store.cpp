...
void LMDBTreeStore::get_stats(TreeDBStats& stats, ReadTransaction& tx)
{
    ...
    auto db_stats = [&](LMDBDatabase& db) {
        azteclabs::lmdblib::DBStats s = db.get_stats(tx);
        return DBStats{ .name = s.name, .numDataItems = s.numDataItems, .totalUsedSize = s.totalUsedSize };
    };
    stats.blocksDBStats = db_stats(*_blockDatabase);
    stats.leafPreimagesDBStats = db_stats(*_leafHashToPreImageDatabase);
    stats.leafIndicesDBStats = db_stats(*_leafKeyToIndexDatabase);
    stats.nodesDBStats = db_stats(*_nodeDatabase);
    stats.blockIndicesDBStats = db_stats(*_indexToBlockDatabase);
}
