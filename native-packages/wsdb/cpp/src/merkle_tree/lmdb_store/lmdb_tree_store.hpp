...
struct BlockPayload {
    ...
    MSGPACK_DEFINE_MAP(size, blockNumber, root)
...
struct NodePayload {
    ...
    MSGPACK_DEFINE_MAP(left, right, ref)
...
// Append-only trees persist their leaf preimages as IndexedLeaf<fr>, which never crosses the
// IPC boundary and so has no wire record; this is its storage record.
struct IndexedFrPayload {
    fr leaf;
    index_t nextIndex;
    fr nextKey;
    MSGPACK_DEFINE_MAP(leaf, nextIndex, nextKey)
};
template <> struct Wire<IndexedLeaf<fr>> {
    using type = IndexedFrPayload;
    static type to(const IndexedLeaf<fr>& d)
    {
        return { .leaf = d.leaf, .nextIndex = d.nextIndex, .nextKey = d.nextKey };
    }
    static IndexedLeaf<fr> from(const type& w) { return { w.leaf, w.nextIndex, w.nextKey }; }
};
...
bool LMDBTreeStore::read_leaf_by_hash(const fr& leafHash, LeafType& leafData, TxType& tx)
{
    ...
    if (success) {
        wire_t<LeafType> wire_leaf;
        msgpack::unpack((const char*)data.data(), data.size()).get().convert(wire_leaf);
        leafData = from_wire<LeafType>(wire_leaf);
    }
    return success;
}
...
void LMDBTreeStore::write_leaf_by_hash(const fr& leafHash, const LeafType& leafData, WriteTransaction& tx)
{
    ...
    msgpack::pack(buffer, to_wire(leafData));
