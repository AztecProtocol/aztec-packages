...
TEST_F(LMDBTreeStoreTest, fr_keys_serialise_as_canonical_little_endian)
{
    ...
    for (const auto& c : cases) {
        FrKeyType key(azteclabs::wsdb::fr{ std::string(c.big_endian_hex) });
...
{
    index_t index = 47;
    azteclabs::wsdb::fr key = get_value(5);
    LMDBTreeStore store(_directory, "DB1", _mapSize, _maxReaders);
    {
        LMDBWriteTransaction::Ptr transaction = store.create_write_transaction();
...
TEST_F(LMDBTreeStoreTest, can_write_and_read_nodes)
{
    ...
    nodePayload.ref = 4;
    azteclabs::wsdb::fr key = get_value(6);
    LMDBTreeStore store(_directory, "DB1", _mapSize, _maxReaders);
    {
        LMDBWriteTransaction::Ptr transaction = store.create_write_transaction();
...
TEST_F(LMDBTreeStoreTest, can_write_and_read_leaves_by_hash)
{
    ...
    azteclabs::wsdb::fr key = get_value(2);
    LMDBTreeStore store(_directory, "DB1", _mapSize, _maxReaders);
    {
        LMDBWriteTransaction::Ptr transaction = store.create_write_transaction();
