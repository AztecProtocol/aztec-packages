...
template <typename LeafValueType> struct BatchInsertionResult {
    std::vector<LeafUpdateWitnessData<LeafValueType>> lowLeafWitnessData;
    std::vector<std::pair<LeafValueType, index_t>> sortedLeaves;
    fr_sibling_path subtreePath;
};

template <typename LeafValueType> struct SequentialInsertionResult {
    std::vector<LeafUpdateWitnessData<LeafValueType>> lowLeafWitnessData;
    std::vector<LeafUpdateWitnessData<LeafValueType>> insertionWitnessData;
};

template <typename LeafValueType> struct AddIndexedDataResponse {
    AddDataResponse add_data_result;
    fr_sibling_path subtreePath;
    std::shared_ptr<std::vector<std::pair<LeafValueType, index_t>>> sortedLeaves;
...
template <typename LeafValueType> struct AddIndexedDataSequentiallyResponse {
    AddDataResponse add_data_result;
    std::shared_ptr<std::vector<LeafUpdateWitnessData<LeafValueType>>> lowLeafWitnessData;
    std::shared_ptr<std::vector<LeafUpdateWitnessData<LeafValueType>>> insertionWitnessData;
...
using SiblingPathAndIndex = wire::SiblingPathAndIndex;
