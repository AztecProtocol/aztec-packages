...
#include "merkle_tree/nullifier_tree/nullifier_memory_tree.hpp"
#include "merkle_tree/response.hpp"
#include "merkle_tree/test_fixtures.hpp"
...
using namespace azteclabs::wsdb;
...
void test_batch_insert(uint32_t batchSize, std::string directory, uint64_t mapSize, uint64_t maxReaders)
{
    ...
    for (uint32_t i = 0; i < num_batches; i++) {
        ...
        std::shared_ptr<std::vector<LeafUpdateWitnessData<NullifierLeafValue>>> tree2_low_leaf_witness_data;
        {
            ...
            CompletionCallback completion =
                [&](const TypedResponse<AddIndexedDataResponse<NullifierLeafValue>>& response) {
                    tree1_low_leaf_witness_data = response.inner.lowLeafWitnessData;
                    signal.signal_level();
                };
...

        {
            ...
            CompletionCallback completion =
                [&](const TypedResponse<AddIndexedDataResponse<NullifierLeafValue>>& response) {
                    tree2_low_leaf_witness_data = response.inner.lowLeafWitnessData;
                    signal.signal_level();
                };
...
void test_sequential_insert_vs_batch(uint32_t batchSize, std::string directory, uint64_t mapSize, uint64_t maxReaders)
{
    ...
    for (uint32_t i = 0; i < num_batches; i++) {
...

        {
            ...
            SequentialCompletionCallback completion =
                [&](const TypedResponse<AddIndexedDataSequentiallyResponse<NullifierLeafValue>>& response) {
                    sequential_tree_1_low_leaf_witness_data = response.inner.lowLeafWitnessData;
                    sequential_tree_1_insertion_witness_data = response.inner.insertionWitnessData;
                    signal.signal_level();
                };
...

        {
            ...
            SequentialCompletionCallback completion =
                [&](const TypedResponse<AddIndexedDataSequentiallyResponse<NullifierLeafValue>>& response) {
                    sequential_tree_2_low_leaf_witness_data = response.inner.lowLeafWitnessData;
                    sequential_tree_2_insertion_witness_data = response.inner.insertionWitnessData;
                    signal.signal_level();
                };
