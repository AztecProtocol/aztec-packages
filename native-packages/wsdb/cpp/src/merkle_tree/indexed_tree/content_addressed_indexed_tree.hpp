    ...
    bool capture_witness)
{
    ...
    struct IntermediateResults {
        ...
        std::shared_ptr<std::vector<LeafUpdateWitnessData<LeafValueType>>> lowLeafWitnessData;
        fr_sibling_path subtreePath;
    ...
    auto final_completion = [=](const TypedResponse<AddDataResponse>& add_data_response) {
        ...
        if (add_data_response.success) {
            if (capture_witness) {
                response.inner.subtreePath = std::move(results->subtreePath);
                response.inner.sortedLeaves = std::move(values_to_be_sorted);
                response.inner.lowLeafWitnessData = std::move(results->lowLeafWitnessData);
            }
    ...
    auto sibling_path_completion = [=, this](const TypedResponse<GetSiblingPathResponse>& response) {
        ...
        if (capture_witness) {
            results->subtreePath = std::move(response.inner.path);
        }
    ...
    UpdatesCompletionCallback updates_completion =
        [=, this](const TypedResponse<UpdatesCompletionResponse>& updates_response) {
            if (!updates_response.success) {
                ...
                results->lowLeafWitnessData = updates_response.inner.update_witnesses;
            }
    ...
    bool capture_witness)
{
    ...
    auto final_completion = [=, this](const TypedResponse<UpdatesCompletionResponse>& updates_completion_response) {
        ...
        if (updates_completion_response.success) {
            ...
            if (capture_witness) {
                // Split results->update_witnesses between lowLeafWitnessData and insertionWitnessData
                response.inner.insertionWitnessData =
                    std::make_shared<std::vector<LeafUpdateWitnessData<LeafValueType>>>();
                response.inner.insertionWitnessData->reserve(results->updates_to_perform.size());

                response.inner.lowLeafWitnessData =
                    std::make_shared<std::vector<LeafUpdateWitnessData<LeafValueType>>>();
                response.inner.lowLeafWitnessData->reserve(results->updates_to_perform.size());
                ...
                for (size_t i = 0; i < results->updates_to_perform.size(); ++i) {
                    ...
                    response.inner.lowLeafWitnessData->push_back(low_leaf_witness);
                    ...
                    if (results->updates_to_perform.at(i).new_leaf.has_value()) {
                        LeafUpdateWitnessData<LeafValueType> insertion_witness =
                            updates_completion_response.inner.update_witnesses->at(current_witness_index++);
                        response.inner.insertionWitnessData->push_back(insertion_witness);
                    } else {
                        // If it's an update, append an empty witness
                        response.inner.insertionWitnessData->push_back(LeafUpdateWitnessData<LeafValueType>(
