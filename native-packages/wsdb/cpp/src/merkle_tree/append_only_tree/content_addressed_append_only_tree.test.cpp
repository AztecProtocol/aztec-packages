...
#include "merkle_tree/memory_tree.hpp"
...
using Store = ContentAddressedCachedTreeStore<fr>;
                                 ...
                                 bool expected_result = true)
{
    ...
    auto completion = [&](const TypedResponse<FindLeafPathResponse>& response) -> void {
        ...
        if (expected_result) {
            ...
            EXPECT_EQ(from_wire<fr_sibling_path>(response.inner.leaf_paths[0].value().path), expected_sibling_path);
