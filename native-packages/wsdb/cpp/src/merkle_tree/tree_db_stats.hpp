...
using DBStats = bb::lmdblib::DBStats;
...
struct TreeDBStats {
  ...
  TreeDBStats(uint64_t mapSize, uint64_t physicalFileSize)
      : mapSize(mapSize), physicalFileSize(physicalFileSize) {}
  TreeDBStats(uint64_t mapSize, uint64_t physicalFileSize,
              const DBStats &blockStats, const DBStats &nodesStats,
              const DBStats &leafPreimagesDBStats,
              const DBStats &leafIndicesStats, const DBStats &blockIndicesStats)
      : mapSize(mapSize), physicalFileSize(physicalFileSize),
        blocksDBStats(blockStats), nodesDBStats(nodesStats),
  ...
  SERIALIZATION_FIELDS(mapSize, physicalFileSize, blocksDBStats, nodesDBStats,
                       leafPreimagesDBStats, leafIndicesDBStats,
                       blockIndicesDBStats)

  bool operator==(const TreeDBStats &other) const {
    return mapSize == other.mapSize &&
           physicalFileSize == other.physicalFileSize &&
           blocksDBStats == other.blocksDBStats &&
           nodesDBStats == other.nodesDBStats &&
           leafPreimagesDBStats == other.leafPreimagesDBStats &&
  ...
  friend std::ostream &operator<<(std::ostream &os, const TreeDBStats &stats) {
    os << "Map Size: " << stats.mapSize
       << ", Physical File Size: " << stats.physicalFileSize << " Blocks DB "
       << stats.blocksDBStats << ", Nodes DB " << stats.nodesDBStats
       << ", Leaf Pre-images DB " << stats.leafPreimagesDBStats
       << ", Leaf Indices DB " << stats.leafIndicesDBStats
       << ", Block Indices DB " << stats.blockIndicesDBStats;
    return os;
  }
};
...
struct RemoveHistoricResponse {
  ...
  operator=(const RemoveHistoricResponse &other) = default;
  RemoveHistoricResponse &
  operator=(RemoveHistoricResponse &&other) noexcept = default;
};
