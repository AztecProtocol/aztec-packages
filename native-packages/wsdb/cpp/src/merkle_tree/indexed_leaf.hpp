...
struct PublicDataLeafValue {
    ...
    size_t hash() const noexcept { return std::hash<fr>{}(value) ^ (std::hash<fr>{}(slot) << 1); }
...
template <typename LeafType> struct IndexedLeaf {
    ...
    MSGPACK_DEFINE_MAP(leaf, nextIndex, nextKey)
    ...
    // The next-pointer check keeps a real low leaf whose value happens to be zero (e.g. the head of the list)
    // from being mistaken for padding.
