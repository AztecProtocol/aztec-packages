 ...
 * serialisation (big-endian): byte order == numeric order. It carries no serialisation of its own:
 * the IPC wire and the lmdb store both encode it as the generated `wire::Fr` (a 32-byte bin),
 * which it converts to and from implicitly.
 */
struct FieldElement {
    ...
    // The generated wire aliases (`wire::Fr`, `wire::Nullifier`, ...) are the same 32
    // canonical bytes under a nominal tag, so they convert both ways implicitly. Detected
    // by the marker typedef the codegen stamps on `ipc::Bin32Alias`, which keeps this header
    // independent of the generated types.
    template <typename W>
        requires requires { typename W::IPC_CODEGEN_BIN32_ALIAS; }
    FieldElement(const W& w)
        : bytes(w.value)
    {}
    template <typename W>
        requires requires { typename W::IPC_CODEGEN_BIN32_ALIAS; }
    ...
    // Construct from a small integer (canonical big-endian), matching bb::fr(uint64_t).
    ...
    // Construct from a "0x..."-style hex string (big-endian), matching bb::fr(std::string).
    ...
    // Test/fixture helper, mirroring bb::fr::random_element().
    ...
    {
        return bytes == w.value;
    }

    [[nodiscard]] const uint8_t* data() const { return bytes.data(); }
    uint8_t* data() { return bytes.data(); }

    // Raw 32-byte (de)serialisation, mirroring bb::fr's buffer API.
