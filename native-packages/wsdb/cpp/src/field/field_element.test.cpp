...
// The wire scalar FieldElement serialises through is msgpack byte-identical to bb::fr => on-disk lmdb data
// and the IPC wire are unchanged.
TEST(FieldElementMsgpack, ByteIdenticalToBbFr)
{
    for (int i = 0; i < 256; ++i) {
        bb::fr a = bb::fr::random_element();
        msgpack::sbuffer fe_buf;
        msgpack::pack(fe_buf, azteclabs::wsdb::wire::Fr(to_fe(a)));
        ...
        azteclabs::wsdb::wire::Fr rt;
        msgpack::unpack(fe_buf.data(), fe_buf.size()).get().convert(rt);
        EXPECT_EQ(azteclabs::wsdb::FieldElement(rt), to_fe(a));
    }
}
