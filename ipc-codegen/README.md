...
Produces `myservice_types.hpp`, `myservice_ipc_client.{hpp,cpp}`,
`myservice_ipc_server.hpp`, plus the `msgpack_struct_map_impl.hpp` template.
Cross-includes use the supplied `--cpp-include-dir` prefix
