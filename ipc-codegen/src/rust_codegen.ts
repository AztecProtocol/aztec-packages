...
`;
  ...
  private generateApiMethod(command: {name: string, fields: Field[], responseType: string}): string {
    ...
    // Extract error type name from the error import (e.g., 'BarretenbergError' from 'crate::error::{BarretenbergError, Result}')
    const errorType = this.opts.errorImport.match(/\{(\w+),/)?.[1] ?? 'BarretenbergError';
...
` : '';
    ...
    const errorType = errorImport.match(/\{(\w+),/)?.[1] ?? 'BarretenbergError';
