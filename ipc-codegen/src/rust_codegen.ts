...
export class RustCodegen {
  ...
  private needsSerdeFixedBytes(type: Type): boolean {
    return (
      type.kind === "array" && type.size! <= 32 && this.isU8(type.element!)
    );
  }
...
`;
  ...
  /**
   * In-process FFI entry (the ipc-codegen FFI backend contract) over the generated dispatch: the
   * byte-level call, malloc-compatible buffers, and a macro defining the exported C symbols.
   */
  /**
   * Exported symbol names carry the service (`bb_ipc_ffi_entry`), so libraries of several services
   * can be linked into one binary. A schema with no service/prefix keeps the bare names.
   */
  private ffiSymbol(name: string): string {
    const prefix = this.opts.prefix ? `${toSnakeCase(this.opts.prefix)}_` : "";
    return `${prefix}${name}`;
  }

  /**
   * Client-side FFI backend: the generated client over a linked library that exports this
   * service's FFI entry (e.g. one built from the `--server --ffi` output, or a wasm reactor's
   * native twin).
   */
  generateFfiBackend(): string {
    const { prefix, apiStructName } = this.opts;
    const entry = this.ffiSymbol("ipc_ffi_entry");
    const free = this.ffiSymbol("ipc_ffi_free");

    return `//! AUTOGENERATED - DO NOT EDIT
//! FFI client backend for ${prefix || "service"}: calls the service's in-process FFI entry directly,
//! with no process and no IPC.
//!
//! Link against a library exporting this service's ipc-codegen FFI contract (\`${entry}\` and
//! \`${free}\`) and add the \`-L\` / \`-l\` directives to your \`build.rs\` (or \`.cargo/config.toml\`).
//!
//! \`\`\`ignore
//! let mut api = ${apiStructName}::new(FfiBackend::new()?);
//! \`\`\`
...
extern "C" {
    /// Execute a msgpack-encoded command and return the msgpack-encoded response.
    ///
    /// # Safety
    /// - \`input\` must point to valid memory of \`input_len\` bytes
    /// - \`output\` and \`output_len\` must be valid pointers
    /// - the caller must release \`*output\` with [\`ipc_ffi_free\`]
    #[link_name = "${entry}"]
    fn ipc_ffi_entry(input: *const u8, input_len: usize, output: *mut *mut u8, output_len: *mut usize);

    /// Release a buffer the entry handed out.
    ///
    /// # Safety
    /// \`ptr\` must have come from the entry and not have been freed already.
    #[link_name = "${free}"]
    fn ipc_ffi_free(ptr: *mut u8);
}

/// FFI backend that calls the service's library directly via its C ABI.
///
/// The most direct backend (no process spawn, no IPC), but the library has to be linked at
/// build time.
///
/// # Thread Safety
///
/// Not thread-safe: the FFI entry serializes nothing, so each thread should have its own
/// \`FfiBackend\` or callers should synchronize externally.
...
impl Backend for FfiBackend {
    fn call(&mut self, input: &[u8]) -> Result<Vec<u8>> {
        ...
        // SAFETY: input is valid for input.len() bytes; output_ptr/output_len are valid stack slots.
        unsafe {
            ipc_ffi_entry(input.as_ptr(), input.len(), &mut output_ptr, &mut output_len);
        }

        if output_ptr.is_null() {
            return Err(IpcError::Backend("FFI entry returned null pointer".to_string()));
        }

        if output_len == 0 {
            // SAFETY: the entry allocated output_ptr; it is freed exactly once.
            unsafe { ipc_ffi_free(output_ptr) };
            return Err(IpcError::Backend("FFI entry returned empty response".to_string()));
        }

        // SAFETY: output_ptr is valid for output_len bytes until freed below.
...
`;
  ...
  generateFfi(): string {
    ...
    const snake = toSnakeCase(prefix || "ipc");
    const macroName = `export_${snake}_ffi`;
    const entry = this.ffiSymbol("ipc_ffi_entry");
    const alloc = this.ffiSymbol("ipc_ffi_alloc");
    const free = this.ffiSymbol("ipc_ffi_free");

    return `//! AUTOGENERATED - DO NOT EDIT
//! In-process FFI entry for ${prefix || "service"}: the ipc-codegen FFI backend contract, with the
//! symbols prefixed by the service so several services can be linked into one binary.
//!
//! \`${macroName}!\` defines the exported C-ABI symbols \`${entry}\`, \`${alloc}\` and
//! \`${free}\` over one handler, built with \`Default\` on the first call and kept for the life
//! of the process, so stateful command sequences share state the way they would on one transport
//! connection. Invoke it once at the crate root of a \`cdylib\`/\`staticlib\`, or of a wasm reactor:
//!
//! \`\`\`ignore
//! ${macroName}!(crate::generated::${snake}_ffi, MyHandler);
//! \`\`\`
//!
//! Needs the \`libc\` crate for the malloc-compatible buffers.

use super::${snake}_server::{handle_request, Handler};

/// One FFI round trip: decode and dispatch \`input\`, leaving the response payload in a buffer
/// from [\`ffi_alloc\`] that the caller releases with [\`ffi_free\`].
///
/// # Safety
/// \`input\` must be valid for \`input_len\` bytes; \`output\` and \`output_len\` must be valid for writes.
pub unsafe fn ffi_call(
    ...
    let request: &[u8] = if input_len == 0 {
        &[]
    } else {
        unsafe { std::slice::from_raw_parts(input, input_len) }
    };
    let response = handle_request(handler, request);
    let buf = ffi_alloc(response.len());
    unsafe {
...
/// malloc-compatible allocation for buffers crossing the FFI boundary in either direction.
pub fn ffi_alloc(size: usize) -> *mut u8 {
    // A zero-size request still yields a distinct, freeable pointer.
    unsafe { libc::malloc(size.max(1)) as *mut u8 }
}

/// Release a buffer from [\`ffi_alloc\`].
///
/// # Safety
/// \`ptr\` must have come from [\`ffi_alloc\`] and not have been freed already.
pub unsafe fn ffi_free(ptr: *mut u8) {
    unsafe { libc::free(ptr as *mut libc::c_void) };
}

/// Define the exported C-ABI FFI symbols over \`$handler\` (a \`Handler + Default + Send\` type).
/// \`$ffi\` is the path of this module, which the expansion calls back into.
#[macro_export]
macro_rules! ${macroName} {
    ($ffi:path, $handler:ty) => {
        ...
        pub unsafe extern "C" fn ${entry}(
            ...
            use $ffi as ffi;
            static HANDLER: ::std::sync::Mutex<Option<$handler>> = ::std::sync::Mutex::new(None);
            let mut guard = HANDLER.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            let handler = guard.get_or_insert_with(<$handler as ::std::default::Default>::default);
            unsafe { ffi::ffi_call(handler, input, input_len, output, output_len) };
        }
