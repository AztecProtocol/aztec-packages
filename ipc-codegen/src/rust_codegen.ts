...
`;
  ...
  generateFfi(): string {
    ...
    return `//! AUTOGENERATED - DO NOT EDIT
//! In-process FFI entry for ${prefix || "service"}: the ipc-codegen FFI backend contract.
//!
//! \`${macroName}!\` defines the exported C-ABI symbols \`ipc_ffi_entry\`, \`ipc_ffi_alloc\` and
//! \`ipc_ffi_free\` over one handler, built with \`Default\` on the first call and kept for the life
...
macro_rules! ${macroName} {
    ($ffi:path, $handler:ty) => {
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn ipc_ffi_alloc(size: usize) -> *mut u8 {
            use $ffi as ffi;
            ffi::ffi_alloc(size)
        }

        #[unsafe(no_mangle)]
        pub unsafe extern "C" fn ipc_ffi_free(ptr: *mut u8) {
            use $ffi as ffi;
