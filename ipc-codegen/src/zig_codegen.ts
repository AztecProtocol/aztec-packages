...
export class ZigCodegen {
  ...
  /**
   * Client-side FFI backend over a linked library exporting this service's FFI entry. The symbols
   * carry the service (`bb_ipc_ffi_entry`) so several services can be linked into one binary.
   */
  generateFfiBackend(): string {
    const sym = this.opts.prefix ? `${toSnakeCase(this.opts.prefix)}_` : "";
    return `//! AUTOGENERATED - DO NOT EDIT
//! FFI client backend for ${this.opts.prefix || "service"}: calls the service's in-process FFI
//! entry directly (\`${sym}ipc_ffi_entry\`), with no process and no IPC. Link the library
//! exporting it in your build.zig.
//!
//! Satisfies the backend interface: call(request) -> response, destroy().
const std = @import("std");

extern fn ${sym}ipc_ffi_entry(input: [*]const u8, input_len: usize, output: *[*]u8, output_len: *usize) void;
extern fn ${sym}ipc_ffi_free(ptr: [*]u8) void;

/// Allocator contract: callers free returned slices with this allocator
/// (the generated client uses std.heap.page_allocator), so the FFI buffer is
/// copied into it and released with the service's own free — freeing it with
/// a Zig allocator is undefined behaviour.
...
pub const FfiBackend = struct {
    ...
    pub fn call(self: *FfiBackend, request: []const u8) ![]u8 {
        ...
        var out_len: usize = 0;
        ${sym}ipc_ffi_entry(request.ptr, request.len, &out_ptr, &out_len);
        defer ${sym}ipc_ffi_free(out_ptr);
