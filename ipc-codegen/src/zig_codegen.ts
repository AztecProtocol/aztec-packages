...
`;
  ...
  generateServer(schema: CompiledSchema): string {
    ...
    // Dispatch cases: match command name → deserialize → call async handler with
    // a Responder → return the response it produced.
    const dispatchCases = schema.commands
      .map((c) => {
        ...
        return `            if (std.mem.eql(u8, cmd_name, "${c.name}")) {
                ...
                var responder = Responder(types.${zigRespName}){ .resp_name = "${c.responseType}" };
                self.handler.${methodName}(cmd, &responder);
                return responder.result orelse makeErrorFmt("${c.name}: handler did not respond", .{});
            }`;
      })
      .join("\n");
...
//!
//! The handler is any type with one (async) method per command, producing its
//! result via responder.ok(value) / responder.err(message):
//!     pub fn ${handlerMethodNames[0] ?? "command"}(self: *@This(), cmd: types.${toPascalCase(schema.commands[0]?.name ?? "Command")}, respond: *Responder(types.${toPascalCase(schema.commands[0]?.responseType ?? "Response")})) void
//! respond.err(message) produces the schema error variant.
//!
...
/// Typed response callback handed to each handler. The handler calls ok(resp)
/// or err(message) exactly once (synchronously here; an async transport could
/// defer). It encodes the response and stores it for the dispatcher to return.
pub fn Responder(comptime RespType: type) type {
    return struct {
        const Self = @This();
        resp_name: []const u8,
        result: ?DispatchResult = null,

        pub fn ok(self: *Self, resp: RespType) void {
            const payload = resp.toPayload(alloc) catch |e| {
                self.result = makeErrorFmt("encode of response failed: {s}", .{@errorName(e)});
                return;
            };
            self.result = .{ .resp_name = self.resp_name, .resp_payload = payload };
        }

        pub fn err(self: *Self, message: []const u8) void {
            self.result = makeErrorFmt("{s}", .{message});
        }
    };
}
