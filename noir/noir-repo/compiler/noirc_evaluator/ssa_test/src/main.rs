...
fn main() {
    ...
    let u2_var = Variable{ variable_type: VariableType::Unsigned, variable_size: 2};
    let i2_var = Variable{ variable_type: VariableType::Signed, variable_size: 1};
    let u100_var = Variable{ variable_type: VariableType::Unsigned, variable_size: 100};
    all_artifacts.push(InstructionArtifacts::new_div(&u2_var, &u2_var));
    all_artifacts.push(InstructionArtifacts::new_div(&i2_var, &i2_var));
    all_artifacts.push(InstructionArtifacts::new_div(&u100_var, &u100_var));

    let u33_var = Variable{ variable_type: VariableType::Unsigned, variable_size: 33};
    all_artifacts.push(InstructionArtifacts::new_xor(&u33_var, &u33_var));
    all_artifacts.push(InstructionArtifacts::new_and(&u33_var, &u33_var));
