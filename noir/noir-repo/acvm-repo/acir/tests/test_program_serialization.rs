...
use acir::{
    circuit::{
        ...
        opcodes::{BlackBoxFuncCall, BlockId, FunctionInput, MemOp, WitnessInput},
...
fn multi_scalar_mul_circuit() {
    let multi_scalar_mul: Opcode<FieldElement> =
        Opcode::BlackBoxFuncCall(BlackBoxFuncCall::MultiScalarMul {
            points: vec![
                FunctionInput::Witness(WitnessInput { witness: Witness(1), num_bits: 128 }),
                FunctionInput::Witness(WitnessInput { witness: Witness(2), num_bits: 128 }),
                FunctionInput::Witness(WitnessInput { witness: Witness(3), num_bits: 1 }),
            ],
            scalars: vec![
                FunctionInput::Witness(WitnessInput { witness: Witness(4), num_bits: 128 }),
                FunctionInput::Witness(WitnessInput { witness: Witness(5), num_bits: 128 }),
            ],
...
fn pedersen_circuit() {
    let pedersen = Opcode::BlackBoxFuncCall(BlackBoxFuncCall::PedersenCommitment {
        inputs: vec![FunctionInput::Witness(WitnessInput {
            witness: Witness(1),
    ...
    let expected_serialization: Vec<u8> = vec![
        31, 139, 8, 0, 0, 0, 0, 0, 0, 255, 93, 138, 219, 10, 0, 0, 4, 67, 221, 30, 252, 255, 7, 11,
        161, 176, 58, 109, 107, 67, 40, 73, 128, 157, 211, 245, 245, 196, 186, 83, 192, 107, 219,
        62, 251, 255, 29, 57, 150, 69, 205, 11, 117, 0, 0, 0,
    ];
    assert_eq!(bytes, expected_serialization)
}
...
fn schnorr_verify_circuit() {
    ...
    let signature: [FunctionInput<FieldElement>; 64] = (3..(3 + 64))
        .map(|i| FunctionInput::Witness(WitnessInput { witness: Witness(i), num_bits: 8 }))
        .collect::<Vec<_>>()
        .try_into()
        .unwrap();
