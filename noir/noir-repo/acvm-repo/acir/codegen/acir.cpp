...
struct BlackBoxOp {
  ...
  std::variant<Blake2s, Blake3, Keccak256, Keccakf1600, EcdsaSecp256k1,
...
struct BlackBoxFuncCall {
  ...
  std::variant<AND, XOR, RANGE, Blake2s, Blake3, SchnorrVerify,
...
struct Directive {
  ...
  struct BlackBoxFuncCall {
    ...
    struct EmbeddedCurveAdd {
      ...
      friend bool operator==(const EmbeddedCurveAdd &,
                             const EmbeddedCurveAdd &);
      std::vector<uint8_t> bincodeSerialize() const;
      static EmbeddedCurveAdd bincodeDeserialize(std::vector<uint8_t>);
    };
    ...
    std::variant<AND, XOR, RANGE, Blake2s, Blake3, SchnorrVerify,
                 PedersenCommitment, PedersenHash, EcdsaSecp256k1,
                 EcdsaSecp256r1, FixedBaseScalarMul, EmbeddedCurveAdd,
                 Keccak256, Keccak256VariableLength, Keccakf1600,
                 RecursiveAggregation, BigIntAdd, BigIntSub, BigIntMul,
                 BigIntDiv, BigIntFromLeBytes, BigIntToLeBytes,
                 Poseidon2Permutation, Sha256Compression>
        value;
  ...
  struct BinaryFieldOp {
    ...
    std::variant<Add, Sub, Mul, Div, IntegerDiv, Equals, LessThan,
                 LessThanEquals>
        value;
  ...
  struct BlackBoxOp {
    ...
    std::variant<Sha256, Blake2s, Blake3, Keccak256, Keccakf1600,
                 EcdsaSecp256k1, EcdsaSecp256r1, SchnorrVerify,
                 PedersenCommitment, PedersenHash, FixedBaseScalarMul,
  ...
  template <typename Serializer>
  void
  serde::Serializable<Program::BlockId>::serialize(const Program::BlockId &obj,
                                                   Serializer &serializer) {
    serializer.increase_container_depth();
    serde::Serializable<decltype(obj.value)>::serialize(obj.value, serializer);
    serializer.decrease_container_depth();
