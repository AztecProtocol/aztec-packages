...
struct BinaryIntOp {
  ...
  std::variant<Add, Sub, Mul, SignedDiv, UnsignedDiv, Equals, LessThan,
               LessThanEquals, And, Or, Xor, Shl, Shr>
      value;
...
inline bool operator==(const BinaryIntOp::SignedDiv &lhs,
...
inline BinaryIntOp::SignedDiv
BinaryIntOp::SignedDiv::bincodeDeserialize(std::vector<uint8_t> input) {
  auto deserializer = serde::BincodeDeserializer(input);
  auto value =
      serde::Deserializable<BinaryIntOp::SignedDiv>::deserialize(deserializer);
  if (deserializer.get_buffer_offset() < input.size()) {
    throw serde::deserialization_error("Some input bytes were not read");
  }
  return value;
...
template <typename Deserializer>
Program::Value
serde::Deserializable<Program::Value>::deserialize(Deserializer &deserializer) {
  deserializer.increase_container_depth();
  Program::Value obj;
  obj.inner =
