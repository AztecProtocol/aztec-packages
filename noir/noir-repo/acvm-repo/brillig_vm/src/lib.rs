...
impl<'a, B: BlackBoxFunctionSolver> VM<'a, B> {
    ...
    pub fn process_opcode(&mut self) -> VMStatus {
        ...
        match opcode {
            ...
            Opcode::Const { destination, value, bit_size } => {
                ...
                let memory_value = if *bit_size == FieldElement::max_num_bits() {
                    MemoryValue::new_field(*value)
                } else {
                    MemoryValue::new_integer(
    ...
    fn get_memory_values(
        ...
        match (input, value_type) {
            (ValueOrArray::MemoryAddress(value_index), HeapValueType::Simple(_)) => {
                self.memory.read(value_index).unify_to_field().into()
            }
            (
                ...
                self.read_slice_of_values_from_memory(start, size, value_types)
                    .into_iter()
                    .map(|mem_value| mem_value.unify_to_field())
                    .collect::<Vec<_>>()
                    .into()
            }
            (
    ...
    fn cast(&self, bit_size: u32, source_value: MemoryValue) -> MemoryValue {
        let lhs_big = source_value.unify_to_integer();
        let mask = BigUint::from(2_u32).pow(bit_size) - 1_u32;
        let masked = lhs_big & mask;
        if bit_size == FieldElement::max_num_bits() {
            MemoryValue::new_field(FieldElement::from_be_bytes_reduce(&masked.to_bytes_be()))
        } else {
            MemoryValue::new_integer(masked, bit_size)
        }
    }
}
