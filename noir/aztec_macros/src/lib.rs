...
fn inject_compute_note_hash_and_nullifier(
    ...
    // First we need to fetch all of the different note types a contract might use. These are all of the types that
    // implement the NoteInterface trait, which provides the get_note_type_id() function.
    
    let note_types: Vec<String> = unresolved_traits_impls.iter()
    ...
    // We can now generate a version of compute_note_hash_and_nullifier tailored for the contracts in this crate.
    let fn_compute_note_hash_and_nullifier = generate_compute_note_hash_and_nullifier(&note_types);
    
    // Then we fetch all of the modules which correspond to contracts
    let contract_module_file_ids: Vec<(LocalModuleId, FileId)> = context.def_map(crate_id).unwrap().modules().iter()
        .filter(|(_, module)| module.is_contract)
        .map(|(idx, module)| (LocalModuleId(idx), module.location.file))
        .collect();

    // And inject the newly created function into each of them. We perform two separate loops as def_map does not allow
    // for mutable iteration.
    for (module_id, file_id) in contract_module_file_ids {
        ...
        let location = Location::new(fn_compute_note_hash_and_nullifier.span(), file_id);

        dbg!(&module_id);
        dbg!(&file_id);
        
        // This is the same thing the ModCollector does when collecting functions. The only part we are not doing is we
        // are skipping the declaration of the function (i.e. adding it to the scope/namespace of the module) because we 
        // can't mutate the modules. This should not be an issue, since this function will not be called by any other
        // contract function.
        let func_id = context.def_interner.push_empty_fn();
        context.def_interner.push_function(func_id, &fn_compute_note_hash_and_nullifier.def, ModuleId { krate: *crate_id, local_id: module_id }, location);

        collected_functions.iter_mut().find(|fns| fns.file_id == file_id).unwrap().push_fn(module_id, func_id, fn_compute_note_hash_and_nullifier.clone());
    }

    Ok(())
}

fn generate_compute_note_hash_and_nullifier(_note_types: &Vec<String>) -> NoirFunction {
    ...
    let (function_ast, errors) = parse_program(unformatted_source);
    if !errors.is_empty() {
        dbg!(errors.clone());
    }
