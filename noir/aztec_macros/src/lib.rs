...
fn inject_compute_note_hash_and_nullifier(
    ...
    let mut contract_module_file_ids: Vec<(LocalModuleId, FileId)> = context.def_map(crate_id).unwrap().modules().iter()
    ...
    let (module_id, file_id) = contract_module_file_ids.pop().unwrap();
    ...
    note_types.extend(unresolved_traits_impls.iter()
        .filter(|trait_impl| trait_impl.trait_path.segments.last().unwrap().0.contents == "NoteInterface")
    ...
    // We don't have a reasonable location for the source code of this autogenerated function, but it should not
    // result in errors anyway so this should not matter. 
    let location = Location::new(func.span(), file_id);
    ...
    let Ok(_) = context.def_map_mut(crate_id).unwrap()
        .modules_mut()[module_id.0]
        .declare_function(
            func.name_ident().clone(), func_id
        )
    else {
        panic!("Failed to declare the autogenerated compute_note_hash_and_nullifier function, likely due to a duplicate definition. This should not happen");
    };

    collected_functions.iter_mut().find(|fns| fns.file_id == file_id).unwrap().push_fn(module_id, func_id, func.clone());
