...
fn run_command(
  ...
  if args.output_witness.is_some() {
    save_witness_to_dir(&output_witness_string, &args.working_directory, &args.output_witness.unwrap())?;
  }
...
pub(crate) fn execute_program_from_witness(
  ...
  let result = execute_circuit(&circuit, inputs_map.clone(), &blackbox_solver, &mut DefaultForeignCallExecutor::new(true, foreign_call_resolver_url)).map_err(|e| CliError::CircuitExecutionError(e));
  result
}
