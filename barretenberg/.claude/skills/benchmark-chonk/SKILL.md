...
This populates `barretenberg/cpp/example-app-ivc-inputs-out/<flow>/ivc-inputs.msgpack`.
...
- and more under `barretenberg/cpp/example-app-ivc-inputs-out/` after downloading
...
The benchmark script defaults to `ecdsar1+transfer_0_recursions+sponsored_fpc`. If a flow fails, the pinned inputs remain in `barretenberg/cpp/example-app-ivc-inputs-out/<flow>/ivc-inputs.msgpack`; rerun the same case by passing the flow directory:

```bash
HARDWARE_CONCURRENCY=8 barretenberg/cpp/scripts/ci_benchmark_ivc_flows.sh native \
  barretenberg/cpp/example-app-ivc-inputs-out/<flow>
```
...
HARDWARE_CONCURRENCY=8 ./build/bin/bb prove \
  -o $OUTPUT_DIR \
  --ivc_inputs_path example-app-ivc-inputs-out/$FLOW/ivc-inputs.msgpack \
  --scheme chonk \
  -v \
  --print_bench \
...
cp example-app-ivc-inputs-out/$FLOW/ivc-inputs.msgpack $OUTPUT_DIR/
