/**
 * Echo IPC client (TypeScript) — uses template IPC client.
...
async function run() {
  ...
  const [name1, resp1] = await client.call('EchoBytes', { data: testData });
  ...
  // Test 2: EchoFields
  const [name2, resp2] = await client.call('EchoFields', { a: 42, b: 999999, name: 'hello wire compat' });
  ...
  const [name3, resp3] = await client.call('EchoNested', { inner });
