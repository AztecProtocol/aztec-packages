...
out.write(' * the 8MB chunk size - callers should round up their download size to ensure this.\n')
...
out.write('{\n')
out.write('    if (data.size() % CRS_HASH_CHUNK_SIZE != 0) {\n')
out.write('        throw_or_abort(\"CRS data size (\" + std::to_string(data.size()) +\n')
out.write('                       \" bytes) is not a multiple of the chunk size (\" + std::to_string(CRS_HASH_CHUNK_SIZE) +\n')
out.write('                       \" bytes). Downloads must be aligned to 8MB chunk boundaries for hash verification.\");\n')
out.write('    }\n')
out.write('\n')
out.write('    size_t num_chunks = data.size() / CRS_HASH_CHUNK_SIZE;\n')
out.write('    size_t chunks_to_verify = std::min(num_chunks, CRS_NUM_CHUNK_HASHES);\n')
