...
import {Add2HonkVerificationKey as VK} from "./keys/Add2HonkVerificationKey.sol";
...
contract HonkVerifier is IVerifier {
    ...
    function verify(bytes calldata proof, bytes32[] calldata publicInputs) public view override returns (bool) {
        ...
        // Sumcheck
        bool sumcheckVerified = verifySumcheck(p, tp);
        if (!sumcheckVerified) revert SumcheckFailed();

        // Zeromorph
        bool zeromorphVerified = verifyZeroMorph(p, vk, tp);
    ...
    function partiallyEvaluatePOW(
        ...
        Fr roundChallenge,
        uint256 round
    ) internal view returns (Fr currentEvaluation) {
    ...
    function accumulateRelationEvaluations(Honk.Proof memory proof, TranscriptParameters memory tp, Fr powPartialEval)
        internal
        view
    ...
    function verifyZeroMorph(Honk.Proof memory proof, Honk.VerificationKey memory vk, TranscriptParameters memory tp)
        internal
        view
        returns (bool verified)
    {
    ...
    function computeCZeta(Honk.Proof memory proof, TranscriptParameters memory tp)
        internal
        view
