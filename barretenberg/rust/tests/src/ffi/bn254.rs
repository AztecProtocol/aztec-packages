...
fn test_bn254_fr_sqrt_of_zero() {
    ...
    let zero = Fr::from_be_bytes([0u8; 32]);
...
fn test_bn254_fr_sqrt_of_one() {
    ...
    let response = api
        .bn254_fr_sqrt(one.clone())
        .expect("bn254_fr_sqrt failed");
...
fn test_bn254_fr_sqrt_deterministic() {
    ...
    let input = Fr::from_u64(16); // Perfect square

    let response1 = api
        .bn254_fr_sqrt(input.clone())
        .expect("bn254_fr_sqrt failed");
...
fn test_bn254_g1_mul_consistency() {
    ...
    let result_3g = api
        .bn254_g1_mul(g1.clone(), three.clone())
        .expect("bn254_g1_mul(3) failed");
    let result_2_of_3g = api
        .bn254_g1_mul(result_3g.point.clone(), two.clone())
...
fn test_bn254_g2_mul_consistency() {
    ...
    let three = Fr::from_u64(3);
    let result_3g = api
        .bn254_g2_mul(g2.clone(), three.clone())
        .expect("bn254_g2_mul(3) failed");

    // Compute 6*G2 two ways: 2*(3*G2) vs 3*(2*G2)
    let two = Fr::from_u64(2);
    let result_2_of_3g = api
        .bn254_g2_mul(result_3g.point.clone(), two.clone())
...
fn test_bn254_fq_sqrt() {
    ...
    let response = api
        .bn254_fq_sqrt(four.into())
        .expect("bn254_fq_sqrt failed");

    assert!(
        response.is_square_root,
        "Square root of four in Fq should exist"
    );

    let mut expected = [0u8; 32];
    expected[31] = 2;
    assert_eq!(response.value, expected.into());
