...
//! These tests verify the AES-128-CBC API compatibility.
//!
...
fn test_aes_empty_input() {
    ...
    assert_eq!(encrypt_response.ciphertext.len(), 16);
...
#[test]
fn test_aes_different_ivs_produce_different_outputs() {
    require_bb_binary!();
    let bb_path = get_bb_binary_path();
    ...
    let plaintext = b"Test message for IV difference";
    let key = [
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f,
        0x3c,
    ];
    ...
    let encrypt_response1 = api
        .aes_encrypt(&padded_plaintext, &iv1, &key, length)
        .expect("aes_encrypt failed");
    let encrypt_response2 = api
        .aes_encrypt(&padded_plaintext, &iv2, &key, length)
        .expect("aes_encrypt failed");
