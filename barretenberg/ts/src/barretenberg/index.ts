...
export class Barretenberg extends BarretenbergApi {
  ...
  async initSRSForCircuitSize(circuitSize: number): Promise<void> {
    const minSRSSize = 2 ** 9 + 1; // 2**9 is the dyadic size for the SmallSubgroupIPA MSM.
    const crs = await Crs.new(Math.max(circuitSize + 1, minSRSSize), this.options.crsPath, this.options.logger);
