...
export class SchemaCompiler {
  ...
  compile(): string {
    ...
    // For non-api_types files, generate imports based on tracked usage
    const needsApiTypesImport = this.importStrategy.getTypePrefix() === 'apiTypes.';
    
    if (needsApiTypesImport && (this.usedTypes.size > 0 || this.functionMetadata.length > 0)) {
      parts.push('');
      // Import conversion functions namespace
      parts.push(`import * as apiTypes from './api_types.js';`);
      
      // Import types that were actually used (excluding types that are defined locally)
      const typesToImport = new Set(this.usedTypes);
      // Remove types that are defined in this file
      for (const typeInfo of this.typeCache.values()) {
        if (typeInfo.declaration) {
          typesToImport.delete(typeInfo.typeName);
        }
      }
      
      if (typesToImport.size > 0) {
        const sortedImports = Array.from(typesToImport).sort();
        parts.push(`import type {`);
        for (const type of sortedImports) {
          parts.push(`  ${type},`);
        }
        parts.push(`} from './api_types.js';`);
      }
    }
