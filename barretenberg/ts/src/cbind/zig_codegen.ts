...
export class ZigCodegen {
  ...
  // Type mapping for API method parameters (adds types. prefix for struct types)
  private mapApiType(type: Type): string {
    switch (type.kind) {
      case 'primitive':
        switch (type.primitive) {
          case 'bool': return 'bool';
      ...
      case 'vector':
        return `[]const ${this.mapApiType(type.element!)}`;

      case 'array':
        return `[${type.size}]${this.mapApiType(type.element!)}`;

      case 'optional':
        return `?${this.mapApiType(type.element!)}`;

      case 'struct':
        return `types.${toPascalCase(type.struct!.name)}`;
    }
...
`;
  ...
  private generateApiMethod(command: {name: string, fields: Field[], responseType: string}): string {
    ...
    const params = command.fields.map(f => {
      const zigType = this.mapApiType(f.type);
...
//!
//! Note: Full msgpack serialization is TODO. Currently uses placeholder implementation.
...
pub const MockBackend = struct {
    ...
    fn callImpl(ptr: *anyopaque, input: []const u8) anyerror![]u8 {
        ...
        // Return a mock 32-byte hash response (zeros)
...
pub const BarretenbergApi = struct {
    ...
    fn execute(self: *Self, command: types.Command) !types.Response {
        _ = command;
        // Call backend with placeholder (serialization TODO)
        const output_buffer = try self.backend_impl.call("");
        defer self.allocator.free(output_buffer);

        // Return placeholder response (deserialization TODO)
        return types.Response{ .blake2s_response = .{ .hash = output_buffer } };
    }
