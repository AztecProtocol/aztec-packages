...
export class ZigCodegen {
  ...
  private generateStruct(struct: Struct, isCommand: boolean): string {
    ...
    // Commands need __typename field for msgpack
    const typenameField = isCommand
      ? `    __typename: []const u8 = "${struct.name}",\n`
      : '';
...
${typenameField}${fields}
  ...
  private generateCommandUnion(schema: CompiledSchema): string {
    ...
    const variants = names
      .map(name => {
        const zigName = this.toPascalCase(name);
        return `    ${zigName}: ${zigName},`;
      })
      .join('\n');

    return `/// Command union - wraps all possible commands
...
${variants}
  ...
  private generateResponseUnion(schema: CompiledSchema): string {
    ...
    return `/// Response union - wraps all possible responses
