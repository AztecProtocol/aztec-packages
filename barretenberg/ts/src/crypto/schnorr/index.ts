/**
 * Schnorr signature operations using barretenberg bbapi.
 */
...
export class Schnorr {
   ...
   * @param privateKey - The private key (Bn254Fr = GrumpkinScalar).
   * @returns A grumpkin public key.
   */
  public async computePublicKey(privateKey: Bn254Fr): Promise<GrumpkinPoint> {
    await BarretenbergSync.initSingleton();
    const api = BarretenbergSync.getSingleton();
    const response = api.schnorrComputePublicKey({ privateKey: privateKey.toBuffer() });
    return GrumpkinPoint.fromBuffer(
      Buffer.concat([Buffer.from(response.publicKey.x), Buffer.from(response.publicKey.y)]),
    );
  }
   ...
   */
  public async constructSignature(msg: Uint8Array, privateKey: Bn254Fr) {
    await BarretenbergSync.initSingleton();
    const api = BarretenbergSync.getSingleton();
   ...
   */
  public async verifySignature(msg: Uint8Array, pubKey: GrumpkinPoint, sig: SchnorrSignature) {
    await BarretenbergSync.initSingleton();
    const api = BarretenbergSync.getSingleton();
