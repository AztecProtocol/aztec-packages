/**
 * Secp256k1 elliptic curve operations using barretenberg bbapi.
 */
...
export class Secp256k1 {
  // prettier-ignore
  static generator = new Uint8Array([
   ...
   * @returns Result of the multiplication (64 bytes: x || y).
   */
  public async mul(point: Uint8Array, scalar: Uint8Array) {
    ...
    result.set(response.point.x, 0);
    result.set(response.point.y, 32);
    return result;
  }
   ...
   */
  public async reduce512BufferToFr(uint512Buf: Uint8Array) {
    await BarretenbergSync.initSingleton();
    const api = BarretenbergSync.getSingleton();
