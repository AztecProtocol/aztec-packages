...
export class BarretenbergApi {
  ...
  async acirCreateGoblinProof(acirComposerPtr: Ptr, constraintSystemBuf: Uint8Array, witnessBuf: Uint8Array): Promise<Uint8Array> {
  ...
  async acirVerifyGoblinProof(acirComposerPtr: Ptr, proofBuf: Uint8Array): Promise<boolean> {
    ...
    const outTypes: OutputType[] = [BoolDeserializer()];
    const result = await this.wasm.callWasmExport('acir_verify_goblin_proof', inArgs, outTypes.map(t=>t.SIZE_IN_BYTES));
    const out = result.map((r, i) => outTypes[i].fromBuffer(r));
    return out[0];
  }
...
export class BarretenbergApiSync {
  ...
  acirVerifyGoblinProof(acirComposerPtr: Ptr, proofBuf: Uint8Array): boolean {
    const inArgs = [acirComposerPtr, proofBuf].map(serializeBufferable);
    const outTypes: OutputType[] = [BoolDeserializer()];
    const result = this.wasm.callWasmExport('acir_verify_goblin_proof', inArgs, outTypes.map(t=>t.SIZE_IN_BYTES));
    const out = result.map((r, i) => outTypes[i].fromBuffer(r));
    return out[0];
  }
