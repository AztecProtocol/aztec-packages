...
export class BarretenbergApi {
  ...
  async pedersenCommit(inputsBuffer: Fr[], hashIndex: number): Promise<Point> {
    const result = await this.binder.callWasmExport('pedersen_commit', [inputsBuffer, hashIndex], [Point]);
    return result[0];
  }
  ...
  async schnorrMultisigConstructSignatureRound2(message: Uint8Array, privateKey: Fr, signerRoundOnePrivateBuf: Buffer128, signerPubkeysBuf: Buffer128[], roundOnePublicBuf: Buffer128[]): Promise<[Fr, boolean]> {
    const result = await this.binder.callWasmExport('schnorr_multisig_construct_signature_round_2', [message, privateKey, signerRoundOnePrivateBuf, signerPubkeysBuf, roundOnePublicBuf], [Fr, BoolDeserializer()]);
    return result as any;
  }
