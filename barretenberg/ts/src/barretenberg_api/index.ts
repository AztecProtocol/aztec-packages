...
export class BarretenbergApi {
  ...
  async acirCreateProof(acirComposerPtr: Ptr, acirVec: Uint8Array, witnessVec: Uint8Array, isRecursive: boolean): Promise<[Uint8Array, Uint8Array]> {
    const result = await this.binder.callWasmExport('acir_create_proof', [acirComposerPtr, acirVec, witnessVec, isRecursive], [BufferDeserializer(), BufferDeserializer()]);
    return result as any;
  }
  ...
  async acirVerifyProof(acirComposerPtr: Ptr, publicInputsBuf: Uint8Array, proofBuf: Uint8Array, isRecursive: boolean): Promise<boolean> {
    const result = await this.binder.callWasmExport('acir_verify_proof', [acirComposerPtr, publicInputsBuf, proofBuf, isRecursive], [BoolDeserializer()]);
    return result[0];
  }
  ...
  async acirSerializeProofIntoFields(acirComposerPtr: Ptr, publicInputsBuf: Uint8Array, proofBuf: Uint8Array): Promise<Fr[]> {
    const result = await this.binder.callWasmExport('acir_serialize_proof_into_fields', [acirComposerPtr, publicInputsBuf, proofBuf], [VectorDeserializer(Fr)]);
    return result[0];
  }
