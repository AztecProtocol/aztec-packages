...
export class BarretenbergApi {
  ...
  async acirCreateGoblinProof(acirComposerPtr: Ptr, constraintSystemBuf: Uint8Array, witnessBuf: Uint8Array, isRecursive: boolean): Promise<Uint8Array> {
    const inArgs = [acirComposerPtr, constraintSystemBuf, witnessBuf, isRecursive].map(serializeBufferable);
    const outTypes: OutputType[] = [BufferDeserializer()];
    const result = await this.wasm.callWasmExport('acir_create_goblin_proof', inArgs, outTypes.map(t=>t.SIZE_IN_BYTES));
    const out = result.map((r, i) => outTypes[i].fromBuffer(r));
    return out[0];
  }
  ...
  async acirVerifyProofGoblin(acirComposerPtr: Ptr, proofBuf: Uint8Array, isRecursive: boolean): Promise<boolean> {
    const inArgs = [acirComposerPtr, proofBuf, isRecursive].map(serializeBufferable);
    const outTypes: OutputType[] = [BoolDeserializer()];
    const result = await this.wasm.callWasmExport('acir_verify_proof_goblin', inArgs, outTypes.map(t=>t.SIZE_IN_BYTES));
    const out = result.map((r, i) => outTypes[i].fromBuffer(r));
    return out[0];
  }
...
export class BarretenbergApiSync {
  ...
  acirCreateGoblinProof(acirComposerPtr: Ptr, constraintSystemBuf: Uint8Array, witnessBuf: Uint8Array, isRecursive: boolean): Uint8Array {
    const inArgs = [acirComposerPtr, constraintSystemBuf, witnessBuf, isRecursive].map(serializeBufferable);
    const outTypes: OutputType[] = [BufferDeserializer()];
    const result = this.wasm.callWasmExport('acir_create_goblin_proof', inArgs, outTypes.map(t=>t.SIZE_IN_BYTES));
    const out = result.map((r, i) => outTypes[i].fromBuffer(r));
    return out[0];
  }
  ...
  acirVerifyProofGoblin(acirComposerPtr: Ptr, proofBuf: Uint8Array, isRecursive: boolean): boolean {
    const inArgs = [acirComposerPtr, proofBuf, isRecursive].map(serializeBufferable);
    const outTypes: OutputType[] = [BoolDeserializer()];
    const result = this.wasm.callWasmExport('acir_verify_proof_goblin', inArgs, outTypes.map(t=>t.SIZE_IN_BYTES));
    const out = result.map((r, i) => outTypes[i].fromBuffer(r));
    return out[0];
  }
