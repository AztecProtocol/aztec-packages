 ...
 *  the small-MSM regime) instead of K sequential per-MSM dispatches. Default ON —
 *  on the real Chonk prove it verifies (vks_match=true) and beats the per-MSM GPU
 *  path (6.0 s vs 8.1 s on M4 Pro).
 *
 *  Correctness rests on two invariants, regression-guarded by the dev harness
 *  `?autorun=msm-union-validate` (per-member staged-combine ≡ CPU MSM):
 *    1. `union_runner` reads the early-exit STAGED partials — `run()` leaves the
 *       host `windowSums` empty (the staged-partials refactor); reading it yields
 *       0-window infinities.
 *    2. packs are grouped by (srsOffset, c) — a pack mixing `pickC(n)` values feeds
 *       prepareBatch's different-c envelope, which computes wrong (on-curve but
 *       incorrect) per-member sums and a non-uniform `partialsPerWindow` slice.
 *  Set `globalThis.__msm_union_bridge = false` to force the legacy per-MSM path. */
...
export class WebGpuMsmHost {
  ...
  private async runBatchMsmUnion(
    ...
    for (const r of out.results) {
      // The union ships early-exit staged partials (windowSums is host-empty), so write
      // the member's raw staged bytes and pack the meta as (partialsPerWindow<<16)|numWindows
      // — identical to the per-MSM path so the C++ hook runs finish_and_combine_windows.
      new Uint8Array(wasm.buffer, resultsBase + r.resultOff, r.stagedBytes.byteLength).set(r.stagedBytes);
      metaOut[r.descIdx * 2 + 0] = ((r.partialsPerWindow << 16) | r.numWindows) >>> 0;
  ...
  private async getOrCreateUnionMsm(maxN: number): Promise<MsmV2> {
    ...
    const device = await this.getDevice();
    const fresh = await MsmV2.create(device, maxN, this.pool!, {
      warmupRuns: 0,
      combineOnHost: false,
      profile: false,
      ...this.msmFlags(),
