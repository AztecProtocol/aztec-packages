...
export class MsmV2Pool {
  ...
  ensureScratch(dims: ScratchDims, padPts: Pt[], R: bigint): SharedScratch {
    ...
    let grew = false;
    const grow = <K extends keyof ScratchDims>(cond: boolean, name: K): boolean => {
      if (cond) {
        cur[name] = dims[name];
        return true;
      }
...
export class MsmV2 {
  ...
  encodeIntoBatch(
    ...
    // off lparams.w). reduceLevelKinds is no longer consulted.
    {
      ...
      // the partials for the native finish + combine (the GPU pipeline ends
      // here; early exit is the only halving result path).
      const depthCount =
  ...
  async run(): Promise<{
    ...
    if (!this.splitSubmitDiag) {
      ...
      // Wrapped in an object so control-flow analysis keeps the declared type
      // at the read site below — a plain `let` mutated only inside the async
      // `.then` callback gets narrowed to its `null` initializer otherwise.
      const lost: { info: { message: string } | null } = { info: null };
      void this.device.lost.then(i => {
        lost.info = i as unknown as { message: string };
      });
      for (const [ph, cmd] of this.splitCmdBuffers) {
        device.queue.submit([cmd]);
        await device.queue.onSubmittedWorkDone();
        await Promise.resolve();
        if (lost.info) throw new Error(`[split] DEVICE_LOST at phase=${ph}: ${lost.info.message}`);
        // eslint-disable-next-line no-console
