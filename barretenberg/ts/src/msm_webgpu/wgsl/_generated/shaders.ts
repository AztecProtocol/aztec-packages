...
export const ap_ladder_a0 = `// LADDER A rung 0: apA0_loadstore — apply_precomputed memory floor.
//
...
export const ap_ladder_a1 = `// LADDER A rung 1: apA1_mul1 — apA0_loadstore + ONE montmul.
//
...
export const ap_ladder_a2 = `// LADDER A rung 2: apA2_mul2 — apA1_mul1 + 2nd montmul (the square).
//
...
export const ap_ladder_a3 = `// LADDER A rung 3: apA3_mul3 — apA2_mul2 + 3rd montmul.
//
...
export const ap_ladder_a4 = `// LADDER A rung 4: apA4_subs — apA3_mul3 + the 5 fr_sub of the real
...
export const ap_ladder_a5 = `// LADDER A rung 5: apA5_full — the REAL apply_precomputed_k1 formula.
//
...
export const ba_affine_only_bench = `// Isolation bench #1: the affine-add FORMULA only (post-inverse), with all
...
export const ba_apply_il_bench = `// Two-kernel pipeline stage 2: software-pipelined (stage-interleaved)
...
export const ba_apply_ilp_bench = `// Software-pipelined (stage-interleaved) fully-coalesced SoA+vec4
...
export const ba_apply_m_bench = `// Two-kernel pipeline stage 2: affine apply, M distinct pairs per thread.
//
...
export const ba_apply_serial_bench = `// Interleave-free M-serial affine apply.
//
...
export const ba_apply_soa_bench = `// Fully-coalesced SoA + vec4 lean-affine apply. One thread = one pair.
//
...
export const ba_apply_tight_bench = `// Two-kernel pipeline stage 2: affine apply, minimal-temporary form.
//
...
export const ba_arith_floor_bench = `// Profiling kernel: the TRUE arithmetic floor of one batch-affine pair —
...
export const ba_blelloch_scan_bench = `// Decisive primitive: cooperative block Blelloch (work-efficient)
...
export const ba_coop_inv_bench = `// Two-level COOPERATIVE Montgomery batch-inversion batch-affine kernel.
//
...
export const ba_fused_il_bench = `// Fused single-kernel BATCHED-inverse batch-affine, software-pipelined.
//
...
export const ba_fused_soa_bench = `// Single-kernel batched-inverse batch-affine, fully-coalesced SoA+vec4.
//
...
export const ba_fused_tight_bench = `// Fused single-kernel batch-affine, minimal private storage + lean formula.
//
...
export const ba_hs_scan_bench = `// Hillis-Steele inclusive prefix-product scan (work-INefficient
...
export const ba_invonly_bench = `// Two-kernel pipeline, stage 1: inverse-only.
//
...
export const ba_invonly_soa_bench = `// Two-kernel pipeline stage 1 (SoA): inverse-only.
//
...
export const ba_invonly_wg_bench = `// Two-kernel pipeline stage 1, workgroup-amortised inversion.
//
...
export const ba_msm_bucket_bench = `// MSM-integrated bucket-accumulate batch-affine kernel.
//
...
export const ba_msm_bucket_f32 = `// f32-22 MSM-integrated bucket-accumulate batch-affine kernel.
//
...
export const ba_pipe_inv_bench = `// Per-thread software-pipelined double-buffered batch-affine kernel.
//
...
export const ba_pipe_inv_noinv_bench = `// _noinv twin of ba_pipe_inv_bench: BOTH fr_inv_by_a(...) calls replaced
//
// Goal: HIDE the serial data-dependent latency of fr_inv_by_a (the
// modular inversion) rather than reduce its frequency. The cooperative
// count-reduction approach regressed because the ~48 ns serial inversion
...
//   2. forward-product B -> accB           (g montmuls; reads only B's
//      streamed points; independent of A and of accA)
//   3. invA = accA            (long-latency, data-dependent
//      ONLY on accA)
//   4. invB = accB            (long-latency, data-dependent
//      ONLY on accB; NOT dependent on invA -> issued back-to-back so the
//      two inversions' latencies overlap, and the scheduler can also
//      overlap them with step-2/5 montmuls)
//   5. backward-peel + lean affine apply A using invA  (the montmuls
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // data-dependent latencies overlap with each other and with the
    // surrounding montmul work.
    var invA: BigInt = accA;
    var invB: BigInt = accB;
...
export const ba_rbs_bwd_bench = `// Register-blocked batch-affine, backward kernel: per-thread segment-
...
export const ba_rbs_fwd_bench = `// Register-blocked work-efficient prefix-product forward kernel
...
export const ba_rbs_scan_bench = `// Register-blocked work-efficient parallel prefix-product scan over
...
export const ba_rbs_seed_bench = `// Register-blocked batch-affine, seed kernel: the ONLY kernel
...
export const ba_rev_packed_carry_bench = `// MSM-integrated bucket-accumulate batch-affine kernel — packed 8x u32
...
export const ba_seg_bwd_bench = `// Segmented batch-affine, kernel 3/3: backward peel + affine formula.
//
...
export const ba_seg_fwd_bench = `// Segmented batch-affine, kernel 1/3: forward sweep.
//
...
export const ba_seg_inv_bench = `// Segmented batch-affine, kernel 2/3: the ONLY kernel containing the
...
export const ba_serial_bench = `// Isolation bench #2: the PROPOSED batch-affine structure.
//
...
export const ba_sg_bench = `// Single fused subgroup-shuffle batch-affine: ONE kernel, ONE global pass,
...
export const ba_tile_apply_bench = `// Fully-parallel "tile" batch-affine apply kernel: ONE thread = ONE pair
...
export const ba_tile_scan_bench = `// Fully-parallel "tile" batch-affine forward kernel: ONE thread = ONE
...
export const ba_wgamort_bench = `// Single-kernel, TWO-LEVEL workgroup-amortised batch-affine.
//
...
export const ba_wgtile_bench = `// Single fused workgroup-tile batch-affine: ONE kernel, ONE global pass,
...
export const field_binop_bench = `// Per-thread chained binary-field-op micro-benchmark (BN254 base field,
...
export const jac_add_bench = `// Per-thread chained UNCONDITIONAL Jacobian point-addition micro-benchmark
...
export const mb_abl_fwdonly = `// ABLATION 2 of msm_bucket_s16: forward accumulator-structured montmul.
//
...
export const mb_abl_loadonly = `// ABLATION 1 of msm_bucket_s16: pure memory floor.
//
// Identical SoA+vec4 layout, 8192-thread launch and S=16 streamed-point
...
export const mb_abl_noinv = `// ABLATION 3 of msm_bucket_s16: amortised inversion removed.
//
...
export const mb_abl_nopeel = `// MSM-integrated bucket-accumulate batch-affine kernel.
//
// Models the MSM bucket inner loop: each thread owns ONE resident
// accumulator A (kept in registers across the whole S-point chunk) and
// folds a streamed chunk of S independent points P_0..P_{S-1} into it
...
export const mb_ladder_b0 = `// LADDER B rung 0: mbB0_loadstore — msm_bucket_s16 memory floor.
//
// Identical SoA+vec4 layout, S=16 per-thread chunk and 8192-thread
// launch geometry as ba_msm_bucket_bench. Streams every pair with the
// SAME coalesced access pattern as the real kernel (forward: S loads of
// p_x; backward: S loads of p_x + p_y) but performs ZERO field
// arithmetic. Every loaded limb XOR-folds into the stored output so no
// load can be dead-code-eliminated. Geometry HELD for all B-rungs.
...
export const mb_ladder_b1 = `// LADDER B rung 1: mbB1_fwd — mbB0_loadstore + forward prefix-product.
//
...
export const mb_ladder_b2 = `// LADDER B rung 2: mbB2_inv — mbB1_fwd + the ONE fr_inv_by_a(acc).
//
...
export const mb_ladder_b3 = `// LADDER B rung 3: mbB3_bwd_muls — mbB2_inv + backward per-pair montmuls
// only (NO subs yet).
//
...
export const mb_ladder_b4 = `// LADDER B rung 4: mbB4_bwd_subs — mbB3_bwd_muls + backward per-pair
...
export const mb_ladder_b5 = `// MSM-integrated bucket-accumulate batch-affine kernel.
//
// Models the MSM bucket inner loop: each thread owns ONE resident
// accumulator A (kept in registers across the whole S-point chunk) and
// folds a streamed chunk of S independent points P_0..P_{S-1} into it
...
export const mm_distinct_f32 = `// f32-22 distinct-operand montmul microbench. The f32-22 sibling of the
...
export const montmul_pressure_bench = `// Register-pressure montmul microbench. Each thread keeps L independent
...
export const montmul_realism_bench = `// Montmul-realism microbench. Isolates realistic Montgomery-multiply
...
export const montmul_resident_bench = `// Profiling kernel: chained montmul with a RESIDENT private array of
...
export const mont_karat_lean = `// Register-footprint-reduced variant of mont_pro_product_karat_yuval.
//
