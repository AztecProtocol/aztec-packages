...
override RED_EXTRA: u32 = 0u;
override MV_TERMS: u32 = 20u;
fn red_m(t: u32, m: u32, minv: u32, z: u32) -> u32 { var r = rns_montred(t, m, minv); for (var e = 0u; e < RED_EXTRA; e++) { r = rns_plain(r, m, z); } return r; }
fn rns_modmul(a: Rns2, b: Rns2) -> Rns2 {
  var sM: array<u32, RNS_T>;
  var sN: array<u32, RNS_T>;
  for (var i = 0u; i < RNS_T; i++) { sM[i] = red_m(a.m[i]*b.m[i], RNS_M_MOD[i], MONT_MINV_M[i], RNS_M_Z[i]); }
  for (var j = 0u; j < RNS_T; j++) { sN[j] = red_m(a.n[j]*b.n[j], RNS_N_MOD[j], MONT_MINV_N[j], RNS_N_Z[j]); }
  ...
  for (var i = 0u; i < RNS_T; i++) {
    xi[i] = red_m(sM[i]*PRE_M[i], RNS_M_MOD[i], MONT_MINV_M[i], RNS_M_Z[i]);
  ...
  for (var j = 0u; j < RNS_T; j++) {
    ...
    for (var i = 0u; i < MV_TERMS; i++) { acc += xi[i] * CRNS_A_MN[i * RNS_T + j]; }
  ...
  for (var i = 0u; i < RNS_T; i++) {
    ...
    for (var j = 0u; j < MV_TERMS; j++) { acc += eta[j] * CRNS_A_NM[j * RNS_T + i]; }
