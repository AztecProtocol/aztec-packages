...
// Prefix scratch (S prefix-product + inverse slots per thread, 2 vec4 each).
// There is no cross-thread aliasing — each thread touches only its own
// [base .. base + S*2) region — so no workgroup barrier is needed regardless
// of where the storage lives. Three placements (build knob `pref` in
// shader_manager): workgroup-shared (KNOB 1, 16 KB at TPB=64,S=8 — the
// occupancy limiter the swarm identified), device storage (frees workgroup
// memory entirely, indexed by global thread), or private (per-invocation,
// module-scope var<private>).
{{#pref_device}}
