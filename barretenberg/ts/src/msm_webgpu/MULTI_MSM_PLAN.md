...
- **Current state + the next-step recipe live in `MULTI_MSM_HANDOFF.md` — read it
  first.** In short: ROLLOUT step 1 (host scheduler + bin-packer + runtime
  batch-of-1 check) is DONE, committed (`6496810343`, `d0125e504c`), and validated
  (22 jest tests; batch-of-1 byte-identical; golden 14–17 unchanged). Next is
  step 2 **coupled with** step 4 (global-window bid + the one-dispatch-over-the-
  union dispatch) — the handoff explains why step 2 cannot land alone.
- **Reduce optimisation is owned by other agents/worktrees — do NOT work on it
  here.** It is near its compute floor at the production c.
- **This doc is the design spec.** Approved framing: the acceptance criterion is
  the operator's (no starvation of the solo-small-MSM-starved stages); the
  mechanism (concatenated super-MSM, one dispatch over the pack, adapt-don't-
  rewrite) is the proposed means, justified below.
- **Open follow-on, separate & lower priority:** split-c large-`c_lo` buffer sizing
  (region-split buffers are sized for `c_w ≤ pickC`; the lever is gated behind a
  fits-guard until they're sized). Does NOT block multi-MSM.
