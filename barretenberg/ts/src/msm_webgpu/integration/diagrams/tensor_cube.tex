...
\begin{scope}[
    ...
    % Carry (P_5) keeps its i position across levels.
    % =================================================================
    ...
    \foreach \xc/\lbl in {\xA/$P_1$, \xB/$P_2$, \xC/$P_3$, \xD/$P_4$, \xE/$P_5$}
        \draw[feedEdge] (\xc, \khl+0.5, \jSlice) -- (\xc, \khl+0.5, \zLeaf);

    % Level 0 — leaves with labels (all placed above for consistency).
    \node[treeLeaf,label={[font=\footnotesize, text=axisColor]above:$P_1$}]
        (Ta) at (\xA, \khl+0.5, \zLeaf) {};
    \node[treeLeaf,label={[font=\footnotesize, text=axisColor]above:$P_2$}]
        (Tb) at (\xB, \khl+0.5, \zLeaf) {};
    \node[treeLeaf,label={[font=\footnotesize, text=axisColor]above:$P_3$}]
        (Tc) at (\xC, \khl+0.5, \zLeaf) {};
    \node[treeLeaf,label={[font=\footnotesize, text=axisColor]above:$P_4$}]
        (Td) at (\xD, \khl+0.5, \zLeaf) {};
    \node[treeLeaf,label={[font=\footnotesize, text=axisColor]above:$P_5$}]
