...
${packLines.join('\n')}
  ...
  public gen_ba_halve_shader(workgroup_size: number, cpairs: number): string {
    ...
    // is a 2-bit flag (ps/act). NO equal-point or negation branches, exactly
    // like the walker: the operands are disjoint subset-sums of fixed random
    // SRS points, so a collision (equal or negated x) is dlog-hard to
    // produce even adversarially. Absence (empty buckets) is structural and
    // fully handled via is_present.
    const gather: string[] = [];
    ...
    gather.push(`        let f = select(0u, 1u, ps) | select(0u, 2u, act);`);
    gather.push(`        fl = fl | (f << (2u * k));`);
    gather.push(`        if (act) {`);
    gather.push(`            // den = xs - xd, never zero for SRS subset-sums.`);
    gather.push(`            chain = montgomery_product_f8(chain, fr_sub_f8(load_x(src, M_RED), load_x(dst, M_RED)));`);
    gather.push(`        }`);
    gather.push(`        if (k + 1u < cd) {`);
    gather.push(`            // pp[k] must be written even when the multiply was skipped`);
    gather.push(`            // (identity denominator) — the peel loads it unconditionally.`);
    ...
    apply.push(`            if (k > 0u) {`);
    apply.push(`                inv_acc = montgomery_product_f8(inv_acc, fr_sub_f8(xs, xd));`);
    apply.push(`            }`);
    apply.push(`            let num = fr_sub_f8(load_y(src, M_RED), yd);`);
  ...
  public gen_halve_finish_arrays_shader(stride: number, finisherDepth: number, _inputsJac: boolean): string {
    ...
    const Lf = B >> df;
    // Floor at a full wave: sub-wave workgroups (32 at small Lf) returned
    // wrong results on Adreno — barriers in partial waves are driver-bug
    // territory, and no proven cooperative kernel here runs below 64.
    const workgroup_size = Math.max(64, Lf >> 1);
    if (Lf * 96 > 24 * 1024) {
    ...
    body.push(`    let dn = lparams.w;`);
    body.push(`    if (dn == 1u) {`);
    body.push(`        // Dump mode 1 (depth bisection): stage window w's RAW W-region`);
    body.push(`        // slot a straight from global — no shared memory, no reduction.`);
    body.push(`        // Diffing these records across devices after N wide depths`);
    body.push(`        // localizes the first divergent pass.`);
    body.push(`        if (t == 0u) {`);
    body.push(`            let g = base + a;`);
    body.push(`            var dz = array<u32, 8>(0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u);`);
    ...
    body.push(`        var z = array<u32, 8>(0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u);`);
    body.push(`        // is_present gates BOTH entry kinds: a Jacobian-phase`);
    body.push(`        // cancellation clears the flag but leaves a stale nonzero z`);
    body.push(`        // in red_z — reading z alone would resurrect the slot.`);
    body.push(`        if (is_present[g] != 0u) {`);
    body.push(`            if (ij) {`);
    ...
    body.push(`    var iters = lgLf + select(r - a, lgLf, a == 0u);`);
    body.push(`    if (dn >= 2u) {`);
    body.push(`        // Dump mode N>=2 (iteration bisection): run exactly dn-2 master`);
    body.push(`        // loop iterations, then stage each workgroup's slot 0 — rung 2`);
    body.push(`        // fingerprints the cooperative load + shared round-trip alone.`);
    ...
    body.push(`                let y0 = sl_y(dslot);`);
    body.push(`                let dA = montgomery_product_f8(x0, x0);`);
    body.push(`                let dB = montgomery_product_f8(y0, y0);`);
    body.push(`                let dC = montgomery_product_f8(dB, dB);`);
    body.push(`                let XpB = fr_add_f8(x0, dB);`);
    ...
    body.push(`                let dF = montgomery_product_f8(dE, dE);`);
    body.push(`                let X3d = fr_sub_f8(dF, fr_dbl_f8(dD));`);
    body.push(`                let C8 = fr_dbl_f8(fr_dbl_f8(fr_dbl_f8(dC)));`);
    body.push(`                let Y3d = fr_sub_f8(montgomery_product_f8(dE, fr_sub_f8(dD, X3d)), C8);`);
    body.push(`                let Z3d = fr_dbl_f8(montgomery_product_f8(y0, z0));`);
    body.push(`                ss_x(dslot, X3d);`);
    ...
    body.push(`        }`);
    body.push(`        if (valid) {`);
    body.push(`            // add-2007-bl, inline on components. Absence (z == 0) is the`);
    body.push(`            // only special case — equal/negated operands are dlog-hard for`);
    body.push(`            // disjoint subset-sums of SRS points, so no equal-point branch`);
    body.push(`            // (the walker's discipline).`);
    body.push(`            let zs = sl_z(si);`);
    body.push(`            if (!fr_is_zero_f8(zs)) {`);
    body.push(`                let zd = sl_z(di);`);
    body.push(`                if (fr_is_zero_f8(zd)) {`);
    body.push(`                    ss_x(di, sl_x(si));`);
    body.push(`                    ss_y(di, sl_y(si));`);
    body.push(`                    ss_z(di, zs);`);
    body.push(`                } else {`);
    body.push(`                    let Z1Z1 = montgomery_product_f8(zd, zd);`);
    body.push(`                    let Z2Z2 = montgomery_product_f8(zs, zs);`);
    body.push(`                    let xd = sl_x(di);`);
    body.push(`                    let xs = sl_x(si);`);
    body.push(`                    let U1 = montgomery_product_f8(xd, Z2Z2);`);
    body.push(`                    let U2 = montgomery_product_f8(xs, Z1Z1);`);
    body.push(`                    let yd = sl_y(di);`);
    body.push(`                    let ys = sl_y(si);`);
    ...
    body.push(`                    let rr = fr_dbl_f8(fr_sub_f8(S2, S1));`);
    body.push(`                    let V = montgomery_product_f8(U1, I);`);
    body.push(`                    let X3 = fr_sub_f8(fr_sub_f8(montgomery_product_f8(rr, rr), J), fr_dbl_f8(V));`);
    body.push(`                    let S1J = montgomery_product_f8(S1, J);`);
    body.push(`                    let Y3 = fr_sub_f8(montgomery_product_f8(rr, fr_sub_f8(V, X3)), fr_dbl_f8(S1J));`);
    body.push(`                    let ZpZ = fr_add_f8(zd, zs);`);
    body.push(`                    let Z3 = montgomery_product_f8(fr_sub_f8(fr_sub_f8(montgomery_product_f8(ZpZ, ZpZ), Z1Z1), Z2Z2), H);`);
    ...
    body.push(`    }`);
    body.push(`    if (t == 0u) {`);
    body.push(`        // dn == 2 (zero iterations) stages each workgroup's local slot`);
    body.push(`        // min(a, Lf-1) instead of slot 0, so the bisection fingerprints`);
    body.push(`        // EVERY loaded slot index across the workgroup set — slot 0`);
    body.push(`        // alone cannot exonerate the cooperative load.`);
    body.push(`        let sslot = select(0u, min(a, Lf - 1u), dn == 2u);`);
    body.push(`        if (dn < 2u) {`);
    body.push(`            // Component-wise writeback — the home slot feeds pass 2 and`);
    body.push(`            // struct-valued call chains miscompile on Adreno.`);
    body.push(`            store_x(base + off, M_RED, sl_x(sslot));`);
    body.push(`            store_y(base + off, M_RED, sl_y(sslot));`);
    body.push(`            store_z(base + off, sl_z(sslot));`);
    body.push(`        }`);
    body.push(`        stage_set(w * (df + 1u) + a, sload(sslot));`);
    body.push(`    }`);
    const dec = this.decoupledPackUnpackWgsl();
    const { p8_consts, r8_csv, f8_words } = this.f8Context();
    return mustache.render(
  ...
  public gen_halve_finish_root_shader(stride: number, finisherDepth: number): string {
    ...
    // Full-wave floor — see gen_halve_finish_arrays_shader.
    const workgroup_size = Math.max(64, Math.ceil(n / 2));
    ...
    body.push(`        // Component-wise — no struct-valued call chains (the shape that`);
    body.push(`        // miscompiled on Adreno in the pass-1 finisher).`);
    body.push(`        ss_x(i, load_x(g, cparams.x));`);
    body.push(`        ss_y(i, load_y(g, cparams.x));`);
    body.push(`        ss_z(i, load_zp(g));`);
    body.push(`    }`);
    body.push(`    workgroupBarrier();`);
    body.push(`    var step = 1u;`);
    body.push(`    loop {`);
    body.push(`        if (step >= n) { break; }`);
    body.push(`        let di = 2u * step * t;`);
    body.push(`        let si = di + step;`);
    body.push(`        if (si < n) {`);
    body.push(`            // add-2007-bl, inline on components — absence (z == 0) is the`);
    body.push(`            // only special case; equal/negated operands are dlog-hard for`);
    body.push(`            // disjoint subset-sums of SRS points.`);
    body.push(`            let zs = sl_z(si);`);
    body.push(`            if (!fr_is_zero_f8(zs)) {`);
    body.push(`                let zd = sl_z(di);`);
    body.push(`                if (fr_is_zero_f8(zd)) {`);
    ...
    body.push(`    }`);
    body.push(`    if (t == 0u) {`);
    body.push(`        store_x(base, cparams.x, sl_x(0u));`);
    body.push(`        store_y(base, cparams.x, sl_y(0u));`);
    body.push(`        store_z(base, sl_z(0u));`);
