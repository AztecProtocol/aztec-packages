/**
 * Code generation for aztec-cdb TypeScript and C++ bindings.
 *
 * Uses the same codegen pipeline as bb.js but targets the aztec-cdb binary schema.
...
async function generate() {
  const cdbBuildPath = process.env.CDB_BINARY_PATH || join(__dirname, '../../../cpp/build/bin/aztec-cdb');

  // Get schema from aztec-cdb
  console.log('Fetching msgpack schema from aztec-cdb...');
  const { stdout } = await execAsync(`${cdbBuildPath} msgpack schema`);
  ...
  const files = [
    ...
    { path: '../../../cpp/src/barretenberg/cdb/cdb_ipc_client_generated.hpp', content: cppGen.generateHeader(compiled) },
    { path: '../../../cpp/src/barretenberg/cdb/cdb_ipc_client_generated.cpp', content: cppGen.generateImpl(compiled) },
  ];
