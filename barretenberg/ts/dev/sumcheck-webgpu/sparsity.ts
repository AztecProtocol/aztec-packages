...
export type RowStructure = 'block' | 'scattered';
...
/**
 * Active-edge compaction plan (Tier 2, scattered). For each round, the dense list of
...
/**
 * The brief's success metric: relations evaluated per active row. Reports, for round 0
 * (where ~half the O(n) field work lives), the dense baseline (NUM_RELATIONS — every
 * relation on every row, what the un-skipped engine does), the `ideal` (Σ_r density_r —
 * the relations a real prover's `skip()` would actually evaluate per used row), and the
 * `achieved` count the GPU skip path dispatches, derived from the SAME round-0 dispatch
 * sizing the engines use (block: each relation over its active prefix `activeRowsByRel`;
 * scattered: compacted relations over their gathered active-pair count, the rest over the
 * full used prefix under Tier-0 trim). Pure host arithmetic — no GPU needed — so the
 * achieved-vs-ideal number can be logged on any device. Normalized per *used* row
 * (usedRows>>1 round-0 pairs), so dense == NUM_RELATIONS exactly and block ≈ ideal.
 */
export interface RelationsPerRow {
  dense: number;     // NUM_RELATIONS — the un-skipped baseline (14/row)
  ideal: number;     // Σ_r density_r — a real prover's skip()-gated work
  achieved: number;  // what the GPU skip path actually dispatches (round 0)
  mode: 'dense' | 'block' | 'scattered';
}

export function relationsPerRow(profile: CircuitProfile, n: number): RelationsPerRow {
  const ideal = profile.density.reduce((a, b) => a + b, 0);
  if (!profileIsSparse(profile)) return { dense: NUM_RELATIONS, ideal, achieved: NUM_RELATIONS, mode: 'dense' };
  const L = usedRows(profile, n);
  const usedPairs = Math.max(1, L >> 1); // round-0 active edge-pairs over the used prefix
  let dispatched = 0;
  if (profile.structure === 'block') {
    const byRel = activeRowsByRel(profile, n)!; // per-relation active prefix (even row counts)
    for (const desc of ALL_RELATIONS) dispatched += Math.max(1, byRel[desc.relationIndex] >> 1);
    return { dense: NUM_RELATIONS, ideal, achieved: dispatched / usedPairs, mode: 'block' };
  }
  // scattered: compacted relations dispatch their round-0 active-pair count; the rest
  // (density >= COMPACTION_MAX_DENSITY) run the full used prefix under Tier-0 trim.
  const comp = compactionPlan(profile, n);
  for (const desc of ALL_RELATIONS) {
    const rounds = comp?.roundsByRel[desc.relationIndex];
    dispatched += rounds ? rounds[0]?.count ?? 0 : usedPairs;
  }
  return { dense: NUM_RELATIONS, ideal, achieved: dispatched / usedPairs, mode: 'scattered' };
}
