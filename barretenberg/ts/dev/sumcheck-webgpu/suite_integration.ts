...
//
// Drives the relation isolation kernels through the full CPU tail on real GPU
// output: for each relation, build edges with per-edge scaling = gate
// separator edgeScaling(j), dispatch, decode, sum over edges (reduceEdges) into
// the relation's 345-Fr slice; assemble all slices (absent relations zero), then
...
// pipeline fed by the polynomial references (the C++-mirror goldens).
//
// Scope: currently the simple-group relations (no relation_parameters). Each
// relation uses its own random edges — this validates the GPU->reduce->assemble->
// tail plumbing and the gate-separator wiring, not a shared execution trace.
// Round 0 (periodicity 2, c_i = 1); the c_i/later-round paths are covered by the
// no-GPU gate_separator tests.

import { GateSeparatorPolynomial } from '../../src/msm_webgpu/gate_separator.js';
import {
  NUM_RELATIONS, RELATION_NAMES, assembleAccumulator, reduceEdges,
} from '../../src/msm_webgpu/accumulator.js';
import { SIMPLE_GROUP } from './descriptors.js';
import {
  type Suite, type SuiteCtx,
  WG, makeRng, dispatchRelation, packEdgeRows, fromMont, le32ToBi,
} from './harness.js';
...
async function run({ device, n, log }: SuiteCtx): Promise<boolean> {
  ...
  for (const desc of SIMPLE_GROUP) {
    ...
    const { bytes, ms } = await dispatchRelation(device, n, desc.shader(), desc.entry, inBytes, desc.outLen);
    totalMs += ms;
    ...
    const refSlice = reduceEdges(inputs.map(inp => desc.polyRef(inp.e, inp.s)), desc.outLen);

    // Localize any GPU mismatch to (relation, slice index) before the reduction.
    let mism = 0;
    let first = '';
    for (let k = 0; k < desc.outLen; k++) {
      if (gpuSlice[k] !== refSlice[k]) { mism++; if (mism <= 3) first += `\n    ${desc.id} slice[${k}] got=${gpuSlice[k]} want=${refSlice[k]}`; }
    }
    if (mism > 0) { log('err', `  ${desc.id.padEnd(10)} ✗  ${mism}/${desc.outLen} slice MISMATCH${first}`); return false; }
    log('ok', `  ${desc.id.padEnd(10)} ✓  slice reduced (${n} edges, ${ms.toFixed(1)} ms)`);
  ...
  const present = SIMPLE_GROUP.map(desc => RELATION_NAMES[desc.relationIndex]).join(', ');
  log('ok', `  round univariate ✓  length-${gpuRU.length} over {${present}}  (${totalMs.toFixed(1)} ms GPU)`);
