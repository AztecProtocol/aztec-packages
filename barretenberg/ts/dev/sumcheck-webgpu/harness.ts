...
//     logderiv `[read_counts, q_lookup]`, databus `[q_busread, 5×read_counts]`).
...
export interface RelationDescriptor {
  ...
  // (gpu_pipeline.injectSkipPrelude) and the sparse-instance generator
  // (sparsity.applySkipZeroing) both read this; the same column indices appear as
  // the "skip path" rows in `build` above.
  skip: SkipPredicate;
}
