...
export async function runSingleSubmitSumcheck(
  ...
  for (let i = 0; i < rounds; i++) {
    ...
    if (useShared) {
      ...
      // whole columns, so storage-offset-aligned) and folds it independently. Full fold per
      // chunk (band_count = pairs, band_start = 0): the band-aware fold shader requires both.
      ...
      for (let c0 = 0; c0 < NUM_GLOBAL_ENTITIES; c0 += maxCols) {
        ...
        const fBg = device.createBindGroup({
          ...
          entries: [
            ...
            { binding: 2, resource: { buffer: create_and_write_ub(device, u32x4(numOut, pairs, pairs, 0)) } },
