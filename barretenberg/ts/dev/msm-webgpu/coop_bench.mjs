// Cooperative RNS modmul (16 lanes/modmul) — on-device validate + bench vs f8.
// MUST run with the T=16/W=16 oracle config:
//   RNS_T=16 RNS_W=16 RNS_RANK_F=27 node dev/msm-webgpu/coop_bench.mjs [Kperf=262144] [chain=64] [reps=30]
import { readFileSync } from 'fs';
import { fileURLToPath } from 'url';
import { dirname, join } from 'path';
import http from 'http';
import { chromium } from 'playwright-core';
import { computeParams, rnsModmul, T, W } from './rns_params.mjs';
...
// ---- cooperative RNS inputs (residues in both bases) + expected ----
...
const coopWgsl = ['enable subgroups;', readFileSync(join(wgslDir, 'rns', 'rns_constants_16x16.wgsl'), 'utf8'), readFileSync(join(wgslDir, 'rns', 'rns_field_coop.wgsl'), 'utf8')].join('\n');

// ---- f8 baseline (same as modmul_bench) ----
...
fn unpack256_to_limbs(x: array<u32, 8>) -> BigInt {
  var r: BigInt;
  for (var i = 0u; i < 20u; i++) { let bit=i*13u; let w=bit>>5u; let off=bit&31u; var v=x[w]>>off; if(off>19u&&w<7u){v=v|(x[w+1u]<<(32u-off));} r.limbs[i]=v&0x1fffu; }
  return r;
}`,
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let base=gid.x*16u; var a: array<u32,8>; var b: array<u32,8>;
  for (var i=0u;i<8u;i++){a[i]=ins[base+i];b[i]=ins[base+8u+i];}
  var r=a; for(var k=0u;k<${chain}u;k++){r=montgomery_product_f8(r,b);}
...
const result = await page.evaluate(async ({ coopWgsl, f8Wgsl1, f8WgslN, rnsIn, rnsExp, f8In, f8Exp, KVAL, Kperf, CHAIN, reps, lanesPerOp }) => {
  ...
  const out = { info: { vendor: info.vendor, architecture: info.architecture }, hasSg, feats, sgSize: info.subgroupMinSize ?? info.subgroupMaxSize ?? null };
  ...
  const run = async (pipe, K, inArr, outU32, lanes) => {
  ...
  const validate = async (pipe, inArr, expArr, outU32, lanes) => {
    ...
    const x = await run(pipe, K, new Uint32Array(inArr), outU32, lanes);
  ...
  const perf = async (pipe, inArr, inU32, outU32, lanes, K) => {
    // cap so workgroups = ceil(K*lanes/64) <= 65535
    const maxK = Math.floor((65535 * 64) / lanes);
    K = Math.min(K, maxK);
    const cin = new Uint32Array(inArr); const pin = new Uint32Array(K * inU32);
    for (let i = 0; i < pin.length; i++) pin[i] = cin[i % cin.length];
    const x = await run(pipe, K, pin, outU32, lanes);
  ...
  // cooperative
  if (!hasSg) { out.coopErr = 'no subgroups feature'; }
  else {
    const cv = await build(coopWgsl, { CHAIN: 1 });
    if (cv.err) { out.coopErr = cv.err; }
    else {
      out.coop = await validate(cv.pipe, rnsIn, rnsExp, 32, lanesPerOp);
      const cp = await build(coopWgsl, { CHAIN });
      if (cp.err) out.coopErr = cp.err; else out.coopPerf = await perf(cp.pipe, rnsIn, 64, 32, lanesPerOp, Kperf);
    }
  }
  // f8
  const fv = await build(f8Wgsl1, {});
  if (fv.err) { out.f8Err = fv.err; } else {
    out.f8 = await validate(fv.pipe, f8In, f8Exp, 8, 1);
    const fp = await build(f8WgslN, {});
    out.f8Perf = await perf(fp.pipe, f8In, 16, 8, 1, Kperf);
  }
  return out;
}, { coopWgsl, f8Wgsl1: f8Wgsl(1), f8WgslN: f8Wgsl(CHAIN), rnsIn, rnsExp, f8In, f8Exp, KVAL, Kperf, CHAIN, reps, lanesPerOp: 16 });
...
const nsmul = (perf) => (perf.median * 1e6) / (perf.K * CHAIN);
const show = (name, v, perf) => { if (!perf) return; const ns = nsmul(perf), mn = (perf.min * 1e6) / (perf.K * CHAIN); console.log(`${name.padEnd(5)} mism=${v ? v.mism : '?'}${v && v.mism ? ' ' + JSON.stringify(v.firsts) : ''}  K=${perf.K} median ${perf.median.toFixed(2)}ms -> ${ns.toFixed(3)} ns/mul (min ${mn.toFixed(3)})`); };
show('f8', result.f8, result.f8Perf);
show('coop', result.coop, result.coopPerf);
if (result.coopPerf && result.f8Perf) console.log(`\ncoop / f8 = ${(nsmul(result.coopPerf) / nsmul(result.f8Perf)).toFixed(2)}x  (ns/mul; <1 means coop faster)`);
