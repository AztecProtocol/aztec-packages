...
# Use wasi-sdk builds (production)
...
# Note: For debugging memory issues with address sanitizer, build with:
#   ../cpp/bootstrap.sh build_emscripten_threads_asan
# Then create a custom bb_backend to load the emscripten WASM + JS glue code from:
#   ../cpp/build-emscripten-threads-asan/bin/barretenberg-debug.wasm.{wasm,js}
