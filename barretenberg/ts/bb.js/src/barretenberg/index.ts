...
export class BarretenbergSync extends SyncApi {
  ...
  static async initSingleton(options: BackendOptions = {}) {
    if (!barretenbergSyncSingletonPromise) {
      // unref for the same reason the async singleton does: nothing destroys a
      // singleton, so its backend must not hold the event loop open at exit.
      barretenbergSyncSingletonPromise = BarretenbergSync.new({ ...options, unref: true });
    }
