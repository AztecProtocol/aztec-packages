...
if [ "${CI:-0}" -eq 1 ]; then
  ...
  FLOW=prove_then_verify ./run_acir_tests.sh ecdsa_secp256r1_3x &
  # Run a single arbitrary test not involving recursion through bb.js for UltraHonk
  FLOW=prove_and_verify_ultra_honk ./run_acir_tests.sh 6_array assert_statement &
  # Run the prove then verify flow for UltraHonk. This makes sure we have the same circuit for different witness inputs.
  FLOW=prove_then_verify_ultra_honk ./run_acir_tests.sh 6_array assert_statement &
  ...
  FLOW=fold_and_verify_program ./run_acir_tests.sh fold_basic &
