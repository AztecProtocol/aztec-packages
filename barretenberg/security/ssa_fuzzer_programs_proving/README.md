...
## Including acir_components_count (optional)

To include the `acir_components_count` binary in the image (for ACIR/circuit component consistency checks), build it and copy it into this directory before building the image:

```bash
# From barretenberg/cpp
cmake --preset default && cmake --build build --target acir_components_count

# From barretenberg/security/ssa_fuzzer_programs_proving
./prepare_binaries.sh
```

Then build the image as usual; the binary will be at `/app/proving/acir_components_count` in the container.