...
ssh $BB_SSH_in "
	set -eux ;
	cd /mnt/user-data/$USER/aztec-packages/barretenberg/cpp/ ;
	cmake --preset $PRESET && cmake --build --preset $PRESET --target $BENCHMARK ;
	! [ -d /mnt/user-data/$USER/tracy ] && git clone https://github.com/wolfpld/tracy /mnt/user-data/$USER/tracy ;
	cd /mnt/user-data/$USER/tracy/capture ;
        git checkout 075395620a504c0cdcaf9bab3d196db16a043de7 ;
	sudo apt-get install -y libdbus-1-dev libdbus-glib-1-dev ;
	mkdir -p build && cd build && cmake .. && make -j ;
	sudo ./tracy-capture -a 127.0.0.1 -f -o trace-$BENCHMARK & ;
	sleep 0.1 ;
	cd /mnt/user-data/$USER/aztec-packages/barretenberg/cpp/build-$PRESET ;
	ninja $BENCHMARK ;
	sudo $COMMAND ;
" &
