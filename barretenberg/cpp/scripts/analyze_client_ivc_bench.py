...
def print_contributions(prefix, ivc_bench_json, benchmark, relations):

    # Read JSON file and extract benchmark
    try:
        with open(prefix / ivc_bench_json, "r") as read_file:
            read_result = json.load(read_file)
            bench = next((_bench for _bench in read_result["benchmarks"] if _bench["name"] == benchmark), None)
            if not bench:
                raise ValueError(f"Benchmark '{benchmark}' not found in the JSON file.")
    except FileNotFoundError:
        print(f"File not found: {prefix / ivc_bench_json}")
        return
    
    # Filter and sum up kept times
    bench_components = {key: bench[key] for key in relations if key in bench}
