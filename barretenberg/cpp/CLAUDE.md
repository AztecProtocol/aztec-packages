...
There are two routes once you have permission:

1. **Auto-regen via `VK-UPDATE:` commit.** Push an empty acknowledgement commit
   on your PR and CI will regenerate the inputs, upload the new tarball, and
   push the pin update back to your branch with `[skip ci]`:
   ...
   CI's `ci-fast` / `ci-full` lanes call `scripts/ci_vk_update.sh` after a
   failing test pass; it detects the `VK-UPDATE:` commit and runs
   `regenerate_chonk_inputs.sh --push --reason ...`. The pushed commit is
   tagged `[skip ci]` so CI does not loop.

2. **Manual local regen** (when CI is unavailable, or to inspect the diff
   first):
   ...
   This delegates to `regenerate_chonk_inputs.sh --no-commit`: it builds bb
   native (AVM=ON), regenerates inputs via the e2e client flows, proves +
   verifies each flow, uploads the new tarball, and updates
   `barretenberg/cpp/scripts/chonk-inputs.hash` in your working tree. Review
   the diff, then commit and push yourself.
...
Example IVC inputs (msgpack files) for `bb prove --scheme chonk` come from a
single canonical pinned tarball stored in S3. The short hash of the pinned
tarball lives in `barretenberg/cpp/scripts/chonk-inputs.hash` and every Chonk
benchmark and the VK consistency test consume it via
`barretenberg/cpp/scripts/chonk_inputs_lib.sh`.

Populate the inputs locally:

```bash
yarn-project/end-to-end/bootstrap.sh build_bench
```

This downloads + extracts the pinned tarball into
`yarn-project/end-to-end/example-app-ivc-inputs-out/<flow>/ivc-inputs.msgpack`.
Set `BUILD_BENCH_REGEN=1` to instead regenerate inputs from scratch by running
the e2e client flows (slow path — used by `regenerate_chonk_inputs.sh` when
the pin needs to move).
