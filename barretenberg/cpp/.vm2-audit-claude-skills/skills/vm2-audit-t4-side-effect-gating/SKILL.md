---
name: vm2-audit-t4-side-effect-gating
...
## Severity Assessment
- **Soundness** (malicious prover exploits): Critical/High/Medium based on side effect type
...
| Side Effect Type | Soundness | Completeness (if tracegen triggers) |
|------------------|-----------|-------------------------------------|
| Tree write (nullifier, note_hash, storage) | Critical | Critical |
| Public input write | High | Critical |
| Log/message | Medium | High |
...
| Item | Value |
|------|-------|
| Skill | `vm2-audit-t4-side-effect-gating` |
| Target | `{path}` |
...
### Finding Format
- **ID**: `vm2-audit-t4-side-effect-gating-{file}-{line}-{selector}`
