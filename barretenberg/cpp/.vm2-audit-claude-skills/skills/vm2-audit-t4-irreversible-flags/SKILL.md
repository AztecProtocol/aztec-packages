---
name: vm2-audit-t4-irreversible-flags
...
## Severity Assessment
- **Soundness** (malicious prover exploits): High if reversible flag affects security
...
|---------|-----------|--------------|
| Reversible security flag (padding, error) | High | Critical if tracegen triggers |
| Reversible computation flag | Medium | High if tracegen triggers |
| Flag with explicit reset mechanism | Low | Low (intentional) |
...
| Item | Value |
|------|-------|
| Skill | `vm2-audit-t4-irreversible-flags` |
| Target | `{path}` |
| Flags Analyzed | `{N}` |
| Findings | `{e.g., "1 High"}` |
...
### Finding Format
- **ID**: `vm2-audit-t4-irreversible-flags-{file}-{line}-{flag}`
