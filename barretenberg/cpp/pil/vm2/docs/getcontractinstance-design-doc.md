...
**Used by opcode component and gadget**: - **Memory**: Used by the memory-aware opcode component to write the opcode results to memory. - _Note_: this opcode relies on `Execution` to perform address resolution, tag checking and memory reads.
**Used during core contract instance retrieval**: - **Nullifier Check**: Validates deployment nullifier existence. Used during contract instance retrieval. - **Address Derivation**: Ensures contract addresses match their constituent parts. - **Update Check**: Validates contract class ID updates
