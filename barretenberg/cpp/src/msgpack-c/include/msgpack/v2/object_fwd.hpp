...
MSGPACK_API_VERSION_NAMESPACE(v2)
{
    ...
    struct object : v1::object {
         ...
         */
        template <typename T> explicit object(const T& v) { *this << v; }
      ...
      public:
         ...
         * @tparam T The type of v.
         * @param v The value you want to get. `v` is output parameter. `v` is overwritten by converted value from the
         * object.
         * @return The reference of `v`.
         */
        template <typename T> T& convert(T& v) const { return v1::object::convert(v); }
    ...
    template <typename T> struct as<T, typename std::enable_if<v1::has_as<T>::value>::type> : v1::adaptor::as<T> {};
    ...
    template <typename T> struct has_as {
      private:
        template <typename U>
        static auto check_(U*) -> typename std::enable_if<
            // check v2 specialization
            std::is_same<decltype(adaptor::as<U>()(std::declval<msgpack::object>())), U>::value ||
                // check v1 specialization
                v1::has_as<U>::value,
            std::true_type>::type;
        template <typename...> static std::false_type check_(...);
