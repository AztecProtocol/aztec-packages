...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    class zone {
      private:
        struct finalizer {
            finalizer(void (*func)(void*), void* data, finalizer* next)
                : m_func(func)
                , m_data(data)
                , m_next(next)
            {}
        ...
        struct finalizer_array {
            ...
            void push(void (*func)(void* data), void* data) { m_head = new finalizer(func, data, m_head); }
        ...
        struct chunk_list {
            chunk_list(size_t chunk_size, char* ptr)
                : m_free(chunk_size)
                , m_ptr(ptr)
                , m_head(MSGPACK_NULLPTR)
            {}
      ...
      public:
        ...
        bool allocated() { return m_chunk_list != MSGPACK_NULLPTR; }

        void push_finalizer(void (*func)(void*), void* data);

        template <typename T> void push_finalizer(msgpack::unique_ptr<T> obj);
      ...
      private:
        ...
        template <typename T> static void object_destruct(void* obj);

        template <typename T> static void object_delete(void* obj);
    ...
    inline char* zone::get_aligned(char* ptr, size_t align)
    {
        ...
        return reinterpret_cast<char*>(reinterpret_cast<uintptr_t>(ptr + (align - 1)) &
    ...
    template <typename T> inline void zone::push_finalizer(msgpack::unique_ptr<T> obj)
    {
    ...
    template <typename T> void zone::object_delete(void* obj)
    {
        delete static_cast<T*>(obj);
    }

    template <typename T> void zone::object_destruct(void* obj)
    {
        static_cast<T*>(obj)->~T();
    }
    ...
    template <typename T, typename... Args> T* zone::allocate(Args... args)
    {
