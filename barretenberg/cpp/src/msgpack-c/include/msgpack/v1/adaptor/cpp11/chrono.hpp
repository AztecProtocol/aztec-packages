...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename Target,
              ...
              typename =
                  typename std::enable_if<std::is_integral<Target>::value && std::is_integral<Source>::value>::type>
    ...
    template <typename Target, typename Source> struct would_underflow<Target, Source, false, true> {
    ...
    template <typename Target, typename Source> struct would_underflow<Target, Source, true, true> {
    ...
    template <typename Target, typename Source> struct would_overflow<Target, Source, false, true> {
    ...
    template <typename Target, typename Source> struct would_overflow<Target, Source, true, false> {
    ...
    template <
        typename Target,
        typename Source,
        typename = typename std::enable_if<std::is_integral<Target>::value && std::is_integral<Source>::value>::type>
    ...
    template <typename Clock, typename Duration> struct as<std::chrono::time_point<Clock, Duration>> {
        typename std::chrono::time_point<Clock, Duration> operator()(msgpack::object const& o) const
        {
            ...
            case 8: {
                ...
                tp += std::chrono::duration_cast<Duration>(std::chrono::nanoseconds(nanosec));
                tp += std::chrono::seconds(sec);
            } break;
            case 12: {
                uint32_t nanosec;
                ...
                if (sec > 0) {
                    ...
                    if (nanosec == 0) {
                        ...
                        tp += std::chrono::duration_cast<Duration>(std::chrono::nanoseconds(ns));
                    }
                }
            } break;
            default:
    ...
    template <typename Clock, typename Duration> struct convert<std::chrono::time_point<Clock, Duration>> {
    ...
    template <typename Clock, typename Duration> struct pack<std::chrono::time_point<Clock, Duration>> {
                                            ...
                                            std::chrono::time_point<Clock, Duration> const& v) const
        {
            ...
            int64_t nano_num = Duration::period::ratio::num * (1000000000L / Duration::period::ratio::den);
            ...
            sec += count * Duration::period::ratio::num / Duration::period::ratio::den;

            if ((sec >> 34) == 0) {
                uint64_t data64 =
                    (detail::integral_cast<uint64_t>(nanosec) << 34) | detail::integral_cast<uint64_t>(sec);
    ...
    template <typename Clock, typename Duration> struct object_with_zone<std::chrono::time_point<Clock, Duration>> {
