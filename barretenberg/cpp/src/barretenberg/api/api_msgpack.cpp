...
int process_msgpack_commands(std::istream& input_stream)
{
    ...
    while (!input_stream.eof()) {
        ...
        // Decode the request, execute it, and serialize the response. A request that cannot be decoded (malformed
        // msgpack, an unknown command name, a missing command field) is answered with an ErrorResponse so the
        // stream stays usable and the client learns what was wrong.
        ...
        try {
            ...
            // Drop anything a partially completed pack left behind so the frame holds only the error.
            response_buffer.clear();
            std::cerr << "Error processing msgpack request: " << e.what() << '\n';
