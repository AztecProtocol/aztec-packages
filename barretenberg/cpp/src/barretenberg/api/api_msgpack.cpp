...
int process_msgpack_commands(std::istream& input_stream)
{
    ...
    while (!input_stream.eof()) {
        ...
        // It turns a malformed envelope, an unknown command and a throwing
        // handler into error frames itself, but a payload that fails to unpack or
        // convert still throws: catch that here rather than let it take down a
        // process that is mid-stream and owes the caller a response.
        ...
        try {
            ...
            response = bbapi::detail::make_error(e.what());
        }
#endif
