...
namespace bb::bbrpc {
/**
 * @brief Type-erased base so BBRpcCircuitState can iterate over all caches without knowing their
 *        template parameters.
 */
...
/* =================================================================================================
 *  IVC session state
 *  One IVC per request is allowed.
 * ================================================================================================= */

class BBRpcIvcState {
  public:
    /**
     * @brief Clears the IVC state.
     *
     * This function is registered to be called when the BBRpcIvcState is cleared.
     */
    void clear() { ivc_sessions.clear(); }

    /**
     * @brief Drops a specific IVC session.
     *
     * This function is registered to be called when a specific IVC session is dropped.
     */
    void drop(RequestId request_id) { ivc_sessions.erase(request_id); }

  private:
    std::unordered_map<RequestId, ClientIVC> ivc_sessions;
};
...
struct BBRpcSession {
    BBRpcIvcState ivc_state;
    BBRpcCircuitRegistry circuit_registry;
};
