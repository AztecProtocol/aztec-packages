...
class API {
  public:
    ...
    struct Flags {
        ...
        std::string crs_path{ "" }; // the location of reference strings for commitment schemes
    ...
    virtual bool check(const Flags& flags, const std::string& bytecode_path, const std::string& witness_path) = 0;

    virtual bool verify(const Flags& flags,
                        const std::string& public_inputs_path,
                        const std::string& proof_path,
                        const std::string& vk_path) = 0;

    virtual void write_vk(const Flags& flags, const std::string& bytecode_path, const std::string& output_path) = 0;

    virtual void gates(const Flags& flags, const std::string& bytecode_path) = 0;
