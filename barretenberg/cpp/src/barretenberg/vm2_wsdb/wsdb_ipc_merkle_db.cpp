...
#include "barretenberg/vm2_wsdb/wsdb_wire.hpp"
...
avm2::TreeSnapshots WsdbIpcMerkleDB::get_tree_roots() const
{
    ...
    auto wire_rev = to_wire(revision_);

    auto l1_info = client_.get_tree_info(
        wsdb::WsdbGetTreeInfo{ .treeId = to_wire(MerkleTreeId::L1_TO_L2_MESSAGE_TREE), .revision = wire_rev });
    auto nh_info = client_.get_tree_info(
        wsdb::WsdbGetTreeInfo{ .treeId = to_wire(MerkleTreeId::NOTE_HASH_TREE), .revision = wire_rev });
    auto null_info = client_.get_tree_info(
        wsdb::WsdbGetTreeInfo{ .treeId = to_wire(MerkleTreeId::NULLIFIER_TREE), .revision = wire_rev });
    auto pd_info = client_.get_tree_info(
        wsdb::WsdbGetTreeInfo{ .treeId = to_wire(MerkleTreeId::PUBLIC_DATA_TREE), .revision = wire_rev });

    avm2::TreeSnapshots snapshots{
        .l1_to_l2_message_tree = avm2::AppendOnlyTreeSnapshot{ .root = from_wire<bb::fr>(l1_info.root),
                                                               .next_available_leaf_index = l1_info.size },
        .note_hash_tree = avm2::AppendOnlyTreeSnapshot{ .root = from_wire<bb::fr>(nh_info.root),
                                                        .next_available_leaf_index = nh_info.size },
        .nullifier_tree = avm2::AppendOnlyTreeSnapshot{ .root = from_wire<bb::fr>(null_info.root),
                                                        .next_available_leaf_index = null_info.size },
        .public_data_tree = avm2::AppendOnlyTreeSnapshot{ .root = from_wire<bb::fr>(pd_info.root),
...
SiblingPath WsdbIpcMerkleDB::get_sibling_path(MerkleTreeId tree_id, index_t leaf_index) const
{
    auto resp = client_.get_sibling_path(wsdb::WsdbGetSiblingPath{
        .treeId = to_wire(tree_id), .revision = to_wire(revision_), .leafIndex = leaf_index });
    return from_wire<SiblingPath>(resp.path);
}
...
{
    auto resp = client_.find_low_leaf(
        wsdb::WsdbFindLowLeaf{ .treeId = to_wire(tree_id), .revision = to_wire(revision_), .key = to_wire(value) });
...
avm2::FF WsdbIpcMerkleDB::get_leaf_value(MerkleTreeId tree_id, index_t leaf_index) const
{
    auto resp = client_.get_leaf_value(
        wsdb::WsdbGetLeafValue{ .treeId = to_wire(tree_id), .revision = to_wire(revision_), .leafIndex = leaf_index });
    ...
    return from_wire<bb::fr>(resp.value.value());
}
...
IndexedLeaf<PublicDataLeafValue> WsdbIpcMerkleDB::get_leaf_preimage_public_data_tree(index_t leaf_index) const
{
    auto resp = client_.get_public_data_leaf_preimage(
        wsdb::WsdbGetPublicDataLeafPreimage{ .revision = to_wire(revision_), .leafIndex = leaf_index });
    if (!resp.preimage.has_value()) {
        throw std::runtime_error("Invalid get_leaf_preimage_public_data_tree request for index " +
                                 std::to_string(leaf_index));
    }
    return from_wire<IndexedLeaf<PublicDataLeafValue>>(resp.preimage.value());
}
...
IndexedLeaf<NullifierLeafValue> WsdbIpcMerkleDB::get_leaf_preimage_nullifier_tree(index_t leaf_index) const
{
    auto resp = client_.get_nullifier_leaf_preimage(
        wsdb::WsdbGetNullifierLeafPreimage{ .revision = to_wire(revision_), .leafIndex = leaf_index });
    if (!resp.preimage.has_value()) {
        throw std::runtime_error("Invalid get_leaf_preimage_nullifier_tree request for index " +
                                 std::to_string(leaf_index));
    }
    return from_wire<IndexedLeaf<NullifierLeafValue>>(resp.preimage.value());
}
...
{
    auto resp = client_.sequential_insert_public_data(
        wsdb::WsdbSequentialInsertPublicData{ .leaves = { to_wire(leaf_value) }, .forkId = revision_.forkId });
    invalidate_tree_roots_cache();
    return from_wire<SequentialInsertionResult<PublicDataLeafValue>>(resp.result);
}
...
{
    auto resp = client_.sequential_insert_nullifier(
        wsdb::WsdbSequentialInsertNullifier{ .leaves = { to_wire(leaf_value) }, .forkId = revision_.forkId });
    invalidate_tree_roots_cache();
    return from_wire<SequentialInsertionResult<NullifierLeafValue>>(resp.result);
}
...
void WsdbIpcMerkleDB::append_leaves(MerkleTreeId tree_id, std::span<const avm2::FF> leaves)
{
    ...
    client_.append_leaves(wsdb::WsdbAppendLeaves{
        .treeId = to_wire(tree_id), .leaves = std::move(wire_leaves), .forkId = revision_.forkId });
    invalidate_tree_roots_cache();
}
...
void WsdbIpcMerkleDB::pad_tree(MerkleTreeId tree_id, size_t num_leaves)
{
    ...
    case MerkleTreeId::NULLIFIER_TREE: {
        ...
        for (size_t i = 0; i < num_leaves; i++) {
            padding_leaves.push_back(to_wire(empty_leaf));
        }
    ...
    case MerkleTreeId::NOTE_HASH_TREE: {
        ...
        client_.append_leaves(wsdb::WsdbAppendLeaves{ .treeId = to_wire(MerkleTreeId::NOTE_HASH_TREE),
