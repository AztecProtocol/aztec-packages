...
//  *
//  * Once we have our 127-bit scalar multipliers, we determine the optimal number of pippenger rounds, given the number
//  of *points we're multiplying. Once we have the number of rounds, `m`, we need to split our scalar into `m`
//  bit-slices. *Each pippenger round will work on one bit-slice.
//  *
//  * Pippenger's algorithm works by, for each round, iterating over the points we're multiplying. For each point, we
//  *examing the point's scalar multiplier and extract the bit-slice associated with the current pippenger round (we
...
//  * sum = bucket[0] + 2 * bucket[1] + 3 * bucket[2] + 4 * bucket[3] + 5 * bucket[4] + 6 * bucket[5] + 7 * bucket[6] +
//  8 * *bucket[7].
//  *
//  * At the end of each pippenger round, the bucket sum will contain the scalar multiplication result for one bit
//  slice.
//  * For example, say we have 16 rounds, where each bit slice contains 8 bits (8 * 16 = 128, enough to represent our
//  127 *bit scalars). At the end of the first round, we will have taken the 8 most significant bits from every scalar
//  *multiplier. Our bucket sum will be the result of a mini-scalar-multiplication, where we have multiplied every point
//  by *the 8 most significant bits of each point's scalar multiplier.
//  *
...
//  *
//  * To minimize the number of point additions per round, we want fewer rounds. But fewer rounds increases the number
//  of *bucket concatenations. The more points we have, the greater the time saving when reducing the number of rounds,
//  which *means we can afford to have more buckets per round.
//  *
//  * For a concrete example, with 2^20 points, the sweet spot is 2^15 buckets - with 2^15 buckets we can evaluate our
//  127 *bit scalar multipliers in 8 rounds (we can represent b-bit windows with 2^{b-1} buckets, more on that below).
//  *
...
//  *require 9 multiple-scalar-multiplications with 2^20 points, efficiently using Pippenger's algorithm is essential
//  for *fast provers
//  *
...
//  *
//  * We actually use a slightly different representation than simple non-adjacent form. To represent b bits, a bit
//  slice *contains values from (-2^{b} - 1, ..., -1, 1, ..., 2^{b} - 1). i.e. we only have odd values. We do this to
//  eliminate *0-valued windows, as having a conditional branch in our hot loop to check if an entry is 0 is somethin we
//  want to *avoid.
//  *
//  * The above representation can be used to represent any binary number as long as we add a 'skew' factor. Each scalar
//  *multiplier's `skew` tracks if the scalar multiplier is even or odd. If it's even, `skew = true`, and we add `1` to
//  our *multiplier to make it odd.
//  *
...
//  *  We currently don't multi-thread the inner sorting algorithm, and just split our threads over the number of
//  rounds.
...
//             add_affine_points_with_edge_cases<Curve>(state.point_pairs_1 + start, points_in_round,
//             state.scratch_space);
//         } else {
...
//     compute_wnaf_states<Curve>(state.point_schedule, state.skew_table, state.round_counts, scalars,
//     num_initial_points); organize_buckets(state.point_schedule, num_initial_points * 2); typename Curve::Element
//     result =
...
//  *
//  * It's...unsafe, because we assume that the incomplete addition formula exceptions are not triggered i.e. that all
//  the
...
//  * there should be no equal points in the SRS), because triggering the incomplete addition exceptions is about as
//  hard *as solving the disrete log problem. This is ok for the prover, but GIANT RED CLAXON WARNINGS FOR THE VERIFIER
//  Don't *use this in a verification algorithm! That would be a really bad idea. Unless you're a malicious adversary,
...
//                                                                 scalars, const size_t num_initial_points,
...
// template curve::BN254::Element pippenger_unsafe<curve::BN254>(PolynomialSpan<const curve::BN254::ScalarField>
// scalars,
//                                                               std::span<const curve::BN254::AffineElement> points,
//                                                               pippenger_runtime_state<curve::BN254>& state);
...
// template curve::Grumpkin::Element pippenger<curve::Grumpkin>(PolynomialSpan<const curve::Grumpkin::ScalarField>
// scalars,
//                                                              std::span<const curve::Grumpkin::AffineElement> points,
//                                                              pippenger_runtime_state<curve::Grumpkin>& state,
//                                                              bool handle_edge_cases = true);
