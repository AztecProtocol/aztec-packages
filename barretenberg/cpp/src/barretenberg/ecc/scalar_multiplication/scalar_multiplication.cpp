 ...
 */
// Precomputed per-window slice params for Constantine's signed-Booth recoder.
//
 ...
 *
 *        The pippenger function pre-sizes each span large enough for that call's worst
                  ...
                  std::span<const typename Curve::AffineElement> dedup_extra_points = {}) noexcept
{
    ...
    while (bucket <= chunk_bucket_hi && pos < chunk_hi) {
        ...
        for (size_t i = run_lo; i < run_hi; ++i) {
            ...
            // when active, since cluster-rep scheduling is uniform per MSM.
    ...
    size_t min_pts_per_thread_override) noexcept
{
    ...
    // Cost-model window-size selection (mirrors MSM<Curve>::get_optimal_log_num_buckets,
    ...
    auto thread_body = [&](size_t tid) {
        BB_BENCH_NAME("MSM::jacobian_fast/worker");
...
/**
 * @brief Round-parallel Pippenger MSM. Windows process sequentially (high-to-low) but each
...
// Jacobian→affine inversion at the caller boundary (when `MSM<>::msm` constructs an
 ...
 * Sharing the rpmsm pool with the main pippenger keeps per-call dispatch cheap.
 */
template <typename Curve>
                                             ...
                                             std::span<const typename Curve::AffineElement> all_points) noexcept
{
    ...
    //      extra dispatch tax at the trivial-MSM sizes this function handles.
    ...
    bb::parallel_for(num_threads, [&](size_t tid) {
        BB_BENCH_NAME("MSM::trivial_msm_threaded/worker");
                                          ...
                                          bool dedup_active = false) noexcept
{
    ...
    // (e.g. PerMsmChonk's 256 separate per-circuit MSMs) where every per-MSM
...
//     bytes. When empty, allocate per-MSM via make_unique_for_overwrite and free at
                                                 ...
                                                 std::span<std::byte> external_arena) noexcept
{
    ...
    // MIN_PTS_PER_THREAD_FOR_PIPPENGER points — pippenger's per-window scaffolding loses
    ...
    if (use_glv) {
        ...
        bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
            BB_BENCH_NAME("MSM::glv_from_mont_split/worker");
        ...
        bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
            BB_BENCH_NAME("MSM::from_montgomery/worker");
            auto& th_hist = per_thread_msb_hist[chunk.thread_index];
            for (size_t i : chunk.range(n_input)) {
    ...
    // ---------------------------------------------------------------------------------------
    // Phase 2 — bail to trivial_msm_threaded when n_active is too small to amortise pippenger's
    ...
    // ---------------------------------------------------------------------------------------
    {
        ...
        if (pts_per_thread < MIN_PTS_PER_THREAD_FOR_PIPPENGER) {
            bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
                BB_BENCH_NAME("MSM::trivial_msm_to_montgomery/worker");
    ...
    // Pick windows_in_batch so per-MSM working set fits in ~32 MB. Empirically 32 MB
    ...
    if (dedup_active) {
        ...
        BB_BENCH_NAME("MSM::dedup/redirect_invalid_fill");
        uint32_t* const rl = dedup_state.redirect_lookup.data();
        bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
            BB_BENCH_NAME("MSM::dedup/redirect_invalid_fill/worker");
            for (size_t i : chunk.range(n)) {
    ...
    // Phase A runs at most once per MSM (not per batch). Cluster membership is determined
    ...
    auto run_batch = [&](size_t batch_start,
        ...
        auto stage1_digit_extract = [&]<bool DedupKnown>(size_t tid) noexcept {
            BB_BENCH_NAME("MSM::Stage1_digit_extract/worker");
        ...
        bb::parallel_for(num_threads, [&](size_t tid) {
            BB_BENCH_NAME("MSM::Stage2_bucket_histogram/worker");
        ...
        {
            BB_BENCH_NAME("MSM::Stage2_3_bucket_offsets");
            ...
            if (offset_threads <= 1) {
                ...
                bb::parallel_for(offset_threads, [&](size_t tid) {
                    BB_BENCH_NAME("MSM::Stage2_3_bucket_offsets/worker");
        ...
        if (dedup_known_for_batch) {
            bb::parallel_for(num_threads, [&](size_t tid) {
                BB_BENCH_NAME("MSM::Stage4_digit_scatter/worker");
        ...
        // Phase A: schedule-based dedup detection. Runs at most ONCE per MSM (gated on
        ...
        if (dedup_active && windows_in_batch > 0 && !phase_a_done) {
            ...
            {
                BB_BENCH_NAME("MSM::PhaseA_dedup_detect_hash");
                ...
                bb::parallel_for(num_threads, [&, w0_bucket_start](size_t tid) noexcept {
                    BB_BENCH_NAME("MSM::PhaseA_dedup_detect/worker");
        ...
        {
            BB_BENCH_NAME("MSM::Stage5_chunk_partition");
...

        {
            ...
            auto bucket_partials_per_thread_lambda = [&](size_t tid) {
                BB_BENCH_NAME("MSM::Stage6a_bucket_partials/worker");
            ...
            auto bucket_reduce_cross_thread_lambda = [&](size_t tprime) {
                BB_BENCH_NAME("MSM::Stage6b_reduce_cross_thread/worker");
        ...
        // with the per-chunk contributions already accumulated above.)
        {
            ...
            bb::parallel_for(reduce_threads, [&](size_t rid) {
                BB_BENCH_NAME("MSM::Stage7_combine/worker");
    ...
    if (!use_glv) {
        bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
            BB_BENCH_NAME("MSM::to_montgomery/worker");
            for (size_t i : chunk.range(n_input)) {
...
// Multi-MSM driver for `MSM<>::batch_multi_scalar_mul`. The hot path
...
// K-multiplies the per-thread working set and forces windows_in_batch=1; the single-MSM
// hot path is tuned to fit ~4 MiB in L2 and we want to preserve that. The loop is just
//   for m in 0..K: run single-MSM dispatch for MSM m.
// The only cross-MSM amortisation is the GLV-doubled point set: when every member of a
// shared-SRS-prefix group wants GLV, we double the prefix once into a shared buffer and
// each per-MSM call aliases its prefix instead of doubling its own.
namespace round_parallel_detail {
...
template <typename Curve> struct BatchMsmGlvGroup {
    ...
    std::span<typename Curve::AffineElement> doubled;        // length 2 * group_max_n; aliases a prefix of
                                                             ...
                                                             // entries are the per-MSM view for n ≤ Nmax.
                                      ...
                                      std::span<const uint8_t> dedup_hints = {}) noexcept
{
    ...
    BB_BENCH_NAME("MSM::pippenger_round_parallel_batched");
    ...
    // group_max_n. group_uses_glv is a per-group bool but the per-MSM internal dispatch keeps
    // each MSM's own GLV decision in case shared doubling is skipped.
    ...
    for (size_t g = 0; g < glv_groups.size(); ++g) {
        // GLV decision is per-group on group_max_n. Within a group, every MSM has
    ...
    // which constructs each MSM's point span as `get_monomial_points().subspan(start_index)`
    ...
    {
        BB_BENCH_NAME("MSM::pippenger_round_parallel_batched/glv_double_points");
        ...
        if (min_base != nullptr) {
            ...
            bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
                BB_BENCH_NAME("MSM::batch_glv_double/worker");
    ...
    // Shared dynamically-sized arena for all per-MSM internal calls. Sized to the max
    // requirement across the batch so each MSM finds enough space. Single allocation
    // across the batch (vs one per MSM if we passed {} down). Freed at return.
    // dedup_active varies per MSM (gated by per-MSM hint), so the budget query must
    ...
    // Per-MSM dispatch. Each call runs the full single-MSM pipeline (its own from-Mont and
    // to-Mont, schedule, Stage 1-6b). The only batched amortisation we share is the doubled
    // SRS prefix above; the rest of the hot path runs at single-MSM cost.
    for (size_t m = 0; m < K; ++m) {
                                  ...
                                  bool dedup_hint) noexcept
{
    ...
    bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
        BB_BENCH_NAME("MSM::pu_from_montgomery/worker");
    ...
    bb::parallel_for(bb::get_num_cpus(), [&](const ThreadChunk& chunk) {
        BB_BENCH_NAME("MSM::pu_to_montgomery/worker");
        for (size_t i : chunk.range(n_used)) {
    ...
    std::span<const uint8_t> dedup_hints) noexcept
{
    ...
    // Adapt the new (single shared points span + per-MSM PolynomialSpan scalars) API to
    // the internal dispatcher, which still takes one point sub-span per MSM. Each MSM's
    ...
    if (handle_edge_cases) {
        ...
        for (size_t i = 0; i < k; ++i) {
            ...
            results[i] =
                AffineElement(pippenger<Curve>(scalar_span, point_subspans[i], handle_edge_cases, hint_for(i)));
        }
        return results;
    }
