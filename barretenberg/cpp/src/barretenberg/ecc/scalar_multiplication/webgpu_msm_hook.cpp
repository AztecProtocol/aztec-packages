    ...
    std::span<const std::string> labels) noexcept
{
    ...
    for (size_t i = 0; i < batch_size; ++i) {
        ...
        // counters where every scalar lands in one bucket) or because the GPU
        // is a wash against the batched WASM Pippenger at that specific size.
...
//
// The browser bench harness benchmarks the in-tree
