...
template <typename Curve> class MSM {
  public:
    ...
    static std::vector<ThreadWorkUnits> get_work_units(std::vector<std::span<FF>>& scalars,
    ...
    static AffineElement small_pippenger_low_memory_with_transformed_scalars(
        std::span<const FF> scalars,
        std::span<const AffineElement> points,
        std::span<const uint32_t> nonzero_scalar_indices);
    static AffineElement pippenger_low_memory_with_transformed_scalars(std::span<const FF> scalars,
                                                                       std::span<const AffineElement> points,
                                                                       std::span<const uint32_t> scalar_indices);
    static Element evaluate_small_pippenger_round(std::span<const FF> scalars,
                                                  std::span<const AffineElement> points,
                                                  std::span<const uint32_t> nonzero_scalar_indices,
    ...
    static Element evaluate_pippenger_round(std::span<const FF> scalars,
                                            std::span<const AffineElement> points,
                                            std::span<const uint32_t> scalar_indices,
                                            const size_t round_index,
