...
template <class Curve> class ScalarMultiplicationTest : public ::testing::Test {
  public:
    ...
    // ======================= Safe Mode Edge Case Tests =======================
    // These tests verify that pippenger with handle_edge_cases=true correctly
    // handles scenarios that would fail with the unsafe affine variant.
    // Tests cover sizes both below and above AFFINE_TRICK_THRESHOLD (128).

    void test_safe_mode_duplicate_points_helper(size_t num_pts)
    {
        AffineElement base_point = generators[0];
    ...
    void test_safe_mode_duplicate_points()
    {
        // Test below AFFINE_TRICK_THRESHOLD (128)
        test_safe_mode_duplicate_points_helper(50);
        // Test above AFFINE_TRICK_THRESHOLD
        test_safe_mode_duplicate_points_helper(200);
    }

    void test_safe_mode_point_and_negation_helper(size_t num_pairs)
    {
    ...
    void test_safe_mode_point_and_negation()
    {
        // Test below AFFINE_TRICK_THRESHOLD (128) - 30 pairs = 60 points
        test_safe_mode_point_and_negation_helper(30);
        // Test above AFFINE_TRICK_THRESHOLD - 100 pairs = 200 points
        test_safe_mode_point_and_negation_helper(100);
    }

    void test_safe_mode_mixed_duplicates_and_unique()
    {
    ...
    void test_safe_mode_all_same_point_different_scalars()
    {
...
TYPED_TEST(ScalarMultiplicationTest, SafeModeDuplicatePoints)
{
    this->test_safe_mode_duplicate_points();
}
TYPED_TEST(ScalarMultiplicationTest, SafeModePointAndNegation)
{
    this->test_safe_mode_point_and_negation();
}
TYPED_TEST(ScalarMultiplicationTest, SafeModeMixedDuplicatesAndUnique)
{
    this->test_safe_mode_mixed_duplicates_and_unique();
}
TYPED_TEST(ScalarMultiplicationTest, SafeModeAllSamePointDifferentScalars)
{
    this->test_safe_mode_all_same_point_different_scalars();
}
