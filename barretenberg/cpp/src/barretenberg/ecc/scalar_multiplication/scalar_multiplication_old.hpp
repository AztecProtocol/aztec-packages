...
//  * assuming the inputs are from a zero-knowledge proof. This is because the scalar multiplier will be uniformly
//  randomly *distributed, and the bucket indices are derived from the scalar multiplier.
//  *
...
//  *adding a point into a bucket takes, on average, only 400 CPU cycles, so the slowdown of managing mutex locks would
...
//  *threads, safe in the knowledge that there will be no race conditions, with one condition. A thread's starting
//  bucket *may be equal to the previous thread's end bucket, so we need to ensure that each thread works on a local
//  array of *buckets. This adds little overhead (for 2^20 points, we have 32,768 buckets. With 8 threads, the amount of
//  bucket *overlap is ~16 buckets, so we could incur 16 extra 'additions' in pippenger's bucket concatenation phase,
//  but this is *an insignificant contribution).
//  *
...
//  *complexity of pippenger is O(n / logn) point additions, and a sequence of smaller pippenger calls will have a
//  smaller
//  *`n`.
//  *
//  * This is the motivation for multi-threading the actual Pippenger algorithm. In addition, the above approach
//  performs *extremely poorly for GPUs, where the number of threads can be as high as 2^10 (for a
//  multi-scalar-multiplication of *2^20 points, this doubles the number of pippenger rounds per thread)
//  *
//  * To give concrete numbers, the difference between calling pippenger on 2^20 points, and calling pippenger 8 times
//  on *2^17 points, is 5-10%. Which means that, for 8 threads, we need to ensure that our sorting algorithm adds less
//  than 5% *to the total runtime of pippenger. Given a single cache miss per point would increase the run-time by 25%,
//  this is not *much room to work with!
//  *
//  * However, a radix sort, combined with the fact that the total number of buckets is quite small (2^16 at most),
//  seems *to be fast enough. Benchmarks indicate (i7-8650U, 8 threads) that, for 2^20 points, the total runtime is
//  <1200ms and *of that, the radix sort consumes 58ms (4.8%)
//  *
//  * One advantage of sorting by bucket order vs point order, is that a 'bucket' is 96 bytes large
//  (sizeof(g1::element), *buckets have z-coordinates). Points, on the other hand, are 64 bytes large (affine points, no
//  z-coordinate). This *makes fetching random point locations in memory more efficient than fetching random bucket
//  locations, as each point *occupies a single cache line. Using __builtin_prefetch to recover the point just before
//  it's needed, seems to improve *the runtime of pippenger by 10-20%.
//  *
