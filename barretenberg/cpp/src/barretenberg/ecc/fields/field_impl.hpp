...
template <class T> constexpr field<T> field<T>::invert() const noexcept
{
    ...
    // constexpr contexts keep Fermat (Bernstein-Yang is not constexpr).
    if (std::is_constant_evaluated()) {
        return pow(modulus_minus_two);
    }
#if defined(__SIZEOF_INT128__)
    // 256-bit moduli (secp256k1/r1) don't fit the < 2^255 BY precondition and
    // fall through to Fermat.
    if constexpr (T::modulus_3 < (1ULL << 63)) {
        constexpr uint256_t p_uint = modulus;
        constexpr uint64_t p_inv = bernstein_yang::p_inv_from_r_inv(T::r_inv);
        field non_mont = from_montgomery_form_reduced();
        uint256_t a{ non_mont.data[0], non_mont.data[1], non_mont.data[2], non_mont.data[3] };
        uint256_t inv = bernstein_yang::invert_bernsteinyang19(a, p_uint, p_inv);
