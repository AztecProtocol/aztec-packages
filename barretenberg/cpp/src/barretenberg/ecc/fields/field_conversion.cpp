...
//  * @details First, this function must take in 2 bb::fr elements because the grumpkin::fr field has a larger modulus
//  than
//  * the bb::fr field, so we choose to send 1 grumpkin::fr element to 2 bb::fr elements to maintain injectivity.
//  * For the implementation, we want to minimize the number of constraints created by the circuit form, which happens
//  to
//  * use 68 bit limbs to represent a grumpkin::fr (as a bigfield). Therefore, our mapping will split a grumpkin::fr
//  into a
//  * 136 bit chunk for the lower two bigfield limbs and the upper chunk for the upper two limbs. The upper chunk ends
//  up
//  * being 254 - 2*68 = 118 bits as a result. This is why we check that the bb::frs must be at most 136 and 118 bits
//  * respectively (to ensure no overflow). Then, we converts the two chunks to a grumpkin::fr using uint256_t
//  conversions.
//  * @param low_bits_in
//  * @param high_bits_in
//  * @return grumpkin::fr
...
//  * @details First, this function must return 2 bb::fr elements because the grumpkin::fr field has a larger modulus
//  than
//  * the bb::fr field, so we choose to send 1 grumpkin::fr element to 2 bb::fr elements to maintain injectivity.
//  * This function is the reverse of convert_from_bn254_frs(std::span<const bb::fr> fr_vec, grumpkin::fr*) by merging
//  the
//  * two pairs of limbs back into the 2 bb::fr elements. For the implementation, we want to minimize the number of
//  * constraints created by the circuit form, which happens to use 68 bit limbs to represent a grumpkin::fr (as a
//  * bigfield). Therefore, our mapping will split a grumpkin::fr into a 136 bit chunk for the lower two bigfield limbs
//  and
//  * the upper chunk for the upper two limbs. The upper chunk ends up being 254 - 2*68 = 118 bits as a result. We
//  * manipulate the value using bitwise masks and shifts to obtain our two chunks.
//  * @param input
