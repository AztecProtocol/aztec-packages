...
#undef BB_VF_YUVAL_REDUCE
    ...
    result.quad_data[1] = wasm_v128_and(
        wasm_i8x16_shuffle(tlo_10, thi_10, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4);
    result.scalar_data[2] = static_cast<uint32_t>(temp_11) & static_cast<uint32_t>(MASK29);
    result.quad_data[2] = wasm_v128_and(
        wasm_i8x16_shuffle(tlo_11, thi_11, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4);
    result.scalar_data[3] = static_cast<uint32_t>(temp_12) & static_cast<uint32_t>(MASK29);
    result.quad_data[3] = wasm_v128_and(
        wasm_i8x16_shuffle(tlo_12, thi_12, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4);
    result.scalar_data[4] = static_cast<uint32_t>(temp_13) & static_cast<uint32_t>(MASK29);
    result.quad_data[4] = wasm_v128_and(
        wasm_i8x16_shuffle(tlo_13, thi_13, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4);
    result.scalar_data[5] = static_cast<uint32_t>(temp_14) & static_cast<uint32_t>(MASK29);
    result.quad_data[5] = wasm_v128_and(
        wasm_i8x16_shuffle(tlo_14, thi_14, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4);
    result.scalar_data[6] = static_cast<uint32_t>(temp_15) & static_cast<uint32_t>(MASK29);
    result.quad_data[6] = wasm_v128_and(
        wasm_i8x16_shuffle(tlo_15, thi_15, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4);
    result.scalar_data[7] = static_cast<uint32_t>(temp_16) & static_cast<uint32_t>(MASK29);
    result.quad_data[7] = wasm_v128_and(
        wasm_i8x16_shuffle(tlo_16, thi_16, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4);
    result.scalar_data[8] = static_cast<uint32_t>(temp_17);
    result.quad_data[8] = wasm_i8x16_shuffle(tlo_17, thi_17, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27);

    return result;
}

// =====================================================================
// dot_product<K> specializations (K in {2, 3}) for Bn254FrParams.
//
// Structure:
...
// in u64 / i64x2.
//
// Like operator*, each pair's Stage 1..5 is written with interleaved scalar
// and quad statements, separated by `bb_vf_barrier_sqq` inout barriers that
// force LLVM's WASM backend to keep the two streams textually adjacent in the
// emitted WAT (so V8 TurboFan schedules them on separate INT/SIMD pipes).
// =====================================================================
...
// Stage 1..5 of the Mont-mul kernel. Reads 9 scalar + 9 quad limbs from left
// and right, produces 17 temp_m accumulators (one per Karatsuba output
// coefficient).
//
// always_inline is critical: caller (dot_product<K>) wants this body's code
// stamped out in-place K times so LLVM can SSA the outputs into register
// locals (vs spilling the returned KaratsubaPartials struct to stack).
    ...
    const VectorField<Bn254FrParams>& left, const VectorField<Bn254FrParams>& right) noexcept
{
    ...
    const v128_t ql0 = left.quad_data[0], ql1 = left.quad_data[1], ql2 = left.quad_data[2], ql3 = left.quad_data[3],
                 ql4 = left.quad_data[4], ql5 = left.quad_data[5], ql6 = left.quad_data[6], ql7 = left.quad_data[7],
                 ql8 = left.quad_data[8];
    ...
    // ============================================================
    // Stage 1: P_lo = left[0..4] * right[0..4]  (25 muls, pl0..pl8)
    // ============================================================
    uint64_t pl0 = sl0 * sri0;
    v128_t pl0_lo = wasm_u64x2_extmul_low_u32x4(ql0, qri0);
    v128_t pl0_hi = wasm_u64x2_extmul_high_u32x4(ql0, qri0);
    bb_vf_barrier_sqq(pl0, pl0_lo, pl0_hi);

    uint64_t pl1 = sl0 * sri1;
    v128_t pl1_lo = wasm_u64x2_extmul_low_u32x4(ql0, qri1);
    v128_t pl1_hi = wasm_u64x2_extmul_high_u32x4(ql0, qri1);
    bb_vf_barrier_sqq(pl1, pl1_lo, pl1_hi);
    pl1 += sl1 * sri0;
    pl1_lo = wasm_i64x2_add(pl1_lo, wasm_u64x2_extmul_low_u32x4(ql1, qri0));
    pl1_hi = wasm_i64x2_add(pl1_hi, wasm_u64x2_extmul_high_u32x4(ql1, qri0));
    bb_vf_barrier_sqq(pl1, pl1_lo, pl1_hi);
    ...
    v128_t pl2_lo = wasm_u64x2_extmul_low_u32x4(ql0, qri2);
    v128_t pl2_hi = wasm_u64x2_extmul_high_u32x4(ql0, qri2);
    bb_vf_barrier_sqq(pl2, pl2_lo, pl2_hi);
    pl2 += sl1 * sri1;
    pl2_lo = wasm_i64x2_add(pl2_lo, wasm_u64x2_extmul_low_u32x4(ql1, qri1));
    pl2_hi = wasm_i64x2_add(pl2_hi, wasm_u64x2_extmul_high_u32x4(ql1, qri1));
    bb_vf_barrier_sqq(pl2, pl2_lo, pl2_hi);
    ...
    v128_t pl3_lo = wasm_u64x2_extmul_low_u32x4(ql0, qri3);
    v128_t pl3_hi = wasm_u64x2_extmul_high_u32x4(ql0, qri3);
    bb_vf_barrier_sqq(pl3, pl3_lo, pl3_hi);
    pl3 += sl1 * sri2;
    pl3_lo = wasm_i64x2_add(pl3_lo, wasm_u64x2_extmul_low_u32x4(ql1, qri2));
    pl3_hi = wasm_i64x2_add(pl3_hi, wasm_u64x2_extmul_high_u32x4(ql1, qri2));
    bb_vf_barrier_sqq(pl3, pl3_lo, pl3_hi);
    ...
    v128_t pl4_lo = wasm_u64x2_extmul_low_u32x4(ql0, qri4);
    v128_t pl4_hi = wasm_u64x2_extmul_high_u32x4(ql0, qri4);
    bb_vf_barrier_sqq(pl4, pl4_lo, pl4_hi);
    pl4 += sl1 * sri3;
    pl4_lo = wasm_i64x2_add(pl4_lo, wasm_u64x2_extmul_low_u32x4(ql1, qri3));
    pl4_hi = wasm_i64x2_add(pl4_hi, wasm_u64x2_extmul_high_u32x4(ql1, qri3));
    bb_vf_barrier_sqq(pl4, pl4_lo, pl4_hi);
    ...
    v128_t pl5_lo = wasm_u64x2_extmul_low_u32x4(ql1, qri4);
    v128_t pl5_hi = wasm_u64x2_extmul_high_u32x4(ql1, qri4);
    bb_vf_barrier_sqq(pl5, pl5_lo, pl5_hi);
    pl5 += sl2 * sri3;
    pl5_lo = wasm_i64x2_add(pl5_lo, wasm_u64x2_extmul_low_u32x4(ql2, qri3));
    pl5_hi = wasm_i64x2_add(pl5_hi, wasm_u64x2_extmul_high_u32x4(ql2, qri3));
    bb_vf_barrier_sqq(pl5, pl5_lo, pl5_hi);
    ...
    v128_t pl6_lo = wasm_u64x2_extmul_low_u32x4(ql2, qri4);
    v128_t pl6_hi = wasm_u64x2_extmul_high_u32x4(ql2, qri4);
    bb_vf_barrier_sqq(pl6, pl6_lo, pl6_hi);
    pl6 += sl3 * sri3;
    pl6_lo = wasm_i64x2_add(pl6_lo, wasm_u64x2_extmul_low_u32x4(ql3, qri3));
    pl6_hi = wasm_i64x2_add(pl6_hi, wasm_u64x2_extmul_high_u32x4(ql3, qri3));
    bb_vf_barrier_sqq(pl6, pl6_lo, pl6_hi);
    ...
    v128_t pl7_lo = wasm_u64x2_extmul_low_u32x4(ql3, qri4);
    v128_t pl7_hi = wasm_u64x2_extmul_high_u32x4(ql3, qri4);
    bb_vf_barrier_sqq(pl7, pl7_lo, pl7_hi);
    pl7 += sl4 * sri3;
    pl7_lo = wasm_i64x2_add(pl7_lo, wasm_u64x2_extmul_low_u32x4(ql4, qri3));
    pl7_hi = wasm_i64x2_add(pl7_hi, wasm_u64x2_extmul_high_u32x4(ql4, qri3));
    bb_vf_barrier_sqq(pl7, pl7_lo, pl7_hi);
    ...
    v128_t pl8_lo = wasm_u64x2_extmul_low_u32x4(ql4, qri4);
    v128_t pl8_hi = wasm_u64x2_extmul_high_u32x4(ql4, qri4);
    bb_vf_barrier_sqq(pl8, pl8_lo, pl8_hi);

    // ============================================================
    // Stage 2: P_hi = left[5..8] * right[5..8]  (16 muls, ph0..ph6)
    // ============================================================
    uint64_t ph0 = sl5 * sri5;
    v128_t ph0_lo = wasm_u64x2_extmul_low_u32x4(ql5, qri5);
    v128_t ph0_hi = wasm_u64x2_extmul_high_u32x4(ql5, qri5);
    bb_vf_barrier_sqq(ph0, ph0_lo, ph0_hi);

    uint64_t ph1 = sl5 * sri6;
    v128_t ph1_lo = wasm_u64x2_extmul_low_u32x4(ql5, qri6);
    v128_t ph1_hi = wasm_u64x2_extmul_high_u32x4(ql5, qri6);
    bb_vf_barrier_sqq(ph1, ph1_lo, ph1_hi);
    ph1 += sl6 * sri5;
    ph1_lo = wasm_i64x2_add(ph1_lo, wasm_u64x2_extmul_low_u32x4(ql6, qri5));
    ph1_hi = wasm_i64x2_add(ph1_hi, wasm_u64x2_extmul_high_u32x4(ql6, qri5));
    bb_vf_barrier_sqq(ph1, ph1_lo, ph1_hi);
    ...
    v128_t ph2_lo = wasm_u64x2_extmul_low_u32x4(ql5, qri7);
    v128_t ph2_hi = wasm_u64x2_extmul_high_u32x4(ql5, qri7);
    bb_vf_barrier_sqq(ph2, ph2_lo, ph2_hi);
    ph2 += sl6 * sri6;
    ph2_lo = wasm_i64x2_add(ph2_lo, wasm_u64x2_extmul_low_u32x4(ql6, qri6));
    ph2_hi = wasm_i64x2_add(ph2_hi, wasm_u64x2_extmul_high_u32x4(ql6, qri6));
    bb_vf_barrier_sqq(ph2, ph2_lo, ph2_hi);
    ...
    v128_t ph3_lo = wasm_u64x2_extmul_low_u32x4(ql5, qri8);
    v128_t ph3_hi = wasm_u64x2_extmul_high_u32x4(ql5, qri8);
    bb_vf_barrier_sqq(ph3, ph3_lo, ph3_hi);
    ph3 += sl6 * sri7;
    ph3_lo = wasm_i64x2_add(ph3_lo, wasm_u64x2_extmul_low_u32x4(ql6, qri7));
    ph3_hi = wasm_i64x2_add(ph3_hi, wasm_u64x2_extmul_high_u32x4(ql6, qri7));
    bb_vf_barrier_sqq(ph3, ph3_lo, ph3_hi);
    ...
    v128_t ph4_lo = wasm_u64x2_extmul_low_u32x4(ql6, qri8);
    v128_t ph4_hi = wasm_u64x2_extmul_high_u32x4(ql6, qri8);
    bb_vf_barrier_sqq(ph4, ph4_lo, ph4_hi);
    ph4 += sl7 * sri7;
    ph4_lo = wasm_i64x2_add(ph4_lo, wasm_u64x2_extmul_low_u32x4(ql7, qri7));
    ph4_hi = wasm_i64x2_add(ph4_hi, wasm_u64x2_extmul_high_u32x4(ql7, qri7));
    bb_vf_barrier_sqq(ph4, ph4_lo, ph4_hi);
    ...
    v128_t ph5_lo = wasm_u64x2_extmul_low_u32x4(ql7, qri8);
    v128_t ph5_hi = wasm_u64x2_extmul_high_u32x4(ql7, qri8);
    bb_vf_barrier_sqq(ph5, ph5_lo, ph5_hi);
    ph5 += sl8 * sri7;
    ph5_lo = wasm_i64x2_add(ph5_lo, wasm_u64x2_extmul_low_u32x4(ql8, qri7));
    ph5_hi = wasm_i64x2_add(ph5_hi, wasm_u64x2_extmul_high_u32x4(ql8, qri7));
    bb_vf_barrier_sqq(ph5, ph5_lo, ph5_hi);
    ...
    v128_t ph6_lo = wasm_u64x2_extmul_low_u32x4(ql8, qri8);
    v128_t ph6_hi = wasm_u64x2_extmul_high_u32x4(ql8, qri8);
    bb_vf_barrier_sqq(ph6, ph6_lo, ph6_hi);

    // ============================================================
    // Stage 3: Cross sums sl_i, sr_i (i32x4 add, NOT i64x2).
    // ============================================================
    uint64_t ssl0 = sl0 + sl5;
    v128_t qsl0 = wasm_i32x4_add(ql0, ql5);
    uint64_t ssr0 = sri0 + sri5;
    v128_t qsr0 = wasm_i32x4_add(qri0, qri5);
    bb_vf_barrier_sq(ssl0, qsl0);
    bb_vf_barrier_sq(ssr0, qsr0);

    uint64_t ssl1 = sl1 + sl6;
    v128_t qsl1 = wasm_i32x4_add(ql1, ql6);
    uint64_t ssr1 = sri1 + sri6;
    v128_t qsr1 = wasm_i32x4_add(qri1, qri6);
    bb_vf_barrier_sq(ssl1, qsl1);
    bb_vf_barrier_sq(ssr1, qsr1);

    uint64_t ssl2 = sl2 + sl7;
    v128_t qsl2 = wasm_i32x4_add(ql2, ql7);
    uint64_t ssr2 = sri2 + sri7;
    v128_t qsr2 = wasm_i32x4_add(qri2, qri7);
    bb_vf_barrier_sq(ssl2, qsl2);
    bb_vf_barrier_sq(ssr2, qsr2);

    uint64_t ssl3 = sl3 + sl8;
    v128_t qsl3 = wasm_i32x4_add(ql3, ql8);
    uint64_t ssr3 = sri3 + sri8;
    v128_t qsr3 = wasm_i32x4_add(qri3, qri8);
    bb_vf_barrier_sq(ssl3, qsl3);
    bb_vf_barrier_sq(ssr3, qsr3);

    // Break the qsl4==ql4 / qsr4==qri4 alias so LLVM doesn't CSE pc8's
    // extmul pair with pl8 (see operator* for the detailed reason).
    uint64_t ssl4 = sl4;
    v128_t qsl4 = ql4;
    uint64_t ssr4 = sri4;
    v128_t qsr4 = qri4;
    bb_vf_barrier_sq(ssl4, qsl4);
    bb_vf_barrier_sq(ssr4, qsr4);

    // ============================================================
    // Stage 4: P_cross = sl * sr  (25 muls, pc0..pc8)
    // ============================================================
    uint64_t pc0 = ssl0 * ssr0;
    v128_t pc0_lo = wasm_u64x2_extmul_low_u32x4(qsl0, qsr0);
    v128_t pc0_hi = wasm_u64x2_extmul_high_u32x4(qsl0, qsr0);
    bb_vf_barrier_sqq(pc0, pc0_lo, pc0_hi);

    uint64_t pc1 = ssl0 * ssr1;
    v128_t pc1_lo = wasm_u64x2_extmul_low_u32x4(qsl0, qsr1);
    v128_t pc1_hi = wasm_u64x2_extmul_high_u32x4(qsl0, qsr1);
    bb_vf_barrier_sqq(pc1, pc1_lo, pc1_hi);
    pc1 += ssl1 * ssr0;
    pc1_lo = wasm_i64x2_add(pc1_lo, wasm_u64x2_extmul_low_u32x4(qsl1, qsr0));
    pc1_hi = wasm_i64x2_add(pc1_hi, wasm_u64x2_extmul_high_u32x4(qsl1, qsr0));
    bb_vf_barrier_sqq(pc1, pc1_lo, pc1_hi);
    ...
    v128_t pc2_lo = wasm_u64x2_extmul_low_u32x4(qsl0, qsr2);
    v128_t pc2_hi = wasm_u64x2_extmul_high_u32x4(qsl0, qsr2);
    bb_vf_barrier_sqq(pc2, pc2_lo, pc2_hi);
    pc2 += ssl1 * ssr1;
    pc2_lo = wasm_i64x2_add(pc2_lo, wasm_u64x2_extmul_low_u32x4(qsl1, qsr1));
    pc2_hi = wasm_i64x2_add(pc2_hi, wasm_u64x2_extmul_high_u32x4(qsl1, qsr1));
    bb_vf_barrier_sqq(pc2, pc2_lo, pc2_hi);
    ...
    v128_t pc3_lo = wasm_u64x2_extmul_low_u32x4(qsl0, qsr3);
    v128_t pc3_hi = wasm_u64x2_extmul_high_u32x4(qsl0, qsr3);
    bb_vf_barrier_sqq(pc3, pc3_lo, pc3_hi);
    pc3 += ssl1 * ssr2;
    pc3_lo = wasm_i64x2_add(pc3_lo, wasm_u64x2_extmul_low_u32x4(qsl1, qsr2));
    pc3_hi = wasm_i64x2_add(pc3_hi, wasm_u64x2_extmul_high_u32x4(qsl1, qsr2));
    bb_vf_barrier_sqq(pc3, pc3_lo, pc3_hi);
    ...
    v128_t pc4_lo = wasm_u64x2_extmul_low_u32x4(qsl0, qsr4);
    v128_t pc4_hi = wasm_u64x2_extmul_high_u32x4(qsl0, qsr4);
    bb_vf_barrier_sqq(pc4, pc4_lo, pc4_hi);
    pc4 += ssl1 * ssr3;
    pc4_lo = wasm_i64x2_add(pc4_lo, wasm_u64x2_extmul_low_u32x4(qsl1, qsr3));
    pc4_hi = wasm_i64x2_add(pc4_hi, wasm_u64x2_extmul_high_u32x4(qsl1, qsr3));
    bb_vf_barrier_sqq(pc4, pc4_lo, pc4_hi);
    ...
    v128_t pc5_lo = wasm_u64x2_extmul_low_u32x4(qsl1, qsr4);
    v128_t pc5_hi = wasm_u64x2_extmul_high_u32x4(qsl1, qsr4);
    bb_vf_barrier_sqq(pc5, pc5_lo, pc5_hi);
    pc5 += ssl2 * ssr3;
    pc5_lo = wasm_i64x2_add(pc5_lo, wasm_u64x2_extmul_low_u32x4(qsl2, qsr3));
    pc5_hi = wasm_i64x2_add(pc5_hi, wasm_u64x2_extmul_high_u32x4(qsl2, qsr3));
    bb_vf_barrier_sqq(pc5, pc5_lo, pc5_hi);
    ...
    v128_t pc6_lo = wasm_u64x2_extmul_low_u32x4(qsl2, qsr4);
    v128_t pc6_hi = wasm_u64x2_extmul_high_u32x4(qsl2, qsr4);
    bb_vf_barrier_sqq(pc6, pc6_lo, pc6_hi);
    pc6 += ssl3 * ssr3;
    pc6_lo = wasm_i64x2_add(pc6_lo, wasm_u64x2_extmul_low_u32x4(qsl3, qsr3));
    pc6_hi = wasm_i64x2_add(pc6_hi, wasm_u64x2_extmul_high_u32x4(qsl3, qsr3));
    bb_vf_barrier_sqq(pc6, pc6_lo, pc6_hi);
    ...
    v128_t pc7_lo = wasm_u64x2_extmul_low_u32x4(qsl3, qsr4);
    v128_t pc7_hi = wasm_u64x2_extmul_high_u32x4(qsl3, qsr4);
    bb_vf_barrier_sqq(pc7, pc7_lo, pc7_hi);
    pc7 += ssl4 * ssr3;
    pc7_lo = wasm_i64x2_add(pc7_lo, wasm_u64x2_extmul_low_u32x4(qsl4, qsr3));
    pc7_hi = wasm_i64x2_add(pc7_hi, wasm_u64x2_extmul_high_u32x4(qsl4, qsr3));
    bb_vf_barrier_sqq(pc7, pc7_lo, pc7_hi);
    ...
    v128_t pc8_lo = wasm_u64x2_extmul_low_u32x4(qsl4, qsr4);
    v128_t pc8_hi = wasm_u64x2_extmul_high_u32x4(qsl4, qsr4);
    bb_vf_barrier_sqq(pc8, pc8_lo, pc8_hi);

    // ============================================================
    // Stage 5: Combine into temp_0..temp_16. See operator* for the formula.
    // ============================================================
...
// In-place accumulate variant of karatsuba_stage_1_5_bn254fr: computes the
// same 17 Stage-5 expressions as `karatsuba_stage_1_5_bn254fr` but writes the
// final result as `temp[m] += ...` (and paired tlo/thi i64x2 adds) so that
// each intermediate pl/ph/pc can be issued and then consumed immediately into
// the accumulator, rather than all 66 partial-products surviving past Stage 5
// into a returned 51-value aggregate.
//
// The reduction in live-range is what lets K=3..6 specializations avoid V8's
// i64x2 regpool spill. (With the return-by-struct variant, LLVM has to keep
// 17 scalar + 34 v128 outputs live across the function boundary before the
// caller consumes them; here the outputs are folded into the caller's
// accumulator as each is produced.)
//
// Caller supplies three 17-element output arrays (references). Each slot must
// already hold a valid running sum (the K=N caller initializes from
// karatsuba_stage_1_5_bn254fr's return for pair 0, then calls this helper for
// pairs 1..N-1).
...
// reference (via named locals `temp_m`, `tlo_m`, `thi_m` in the caller), runs
// 8 Yuval reductions, 1 wasm_reduce, carry propagation, and stores into
// `result`. Shared between dot_product<K=2>, dot_product<K=3>, etc.
//
// This is structurally identical to operator*'s Stages 6..10.
//
// The caller must define named locals `temp_0..temp_16`, `tlo_0..tlo_16`,
// `thi_0..thi_16` before expansion, and a `VectorField<Bn254FrParams> result`
// that this macro populates.
#define BB_VF_RUN_STAGES_6_THROUGH_10()                                                                                \
    do {                                                                                                               \
        ...
        {                                                                                                              \
            ...
            tlo_8 = wasm_i64x2_add(tlo_8, wasm_u64x2_extmul_low_u32x4(rk_q, p0_splat));                                \
            thi_8 = wasm_i64x2_add(thi_8, wasm_u64x2_extmul_high_u32x4(rk_q, p0_splat));                               \
            bb_vf_barrier_sqq(temp_8, tlo_8, thi_8);                                                                   \
                                                                                                                       \
            temp_9 += rk_s * P_WASM[1] + (temp_8 >> 29);                                                               \
            tlo_9 = wasm_i64x2_add(wasm_i64x2_add(tlo_9, wasm_u64x2_extmul_low_u32x4(rk_q, p1_splat)),                 \
                                   wasm_u64x2_shr(tlo_8, 29));                                                         \
            thi_9 = wasm_i64x2_add(wasm_i64x2_add(thi_9, wasm_u64x2_extmul_high_u32x4(rk_q, p1_splat)),                \
                                   wasm_u64x2_shr(thi_8, 29));                                                         \
            bb_vf_barrier_sqq(temp_9, tlo_9, thi_9);                                                                   \
                                                                                                                       \
            temp_10 += rk_s * P_WASM[2];                                                                               \
            tlo_10 = wasm_i64x2_add(tlo_10, wasm_u64x2_extmul_low_u32x4(rk_q, p2_splat));                              \
            thi_10 = wasm_i64x2_add(thi_10, wasm_u64x2_extmul_high_u32x4(rk_q, p2_splat));                             \
            bb_vf_barrier_sqq(temp_10, tlo_10, thi_10);                                                                \
                                                                                                                       \
            temp_11 += rk_s * P_WASM[3];                                                                               \
            tlo_11 = wasm_i64x2_add(tlo_11, wasm_u64x2_extmul_low_u32x4(rk_q, p3_splat));                              \
            thi_11 = wasm_i64x2_add(thi_11, wasm_u64x2_extmul_high_u32x4(rk_q, p3_splat));                             \
            bb_vf_barrier_sqq(temp_11, tlo_11, thi_11);                                                                \
                                                                                                                       \
            temp_12 += rk_s * P_WASM[4];                                                                               \
            tlo_12 = wasm_i64x2_add(tlo_12, wasm_u64x2_extmul_low_u32x4(rk_q, p4_splat));                              \
            thi_12 = wasm_i64x2_add(thi_12, wasm_u64x2_extmul_high_u32x4(rk_q, p4_splat));                             \
            bb_vf_barrier_sqq(temp_12, tlo_12, thi_12);                                                                \
                                                                                                                       \
            temp_13 += rk_s * P_WASM[5];                                                                               \
            tlo_13 = wasm_i64x2_add(tlo_13, wasm_u64x2_extmul_low_u32x4(rk_q, p5_splat));                              \
            thi_13 = wasm_i64x2_add(thi_13, wasm_u64x2_extmul_high_u32x4(rk_q, p5_splat));                             \
            bb_vf_barrier_sqq(temp_13, tlo_13, thi_13);                                                                \
                                                                                                                       \
            temp_14 += rk_s * P_WASM[6];                                                                               \
            tlo_14 = wasm_i64x2_add(tlo_14, wasm_u64x2_extmul_low_u32x4(rk_q, p6_splat));                              \
            thi_14 = wasm_i64x2_add(thi_14, wasm_u64x2_extmul_high_u32x4(rk_q, p6_splat));                             \
            bb_vf_barrier_sqq(temp_14, tlo_14, thi_14);                                                                \
                                                                                                                       \
            temp_15 += rk_s * P_WASM[7];                                                                               \
            tlo_15 = wasm_i64x2_add(tlo_15, wasm_u64x2_extmul_low_u32x4(rk_q, p7_splat));                              \
            thi_15 = wasm_i64x2_add(thi_15, wasm_u64x2_extmul_high_u32x4(rk_q, p7_splat));                             \
            bb_vf_barrier_sqq(temp_15, tlo_15, thi_15);                                                                \
                                                                                                                       \
            temp_16 += rk_s * P_WASM[8];                                                                               \
            tlo_16 = wasm_i64x2_add(tlo_16, wasm_u64x2_extmul_low_u32x4(rk_q, p8_splat));                              \
            thi_16 = wasm_i64x2_add(thi_16, wasm_u64x2_extmul_high_u32x4(rk_q, p8_splat));                             \
...
// Single Yuval reduction step in macro form. `lo`, `p1..p9` are the
// temp/tlo/thi indices. `p1 == lo+1`, `p2 == lo+2`, ..., `p9 == lo+9`, but
// passed explicitly so the macro token-pastes cleanly.
#define BB_VF_DP_YUVAL(lo, p1, p2, p3, p4, p5, p6, p7, p8, p9)                                                         \
    {                                                                                                                  \
        ...
        bb_vf_barrier_sqq(temp_##p1, tlo_##p1, thi_##p1);                                                              \
        asm volatile("" : "+r"(km_q));                                                                                 \
        ...
        bb_vf_barrier_sqq(temp_##p3, tlo_##p3, thi_##p3);                                                              \
        asm volatile("" : "+r"(km_q));                                                                                 \
        ...
        bb_vf_barrier_sqq(temp_##p5, tlo_##p5, thi_##p5);                                                              \
        asm volatile("" : "+r"(km_q));                                                                                 \
        ...
        bb_vf_barrier_sqq(temp_##p7, tlo_##p7, thi_##p7);                                                              \
        asm volatile("" : "+r"(km_q));                                                                                 \
        ...
        bb_vf_barrier_sqq(temp_##p9, tlo_##p9, thi_##p9);                                                              \
    }
...
#define BB_VF_RUN_STAGES_6_THROUGH_10_WITH_M_LINEARS(LINEARS_ARR, M_COUNT, N_SUB)                                      \
    do {                                                                                                               \
        ...
        thi_16 = wasm_i64x2_add(thi_16, wasm_u64x2_shr(thi_15, 29));                                                   \
                                                                                                                       \
        const uint64_t temp_17 = temp_17_linear_acc + (temp_16 >> 29);                                                 \
        const v128_t tlo_17 = wasm_i64x2_add(tlo_17_linear_acc, wasm_u64x2_shr(tlo_16, 29));                           \
        const v128_t thi_17 = wasm_i64x2_add(thi_17_linear_acc, wasm_u64x2_shr(thi_16, 29));                           \
                                                                                                                       \
        /* --- Stage 9/10: Store output (9 limbs), then run N_SUB conditional */                                       \
        /* subtract-p steps to restore coarse form. */                                                                 \
        result.scalar_data[0] = static_cast<uint32_t>(temp_9) & static_cast<uint32_t>(MASK29);                         \
        result.quad_data[0] = wasm_v128_and(                                                                           \
            wasm_i8x16_shuffle(tlo_9, thi_9, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27), mask29_i32x4); \
        result.scalar_data[1] = static_cast<uint32_t>(temp_10) & static_cast<uint32_t>(MASK29);                        \
        ...
        result.quad_data[7] = wasm_v128_and(                                                                           \
            wasm_i8x16_shuffle(tlo_16, thi_16, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27),              \
            mask29_i32x4);                                                                                             \
        result.scalar_data[8] = static_cast<uint32_t>(temp_17) & static_cast<uint32_t>(MASK29);                        \
        result.quad_data[8] = wasm_v128_and(                                                                           \
            wasm_i8x16_shuffle(tlo_17, thi_17, 0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27),              \
            mask29_i32x4);                                                                                             \
                                                                                                                       \
        /* Binary-search normalization: R rounds of conditional sub (2^k · p), k */                                    \
        /* descending from R-1 to 0. R = dp_norm_rounds(K_COUNT, M_COUNT). Each */                                     \
        /* round halves the coarse-form overshoot, so total tail cost is O(log Q) */                                   \
        /* where Q = ceil(2M + K/32 - 1). Unrolled in one pass on locals so the */                                     \
        /* compiler can keep result's limbs in registers across rounds. */                                             \
        bb_vf_binary_search_normalize<(N_SUB)>(result);                                                                \
    } while (0)

// One round of conditional `x -= CONST` if x >= CONST on 9-limb scalar + paired
// i32x4 quad locals. Matches operator-'s op-by-op scalar/quad interleaving and
// barrier pattern so V8 schedules the two streams on separate INT / SIMD pipes.
//
// Inputs are by ref to uint32_t[9] scalar + v128_t[9] quad arrays. Writes the
// sub result back in-place. CONST is a reference to a constexpr std::array of
// 9 × 29-bit limbs (e.g. P_WASM, 2·P_WASM, 4·P_WASM, 8·P_WASM).
//
// Structure per limb (strict source order; clang's WASM backend preserves it):
//   scalar op  ->  quad op  ->  scalar op  ->  quad op  ->  bb_vf_barrier_sqq
// Each limb issues 4 ops (2 scalar + 2 quad) in interleaved order, followed by
// a joint barrier forcing LLVM to keep them textually adjacent in the WAT.
[[gnu::always_inline]] static inline void bb_vf_cond_sub_limb_arrays(uint32_t (&sd)[9],
                                                                     v128_t (&qd)[9],
                                                                     const std::array<uint64_t, 9>& P) noexcept
{
    constexpr uint64_t MASK = 0x1fffffffULL;
    const v128_t mask_splat = wasm_i32x4_splat(static_cast<int32_t>(MASK));

    // Upfront load: read each limb's scalar + quad into a named local ONCE so
    // subsequent uses don't re-read from memory. Interleaved to keep V8's
    // scheduler seeing both streams.
    const uint64_t s0 = sd[0];
    const v128_t q0 = qd[0];
    const uint64_t s1 = sd[1];
    const v128_t q1 = qd[1];
    const uint64_t s2 = sd[2];
    const v128_t q2 = qd[2];
    const uint64_t s3 = sd[3];
    const v128_t q3 = qd[3];
    const uint64_t s4 = sd[4];
    const v128_t q4 = qd[4];
    const uint64_t s5 = sd[5];
    const v128_t q5 = qd[5];
    const uint64_t s6 = sd[6];
    const v128_t q6 = qd[6];
    const uint64_t s7 = sd[7];
    const v128_t q7 = qd[7];
    const uint64_t s8 = sd[8];
    const v128_t q8 = qd[8];

    // Splats of CONST limbs, loaded upfront with "+r" inout barriers so LLVM
    // doesn't CSE or hoist them (same trick as the R_INV splats in Stage 6).
    v128_t cp0 = wasm_i32x4_splat(static_cast<int32_t>(P[0]));
    v128_t cp1 = wasm_i32x4_splat(static_cast<int32_t>(P[1]));
    v128_t cp2 = wasm_i32x4_splat(static_cast<int32_t>(P[2]));
    v128_t cp3 = wasm_i32x4_splat(static_cast<int32_t>(P[3]));
    v128_t cp4 = wasm_i32x4_splat(static_cast<int32_t>(P[4]));
    v128_t cp5 = wasm_i32x4_splat(static_cast<int32_t>(P[5]));
    v128_t cp6 = wasm_i32x4_splat(static_cast<int32_t>(P[6]));
    v128_t cp7 = wasm_i32x4_splat(static_cast<int32_t>(P[7]));
    v128_t cp8 = wasm_i32x4_splat(static_cast<int32_t>(P[8]));
    asm volatile("" : "+r"(cp0), "+r"(cp1), "+r"(cp2), "+r"(cp3), "+r"(cp4));
    asm volatile("" : "+r"(cp5), "+r"(cp6), "+r"(cp7), "+r"(cp8));

    // Limb 0.
    int64_t sdiff0 = static_cast<int64_t>(s0) - static_cast<int64_t>(P[0]);
    v128_t qdiff0 = wasm_i32x4_sub(q0, cp0);
    ...
    bb_vf_barrier_sqq(sr0, qr0, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 1.
    int64_t sdiff1 = static_cast<int64_t>(s1) - static_cast<int64_t>(P[1]) - sborrow;
    v128_t qdiff1 = wasm_i32x4_sub(wasm_i32x4_sub(q1, cp1), qborrow);
    ...
    bb_vf_barrier_sqq(sr1, qr1, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 2.
    int64_t sdiff2 = static_cast<int64_t>(s2) - static_cast<int64_t>(P[2]) - sborrow;
    v128_t qdiff2 = wasm_i32x4_sub(wasm_i32x4_sub(q2, cp2), qborrow);
    ...
    bb_vf_barrier_sqq(sr2, qr2, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 3.
    int64_t sdiff3 = static_cast<int64_t>(s3) - static_cast<int64_t>(P[3]) - sborrow;
    v128_t qdiff3 = wasm_i32x4_sub(wasm_i32x4_sub(q3, cp3), qborrow);
    ...
    bb_vf_barrier_sqq(sr3, qr3, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 4.
    int64_t sdiff4 = static_cast<int64_t>(s4) - static_cast<int64_t>(P[4]) - sborrow;
    v128_t qdiff4 = wasm_i32x4_sub(wasm_i32x4_sub(q4, cp4), qborrow);
    ...
    bb_vf_barrier_sqq(sr4, qr4, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 5.
    int64_t sdiff5 = static_cast<int64_t>(s5) - static_cast<int64_t>(P[5]) - sborrow;
    v128_t qdiff5 = wasm_i32x4_sub(wasm_i32x4_sub(q5, cp5), qborrow);
    ...
    bb_vf_barrier_sqq(sr5, qr5, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 6.
    int64_t sdiff6 = static_cast<int64_t>(s6) - static_cast<int64_t>(P[6]) - sborrow;
    v128_t qdiff6 = wasm_i32x4_sub(wasm_i32x4_sub(q6, cp6), qborrow);
    ...
    bb_vf_barrier_sqq(sr6, qr6, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 7.
    int64_t sdiff7 = static_cast<int64_t>(s7) - static_cast<int64_t>(P[7]) - sborrow;
    v128_t qdiff7 = wasm_i32x4_sub(wasm_i32x4_sub(q7, cp7), qborrow);
    ...
    bb_vf_barrier_sqq(sr7, qr7, qborrow);
    asm volatile("" : "+r"(sborrow));

    // Limb 8.
    int64_t sdiff8 = static_cast<int64_t>(s8) - static_cast<int64_t>(P[8]) - sborrow;
    v128_t qdiff8 = wasm_i32x4_sub(wasm_i32x4_sub(q8, cp8), qborrow);
    ...
    const v128_t q_final_borrow_i32 = wasm_u32x4_shr(qdiff8, 31);
    ...
    // Blend, writing back to the arrays. Interleaved scalar/quad.
    sd[0] = static_cast<uint32_t>((sr0 & simask) | (s0 & smask));
    qd[0] = wasm_v128_bitselect(q0, qr0, q_final_borrow_mask);
    sd[1] = static_cast<uint32_t>((sr1 & simask) | (s1 & smask));
    qd[1] = wasm_v128_bitselect(q1, qr1, q_final_borrow_mask);
    sd[2] = static_cast<uint32_t>((sr2 & simask) | (s2 & smask));
    qd[2] = wasm_v128_bitselect(q2, qr2, q_final_borrow_mask);
    sd[3] = static_cast<uint32_t>((sr3 & simask) | (s3 & smask));
    qd[3] = wasm_v128_bitselect(q3, qr3, q_final_borrow_mask);
    sd[4] = static_cast<uint32_t>((sr4 & simask) | (s4 & smask));
    qd[4] = wasm_v128_bitselect(q4, qr4, q_final_borrow_mask);
    sd[5] = static_cast<uint32_t>((sr5 & simask) | (s5 & smask));
    qd[5] = wasm_v128_bitselect(q5, qr5, q_final_borrow_mask);
    sd[6] = static_cast<uint32_t>((sr6 & simask) | (s6 & smask));
    qd[6] = wasm_v128_bitselect(q6, qr6, q_final_borrow_mask);
    sd[7] = static_cast<uint32_t>((sr7 & simask) | (s7 & smask));
    qd[7] = wasm_v128_bitselect(q7, qr7, q_final_borrow_mask);
    sd[8] = static_cast<uint32_t>((sr8 & simask) | (s8 & smask));
    qd[8] = wasm_v128_bitselect(q8, qr8, q_final_borrow_mask);
}

// Binary-search normalization: R rounds of `x -= (2^k · p) if x >= (2^k · p)`,
// k descending from R-1 down to 0. R is a template parameter so the compiler
// unrolls the full sequence, picking only the rounds actually needed.
template <size_t R>
[[gnu::always_inline]] static inline void bb_vf_binary_search_normalize(VectorField<Bn254FrParams>& result) noexcept
{
    if constexpr (R >= 4) {
        bb_vf_cond_sub_limb_arrays(reinterpret_cast<uint32_t (&)[9]>(result.scalar_data),
                                   reinterpret_cast<v128_t(&)[9]>(result.quad_data),
                                   VectorField<Bn254FrParams>::P8_WASM);
    }
    if constexpr (R >= 3) {
        bb_vf_cond_sub_limb_arrays(reinterpret_cast<uint32_t (&)[9]>(result.scalar_data),
                                   reinterpret_cast<v128_t(&)[9]>(result.quad_data),
                                   VectorField<Bn254FrParams>::P4_WASM);
    }
    if constexpr (R >= 2) {
        bb_vf_cond_sub_limb_arrays(reinterpret_cast<uint32_t (&)[9]>(result.scalar_data),
                                   reinterpret_cast<v128_t(&)[9]>(result.quad_data),
                                   VectorField<Bn254FrParams>::P2_WASM);
    }
    if constexpr (R >= 1) {
        bb_vf_cond_sub_limb_arrays(reinterpret_cast<uint32_t (&)[9]>(result.scalar_data),
                                   reinterpret_cast<v128_t(&)[9]>(result.quad_data),
                                   VectorField<Bn254FrParams>::P1_WASM);
    }
}
...
// output (as returned by karatsuba_stage_1_5_bn254fr). Writes into the
// caller's local `uint64_t temp[17]`, `v128_t tlo[17]`, `v128_t thi[17]`.
...
// Macro: copy 17-element array accumulators into named locals temp_0..temp_16
// (and similarly tlo_m, thi_m) right before invoking
// BB_VF_RUN_STAGES_6_THROUGH_10, which references those named locals by
// token-pasting (temp_##p1, tlo_##lo, etc.). The copy is free in SSA: LLVM
// with always_inline treats temp[5] and temp_5 as the same value.
...
// --------------- dot_product<K=2> ---------------
// Pair 0: seed accumulators from Stage 1..5 output.
// Pair 1: in-place += via karatsuba_stage_1_5_bn254fr_accumulate (pl/ph/pc are
//         consumed into temp[m] as each is produced, so only 17 temp slots +
//         the currently-computing partial are live at once).
// Stages 6..10: load arrays into named locals and run shared macro.
template <>
template <>
...
// =====================================================================
// dot_product<K, M> specializations. Preserves the K-only overload; adds
// K + M <= 6, K >= 1, M >= 1 combinations. For K >= 1, N_SUB = 2 * M
// conditional subtract-p steps are required at the tail (see header bound
// analysis).
// =====================================================================

// (K=1, M=0) delegates to the K-only overload (operator*).
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<1, 0>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 1>& pairs,
    const std::array<VectorField<Bn254FrParams>, 0>& /* linears */) noexcept
{
    return dot_product<1>(pairs);
}

// Shared macro body for dot_product<K, M> with M >= 1. Expects the caller to
// have declared `uint64_t temp[17]`, `v128_t tlo[17]`, `v128_t thi[17]`,
// already seeded from Stage 1..5 for pairs[0] and accumulated for pairs[1..K-1].
#define BB_VF_DP_KM_TAIL(LINEARS_ARR, M_COUNT, N_SUB)                                                                  \
    do {                                                                                                               \
        BB_VF_DP_LOAD_NAMED_FROM_ARRAYS();                                                                             \
        BB_VF_RUN_STAGES_6_THROUGH_10_WITH_M_LINEARS((LINEARS_ARR), (M_COUNT), (N_SUB));                               \
    } while (0)

// --------------- dot_product<K=1, M=1..5> ---------------
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<1, 1>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 1>& pairs,
    const std::array<VectorField<Bn254FrParams>, 1>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
    uint64_t temp[17];
    ...
    v128_t thi[17];
    BB_VF_DP_SEED_FROM_PAIR0(pairs[0].first, pairs[0].second);
    BB_VF_DP_KM_TAIL(linears, 1, 2);
    return result;
}

template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<1, 2>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 1>& pairs,
    const std::array<VectorField<Bn254FrParams>, 2>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
    uint64_t temp[17];
    ...
    v128_t thi[17];
    BB_VF_DP_SEED_FROM_PAIR0(pairs[0].first, pairs[0].second);
    BB_VF_DP_KM_TAIL(linears, 2, 3);
    return result;
}

template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<1, 3>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 1>& pairs,
    const std::array<VectorField<Bn254FrParams>, 3>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
    uint64_t temp[17];
    ...
    v128_t thi[17];
    BB_VF_DP_SEED_FROM_PAIR0(pairs[0].first, pairs[0].second);
    BB_VF_DP_KM_TAIL(linears, 3, 3);
    return result;
}

template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<1, 4>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 1>& pairs,
    const std::array<VectorField<Bn254FrParams>, 4>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
    uint64_t temp[17];
    ...
    v128_t thi[17];
    BB_VF_DP_SEED_FROM_PAIR0(pairs[0].first, pairs[0].second);
    BB_VF_DP_KM_TAIL(linears, 4, 4);
    return result;
}

template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<1, 5>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 1>& pairs,
    const std::array<VectorField<Bn254FrParams>, 5>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
    uint64_t temp[17];
    ...
    v128_t thi[17];
    BB_VF_DP_SEED_FROM_PAIR0(pairs[0].first, pairs[0].second);
    BB_VF_DP_KM_TAIL(linears, 5, 4);
    return result;
}

// --------------- dot_product<K=2, M=1..4> ---------------
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<2, 1>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 2>& pairs,
    const std::array<VectorField<Bn254FrParams>, 1>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
...
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<2, 2>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 2>& pairs,
    const std::array<VectorField<Bn254FrParams>, 2>& linears) noexcept
{
    using namespace vector_field_detail;
...
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<2, 3>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 2>& pairs,
    const std::array<VectorField<Bn254FrParams>, 3>& linears) noexcept
{
    using namespace vector_field_detail;
...
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<2, 4>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 2>& pairs,
    const std::array<VectorField<Bn254FrParams>, 4>& linears) noexcept
{
    using namespace vector_field_detail;
...
// --------------- dot_product<K=3, M=1..3> ---------------
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<3, 1>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 3>& pairs,
    const std::array<VectorField<Bn254FrParams>, 1>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
...
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<3, 2>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 3>& pairs,
    const std::array<VectorField<Bn254FrParams>, 2>& linears) noexcept
{
    using namespace vector_field_detail;
...
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<3, 3>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 3>& pairs,
    const std::array<VectorField<Bn254FrParams>, 3>& linears) noexcept
{
    using namespace vector_field_detail;
...
// --------------- dot_product<K=4, M=1..2> ---------------
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<4, 1>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 4>& pairs,
    const std::array<VectorField<Bn254FrParams>, 1>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
...
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<4, 2>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 4>& pairs,
    const std::array<VectorField<Bn254FrParams>, 2>& linears) noexcept
{
    using namespace vector_field_detail;
...
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<5, 1>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 5>& pairs,
    const std::array<VectorField<Bn254FrParams>, 1>& linears) noexcept
{
    using namespace vector_field_detail;
    VectorField result;
...
// Also instantiate dot_product<K, 0> for K in {2,3,4,5,6} as aliases for the
// K-only overload. This keeps call sites that always pass a linears array
// (even empty) consistent.
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<2, 0>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 2>& pairs,
    const std::array<VectorField<Bn254FrParams>, 0>& /* linears */) noexcept
{
    return dot_product<2>(pairs);
}
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<3, 0>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 3>& pairs,
    const std::array<VectorField<Bn254FrParams>, 0>& /* linears */) noexcept
{
    return dot_product<3>(pairs);
}
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<4, 0>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 4>& pairs,
    const std::array<VectorField<Bn254FrParams>, 0>& /* linears */) noexcept
{
    return dot_product<4>(pairs);
}
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<5, 0>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 5>& pairs,
    const std::array<VectorField<Bn254FrParams>, 0>& /* linears */) noexcept
{
    return dot_product<5>(pairs);
}
template <>
template <>
VectorField<Bn254FrParams> VectorField<Bn254FrParams>::dot_product<6, 0>(
    const std::array<std::pair<VectorField<Bn254FrParams>, VectorField<Bn254FrParams>>, 6>& pairs,
    const std::array<VectorField<Bn254FrParams>, 0>& /* linears */) noexcept
{
    return dot_product<6>(pairs);
}

#undef BB_VF_DP_KM_TAIL
#undef BB_VF_RUN_STAGES_6_THROUGH_10
#undef BB_VF_RUN_STAGES_6_THROUGH_10_WITH_M_LINEARS
