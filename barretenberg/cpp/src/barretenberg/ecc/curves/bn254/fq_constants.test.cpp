...
using namespace bb;
// Tests for the constants in BN-254. These tests are be default disabled; to run, use: `./ecc_tests
// --gtest_also_run_disabled_tests` with the desired filter.
namespace {
uint256_t native_q{
...
{
    // uint256_t from a decimal expansion
    auto from_decimal = [](const std::string& dec_str) {
        uint256_t result = 0;
        for (char c : dec_str) {
...
{
    // beta should be g^(2(q-1)/3) where g is the multiplicative generator
