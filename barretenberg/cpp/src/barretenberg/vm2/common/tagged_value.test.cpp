...
TEST(TaggedValueTest, ErrorCases)
{
    ...
    EXPECT_THROW(TaggedValue::from<uint1_t>(1) / TaggedValue::from<uint1_t>(0), std::runtime_error);
    EXPECT_THROW(u8_val1 / TaggedValue::from<uint8_t>(0), std::runtime_error);
    EXPECT_THROW(u16_val / TaggedValue::from<uint16_t>(0), std::runtime_error);
    EXPECT_THROW(TaggedValue::from<uint32_t>(1) / TaggedValue::from<uint32_t>(0), std::runtime_error);
    EXPECT_THROW(TaggedValue::from<uint64_t>(1) / TaggedValue::from<uint64_t>(0), std::runtime_error);
    EXPECT_THROW(TaggedValue::from<uint128_t>(1) / TaggedValue::from<uint128_t>(0), std::runtime_error);
    EXPECT_THROW(ff_val1 / TaggedValue::from<FF>(0), std::runtime_error);
}
