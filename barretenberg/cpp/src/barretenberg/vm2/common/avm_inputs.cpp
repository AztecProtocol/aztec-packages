...
void set_public_logs_in_cols(const FlatPublicLogs& logs, std::vector<std::vector<FF>>& cols, size_t array_start_row_idx)
{
    cols[0][array_start_row_idx] = logs.length;
    for (size_t i = 0; i < logs.length; ++i) {
        cols[0][array_start_row_idx + i + PUBLIC_LOGS_HEADER_LENGTH] = logs.payload[i];
    }
}
