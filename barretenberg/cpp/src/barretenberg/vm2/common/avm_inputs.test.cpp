...
TEST(AvmInputsTest, ValuesInColumns)
{
    ...
    FlatPublicLogs flat_public_logs = pi.accumulatedData.publicLogs.flatten();
    EXPECT_EQ(flat[col0_offset + public_logs_row], flat_public_logs.length);
    for (size_t j = 0; j < 3; ++j) {
        EXPECT_EQ(flat[col0_offset + public_logs_row + PUBLIC_LOGS_HEADER_LENGTH + j], flat_public_logs.payload[j]);
    }
