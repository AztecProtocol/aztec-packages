...
TEST(EmitUnencryptedLogConstrainingTest, NegativeGhostRowInjectionBlocked)
{
    ...
    // The fix: is_write_memory_value * (1 - sel) = 0 should cause the relation check to fail
    // because is_write_memory_value=1 and sel=0 violates this constraint
    EXPECT_THROW_WITH_MESSAGE(check_relation<emit_unencrypted_log>(trace), "IS_WRITE_MEMORY_VALUE_REQUIRES_SEL");
}
