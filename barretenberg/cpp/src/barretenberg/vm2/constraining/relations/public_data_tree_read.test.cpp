...
TEST_P(PublicDataReadInteractionsTests, PositiveWithInteractions)
{
    ...
    check_interaction<lookup_low_leaf_poseidon2_0>(trace);
    check_interaction<lookup_low_leaf_poseidon2_1>(trace);
    check_interaction<lookup_low_leaf_membership>(trace);
    // It'll complain about inverses being unset if no lookup actually happens
    if (param.low_leaf.value.slot != param.leaf_slot) {
        check_interaction<lookup_low_leaf_slot_validation>(trace);
        check_interaction<lookup_low_leaf_next_slot_validation>(trace);
    }
