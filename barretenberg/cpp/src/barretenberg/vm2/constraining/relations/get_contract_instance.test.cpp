...
TEST(GetContractInstanceConstrainingTest, NegativeGhostRowInjectionBlocked)
{
    ...
    // The fix: is_valid_member_enum * (1 - sel) = 0 should cause the relation check to fail
    EXPECT_THROW_WITH_MESSAGE(check_relation<get_contract_instance>(trace), "IS_VALID_MEMBER_ENUM_REQUIRES_SEL");
}
