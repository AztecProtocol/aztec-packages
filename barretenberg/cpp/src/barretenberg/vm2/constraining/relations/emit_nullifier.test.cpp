...
TEST(EmitNullifierConstrainingTest, NegativeInvalidShouldDoFlag1)
{
    TestTraceContainer trace({
        { { C::execution_sel, 1 },
          { C::execution_sel_emit_nullifier, 1 },
          { C::execution_should_execute_opcode, 1 },
          { C::execution_sel_should_emit_nullifier, 0 } }, // Should be 1 but set to 0
    });
    EXPECT_THROW_WITH_MESSAGE(check_relation<emit_nullifier>(trace, emit_nullifier::SR_SHOULD_EMIT_NULLIFIER),
...
TEST(EmitNullifierConstrainingTest, NegativeInvalidShouldDoFlag2)
{
    TestTraceContainer trace({
        { { C::execution_sel, 1 },
          ...
          { C::execution_sel_should_emit_nullifier, 1 } }, // Should be 0 but set to 1
    });
    EXPECT_THROW_WITH_MESSAGE(check_relation<emit_nullifier>(trace, emit_nullifier::SR_SHOULD_EMIT_NULLIFIER),
                              "SHOULD_EMIT_NULLIFIER");
}
