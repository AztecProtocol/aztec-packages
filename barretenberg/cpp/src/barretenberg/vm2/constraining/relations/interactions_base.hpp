...
template <typename Settings_> struct lookup_settings : public Settings_ {
    ...
    {
        using RetType = std::result_of_t<decltype (&AllEntities::get)(const AllEntities&, ColumnAndShifts)>;
        auto ev = ValueExpressionEvaluator<RetType>([&](const ColumnExpression& expr) { return in.get(expr.column); });
    ...
    static inline auto compute_inverse_exists(const AllEntities& in)
    {
        ...
        auto ev = ValueExpressionEvaluator<View>([&](const ColumnExpression& expr) { return in.get(expr.column); });
        const auto is_operation = View(ev.evaluate(Settings_::SRC_SELECTOR_EXPR));
        const auto is_table_entry = View(ev.evaluate(Settings_::DST_SELECTOR_EXPR));
    ...
    template <typename AllEntities> static inline auto get_entities(AllEntities&& in)
    {
        return []<size_t... ISource, size_t... IDest>(
            ...
            using RetType =
                std::result_of_t<decltype (&std::decay_t<AllEntities>::get)(const AllEntities&, ColumnAndShifts)>;
            auto ev =
                ValueExpressionEvaluator<RetType>([&](const ColumnExpression& expr) { return in.get(expr.column); });
