...
TEST(ExecutionTraceGenTest, InternalCallRet)
{
    ...
    EXPECT_THAT(trace.as_rows(),
                AllOf(Contains(Field(&R::execution_sel_internal_call, 1)),
                      Contains(Field(&R::execution_next_internal_call_id, 2)),
                      Contains(Field(&R::execution_internal_call_id, 1)),
                      Contains(Field(&R::execution_internal_call_return_id, 0)),
                      Contains(Field(&R::execution_rop_0_, 10))));
}
} // namespace
} // namespace bb::avm2::tracegen
