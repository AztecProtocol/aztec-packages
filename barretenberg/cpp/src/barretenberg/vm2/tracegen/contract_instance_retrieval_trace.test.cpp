...
TEST(ContractInstanceRetrievalTraceGenTest, SingleEvent)
{
    ...
    EXPECT_THAT(rows.at(1),
                AllOf(ROW_FIELD_EQ(contract_instance_retrieval_sel, 1),
                      ...
                      // State context
                      ROW_FIELD_EQ(contract_instance_retrieval_timestamp, timestamp),
...
TEST(ContractInstanceRetrievalTraceGenTest, NullifierDoesNotExist)
{
    TestTraceContainer trace;
    ContractInstanceRetrievalTraceBuilder builder;

    // Test constants
    const auto contract_address = FF(0x555);
    const auto nullifier_tree_root = FF(0x444);
    const auto public_data_tree_root = FF(0x333);
    const auto timestamp = 999;
    const auto deployment_nullifier = FF(0x222);
    const auto deployer_protocol_contract_address = FF(0x111);
    const auto expected_rows = 2;
    const auto expected_exists = 0; // Key difference - nullifier doesn't exist
    ...
    // Check that exists is 0 when nullifier_exists is false
    EXPECT_THAT(rows.at(1),
                AllOf(ROW_FIELD_EQ(contract_instance_retrieval_sel, 1),
                      ROW_FIELD_EQ(contract_instance_retrieval_address, contract_address),
                      ROW_FIELD_EQ(contract_instance_retrieval_exists, expected_exists)));
}
...
TEST(ContractInstanceRetrievalTraceGenTest, MultipleEvents)
{
    ...
    const auto base_timestamp = 5000;
    ...
    for (uint32_t i = 0; i < num_events; i++) {
        EXPECT_THAT(
            ...
            AllOf(ROW_FIELD_EQ(contract_instance_retrieval_sel, 1),
                  ...
                  ROW_FIELD_EQ(contract_instance_retrieval_timestamp, base_timestamp + i),
