...
class Sha256TraceBuilder final {
  ...
  private:
    ...
    uint32_t compute_w_with_witness(std::array<uint32_t, 16> prev_w_helpers);
    std::array<uint32_t, 8> compute_compression_with_witness(std::array<uint32_t, 8> state,
                                                             uint32_t round_w,
                                                             uint32_t round_constant,
                                                             uint32_t row);
    void set_helper_cols(std::array<uint32_t, 16> prev_w_helpers);
    void set_init_state_cols(std::array<uint32_t, 8> init_state);
    void set_state_cols(std::array<uint32_t, 8> state);
    void compute_sha256_output(std::array<uint32_t, 8> state, std::array<uint32_t, 8> init_state);
};
