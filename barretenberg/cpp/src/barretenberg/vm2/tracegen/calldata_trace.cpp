    ...
    const simulation::EventEmitterInterface<simulation::CalldataEvent>::Container& events, TraceContainer& trace)
{
    ...
    for (const auto& event : events) {
        ...
        calldata_with_sep.reserve(event.calldata.size() + 1); // +1 for the separator
        ...
        size_t input_size = event.calldata.size();

        // +1 for the separator
        std::array<FF, 4> perm_state = { 0, 0, 0, static_cast<uint256_t>(calldata_length + 1) << 64 };
        ...
        while (input_size > 0) {
            ...
            for (size_t j = 0; j < chunk_size; j++) {
                ...
                perm_state[j] += calldata_with_sep[(round * 3) + j]; // Only used in the permutation computation
            }
            ...
            trace.set(row,
                      { {
                          { C::cd_hashing_sel, 1 },
                          { C::cd_hashing_context_id, context_id },
                          ...
                          { C::cd_hashing_output_0_, perm_state[0] },
                          { C::cd_hashing_output_1_, perm_state[1] },
                          { C::cd_hashing_output_2_, perm_state[2] },
                          { C::cd_hashing_output_3_, perm_state[3] },
