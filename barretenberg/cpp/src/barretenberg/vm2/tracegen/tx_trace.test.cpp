...
TEST(TxTraceGenTest, CollectFeeEvent)
{
    ...
    EXPECT_THAT(rows[static_cast<uint8_t>(TransactionPhase::COLLECT_GAS_FEES)],
                AllOf(ROW_FIELD_EQ(R, tx_sel, 1),
                      ROW_FIELD_EQ(R, tx_phase_value, static_cast<uint8_t>(TransactionPhase::COLLECT_GAS_FEES)),
                      ROW_FIELD_EQ(R, tx_is_padded, 0),
                      ROW_FIELD_EQ(R, tx_start_phase, 1),
                      ROW_FIELD_EQ(R, tx_sel_read_phase_length, 0),
                      ROW_FIELD_EQ(R, tx_end_phase, 1),
                      ROW_FIELD_EQ(R, tx_prev_da_gas_used, prev_da_gas_used),
                      ROW_FIELD_EQ(R, tx_prev_l2_gas_used, prev_l2_gas_used),
                      ROW_FIELD_EQ(R, tx_read_pi_offset, AVM_PUBLIC_INPUTS_EFFECTIVE_GAS_FEES_ROW_IDX),
                      ROW_FIELD_EQ(R, tx_read_pi_length_offset, 0),
                      ROW_FIELD_EQ(R, tx_write_pi_offset, AVM_PUBLIC_INPUTS_TRANSACTION_FEE_ROW_IDX),
                      ROW_FIELD_EQ(R, tx_remaining_phase_counter, 1),
                      ROW_FIELD_EQ(R, tx_remaining_phase_inv, FF(1).invert()),
                      ROW_FIELD_EQ(R, tx_remaining_phase_minus_one_inv, 0),
                      ROW_FIELD_EQ(R, tx_is_collect_fee, 1),
                      ROW_FIELD_EQ(R, tx_effective_fee_per_da_gas, FF(effective_fee_per_da_gas)),
                      ROW_FIELD_EQ(R, tx_effective_fee_per_l2_gas, FF(effective_fee_per_l2_gas)),
                      ROW_FIELD_EQ(R, tx_fee_payer, fee_payer),
                      ROW_FIELD_EQ(R, tx_fee_payer_pi_offset, AVM_PUBLIC_INPUTS_FEE_PAYER_ROW_IDX),
                      ROW_FIELD_EQ(R, tx_fee, fee),
                      ROW_FIELD_EQ(R, tx_fee_payer_balance, fee_payer_balance),
                      ROW_FIELD_EQ(R, tx_end_gas_used_pi_offset, AVM_PUBLIC_INPUTS_END_GAS_USED_ROW_IDX),
                      ROW_FIELD_EQ(R, tx_next_da_gas_used, prev_da_gas_used),
                      ROW_FIELD_EQ(R, tx_next_l2_gas_used, prev_l2_gas_used)));
};
