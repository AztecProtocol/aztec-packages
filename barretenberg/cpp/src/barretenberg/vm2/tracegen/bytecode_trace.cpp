    ...
    TraceContainer& trace)
{
    ...
    uint32_t row = 1;
    for (const auto& event : events) {
        // Only populate fields needed for bc_retrieval.pil constraints and contract_instance_retrieval lookup
        // The actual contract instance validation is handled by the shared ContractInstanceRetrievalEvent
        trace.set(row,
                  { { { C::bc_retrieval_sel, 1 },
                      ...
                      // State context (for lookup into contract_instance_retrieval)
