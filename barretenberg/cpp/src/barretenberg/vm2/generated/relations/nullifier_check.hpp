// AUTOGENERATED FILE
#pragma once

#include <string_view>

#include "barretenberg/common/bb_bench.hpp"
#include "barretenberg/relations/relation_parameters.hpp"
#include "barretenberg/relations/relation_types.hpp"
#include "barretenberg/vm2/generated/columns.hpp"

namespace bb::avm2 {

template <typename FF_> class nullifier_checkImpl {
  public:
    using FF = FF_;

    static constexpr std::array<size_t, 20> SUBRELATION_PARTIAL_LENGTHS = { 3, 3, 3, 3, 3, 3, 3, 3, 3, 5,
                                                                            3, 3, 5, 3, 4, 3, 3, 3, 3, 3 };

    template <typename AllEntities> inline static bool skip(const AllEntities& in)
    {
        using C = ColumnAndShifts;

        return (in.get(C::nullifier_check_sel)).is_zero();
    }

    template <typename ContainerOverSubrelations, typename AllEntities>
    void static accumulate(ContainerOverSubrelations& evals,
                           const AllEntities& in,
                           [[maybe_unused]] const RelationParameters<FF>&,
                           [[maybe_unused]] const FF& scaling_factor);
};

template <typename FF> class nullifier_check : public Relation<nullifier_checkImpl<FF>> {
  public:
    static constexpr const std::string_view NAME = "nullifier_check";

    // Subrelation indices constants, to be used in tests.
    static constexpr size_t SR_PASSTHROUGH_SILOING = 6;
    static constexpr size_t SR_EXISTS_CHECK = 9;
    static constexpr size_t SR_NEXT_NULLIFIER_IS_ZERO_CHECK = 12;

    static std::string get_subrelation_label(size_t index)
    {
        switch (index) {
        case SR_PASSTHROUGH_SILOING:
            return "PASSTHROUGH_SILOING";
        case SR_EXISTS_CHECK:
            return "EXISTS_CHECK";
        case SR_NEXT_NULLIFIER_IS_ZERO_CHECK:
            return "NEXT_NULLIFIER_IS_ZERO_CHECK";
        }
        return std::to_string(index);
    }
};

} // namespace bb::avm2
