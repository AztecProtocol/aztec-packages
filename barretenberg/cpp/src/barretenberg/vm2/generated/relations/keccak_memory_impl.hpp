                                        ...
                                        [[maybe_unused]] const FF_& scaling_factor)
{
    ...
    const auto keccak_memory_LATCH_CONDITION =
        in.get(C::keccak_memory_last) + in.get(C::precomputed_first_row) * (FF(1) - in.get(C::keccak_memory_sel)) *
    ...
    { // COMPUTATION_FINISH_AT_END
        ...
        auto tmp = static_cast<View>(in.get(C::keccak_memory_sel)) *
                   (FF(1) - static_cast<View>(in.get(C::keccak_memory_sel_shift))) *
                   (FF(1) - static_cast<View>(in.get(C::keccak_memory_last)));
