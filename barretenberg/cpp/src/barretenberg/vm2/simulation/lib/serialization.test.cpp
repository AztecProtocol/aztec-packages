...
using simulation::InstructionFetchingError;
...
TEST(SerializationTest, PCOutOfRange)
{
    ...
    } catch (const InstructionFetchingError& error) {
        EXPECT_EQ(error, InstructionFetchingError::PC_OUT_OF_RANGE);
    }
}
...
TEST(SerializationTest, OpcodeOutOfRange)
{
    ...
    try {
        ...
        EXPECT_EQ(error, InstructionFetchingError::OPCODE_OUT_OF_RANGE);
    }
}
...
TEST(SerializationTest, InstructionOutOfRange)
{
    ...
    try {
        ...
        EXPECT_EQ(error, InstructionFetchingError::INSTRUCTION_OUT_OF_RANGE);
    }
}
