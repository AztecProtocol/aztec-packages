...
BytecodeId TxBytecodeManager::get_bytecode(const AztecAddress& address)
{
    ...
    // Use the bytecode commitment as the ID for optimal deduplication
    BytecodeId bytecode_id = klass.public_bytecode_commitment;

    // Check if we've already processed this bytecode (optimal deduplication!)
    ...
    bytecodes.emplace(bytecode_id, shared_bytecode);
