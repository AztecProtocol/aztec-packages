...
TEST_F(BytecodeHashingTest, SimpleHash)
{
    ...
    auto output_hash = bytecode_hasher.compute_public_bytecode_commitment(FF(1), bytecode);

    EXPECT_EQ(output_hash, hash);
    EXPECT_THAT(hashing_events.dump_events(),
                AllOf(SizeIs(1),
                      ElementsAre(AllOf(Field(&BytecodeHashingEvent::bytecode_id, 1),
...
TEST_F(BytecodeHashingTest, Hash)
{
    ...
    bytecode_hasher.compute_public_bytecode_commitment(FF(1), bytecode);
    EXPECT_THAT(hashing_events.dump_events(),
                AllOf(SizeIs(1),
                      ElementsAre(AllOf(Field(&BytecodeHashingEvent::bytecode_id, 1),
