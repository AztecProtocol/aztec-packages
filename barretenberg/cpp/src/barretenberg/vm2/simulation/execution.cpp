...
void Execution::get_env_var(ContextInterface& context, MemoryAddress dst_addr, uint8_t var_enum)
{
    ...
    // TODO(dbanks12): does this make sense to do?
    set_inputs({ TaggedValue::from_tag(ValueTag::U8, var_enum) });

    // Check if var_enum is a valid value in EnvironmentVariable enum
    if (var_enum >= static_cast<uint8_t>(EnvironmentVariable::INVALID_SENTINEL)) {
