...
class BytecodeManagerInterface {
  public:
    ...
    // Does not throw (for use in pre-temporality serialization)
    virtual std::optional<BytecodeId> get_bytecode_id_safe() = 0;
};
...
class BytecodeManager : public BytecodeManagerInterface {
  public:
    ...
    std::optional<BytecodeId> get_bytecode_id_safe() override
    {
        if (!bytecode_id.has_value()) {
            try {
                bytecode_id = tx_bytecode_manager.get_bytecode(address);
