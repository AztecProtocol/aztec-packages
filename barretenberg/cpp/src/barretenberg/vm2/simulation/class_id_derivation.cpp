...
{
    // Check if we've already derived this class_id (optimal deduplication!)
    auto existing_derivation = cached_derivations.find(class_id);
    if (existing_derivation != cached_derivations.end()) {
        // Already processed this class_id - just emit event and return
