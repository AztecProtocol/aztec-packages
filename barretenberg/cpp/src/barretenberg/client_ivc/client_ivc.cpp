...
void ClientIVC::accumulate(ClientCircuit& circuit, const std::shared_ptr<VerificationKey>& precomputed_vk, bool mock_vk)
{
    ...
    // verifier.
    circuit.add_pairing_point_accum(stdlib::recursion::init_default_agg_obj_indices<ClientCircuit>(circuit));
...
HonkProof ClientIVC::construct_and_prove_hiding_circuit()
{
    ...
    num_public_inputs -= bb::PAIRING_POINT_ACCUM_SIZE;            // exclude aggregation object
    ...
    builder.add_pairing_point_accum(stdlib::recursion::init_default_agg_obj_indices<ClientCircuit>(builder));
