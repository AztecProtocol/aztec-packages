...
echo -e '\nComparing Standard Plonk/Honk benchmarks.'
...
# rm -rf $BUILD_DIR
cmake --preset bench > /dev/null && cmake --build --preset bench --target standard_plonk_bench 
cd build-bench
PLONK_BENCH_RESULTS="$BENCH_RESULTS_DIR/plonk_bench.json"
./bin/standard_plonk_bench --benchmark_format=json > $PLONK_BENCH_RESULTS

cd ..
cmake --preset bench > /dev/null && cmake --build --preset bench --target standard_honk_bench 
cd build-bench
HONK_BENCH_RESULTS="$BENCH_RESULTS_DIR/honk_bench.json"
./bin/standard_honk_bench --benchmark_format=json > $HONK_BENCH_RESULTS
