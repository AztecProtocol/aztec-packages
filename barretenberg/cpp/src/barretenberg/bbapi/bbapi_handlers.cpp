...
void handle_poseidon2_absorb_chain(BBApiRequest& /*ctx*/,
                                   wire::BbPoseidon2AbsorbChain&& cmd,
                                   Responder<wire::BbPoseidon2AbsorbChainResponse> respond)
{
    ...
    BB_ASSERT(cmd.inputs.size() % BLOCK_BYTES == 0,
              "Poseidon2AbsorbChain: input size must be a multiple of " << BLOCK_BYTES << " bytes");

    auto state = fr_array_from_wire<4>(cmd.state);
    auto absorbed = Sponge::absorb_blocks(state, many_from_buffer<Sponge::Block>(cmd.inputs));
    respond.ok({ .state = fr_array_to_wire<4>(absorbed) });
}
