...
BbSchnorrComputePublicKey::Response BbSchnorrComputePublicKey::execute(BB_UNUSED BbRequest& request) &&
{
    return { grumpkin::g1::one * private_key };
}

BbSchnorrConstructSignature::Response BbSchnorrConstructSignature::execute(BB_UNUSED BbRequest& request) &&
{
...
BbSchnorrVerifySignature::Response BbSchnorrVerifySignature::execute(BB_UNUSED BbRequest& request) &&
{
    std::string message_str(reinterpret_cast<const char*>(message.data()), message.size());
