...
{
    auto& debug_engine = bb::numeric::get_debug_randomness();

    auto a = debug_engine.get_random_uint64();
    auto b = debug_engine.get_random_uint64();
    EXPECT_NE(a, b);

    debug_engine = bb::numeric::get_debug_randomness(true);
    auto c = debug_engine.get_random_uint64();
    auto d = debug_engine.get_random_uint64();
    EXPECT_EQ(a, c);
    EXPECT_EQ(b, d);

    auto e = bb::numeric::get_randomness().get_random_uint64();
    EXPECT_NE(a, e);
}

TEST(engine, GetExpectedDebugValue)
{
    auto& debug_engine = bb::numeric::get_debug_randomness(true);
