...
ssize_t read_into(int fildes, void* buf, size_t nbyte)
{
...
// This class is horribly unoptmized, but it's easier this way for now
class BufferedRandomEngine : public Engine {
    static constexpr size_t buffer_size = 32 * 1024 * 1024;
    std::array<uint32_t, buffer_size> buffer;
    size_t index = SIZE_MAX;
    int urandom_device = open("/dev/urandom", O_RDONLY);
    std::array<uint32_t, 32> generate_buffered_random_data()
    {
        if (urandom_device < 0) {
            abort();
        }
        if (index == SIZE_MAX || index == buffer_size) {
            size_t n = buffer_size * sizeof(uint32_t);
            char* p = reinterpret_cast<char*>(buffer.data());
            while (n > 0) {
                ssize_t s = read_into(urandom_device, p, n);
                if (s == 0) {
                    abort();
                }
                if (s == -1) {
  ...
  public:
    uint8_t get_random_uint8() override
    {
        auto buf = generate_buffered_random_data();
        uint32_t out = buf[0];
        return static_cast<uint8_t>(out);
    }
...
{
    // return get_debug_engine();
    static BufferedRandomEngine engine;
    return engine;
}
} // namespace numeric::random
