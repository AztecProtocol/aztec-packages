...
/**
 * @brief Test fixed_batch_mul with Grumpkin SRS points and plookup tables (table_bits=8).
 *
 * Uses actual SRS points so load_cached_base_multiples will hit the disk cache (if the cache file
 * was generated by grumpkin_straus_table_gen) or fall back to on-the-fly computation on a cache miss.
 * Either way, verifies that the plookup Straus MSM produces correct results with real SRS points.
 */
TYPED_TEST(CycleGroupTest, TestFixedBatchMulWithSRSPoints)
{
    STDLIB_TYPE_ALIASES;
    auto builder = Builder();

    constexpr size_t num_points = 8;
    constexpr size_t table_bits = 8;

    // Load actual Grumpkin SRS points (downloads if needed)
    auto srs_points = bb::get_grumpkin_g1_data(bb::srs::bb_crs_path(), num_points);
    ...
    for (size_t i = 0; i < num_points; ++i) {
        typename Group::Fr scalar = Group::Fr::random_element(&engine);
        expected += (Element(srs_points[i]) * scalar);
        points.emplace_back(cycle_group_ct(srs_points[i]));
        scalars.emplace_back(cycle_group_ct::cycle_scalar::from_witness(&builder, scalar));
    }

    auto result = cycle_group_ct::fixed_batch_mul(points, scalars, {}, table_bits);
    EXPECT_EQ(result.get_value(), AffineElement(expected));
