  ...
  public:
    ...
    static void test_ecdsa_mul_secp256k1()
    {
        ...
        for (size_t i = 0; i < num_repetitions; ++i) {
            ...
            EXPECT_EQ(output._x.get_value().lo, uint256_t(expected.x));
            EXPECT_EQ(output._y.get_value().lo, uint256_t(expected.y));
        }
    ...
    static void test_secp256k1_ecdsa_mul_skew_handling_regression()
    {
        ...
        // result._x = u2_high_skew ? result._x : acc._x;
        // result._y = u2_high_skew ? result._y : acc._y;
        //
