...
template <typename Builder> class plookup_read {
  ...
  public:
    ...
    static std::pair<field_pt, field_pt> read_pair_from_table(const plookup::MultiTable& multi_table,
                                                              const field_pt& key);
    ...
    // Overload for dynamically-created MultiTables
    static plookup::ReadData<field_pt> get_lookup_accumulators(const plookup::MultiTable& multi_table,
                                                               const field_pt& key_a,
                                                               const field_pt& key_b = 0,
                                                               const bool is_2_to_1_lookup = false);
};
