...
bigfielddyn<Builder>::bigfielddyn(Builder* parent_context, const uint512_t& modulus)
...
{
    Basis target_basis;
    target_basis.modulus = modulus_u512;
    target_basis.num_bits = (size_t)(modulus_u512.get_msb() + 1);
    ASSERT((uint512_t)value < modulus_u512);
}

// TODO(https://github.com/AztecProtocol/barretenberg/issues/850): audit the evaluate_linear_identity function
template <typename Builder>
bigfielddyn<Builder>::bigfielddyn(const field_t<Builder>& low_bits_in,
                                  const field_t<Builder>& high_bits_in,
                                  const uint512_t modulus,
    ...
    , target_basis{ modulus_u512, static_cast<size_t>(modulus_u512.get_msb() + 1) }
{
    ...
    if (low_bits_in.witness_index != IS_CONSTANT) {
        ...
        if constexpr (HasPlookup<Builder>) {
            ...
            low_accumulator = context->decompose_into_base4_accumulators(low_bits_in.witness_index,
                                                                         static_cast<size_t>(NUM_LIMB_BITS * 2),
                                                                         "bigfielddyn: low_bits_in too large.");
    ...
    if (maximum_bitlength > 0) {
        ...
        num_last_limb_bits_with_overflow = maximum_bitlength - (3 * NUM_LIMB_BITS);
    }
    // We create the high limb values similar to the low limb ones above
    const uint64_t num_high_limb_bits = NUM_LIMB_BITS + num_last_limb_bits_with_overflow;
    if (high_bits_in.witness_index != IS_CONSTANT) {
        ...
        if constexpr (HasPlookup<Builder>) {
            ...
            high_accumulator = context->decompose_into_base4_accumulators(high_bits_in.witness_index,
                                                                          ...
                                                                          "bigfielddyn: high_bits_in too large.");
            limb_3.witness_index = high_accumulator[static_cast<size_t>((num_last_limb_bits_with_overflow / 2) - 1)];
...
template <typename Builder>
bigfielddyn<Builder>::bigfielddyn(const bigfielddyn& other)
    : modulus_u512(other.modulus_u512)
    , target_basis{ modulus_u512, static_cast<size_t>(modulus_u512.get_msb() + 1) }
...
template <typename Builder>
bigfielddyn<Builder>::bigfielddyn(bigfielddyn&& other) noexcept
    : modulus_u512(other.modulus_u512)
    , target_basis{ modulus_u512, static_cast<size_t>(modulus_u512.get_msb() + 1) }
    , context(other.context)
    , binary_basis_limbs{ other.binary_basis_limbs[0],
...
/**
 * @brief Creates a bigfielddyn element from a uint512_t.
 * bigfielddyn element is constructed as a witness and not a circuit constant
 *
 * @param ctx
 * @param value
 * @param can_overflow Can the input value have more than log2(modulus) bits?
 * @param maximum_bitlength Provide the explicit maximum bitlength if known. Otherwise bigfielddyn max value will be
 * either log2(modulus) bits iff can_overflow = false, or (4 * NUM_LIMB_BITS) iff can_overflow = true
 * @return bigfielddyn<Builder>
 *
 * @details This method is 1 gate more efficient than constructing from 2 field_ct elements.
 */
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::create_from_u512_as_witness(Builder* ctx,
                                                                       const uint512_t& value,
                                                                       const bool can_overflow,
                                                                       const size_t maximum_bitlength) const
{
    ASSERT((can_overflow == true && maximum_bitlength == 0) ||
           (can_overflow == false && (maximum_bitlength == 0 || maximum_bitlength > (3 * NUM_LIMB_BITS))));
    ...
    if constexpr (HasPlookup<Builder>) {
        ...
        if (maximum_bitlength > 0) {
            ASSERT(maximum_bitlength > 3 * NUM_LIMB_BITS);
            num_last_limb_bits_with_overflow = maximum_bitlength - (3 * NUM_LIMB_BITS);
            uint256_t max_limb_value = (uint256_t(1) << num_last_limb_bits_with_overflow) - 1;
        ...
        ctx->range_constrain_two_limbs(limb_2.witness_index,
                                       limb_3.witness_index,
                                       (size_t)NUM_LIMB_BITS,
                                       (size_t)num_last_limb_bits_with_overflow);
...
template <typename Builder> bigfielddyn<Builder>::bigfielddyn(const byte_array<Builder>& bytes, uint512_t modulus)
{
    ...
    const auto split_byte_into_nibbles = [](Builder* ctx, const field_t<Builder>& split_byte) {
        ...
        lo_nibble.create_range_constraint(4, "bigfielddyn: lo_nibble too large");
        hi_nibble.create_range_constraint(4, "bigfielddyn: hi_nibble too large");
...
template <typename Builder> bigfielddyn<Builder>& bigfielddyn<Builder>::operator=(const bigfielddyn& other)
{
    context = other.context;
    modulus_u512 = other.modulus_u512;
    target_basis = Basis{ modulus_u512, static_cast<size_t>(modulus_u512.get_msb() + 1) };
...
template <typename Builder> bigfielddyn<Builder>& bigfielddyn<Builder>::operator=(bigfielddyn&& other)
{
    context = other.context;
    modulus_u512 = other.modulus_u512;
    target_basis = Basis{ modulus_u512, static_cast<size_t>(modulus_u512.get_msb() + 1) };
...
template <typename Builder> uint512_t bigfielddyn<Builder>::get_value() const
{
...
template <typename Builder> uint512_t bigfielddyn<Builder>::get_maximum_value() const
{
 ...
 *
 * @details Sometimes we need to add a small constrained value to a bigfielddyn element (for example, a boolean value),
 * but we don't want to construct a full bigfielddyn element for that as it would take too many gates. If the maximum
 * value of the field element being added is small enough, we can simply add it to the lowest limb and increase its
 * maximum value. That will create 2 additional constraints instead of 5/3 needed to add 2 bigfielddyn elements and
 * several needed to construct a bigfielddyn element.
 *
 ...
 * @return bigfielddyn<Builder> Result
 */
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::add_to_lower_limb(const field_t<Builder>& other,
...
template <typename Builder> bigfielddyn<Builder> bigfielddyn<Builder>::operator+(const bigfielddyn& other) const
{
 ...
 *
 * Offset `X` will be a multiple of our bigfielddyn modulus `p`
 *
 * i.e `X = m * p`
 *
 ...
 *
 * We must compute the MINIMUM value of `m` that ensures that none of the bigfielddyn limbs will underflow!
 *
...
template <typename Builder> bigfielddyn<Builder> bigfielddyn<Builder>::operator-(const bigfielddyn& other) const
{
    ...
    /**
     * Plookup bigfielddyn subtractoin
     *
 ...
 */
template <typename Builder> bigfielddyn<Builder> bigfielddyn<Builder>::sum(const std::vector<bigfielddyn>& terms)
{
    ASSERT(terms.size() > 0);
 ...
 * */
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::internal_div(const std::vector<bigfielddyn>& numerators,
                                                        const bigfielddyn& denominator,
                                                        bool check_for_zero) const
{
    if (numerators.size() == 0) {
        return bigfielddyn<Builder>(denominator.get_context(), uint256_t(0));
    }
 ...
 **/
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::div_without_denominator_check(const std::vector<bigfielddyn>& numerators,
                                                                         const bigfielddyn& denominator)
{
 ...
 **/
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::div_check_denominator_nonzero(const std::vector<bigfielddyn>& numerators,
                                                                         const bigfielddyn& denominator)
{
 ...
 **/
template <typename Builder> bigfielddyn<Builder> bigfielddyn<Builder>::sqr() const
{
    reduction_check();
    Builder* ctx = context;
 ...
 **/
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::sqradd(const std::vector<bigfielddyn>& to_add) const
{
    reduction_check();
    ...
    if (is_constant()) {
        if (add_constant) {
            ...
            std::vector<bigfielddyn> new_to_add;
 ...
 *
 * @param to_mul bigfielddyn element to multiply by
 * @param to_add Vector of elements to add
 *
 * @return New bigfielddyn elment c
 **/
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::madd(const bigfielddyn& to_mul, const std::vector<bigfielddyn>& to_add) const
{
 ...
 */
template <typename Builder>
void bigfielddyn<Builder>::perform_reductions_for_mult_madd(std::vector<bigfielddyn>& mul_left,
                                                            ...
                                                            const std::vector<bigfielddyn>& to_add)
{
    ...
    if (reduction_required) {
        ...
        auto compute_updates = [this](std::vector<std::tuple<uint1024_t, size_t, size_t>>& maxval_updates,
                                      std::vector<bigfielddyn>& m_left,
                                      std::vector<bigfielddyn>& m_right,
            ...
            for (size_t i = 0; i < number_of_products; i++) {
                ...
                if (m_left[i].is_constant()) {
                    ...
                    if (new_product > original_product) {
                        throw_or_abort("bigfielddyn: This should never happen");
                    }
        ...
        while (reduction_required) {
            ...
            if (!update_size) {
                throw_or_abort("bigfielddyn: Can't reduce further");
            }
 ...
 *
 * @param mul_left Vector of bigfielddyn multiplicands
 * @param mul_right Vector of bigfielddyn multipliers
 * @param to_add Vector of bigfielddyn elements to add to the sum of products
 *
 * @return A reduced value that is the sum of all products and to_add values
 * */
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::mult_madd(const std::vector<bigfielddyn>& mul_left,
                                                     ...
                                                     bool fix_remainder_to_zero)
{
    ...
    std::vector<bigfielddyn> mutable_mul_left(mul_left);
    std::vector<bigfielddyn> mutable_mul_right(mul_right);
    ...
    std::vector<bigfielddyn> new_input_left;
    std::vector<bigfielddyn> new_input_right;
 ...
 **/
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::dual_madd(const bigfielddyn& left_a,
                                                     ...
                                                     const std::vector<bigfielddyn>& to_add)
{
    ...
    std::vector<bigfielddyn> mul_left = { left_a, left_b };
    std::vector<bigfielddyn> mul_right = { right_a, right_b };
 ...
 **/
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::msub_div(const std::vector<bigfielddyn>& mul_left,
                                                    const std::vector<bigfielddyn>& mul_right,
                                                    ...
                                                    bool enable_divisor_nz_check)
{
    ...
    native_big_int product_native;
    product_native.initialize(0, this->modulus_u512);
    ...
    for (size_t i = 0; i < num_multiplications; ++i) {
        ...
        const native_big_int mul_left_native = this->get_native_integer((mul_left[i].get_value() % modulus_u512).lo);
        const native_big_int mul_right_native = this->get_native_integer((mul_right[i].get_value() % modulus_u512).lo);
    ...
    auto sub_native = this->get_native_zero();
    bool sub_constant = true;
    for (const auto& sub : to_sub) {
        sub_native += get_native_integer(uint512_t(sub.get_value() % modulus_u512).lo);
        sub_constant = sub_constant && sub.is_constant();
    }

    native_big_int divisor_native = this->get_native_integer((divisor.get_value() % modulus_u512).lo);

    // Compute the result
    const native_big_int result_native = (product_native - sub_native) / divisor_native;
    ...
    bigfielddyn result = create_from_u512_as_witness(ctx, result_value.lo);

    std::vector<bigfielddyn> eval_left{ result };
    std::vector<bigfielddyn> eval_right{ divisor };
...
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::conditional_negate(const bool_t<Builder>& predicate) const
{
 ...
 * @tparam Builder
 * @tparam T
 * @param other The other bigfielddyn element
 * @param predicate Predicate controlling the result (0 for this, 1 for the other)
 * @return Resulting element
 */
template <typename Builder>
bigfielddyn<Builder> bigfielddyn<Builder>::conditional_select(const bigfielddyn& other,
 ...
 * REDUCTION CHECK
 *
 * When performing bigfielddyn operations, we need to ensure the maximum value is less than:
 ...
 *limit overflow
 **/
template <typename Builder> void bigfielddyn<Builder>::reduction_check(const size_t num_products) const
{
    ...
    if (is_constant()) { // this seems not a reduction check, but actually computing the reduction
        ...
        bigfielddyn reduced(context, uint256_t(reduced_value));
...
template <typename Builder> void bigfielddyn<Builder>::assert_is_in_field() const
{
    ...
    uint256_t modulus_minus_one = modulus_u512.lo - 1;

    uint256_t modulus_minus_one_0 = modulus_minus_one.slice(0, NUM_LIMB_BITS);
    uint256_t modulus_minus_one_1 = modulus_minus_one.slice(NUM_LIMB_BITS, NUM_LIMB_BITS * 2);
    uint256_t modulus_minus_one_2 = modulus_minus_one.slice(NUM_LIMB_BITS * 2, NUM_LIMB_BITS * 3);
    uint256_t modulus_minus_one_3 = modulus_minus_one.slice(NUM_LIMB_BITS * 3, NUM_LIMB_BITS * 4);
    ...
    if constexpr (HasPlookup<Builder>) {
        ...
        context->decompose_into_base4_accumulators(r0.witness_index,
                                                   static_cast<size_t>(NUM_LIMB_BITS),
                                                   "bigfielddyn: assert_is_in_field range constraint 1.");
        context->decompose_into_base4_accumulators(r1.witness_index,
                                                   static_cast<size_t>(NUM_LIMB_BITS),
                                                   "bigfielddyn: assert_is_in_field range constraint 2.");
        context->decompose_into_base4_accumulators(r2.witness_index,
                                                   static_cast<size_t>(NUM_LIMB_BITS),
                                                   "bigfielddyn: assert_is_in_field range constraint 3.");
        context->decompose_into_base4_accumulators(r3.witness_index,
                                                   static_cast<size_t>(NUM_LIMB_BITS),
                                                   "bigfielddyn: assert_is_in_field range constraint 4.");
    }
}

template <typename Builder> void bigfielddyn<Builder>::assert_less_than(const uint256_t upper_limit) const
{
    ...
    if constexpr (Builder::CIRCUIT_TYPE == CircuitType::ULTRA) {
        ...
        context->decompose_into_base4_accumulators(
            r0.witness_index, static_cast<size_t>(NUM_LIMB_BITS), "bigfielddyn: assert_less_than range constraint 1.");
        context->decompose_into_base4_accumulators(
            r1.witness_index, static_cast<size_t>(NUM_LIMB_BITS), "bigfielddyn: assert_less_than range constraint 2.");
        context->decompose_into_base4_accumulators(
            r2.witness_index, static_cast<size_t>(NUM_LIMB_BITS), "bigfielddyn: assert_less_than range constraint 3.");
        context->decompose_into_base4_accumulators(
            r3.witness_index, static_cast<size_t>(NUM_LIMB_BITS), "bigfielddyn: assert_less_than range constraint 4.");
    }
}
...
template <typename Builder> void bigfielddyn<Builder>::assert_equal(const bigfielddyn& other) const
{
    ...
    if (remainder_512 != 0)
        std::cerr << "bigfielddyn: remainder not zero!" << std::endl;
...
template <typename Builder> void bigfielddyn<Builder>::assert_is_not_equal(const bigfielddyn& other) const
{
    ...
    auto diff = base_diff;
    ASSERT(modulus_u512 <= ((uint512_t)1 << 256));
    field_t<Builder> prime_basis(get_context(), (modulus_u512 - 1).lo);
    prime_basis += 1;
...
// not larger
template <typename Builder> void bigfielddyn<Builder>::self_reduce() const
{
    // Warning: this assumes we have run circuit construction at least once in debug mode where large non reduced
 ...
 * */
template <typename Builder>
void bigfielddyn<Builder>::unsafe_evaluate_multiply_add(const bigfielddyn& input_left,
                                                        const bigfielddyn& input_to_mul,
                                                        const std::vector<bigfielddyn>& to_add,
                                                        ...
                                                        const std::vector<bigfielddyn>& input_remainders) const
{
    ...
    bigfielddyn quotient = input_quotient;
    ...
    if constexpr (HasPlookup<Builder>) {
        ...
        const auto convert_constant_to_fixed_witness = [ctx](const bigfielddyn& input) {
            bigfielddyn output(input);
        ...
        bb::non_native_field_witnesses<bb::fr> witnesses23{
            ...
            modulus_u512.lo, // TODO TODO TOD !!! c incorrect, a voir ou on s en sert ???
        };
        ...
        if ((carry_hi_msb + carry_lo_msb) < field_t<Builder>::modulus.get_msb()) {
            ...
            const auto accumulators = ctx->decompose_into_base4_accumulators(
                ...
                "bigfielddyn: carry_combined too large in unsafe_evaluate_multiply_add.");
            ...
            carry_hi.assert_equal(accumulator_midpoint, "bigfielddyn multiply range check failed");
        } else {
            ...
            ctx->decompose_into_base4_accumulators(carry_lo.witness_index,
                                                   ...
                                                   "bigfielddyn: carry_lo too large in unsafe_evaluate_multiply_add.");
            ctx->decompose_into_base4_accumulators(carry_hi.witness_index,
                                                   static_cast<size_t>(carry_hi_msb),
                                                   "bigfielddyn: carry_hi too large in unsafe_evaluate_multiply_add.");
        }
    }
}
/**
 ...
 **/
template <typename Builder>
void bigfielddyn<Builder>::unsafe_evaluate_multiple_multiply_add(const std::vector<bigfielddyn>& input_left,
                                                                 ...
                                                                 const std::vector<bigfielddyn>& input_remainders)
{
    ...
    std::vector<bigfielddyn> remainders(input_remainders);
    std::vector<bigfielddyn> left(input_left);
    std::vector<bigfielddyn> right(input_right);
    bigfielddyn quotient = input_quotient;
    ...
    const auto get_product_maximum = [](const bigfielddyn& left, const bigfielddyn& right) {
    ...
    if constexpr (HasPlookup<Builder>) {
        ...
        // where q is a 'quotient' bigfielddyn and neg_modulus is defined by selector polynomial values
        ...
        bb::non_native_field_witnesses<bb::fr> witnesses{
            ...
            { neg_modulus_limbs[0], neg_modulus_limbs[1], neg_modulus_limbs[2], neg_modulus_limbs[3] },
            modulus_u512.lo, // TODO TODO Jamais 2 asn 3
        };
        ...
        if constexpr (HasPlookup<Builder>) {
            ...
            if ((carry_hi_msb + carry_lo_msb) < field_t<Builder>::modulus.get_msb()) {
                ...
                const auto accumulators = ctx->decompose_into_base4_accumulators(
                    carry_combined.witness_index,
                    static_cast<size_t>(carry_lo_msb + carry_hi_msb),
                    "bigfielddyn: carry_combined too large in unsafe_evaluate_multiple_multiply_add.");
                ...
                ctx->decompose_into_base4_accumulators(
                    ...
                    static_cast<size_t>(carry_lo_msb),
                    "bigfielddyn: carry_lo too large in unsafe_evaluate_multiple_multiply_add.");
                ctx->decompose_into_base4_accumulators(
                    carry_hi.witness_index,
                    static_cast<size_t>(carry_hi_msb),
                    "bigfielddyn: carry_hi too large in unsafe_evaluate_multiple_multiply_add.");
            }
        }
    }
}
...
template <typename Builder>
void bigfielddyn<Builder>::unsafe_evaluate_square_add(const bigfielddyn& left,
                                                      const std::vector<bigfielddyn>& to_add,
    ...
    if constexpr (HasPlookup<Builder>) {
        ...
        if ((carry_hi_msb + carry_lo_msb) < field_t<Builder>::modulus.get_msb()) {
            ...
            const auto accumulators = ctx->decompose_into_base4_accumulators(
                carry_combined.witness_index,
                static_cast<size_t>(carry_lo_msb + carry_hi_msb),
                "bigfielddyn: carry_combined too large in unsafe_evaluate_square_add.");
            field_t<Builder> accumulator_midpoint =
                field_t<Builder>::from_witness_index(ctx, accumulators[static_cast<size_t>((carry_hi_msb / 2) - 1)]);
            carry_hi.assert_equal(accumulator_midpoint, "bigfielddyn multiply range check failed");
        } else {
            ...
            ctx->decompose_into_base4_accumulators(carry_lo.witness_index,
                                                   static_cast<size_t>(carry_lo_msb),
                                                   "bigfielddyn: carry_lo too large in unsafe_evaluate_square_add.");
            ctx->decompose_into_base4_accumulators(carry_hi.witness_index,
                                                   static_cast<size_t>(carry_hi_msb),
                                                   "bigfielddyn: carry_hi too large in unsafe_evaluate_square_add");
        }
    }
}

template <typename Builder>
std::pair<uint512_t, uint512_t> bigfielddyn<Builder>::compute_quotient_remainder_values(
    const bigfielddyn& a, const bigfielddyn& b, const std::vector<bigfielddyn>& to_add) const
{
    uint512_t add_values(0);
    for (const auto& add_element : to_add) {
        add_element.reduction_check();
...
template <typename Builder>
uint512_t bigfielddyn<Builder>::compute_maximum_quotient_value(const std::vector<uint512_t>& as,
...
template <typename Builder>
std::pair<bool, size_t> bigfielddyn<Builder>::get_quotient_reduction_info(
...
template <typename Builder> void bigfielddyn<Builder>::set_value(const uint256_t& value)
{
    // todo; verifier que pas constant !
    // gerer les autres limbs...
    context->variables[prime_basis_limb.witness_index] = value;
    context->variables[binary_basis_limbs[0].element.witness_index] = value;
}

template class bigfielddyn<GoblinUltraCircuitBuilder>;
template class bigfielddyn<UltraCircuitBuilder>;
} // namespace bb::stdlib
