...
//  * and returns rem.
//  *
//  * Note that, if `ov` were not constrained, there is the possibility of returning a malicious value of `rem`. The
//  point
...
//  * prime sometimes called r, i.e., the order of BN254. Let S be the subset of F_q consisting of all of the values
//  range
//  * constrained to width-many bits and their negatives. If width is a power of 2 at most 128, then S is the disjoint
//  * union of [0, 2**width-1] (the positive elements) and [r-2**width, r-1] (the negations). Since we only allow widths
//  up
//  * to 64, we can use range constraints to impose non-negativity. Range constraining r, we have 0 <= r. Since b is
//  range
