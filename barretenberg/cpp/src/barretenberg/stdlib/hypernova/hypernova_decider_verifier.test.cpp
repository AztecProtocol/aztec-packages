...
class HypernovaDeciderVerifierTests : public ::testing::Test {
  ...
  public:
    ...
    static void test_folding(const TamperingMode& mode)
    {
        ...
        bb::HypernovaFoldingProver folding_prover(prover_transcript);
        ...
        bb::HypernovaDeciderProver decider_prover(prover_transcript);
        auto decider_proof = decider_prover.construct_pcs_proof(ck, folded_accumulator);
        ...
        HypernovaDeciderVerifier::Proof stdlib_proof(builder, decider_proof);
        HypernovaDeciderVerifier decider_verifier(verifier_transcript);
        auto pairing_points = decider_verifier.verify_pcs_proof(builder, folded_verifier_accumulator, stdlib_proof);
        ...
        EXPECT_EQ(pp.check(), mode == TamperingMode::None);
        // EXPECT_EQ(first_sumcheck, mode == TamperingMode::None);
        // EXPECT_TRUE(second_sumcheck);
        // EXPECT_TRUE(compare_prover_verifier_accumulators(folded_accumulator,
        // folded_verifier_accumulator.get_value()));
    }
};

TEST_F(HypernovaDeciderVerifierTests, Fold)
{
