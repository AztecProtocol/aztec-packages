...
template <typename Flavor> void OinkRecursiveVerifier_<Flavor>::verify()
{
    ...
    // with the flows
    ASSERT(static_cast<uint32_t>(circuit_size.get_value()) == instance->verification_key->circuit_size);
    ASSERT(static_cast<uint32_t>(public_input_size.get_value()) == instance->verification_key->num_public_inputs);
    ASSERT(static_cast<uint32_t>(pub_inputs_offset.get_value()) == instance->verification_key->pub_inputs_offset);
