...
template <typename Flavor> void OinkRecursiveVerifier_<Flavor>::verify()
{
    ...
    if (static_cast<uint32_t>(public_input_size.get_value()) != verification_key->verification_key->num_public_inputs) {
        throw_or_abort(std::format("OinkRecursiveVerifier::verify: proof public input size ({}) does not match "
                                   "verification key public input size ({})",
                                   static_cast<uint32_t>(public_input_size.get_value()),
