...
TEST_F(SpecialPublicInputsTests, Basic)
{
    ...
    G1Native ecc_op_table_val = G1Native::random_element();
    FFNative pg_acc_hash_val = FFNative::random_element();
    ...
    { // The first circuit propagates the kernel output via its public inputs
        ...
        kernel_output.pairing_inputs =
            PairingInputs{ G1::from_witness(&builder, P0_val), G1::from_witness(&builder, P1_val) };
        ...
        kernel_output.ecc_op_table = G1::from_witness(&builder, ecc_op_table_val);
        kernel_output.pg_acc_hash = FF::from_witness(&builder, pg_acc_hash_val);
    ...
    { // The second circuit reconstructs the kernel inputs from the public inputs
        ...
        EXPECT_EQ(kernel_input.ecc_op_table.get_value(), ecc_op_table_val);
        EXPECT_EQ(kernel_input.pg_acc_hash.get_value(), pg_acc_hash_val);
    }
}
