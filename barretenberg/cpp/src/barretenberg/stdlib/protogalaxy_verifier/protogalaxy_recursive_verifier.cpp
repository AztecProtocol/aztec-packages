    ...
    const std::shared_ptr<Instance>& inst, std::string& domain_separator)
{
    ...
    info("Num gates before oink verifier creation:  ", builder->num_gates);
    OinkVerifier oink_verifier{ builder, inst, transcript, domain_separator };
    info("Num gates before oink verify:             ", builder->num_gates);
    oink_verifier.verify();
    info("Num gates after oink verify:              ", builder->num_gates);
}
