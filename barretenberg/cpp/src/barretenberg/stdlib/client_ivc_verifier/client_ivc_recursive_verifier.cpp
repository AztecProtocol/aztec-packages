...
ClientIVCRecursiveVerifier::Output ClientIVCRecursiveVerifier::verify(const ClientIVC::Proof& proof)
{
    ...
    auto stdlib_decider_vk = std::make_shared<RecursiveDeciderVerificationKey>(builder.get(), stdlib_mega_vk);

    // Perform recursive decider verification
    MegaVerifier verifier{ builder.get(), stdlib_decider_vk, civc_rec_verifier_transcript };
    ...
    GoblinRecursiveVerifierOutput output =
        goblin_verifier.verify(proof.goblin_proof, stdlib_decider_vk->witness_commitments.get_ecc_op_wires());
