...
{
    const size_t N = op_queue->num_subtables(); // actual number of subtables
    ...
    // We use a flat list of (M + 1) * NUM_WIRES + 1 challenges.
    // -------------------------------------------------------------------------
    ...
    for (size_t i = 0; i < M; ++i) {
        for (size_t col = 0; col < NUM_WIRES; ++col) {
            evals.push_back(i < N ? subtable_cols[i][col].evaluate(kappa) : FF(0));
            transcript->send_to_verifier("C_EVAL_" + std::to_string(i) + "_" + std::to_string(col), evals.back());
        }
    }
    ...
    OpeningClaim shplonk_opening_claim = compute_shplonk_opening_claim(
        shplonk_batched_quotient, z, subtable_cols, merged_table, betas, kappa, kappa_inv, degree_check_poly, evals);
