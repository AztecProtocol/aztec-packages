...
/*
 * The below test vector is a subset (covers inputs from 0 to 1024 bytes only) of the test vectors from the Blake3
 * reference implementation. It has been modified to remove the keyed and derive_key context since our implementation
 * does not support those modes. Further, we have converted the string formats from the source file into vector format
 * for convenience.
 *
...
TEST(MiscBlake3s, test_official_vectors)
{
    for (auto v : full_test_vector) {
        ...
        EXPECT_EQ(blake3::blake3s(input), v.hash);
    }
}
