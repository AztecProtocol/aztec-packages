...
template <size_t input_len> std::array<uint8_t, input_len> test_input()
{
...
TEST(misc_blake3s, test_vectors)
{
    ...
    barretenberg::constexpr_for<0, 72, 1>([&]<size_t i>() {
        constexpr auto v = test_vectors[i];
        constexpr size_t input_size = v.input.size();
        std::cout << "i " << input_size << std::endl;
        constexpr std::array<uint8_t, input_size> input =
            convert<input_size>(v.input); // (v.input.begin(), v.input.end());
        blake3::blake3s(input);
        EXPECT_EQ(true, true);
