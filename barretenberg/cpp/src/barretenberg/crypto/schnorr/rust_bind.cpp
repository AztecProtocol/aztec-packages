...
WASM_EXPORT const char* rust_schnorr_compute_public_key(uint8_t const* private_key, uint8_t* public_key_buf)
{
    try {
...
WASM_EXPORT const char* rust_schnorr_negate_public_key(uint8_t const* public_key_buffer, uint8_t* output)
{
    try {
        // Negate the public key (effectively negating the y-coordinate of the public key) and return the resulting
        // public key.
...
WASM_EXPORT const char* rust_schnorr_construct_signature(uint8_t const* message_buf,
    ...
    uint8_t const* message_buf, uint8_t const* pub_key, uint8_t const* sig_s, uint8_t const* sig_e, bool* result)
{
    try {
        ...
        *result = crypto::schnorr::verify_signature<Blake2sHasher, grumpkin::fq, grumpkin::fr, grumpkin::g1>(
            message, pubk, sig);
        return nullptr;
    } catch (const std::exception& e) {
        return e.what(); // return the exception message
...
WASM_EXPORT const char* rust_schnorr_multisig_create_multisig_public_key(uint8_t const* private_key,
...
WASM_EXPORT const char* rust_schnorr_multisig_validate_and_combine_signer_pubkeys(uint8_t const* signer_pubkey_buf,
...
WASM_EXPORT const char* rust_schnorr_multisig_construct_signature_round_1(uint8_t* round_one_public_output_buf,
...
WASM_EXPORT const char* rust_schnorr_multisig_construct_signature_round_2(uint8_t const* message_buf,
                                                                     uint8_t const* private_key,
...
WASM_EXPORT const char* rust_schnorr_multisig_combine_signatures(uint8_t const* message_buf,
                                                            uint8_t const* signer_pubkeys_buf,
