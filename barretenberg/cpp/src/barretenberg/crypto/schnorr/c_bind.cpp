                                             ...
                                             uint8_t* e)
{
    ...
    auto priv_key = from_buffer<grumpkin::fr>(private_key);
    grumpkin::g1::affine_element pub_key = grumpkin::g1::one * priv_key;
    crypto::schnorr::schnorr_key_pair<grumpkin::fr, grumpkin::g1> key_pair = { priv_key, pub_key };
    auto sig = crypto::schnorr::schnorr_construct_signature<Blake2sHasher, grumpkin::fq>(message, key_pair);
    write(s, sig.s);
    write(e, sig.e);
}
    ...
    uint8_t const* message_buf, uint8_t const* pub_key, uint8_t const* sig_s, uint8_t const* sig_e, bool* result)
{
    ...
    crypto::schnorr::schnorr_signature sig = { s, e };
    *result = crypto::schnorr::schnorr_verify_signature<Blake2sHasher, grumpkin::fq, grumpkin::fr, grumpkin::g1>(
        message, pubk, sig);
}
