...
template <typename Hash> struct MultisigTest : public ::testing::Test {
    ...
    using multisig = crypto::schnorr::schnorr_multisig<G, Hash, Blake2sHasher>;
    ...
    static std::optional<crypto::schnorr::schnorr_signature> create_multisig(
        const std::string& message, const std::vector<KeyPair>& accounts, const bool tamper_proof_of_possession = false)
    {
...
TYPED_TEST(MultisigTest, verify_multi_signature_blake2s)
{
    ...
    using KeyPair = crypto::schnorr::schnorr_key_pair<Fr, G>;
    using multisig = crypto::schnorr::schnorr_multisig<G, TypeParam>;
    ...
    bool result = crypto::schnorr::schnorr_verify_signature<Blake2sHasher, Fq, Fr, G>(message, *pub_key, *signature);
