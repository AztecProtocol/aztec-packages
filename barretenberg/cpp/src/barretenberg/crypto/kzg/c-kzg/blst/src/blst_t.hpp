...
template <const size_t N, const vec384 MOD, const limb_t M0, const vec384 RR, const vec384 ONE> class blst_384_t {
  private:
    vec384 val;
  ...
  public:
    ...
    inline blst_384_t(int a)
        : blst_384_t((uint64_t)a)
    {}
...
#error "C++ >= 14 is required to compile <blst>/src/blst_t.hpp for CUDA"
#endif
    template <typename... Ts> constexpr blst_384_t(limb_t a0, Ts... arr)
    {
        limb_t temp[11] = { arr... };
    ...
    static inline const blst_384_t& one() { return *reinterpret_cast<const blst_384_t*>(ONE); }
    ...
    {
        mul_mont_384(val, RR, val, MOD, M0);
        return *this;
    }
    inline blst_384_t& from()
    {
        from_mont_384(val, val, MOD, M0);
        return *this;
    }

    inline void store(limb_t* p) const { vec_copy(p, val, sizeof(val)); }

    inline blst_384_t& operator+=(const blst_384_t& b)
    {
        add_mod_384(val, val, b, MOD);
        return *this;
    }
    ...
    inline blst_384_t operator()(unsigned p) { return *this ^ p; }
    friend inline blst_384_t sqr(const blst_384_t& a) { return a ^ 2; }

    inline bool is_one() const { return vec_is_equal(val, ONE, sizeof(val)); }

    inline int is_zero() const { return vec_is_zero(val, sizeof(val)); }

    inline void zero() { vec_zero(val, sizeof(val)); }
    ...
    static inline blst_384_t csel(const blst_384_t& a, const blst_384_t& b, int sel_a)
    {
        blst_384_t ret;
    ...
    friend inline blst_384_t operator/(const blst_384_t& a, const blst_384_t& b) { return a * b.reciprocal(); }
    inline blst_384_t& operator/=(const blst_384_t& a) { return *this *= a.reciprocal(); }

    inline blst_384_t(const char* hexascii)
    {
        limbs_from_hexascii(val, sizeof(val), hexascii);
        to();
    }
...
template <const size_t N, const vec256 MOD, const limb_t M0, const vec256 RR, const vec256 ONE> class blst_256_t {
    vec256 val;
  ...
  public:
    ...
    inline blst_256_t(int a)
        : blst_256_t((uint64_t)a)
    {}
...
#error "C++ >= 14 is required to compile <blst>/src/blst_t.hpp for CUDA"
#endif
    template <typename... Ts> constexpr blst_256_t(limb_t a0, Ts... arr)
    {
        limb_t temp[7] = { arr... };
    ...
    static inline const blst_256_t& one() { return *reinterpret_cast<const blst_256_t*>(ONE); }
    ...
    {
        mul_mont_sparse_256(val, val, RR, MOD, M0);
        return *this;
    }
    ...
    inline blst_256_t& from(const unsigned char* bytes, size_t n, bool le = false)
    {
        ...
        if (n > 32) {
            vec512 temp{ 0 };
            if (le)
                limbs_from_le_bytes(temp, bytes, n);
            else
                limbs_from_be_bytes(temp, bytes, n);
            redc_mont_256(val, temp, MOD, M0);
            mul_mont_sparse_256(val, RR, val, MOD, M0);
        } else {
            vec_zero(val, sizeof(val));
            if (le)
                limbs_from_le_bytes(val, bytes, n);
            else
                limbs_from_be_bytes(val, bytes, n);
    ...
    inline blst_256_t& operator*=(const blst_256_t& a)
    {
        if (this == &a)
            ...
            mul_mont_sparse_256(val, val, a, MOD, M0);
        return *this;
    }
    friend inline blst_256_t operator*(const blst_256_t& a, const blst_256_t& b)
    {
        ...
        if (&a == &b)
            ...
            mul_mont_sparse_256(ret, a, b, MOD, M0);
        return ret;
    }
    ...
    inline blst_256_t operator()(unsigned p) { return *this ^ p; }
    friend inline blst_256_t sqr(const blst_256_t& a) { return a ^ 2; }
    ...
    static inline blst_256_t csel(const blst_256_t& a, const blst_256_t& b, int sel_a)
    {
        blst_256_t ret;
        vec_select(ret, a, b, sizeof(ret), sel_a);
        return ret;
    ...
    friend inline blst_256_t operator/(const blst_256_t& a, const blst_256_t& b) { return a * b.reciprocal(); }
    inline blst_256_t& operator/=(const blst_256_t& a) { return *this *= a.reciprocal(); }
