...
namespace test_goblin_merge {

namespace {
auto& engine = numeric::random::get_debug_engine();
}

class GoblinMergeTests : public ::testing::Test {
  protected:
 ...
 *
 */
TEST_F(GoblinMergeTests, SimpleCircuit)
{
    ...
    {
        auto prover = composer.create_merge_prover(op_queue);
        auto verifier = composer.create_merge_verifier(10);
        prover.transcript.print();
        EXPECT_EQ(prover.transcript.get_manifest(), verifier.transcript.get_manifest());
        auto proof = prover.construct_proof();
        bool verified = verifier.verify_proof(proof);
        EXPECT_EQ(verified, true);
    }
