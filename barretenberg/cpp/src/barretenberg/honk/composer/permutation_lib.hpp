...
{
    const size_t fixed_size = 1UL << CONST_PROOF_SIZE_LOG_N;
        ...
        parallel_for(thread_data.num_threads, [&](size_t j) {
            ...
            for (size_t i = start; i < end; ++i) {
                ...
                if (current_is_public_input) {
                    ...
                    current_permutation_poly.at(poly_idx) = -FF(current_row_idx + 1 + fixed_size * current_col_idx);
                } else if (current_is_tag) {
                    // Set evaluations to (arbitrary) values disjoint from non-tag values
                    current_permutation_poly.at(poly_idx) = fixed_size * Flavor::NUM_WIRES + current_row_idx;
                } else {
                    // For the regular permutation we simply point to the next location by setting the
                    // evaluation to its idx
                    current_permutation_poly.at(poly_idx) = FF(current_row_idx + fixed_size * current_col_idx);
                }
            }
        });
        wire_idx++;
