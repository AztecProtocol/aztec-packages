...
#include "./ExampleRelation_verifier.hpp"
...
ExampleRelationVerifier_<Flavor>::ExampleRelationVerifier_(
    std::shared_ptr<typename Flavor::VerificationKey> verifier_key)
    : key(verifier_key)
{}

template <typename Flavor>
ExampleRelationVerifier_<Flavor>::ExampleRelationVerifier_(ExampleRelationVerifier_&& other) noexcept
    : key(std::move(other.key))
    , pcs_verification_key(std::move(other.pcs_verification_key))
{}

template <typename Flavor>
ExampleRelationVerifier_<Flavor>& ExampleRelationVerifier_<Flavor>::operator=(ExampleRelationVerifier_&& other) noexcept
{
    key = other.key;
    pcs_verification_key = (std::move(other.pcs_verification_key));
    commitments.clear();
...
/**
 * @brief This function verifies an ExampleRelation Honk proof for given program settings.
 *
 */
template <typename Flavor> bool ExampleRelationVerifier_<Flavor>::verify_proof(const plonk::proof& proof)
{
    ...
    for (auto& commitment : commitments.get_unshifted()) {
        // TODO(@zac-williamson) ensure ExampleRelation polynomial commitments are never points at infinity (#2214)
        if (commitment.y != 0) {
...
template class ExampleRelationVerifier_<honk::flavor::ExampleRelationFlavor>;
