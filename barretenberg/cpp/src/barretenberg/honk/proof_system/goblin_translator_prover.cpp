...
template <typename Flavor> void GoblinTranslatorProver_<Flavor>::execute_grand_product_computation_round()
{
    ...
    FF prime_limb = uint256_t(key->accumulators_binary_limbs_0[1]) +
                    (uint256_t(key->accumulators_binary_limbs_1[1]) << NUM_LIMB_BITS) +
                    (uint256_t(key->accumulators_binary_limbs_2[1]) << NUM_LIMB_BITS * 2) +
                    (uint256_t(key->accumulators_binary_limbs_3[1]) << NUM_LIMB_BITS * 3);
