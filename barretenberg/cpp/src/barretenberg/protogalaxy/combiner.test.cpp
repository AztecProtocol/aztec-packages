...
// TEST(Protogalaxy, FullTestMaybe)
// {
//     constexpr size_t NUM_INSTANCES = 2;
//     using ProverInstance = ProverInstance_<Flavor>;
//     using ProverInstances = ProverInstances_<Flavor, NUM_INSTANCES>;
...
//     auto full_honk_evals = prover.compute_full_honk_evaluations(instances[0]->prover_polynomials, FF(0), params);
//     auto betas = std::vector<FF>{ FF(2) };
//     auto pow_betas = prover.compute_pow_polynomial_at_values(betas);
//     // Compute the corresponding target sum and create a dummy accumulator
//     auto target_sum = FF(0);
//     for (size_t i = 0; i < 2; i++) {
//         target_sum += full_honk_evals[i] * pow_betas[i];
//     }
//     instances[0]->folding_parameters = { betas, target_sum };

//     auto deltas = std::vector<FF>{ FF(2), FF(4) };
//     auto perturbator = prover.compute_perturbator(instances[0], deltas);

//     EXPECT_EQ(perturbator[0], target_sum);

//     instances.alpha = Univariate<FF, 13>(FF(0));

//     auto challenge = FF(2);

//     auto betas_star = prover.update_gate_separation_challenges(challenge, betas, deltas);
//     instances.folding_parameters.gate_separation_challenges = betas;
//     auto combiner = prover.compute_combiner(instances);

//     auto compressed_perturbator = perturbator.evaluate(challenge);
//     auto combiner_quotient = prover.compute_combiner_quotient(compressed_perturbator, combiner);

//     auto combiner_challenge = FF(2);
