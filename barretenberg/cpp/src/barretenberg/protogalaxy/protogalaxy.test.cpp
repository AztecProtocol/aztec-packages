...
template <typename Flavor> class ProtogalaxyTests : public testing::Test {
  public:
    ...
    {
        Builder builder1;
        stdlib::recursion::aggregation_state<Builder>::add_default_pairing_points_to_public_inputs(builder1);
        auto pk_1 = std::make_shared<DeciderProvingKey>(builder1);
        pk_1->relation_parameters.eta = 1;

        Builder builder2;
        builder2.add_variable(3);
        stdlib::recursion::aggregation_state<Builder>::add_default_pairing_points_to_public_inputs(builder2);
    ...
    static void test_fold_with_virtual_size_expansion()
    {
        ...
        for (size_t i = 0; i < 2; ++i) {
            ...
            stdlib::recursion::aggregation_state<MegaCircuitBuilder>::add_default_pairing_points_to_public_inputs(
                builder);
