...
std::tuple<std::vector<typename DeciderProvingKeys::Flavor::FF>,
           std::shared_ptr<typename ProtogalaxyProver_<DeciderProvingKeys>::ComponentEvaluations[]>,
    ...
    const std::shared_ptr<const typename DeciderProvingKeys::DeciderPK>& accumulator)
{
    ...
    const auto compute_first_round_relation_evaluations = [&log_circuit_size]() {
        auto result = std::shared_ptr<ComponentEvaluations[]>(new ComponentEvaluations[1 << log_circuit_size]);
...
FoldingResult<typename DeciderProvingKeys::Flavor> ProtogalaxyProver_<DeciderProvingKeys>::prove()
{
    ...
    std::tie(deltas, component_evaluations, perturbator) = perturbator_round(accumulator);

    std::tie(accumulator->gate_challenges, alphas, relation_parameters, perturbator_evaluation, combiner_quotient) =
        combiner_quotient_round(accumulator->gate_challenges, deltas, keys_to_fold, component_evaluations);
