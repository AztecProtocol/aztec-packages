    ...
    namespace bb
{
        ...
        const FF& perturbator_evaluation)
    {
        ...
        result.accumulator->target_sum =
            perturbator_evaluation * lagranges[0] +
    ...
    std::tuple<std::vector<typename ProverInstances::Flavor::FF>,
               LegacyPolynomial<typename ProverInstances::Flavor::FF>>
        ...
        const std::shared_ptr<const typename ProverInstances::Instance>& accumulator)
    {
        ...
        const LegacyPolynomial<FF> perturbator =
            accumulator->is_accumulator ? Fun::compute_perturbator(accumulator, deltas)
    ...
    ProtogalaxyProver_<ProverInstances>::combiner_quotient_round(
        const std::vector<FF>& gate_challenges, const std::vector<FF>& deltas, const ProverInstances& instances)
    {
