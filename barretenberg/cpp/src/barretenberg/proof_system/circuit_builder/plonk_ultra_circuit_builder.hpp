/**
 * @brief This file contains a frozen-in-time copy of the UltraCircuitBuilder class.
 * @details This duplicate class is being introduced because the requirements for Honk are changing rapidly and it is
 * too much of a hassle to ensure Plonk stays compatible with changes in the circuit builders. Since Plonk is soon to be
 * deprecated entirely, having a duplicate makes things easier and avoids hacks in production code. Intended for use
 * only with plonk and should be deleted along with the rest of plonk when the time comes.
 *
 */
#pragma once
...
class PlonkUltraCircuitBuilder_ : public CircuitBuilderBase<typename Arithmetization_::FF> {
  public:
    ...
    PlonkUltraCircuitBuilder_(const size_t size_hint = 0)
     ...
     */
    PlonkUltraCircuitBuilder_(const size_t size_hint,
                              auto& witness_values,
    ...
    PlonkUltraCircuitBuilder_(const PlonkUltraCircuitBuilder_& other) = default;
    PlonkUltraCircuitBuilder_(PlonkUltraCircuitBuilder_&& other)
    ...
    PlonkUltraCircuitBuilder_& operator=(const PlonkUltraCircuitBuilder_& other) = default;
    PlonkUltraCircuitBuilder_& operator=(PlonkUltraCircuitBuilder_&& other)
    {
    ...
    ~PlonkUltraCircuitBuilder_() override = default;

    bool operator==(const PlonkUltraCircuitBuilder_& other) const = default;
...
using PlonkUltraCircuitBuilder = PlonkUltraCircuitBuilder_<UltraArith<bb::fr>>;
} // namespace bb
