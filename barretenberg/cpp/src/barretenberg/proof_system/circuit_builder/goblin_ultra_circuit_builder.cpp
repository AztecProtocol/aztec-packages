...
template <typename FF> void GoblinUltraCircuitBuilder_<FF>::add_gates_to_ensure_all_polys_are_non_zero()
{
    ...
    // WORKTODO: all of this turned off for now wile debugging basic arith gate failure

    // // All that remains is to handle databus related and poseidon2 related polynomials. In what follows we populate
    // the
