...
template <typename FF> class GoblinUltraCircuitBuilder_ : public UltraCircuitBuilder_<arithmetization::UltraHonk<FF>> {
  ...
  public:
    ...
    GoblinUltraCircuitBuilder_(std::shared_ptr<ECCOpQueue> op_queue_in,
        ...
        , op_queue(op_queue_in)
    {
        // WORKTODO: NOTE: This still works even though the witness indices in the explicit acir gates are +1 offset
        // because we're still adding the const 0 before anything else via the UCB constructor. Once we remove the +1
        // from Noir, we'll need to update the UCB by moving that const 0 to get these tests to pass again.
