...
template <typename FF_> class LookupRelationImpl {
  public:
    ...
    inline static Accumulator new_compute_grand_product_numerator(const auto& new_term,
    ...
    template <typename Accumulator>
    inline static Accumulator new_compute_grand_product_denominator(const auto& extended_edges,
                                                                    const RelationParameters<FF>& relation_parameters)
    {
                               ...
                               const FF& scaling_factor)
    {
        ...
        [&]() {
            ...
            const auto lhs = new_compute_grand_product_numerator<Accumulator>(new_term, relation_parameters);
            const auto rhs = new_compute_grand_product_denominator<Accumulator>(new_term, relation_parameters);
