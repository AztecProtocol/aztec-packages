...
// IPA specifically
 ...
 .   *5. Compute the vector \f$\vec{b}_{k}=(1,\beta,\beta^2,...,\beta^{d-1})\f$ where \f$p(\beta)$\f is the
     ...
     * @details Contains all intermediate values computed during steps 2–7, 9 of the IPA verification protocol.
     ...
     *
     * @details Performs steps 2–7 and 9 of the IPA verification protocol.
    ...
    /**
     * @brief Batch-verify multiple IPA proofs with a single SRS MSM.
     *
     * @details Processes each proof's transcript independently (cheap per-proof work),
     * then combines all proofs using random challenge alpha and performs one large MSM.
     *
