 ...
 * also store elements received from the prover. It also has symbolic hash buffers. (The hash buffers exist only because
 * ipa uses them, as they are methods for the `Transcript` class.)
 *
 */
class MockTranscript {
  public:
    ...
    // Vector of group elements added to the hash buffer.
    std::vector<bb::curve::Grumpkin::AffineElement> hash_buffer_group_elements;
    // Vector of field elements added to the hash buffer. uint256_t is used to ignore field type
    ...
    /**
     * @brief Add something to the hash buffer(s).
     *
     */
    template <typename T> void add_to_hash_buffer(const std::string&, const T& element)
    {
        // GCC breaks explicit specialization, so I have to do this
        if constexpr (std::is_same_v<bb::curve::Grumpkin::AffineElement, T>) {
