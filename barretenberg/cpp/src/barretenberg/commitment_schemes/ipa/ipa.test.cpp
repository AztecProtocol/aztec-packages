...
TEST_F(IPATest, ShpleminiIPAWithShift)
{
    ...
    auto shplonk_verifier_claim = ShpleminiVerifier::compute_opening_claim_for_ipa(
        shplemini_output.commitments, shplemini_output.scalars, shplemini_output.evaluation_point);
    auto result = IPA::reduce_verify(this->vk(), shplonk_verifier_claim, verifier_transcript);
