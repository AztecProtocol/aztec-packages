...
template <class PCS> class ZeroMorphTest : public CommitmentTest<typename PCS::Curve> {
  public:
    ...
    using PolynomialAndEvaluationsTuple

        // Evaluate Phi_k(x) = \sum_{i=0}^k x^i using the direct inefficent formula
        Fr
        Phi(Fr challenge, size_t subscript)
    {
     ...
     *
     */
    bool execute_zeromorph_protocol(size_t NUM_UNSHIFTED, size_t NUM_SHIFTED, size_t NUM_CONCATENATED = 0)
    {
        size_t N = 2;
