...
TYPED_TEST(ShpleminiTest, ShpleminiProverAndVerifierTest)
{
    if constexpr (std::is_same_v<TypeParam, curve::Grumpkin>) {
        GTEST_SKIP() << "Test only runs for curve::BN254";
    } else {
        ...
        Fr masking_term1 = Fr::get_random();
        ...
        for (size_t idx = 0; idx < log_n; idx++) {
            ...
            ASSERT_EQ(libra_polynomial.evaluate(mle_opening_point[idx]),
                      libra_monomial.evaluate(mle_opening_point[idx]));
        ...
        auto opening_claim = ShpleminiProver::template prove<NativeTranscript, LIBRA_UNIVARIATE_LENGTH>(
            Fr{ n },
        ...
        const Fr shplonk_batching_challenge = batch_opening_claim.batching_challenge;

        Fr qudatratic_sum = Fr{ 0 };
        for (size_t idx = 0; idx < n; idx++) {
            qudatratic_sum = mle_opening_point[idx](Fr(1) - mle_opening_point[idx]);
        }

        ShpleminiVerifier::add_zk_data(batch_opening_claim,
                                       RefVector(libra_commitments),
                                       libra_evaluations,
                                       {},
                                       mle_opening_point,
                                       /* quadratic challenge */ Fr(0),
                                       /* shplonk_batching_challenge =*/shplonk_batching_challenge,
                                       /* shplonk_evaluation_challenge = */ shplonk_eval_challenge,
