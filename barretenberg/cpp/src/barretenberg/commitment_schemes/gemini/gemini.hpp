...
template <typename Curve> class GeminiProver_ {
  ...
  public:
     ...
     * combinations (based on challenge rho) of the unshifted and to-be-shifted polynomials by F and G
     * respectively, then A₀ = F + G/X^k. This polynomial is "folded" in Gemini to produce d-1 univariate polynomials
     * Fold_i, i = 1, ..., d-1. The second and third are the partially evaluated batched polynomials
     * A₀₊ = F + G/r^k, and A₀₋ = F + (-1)^k · G/r^k. These are required in order to prove the opening of shifted
     * polynomials G_i/X^k from the commitments to their unshifted counterparts G_i.
    ...
    class PolynomialBatcher {
      ...
      public:
        ...
        /**
         * @brief Compute batched polynomial A₀ = F + G/X^k as the linear combination of all polynomials to be opened,
         * where F is the linear combination of the unshifted polynomials, G is the linear combination of the
         * to-be-shifted polynomials, and k is the shift_exponent.
         *
        ...
        Polynomial compute_batched(const Fr& challenge)
        {
            ...
            if (!batched_shifted_tail_.is_empty()) {
                full_batched += batched_shifted_tail_.shifted(shift_exponent);
            }
         ...
         *   A₀₋(X) = F(X) + (-1)^k · G(X)/r^k
         *
         * @note When k is even (e.g. k=4 for interleaved shifts), A₀₊ = A₀₋. This means the prover
         * computes the same polynomial twice. A future optimization could detect this and avoid the
         * redundant computation and commitment opening.
         *
        ...
        std::pair<Polynomial, Polynomial> compute_partially_evaluated_batch_polynomials(const Fr& r_challenge)
        {
            ...
            if (has_to_be_shifted()) {
                ...
                // since (-r)^k = (-1)^k · r^k. For odd k: subtract. For even k: add.
                if (shift_exponent % 2 == 0) {
                    A_0_neg += batched_to_be_shifted; // A₀₋ += G/r^k (even shift)
                } else {
                    A_0_neg -= batched_to_be_shifted; // A₀₋ -= G/r^k (odd shift)
                }
            }
            if (!batched_shifted_tail_.is_empty()) {
