...
template <typename Fr> class DensePolynomial {
  public:
    ...
    DensePolynomial(DensePolynomial&& other) noexcept;

    // Create a polynomial from the given fields.
    DensePolynomial(std::span<const Fr> coefficients);
     ...
     */
    DensePolynomial(std::span<const Fr> interpolation_points, std::span<const Fr> evaluations);

    // move assignment
    DensePolynomial& operator=(DensePolynomial&& other) noexcept;
    DensePolynomial& operator=(std::span<const Fr> coefficients) noexcept;
    DensePolynomial& operator=(const DensePolynomial& other);
    ~DensePolynomial() = default;

    /**
     * Return a shallow clone of the polynomial. i.e. underlying memory is shared.
     */
    DensePolynomial share() const;
    ...
    bool is_zero()
    {
        if (is_empty()) {
            ASSERT(false);
            info("Checking is_zero on an empty DensePolynomial!");
        }
    ...
    bool operator==(DensePolynomial const& rhs) const;
     ...
     * @param other q(X)
     */
    DensePolynomial& operator+=(std::span<const Fr> other);
     ...
     */
    DensePolynomial& operator*=(Fr scaling_factor);
     ...
     */
    DensePolynomial<Fr> partial_evaluate_mle(std::span<const Fr> evaluation_points) const;
    ...
    static DensePolynomial random(const size_t num_coeffs)
    {
        DensePolynomial p(num_coeffs);
...
template <typename Fr> inline std::ostream& operator<<(std::ostream& os, DensePolynomial<Fr> const& p)
{
    if (p.size() == 0) {
        return os << "[]";
    }
...
/**
 * The static_assert below ensure that that our DensePolynomial class correctly models an
 * `std::ranges::contiguous_range`, and other requirements that allow us to convert a `DensePolynomial<Fr>` to a
 * `std::span<const Fr>`.
 *
