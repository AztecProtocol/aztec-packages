                                   ...
                                   std::string const& class_name)
{
    ...
    os <<
        "            mstore(add(_vk, 0x280), " << (key->contains_pairing_point_accum ? "0x01" : "0x00") << ") // vk.contains_pairing_point_accum\n"
        "            mstore(add(_vk, 0x2a0), " << (key->contains_pairing_point_accum ? key->pairing_point_accumulator_public_input_indices[0] : 0) << ") // vk.pairing_point_accumulator_public_input_indices\n"
...
inline void output_vk_sol_ultra(std::ostream& os, std::shared_ptr<plonk::verification_key> const& key, std::string const& class_name)
{
    ...
    os <<
        "            mstore(add(_vk, 0x640), " << (key->contains_pairing_point_accum ? "0x01" : "0x00") << ") // vk.contains_pairing_point_accum\n"
        "            mstore(add(_vk, 0x660), " << (key->contains_pairing_point_accum ? key->pairing_point_accumulator_public_input_indices[0] : 0) << ") // vk.pairing_point_accumulator_public_input_indices\n"
