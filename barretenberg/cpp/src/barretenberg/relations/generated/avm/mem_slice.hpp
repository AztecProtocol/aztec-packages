...
template <typename FF_> class mem_sliceImpl {
  public:
                           ...
                           [[maybe_unused]] const FF& scaling_factor)
    {
        ...
        {
            Avm_DECLARE_VIEWS(0);
            auto tmp = (slice_sel_cd_cpy * (-slice_sel_cd_cpy + FF(1)));
            tmp *= scaling_factor;
            std::get<0>(evals) += tmp;
        }
        // Contribution 1
        {
            Avm_DECLARE_VIEWS(1);
            auto tmp = (slice_sel_start_cd_cpy * (-slice_sel_start_cd_cpy + FF(1)));
            tmp *= scaling_factor;
            std::get<1>(evals) += tmp;
        }
        // Contribution 2
        {
            Avm_DECLARE_VIEWS(2);
            auto tmp = ((slice_cnt * (-slice_one_min_inv + FF(1))) - slice_sel_cd_cpy);
            tmp *= scaling_factor;
            std::get<2>(evals) += tmp;
        }
        // Contribution 3
        {
            Avm_DECLARE_VIEWS(3);
            auto tmp = ((-slice_sel_cd_cpy + FF(1)) * slice_one_min_inv);
            tmp *= scaling_factor;
            std::get<3>(evals) += tmp;
        }
        // Contribution 4
        {
            Avm_DECLARE_VIEWS(4);
            auto tmp = (slice_sel_cd_cpy * ((slice_cnt - FF(1)) - slice_cnt_shift));
            tmp *= scaling_factor;
            std::get<4>(evals) += tmp;
        }
        // Contribution 5
        {
            Avm_DECLARE_VIEWS(5);
            auto tmp = (slice_sel_cd_cpy * ((slice_addr + FF(1)) - slice_addr_shift));
            tmp *= scaling_factor;
            std::get<5>(evals) += tmp;
        }
        // Contribution 6
        {
            Avm_DECLARE_VIEWS(6);
            auto tmp = (slice_sel_cd_cpy * ((slice_cd_offset + FF(1)) - slice_cd_offset_shift));
            tmp *= scaling_factor;
            std::get<6>(evals) += tmp;
        }
    }
