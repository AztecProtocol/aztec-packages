 ...
 * permutation, but has a one-to-many relationship between elements that are being looked up and the table of values
 * they are being looked up from. In this relation template we use the following terminology:
