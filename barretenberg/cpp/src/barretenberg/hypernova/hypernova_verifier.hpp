...
template <typename Flavor_> class HypernovaFoldingVerifier {
  public:
    ...
    // Pass the instance flavor (Flavor_) to the batching verifier so it uses the correct
    // SumcheckOutput and VerifierCommitments types for the circuit being verified.
