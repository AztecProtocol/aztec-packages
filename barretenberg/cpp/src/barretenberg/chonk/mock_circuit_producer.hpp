...
class PrivateFunctionExecutionMockCircuitProducer {
  ...
  public:
    ...
    /**
     * @brief Construct a producer with an explicit app/kernel pattern for the leading circuits.
     *
     * @param leading_is_kernel_flags specifies the layout of the circuits in the execution stack
     */
    PrivateFunctionExecutionMockCircuitProducer(std::vector<bool> leading_is_kernel_flags, bool large_first_app = true)
        : is_kernel_flags(std::move(leading_is_kernel_flags))
        , large_first_app(large_first_app)
    {
        BB_ASSERT(!is_kernel_flags.empty() && !is_kernel_flags.front(),
                  "First circuit must be an app for the accumulator to initialise correctly.");
        for (size_t i = 0; i < NUM_TRAILING_KERNELS; ++i) {
            is_kernel_flags.emplace_back(true);
        }
