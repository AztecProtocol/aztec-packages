...
TEST_F(ChonkBatchVerifierServiceTests, FullServiceFIFOStreaming)
{
    auto [proof, vk] = generate_proof();
    std::vector<std::shared_ptr<MegaZKFlavor::VKAndHash>> vks = { vk };
    ...
    // Queue proofs (will batch together)
    service.queue(VerifyRequest{ .request_id = 1, .proof = proof, .vk_index = 0 });
    service.queue(VerifyRequest{ .request_id = 2, .proof = proof, .vk_index = 0 });
    service.queue(VerifyRequest{ .request_id = 3, .proof = proof, .vk_index = 0 });
