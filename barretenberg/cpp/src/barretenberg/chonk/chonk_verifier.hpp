...
template <bool IsRecursive> class ChonkVerifier {
    ...
    using GoblinVerifier = std::conditional_t<IsRecursive, GoblinRecursiveVerifier, GoblinVerifier>;
    using HidingKernelVerifier = std::conditional_t<IsRecursive, MegaZKRecursiveVerifier, MegaZKVerifier>;
    using Transcript = GoblinVerifier::Transcript;
  ...
  public:
    ...
    ChonkVerifier(const std::shared_ptr<RecursiveVKAndHash>& stdlib_mega_vk_and_hash)
