 ...
 *
 * @details This is the verifier counterpart to `accumulate()` (prover side). While `accumulate()` creates
...
void Chonk::complete_kernel_circuit_logic(ClientCircuit& circuit)
{
    ...
    // Transcript shared across recursive verification of K_{i-1} (kernel) and A_{i} (app)
    ...
    // ZK CRITICAL: Tail kernel adds masking at op queue start
