...
struct BatchVerifierConfig {
    /** Number of cores dedicated to trusted batch verification. */
    uint32_t num_trusted_cores = 8;
...
struct VerifyResult {
    ...
    MSGPACK_FIELDS(
        request_id, verified, status, error_message, source, time_in_queue_ms, time_in_verify_ms, batch_failure_count);
};
