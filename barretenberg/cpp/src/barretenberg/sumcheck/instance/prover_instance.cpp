    ...
    requires IsGoblinFlavor<Flavor>
{
    ...
    for (size_t idx = 0; idx < calldata.size(); ++idx) {
        public_calldata.set(idx, circuit.get_variable(calldata[idx]));      // calldata values
        calldata_read_counts.set(idx, calldata.get_read_count(idx));        // read counts
        calldata_read_tags.set(idx, calldata_read_counts[idx] > 0 ? 1 : 0); // has row been read or not
    }
    for (size_t idx = 0; idx < secondary_calldata.size(); ++idx) {
        public_secondary_calldata.set(idx, circuit.get_variable(secondary_calldata[idx])); // secondary_calldata values
        secondary_calldata_read_counts.set(idx, secondary_calldata.get_read_count(idx));   // read counts
        secondary_calldata_read_tags.set(idx,
                                         secondary_calldata_read_counts[idx] > 0 ? 1 : 0); // has row been read or not
    }
    for (size_t idx = 0; idx < return_data.size(); ++idx) {
        public_return_data.set(idx, circuit.get_variable(return_data[idx]));      // return data values
        return_data_read_counts.set(idx, return_data.get_read_count(idx));        // read counts
        return_data_read_tags.set(idx, return_data_read_counts[idx] > 0 ? 1 : 0); // has row been read or not
    }
