    ...
    */
                                    ...
                                    std::vector<std::array<FF, 3>>& round_univariate_evaluations)
    {
        ...
        if (round_idx > 0) {
            ...
            *@param round_univariate Sumcheck Round Univariate* @param eval_domain{
                0, 1, ..., BATCHED_RELATION_PARTIAL_LENGTH - 1
            } *
                @param transcript* @param ck Commitment key of size
                    BATCHED_RELATION_PARTIAL_LENGTH* @param round_univariates Auxiliary container to be fed to
                        Shplemini* @param round_univariate_evaluations Auxiliary container to be fed to Shplemini* /
                void commit_to_round_univariate(const size_t round_idx,
                                                bb::Univariate<FF, BATCHED_RELATION_PARTIAL_LENGTH>& round_univariate,
                                                const std::vector<FF>& eval_domain,
                                                const std::shared_ptr<Transcript>& transcript,
         ...
         * - Extract Round Univariate's \f$\tilde{F}\f$ evaluations at \f$0,\ldots, D \f$ from the transcript using \ref
         bb::BaseTranscript::receive_from_prover "receive_from_prover" method from \ref bb::BaseTranscript<
         TranscriptParams > "Base Transcript Class".
         * - \ref bb::SumcheckVerifierRound< Flavor >::check_sum "Check target sum": \f$\quad \sigma_{
         i } \stackrel{?}{=}  \tilde{S}^i(0) + \tilde{S}^i(1)  \f$
         * - Compute the challenge \f$u_i\f$ from the transcript using \ref bb::BaseTranscript::get_challenge
         "get_challenge" method.
         * - \ref bb::SumcheckVerifierRound< Flavor >::compute_next_target_sum "Compute next target sum" :\f$ \quad
         \sigma_{i+1} \gets \tilde{S}^i(u_i) \f$
         * ### Verifier's Data before Final Step
         * Entering the final round, the Verifier has already checked that \f$\quad \sigma_{ d-1 } =
         \tilde{S}^{d-2}(u_{d-2}) \stackrel{?}{=}  \tilde{S}^{d-1}(0) + \tilde{S}^{d-1}(1)  \f$ and computed \f$\sigma_d
         = \tilde{S}^{d-1}(u_{d-1})\f$.
         ...
         * - Compare \f$ \sigma_d \f$ against the evaluation of \f$ \tilde{F} \f$ at \f$P_1(u_0,\ldots, u_{d-1}),
         \ldots, P_N(u_0,\ldots, u_{d-1})\f$:
         * \f{align}{\quad  \sigma_{ d } \stackrel{?}{=} \tilde{F}\left(P_1(u_{0}, \ldots, u_{d-1}),\ldots,
         P_N(u_0,\ldots, u_{d-1})\right)\f}
        ...
        template <typename Flavor> class SumcheckVerifier {
          ...
          public:
            ...
            using FF = typename Flavor::FF;
            /**
             * @brief Container type for the evaluations of Prover Polynomials \f$P_1,\ldots,P_N\f$ at the challenge
             * point \f$(u_0,\ldots, u_{d-1}) \f$.
             *
             */
            using ClaimedEvaluations = typename Flavor::AllValues;
            ...
            // Verifier instantiates sumcheck with circuit size, optionally a different target sum than 0 can be
            // specified.
             ...
             * final round, then use purported evaluations to generate purported full Honk relation value and check
             * against final target sum.
             *
                                          ...
                                          const std::vector<FF>& gate_challenges)
            {
                ...
                // TODO(https://github.com/AztecProtocol/barretenberg/issues/1144): Add proper constraints for taking
                // the log of a field_t link multivariate_d.
                if (multivariate_d == 0) {
                ...
                for (size_t round_idx = 0; round_idx < CONST_PROOF_SIZE_LOG_N; round_idx++) {
                    ...
                    round_univariate = round_univariate =
                    ...
                    if constexpr (IsRecursiveFlavor<Flavor>) {
                        ...
                        // TODO(https://github.com/AztecProtocol/barretenberg/issues/1114): insecure dummy_round
                        // derivation!
                ...
                // Extract claimed evaluations of Libra univariates and compute their sum multiplied by the Libra
                ...
                if constexpr (Flavor::HasZK) {
                    ...
                    if constexpr (!IsRecursiveFlavor<Flavor>) {
                        ...
                        correcting_factor = RowDisablingPolynomial<FF>::evaluate_at_challenge(
                            multivariate_challenge, multivariate_d, builder);
                    }
             ...
             * These evaluations will be proved as a part of Shplemini. The only check that the Verifier performs in
             * this version is the comparison of the target sumcheck sum with the claimed evaluations of the first
             * sumcheck round univariate at 0 and 1.
             *
             * Note that the SumcheckOutput in this case contains a vector of commitments and a vector of arrays (of
             * size 3) of evaluations at 0, 1, and a round challenge.
             *
             * @param relation_parameters
             * @param alpha
             * @param gate_challenges
                ...
                requires IsGrumpkinFlavor<Flavor>
            {
                ...
                for (size_t round_idx = 0; round_idx < CONST_PROOF_SIZE_LOG_N; round_idx++) {
                    // Obtain the round univariate from the transcript
                    const std::string round_univariate_comm_label =
                        "Sumcheck:univariate_comm_" + std::to_string(round_idx);
                    const std::string univariate_eval_label_0 =
                        "Sumcheck:univariate_" + std::to_string(round_idx) + "_eval_0";
                    const std::string univariate_eval_label_1 =
                        "Sumcheck:univariate_" + std::to_string(round_idx) + "_eval_1";
                ...
                if constexpr (IsRecursiveFlavor<Flavor>) {
                    ...
                    const FF correcting_factor = RowDisablingPolynomial<FF>::evaluate_at_challenge(
                        multivariate_challenge, multivariate_d, builder);

                    // Verifier computes full ZK Honk value, taking into account the contribution from the disabled row
                    // and the Libra polynomials
                    full_honk_purported_value =
                        full_honk_purported_value * correcting_factor + libra_evaluation * libra_challenge;
                    ...
                    // Ensure that the sum of the evaluations of the first Sumcheck Round Univariate is equal to the
                    // claimed target total sum
