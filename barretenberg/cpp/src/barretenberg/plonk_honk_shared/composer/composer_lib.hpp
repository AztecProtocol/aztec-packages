                                  ...
                                  size_t dyadic_circuit_size)
{
    ...
    read_counts = typename Flavor::Polynomial(
        circuit.get_tables_size(), dyadic_circuit_size, /*start index*/ static_cast<std::ptrdiff_t>(offset));
    // TODO(AD) we dont need to initialize read_tags
    read_tags = typename Flavor::Polynomial(
        circuit.get_tables_size(), dyadic_circuit_size, /*start index*/ static_cast<std::ptrdiff_t>(offset));
