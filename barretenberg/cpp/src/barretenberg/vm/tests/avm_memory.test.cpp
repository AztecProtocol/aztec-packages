...
{
    trace_builder.calldata_copy(false, 0, 2, 0, std::vector<FF>{ 98, 12 });
...
TEST_F(AvmMemoryTests, readWriteConsistencyValViolation)
{
    ...
    trace_builder.return_op(false, 2, 1);            // Return single memory word at position 2 (36)
    auto trace = trace_builder.finalize();
...
{
    trace_builder.return_op(false, 1, 1); // Return single memory word at position 1
    auto trace = trace_builder.finalize();
...
{
    trace_builder.calldata_copy(false, 0, 2, 0, std::vector<FF>{ 84, 7 });
