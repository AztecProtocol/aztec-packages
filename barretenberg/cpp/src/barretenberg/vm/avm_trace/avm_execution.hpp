...
class Execution {
  public:
    ...
    static std::tuple<AvmFlavor::VerificationKey, bb::HonkProof> prove_vk(std::vector<uint8_t> const& bytecode,
                                                                          std::vector<FF> const& calldata = {});
    static bool verify(AvmFlavor::VerificationKey vk, const HonkProof& proof);
};
