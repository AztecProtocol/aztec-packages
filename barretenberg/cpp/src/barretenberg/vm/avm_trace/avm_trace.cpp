                                                                     ...
                                                                     bool is_gadget_mem_op)
{
    ...
    auto read_dir = mem_trace_builder.read_and_load_from_memory(
        space_id, clk, reg, direct_offset, read_tag, write_tag, is_gadget_mem_op);
                                                                    ...
                                                                    bool is_gadget_mem_op)
{
    ...
    mem_trace_builder.write_into_memory(
        space_id, clk, reg, direct_offset, value, read_tag, write_tag, is_gadget_mem_op);
...
void AvmTraceBuilder::op_poseidon2_permutation(uint8_t indirect, uint32_t input_offset, uint32_t output_offset)
{
    ...
    // TODO: Properly indirect handling for poseidon2, once we have an agreed-upon approach for mem-aware gadgets
    ...
    auto read_a = constrained_read_from_memory(
        call_ptr, clk, direct_src_offset, AvmMemoryTag::FF, AvmMemoryTag::U0, IntermRegister::IA, true);
    auto read_b = constrained_read_from_memory(
        call_ptr, clk, direct_src_offset + 1, AvmMemoryTag::FF, AvmMemoryTag::U0, IntermRegister::IB, true);
    auto read_c = constrained_read_from_memory(
        call_ptr, clk, direct_src_offset + 2, AvmMemoryTag::FF, AvmMemoryTag::U0, IntermRegister::IC, true);
    auto read_d = constrained_read_from_memory(
        call_ptr, clk, direct_src_offset + 3, AvmMemoryTag::FF, AvmMemoryTag::U0, IntermRegister::ID, true);
    ...
    std::array<FF, 4> result = poseidon2_trace_builder.poseidon2_permutation(
        input, poseidon_op_clk, direct_input_offset, direct_output_offset);
    ...
    constrained_write_to_memory(
        call_ptr, clk, direct_dst_offset, ff_result[0], AvmMemoryTag::U0, AvmMemoryTag::FF, IntermRegister::IA, true);
