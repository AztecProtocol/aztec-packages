...
void Stats::time(const std::string& key, std::function<void()> f)
{
    ...
    increment(key, static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(ellapsed).count()));
}
...
std::string Stats::aggregate_to_string(const std::string& key_path) const
{
    uint64_t result = 0;
    for (const auto& [key, value] : stats) {
        if (key.starts_with(key_path)) {
            result += value;
        }
    }
    return key_path + ": " + std::to_string(result);
}
