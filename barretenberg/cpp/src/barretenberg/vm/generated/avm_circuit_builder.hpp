...
class AvmCircuitBuilder {
  public:
    ...
    static constexpr size_t num_fixed_columns = 298;
    static constexpr size_t num_polys = 256;
    std::vector<Row> rows;
    ...
    [[maybe_unused]] bool check_circuit()
    {
        ...
        if (!perm_main_mem_ind_a()) {
            return false;
        };
        if (!perm_main_mem_ind_b()) {
            return false;
        };
        if (!perm_main_mem_ind_c()) {
            return false;
        };
        if (!perm_main_mem_ind_d()) {
            return false;
        };
        if (!lookup_byte_lengths()) {
            return false;
        };
        if (!lookup_byte_operations()) {
            return false;
        };
        if (!lookup_into_kernel()) {
            return false;
        };
        if (!incl_main_tag_err()) {
            return false;
        };
        if (!incl_mem_tag_err()) {
            return false;
        };
        if (!lookup_mem_rng_chk_lo()) {
            return false;
        };
        if (!lookup_mem_rng_chk_hi()) {
            return false;
        };
        if (!lookup_pow_2_0()) {
            return false;
