...
class CopyComposer {
  public:
    ...
    using CircuitConstructor = CopyCircuitBuilder;
    ...
    static constexpr std::string_view NAME_STRING = "Copy";
    ...
    CopyComposer() { crs_factory_ = bb::srs::get_bn254_crs_factory(); }

    CopyComposer(std::shared_ptr<ProvingKey> p_key, std::shared_ptr<VerificationKey> v_key)
        : proving_key(std::move(p_key))
        , verification_key(std::move(v_key))
    {}

    CopyComposer(CopyComposer&& other) noexcept = default;
    CopyComposer(CopyComposer const& other) noexcept = default;
    CopyComposer& operator=(CopyComposer&& other) noexcept = default;
    CopyComposer& operator=(CopyComposer const& other) noexcept = default;
    ~CopyComposer() = default;
    ...
    CopyProver create_prover(CircuitConstructor& circuit_constructor);
    CopyVerifier create_verifier(CircuitConstructor& circuit_constructor);
