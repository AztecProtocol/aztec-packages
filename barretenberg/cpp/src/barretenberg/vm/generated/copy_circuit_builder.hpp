...
class CopyCircuitBuilder {
  public:
    ...
    ProverPolynomials compute_polynomials()
    {
        ...
        polys.copy_main_shift = Polynomial(polys.copy_main.shifted());
    ...
    [[maybe_unused]] bool check_circuit()
    {
        ...
        const auto evaluate_grand_product = [&]<typename GrandProductSettings>(const std::string& copy_name) {
            ...
            for (size_t i = 0; i < num_rows; ++i) {
                ...
                info("rel 0: ", grand_product_result[0]);
                info();
            }
            for (auto r : grand_product_result) {
                if (r != 0) {
                    throw_or_abort(format("Copy ", copy_name, " failed."));
                    return false;
                }
            }
            return true;
