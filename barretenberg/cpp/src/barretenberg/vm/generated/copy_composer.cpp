...
void CopyComposer::compute_witness(CircuitConstructor& circuit)
{
    if (computed_witness) {
        return;
    }
...
CopyProver CopyComposer::create_prover(CircuitConstructor& circuit_constructor)
{
    compute_proving_key(circuit_constructor);
    compute_witness(circuit_constructor);
    compute_commitment_key(circuit_constructor.get_circuit_subgroup_size());

    CopyProver output_state(proving_key, proving_key->commitment_key);

    return output_state;
}

CopyVerifier CopyComposer::create_verifier(CircuitConstructor& circuit_constructor)
{
    auto verification_key = compute_verification_key(circuit_constructor);

    CopyVerifier output_state(verification_key);
...
std::shared_ptr<Flavor::ProvingKey> CopyComposer::compute_proving_key(CircuitConstructor& circuit_constructor)
{
    if (proving_key) {
        return proving_key;
    }
...
std::shared_ptr<Flavor::VerificationKey> CopyComposer::compute_verification_key(CircuitConstructor& circuit_constructor)
{
    if (verification_key) {
