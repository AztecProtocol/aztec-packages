...
TEST(AvmMerkleTree, shouldCheckMembership)
{
    ...
    // Generate some random values that result in a root
    // std::random_device r;
    // std::default_random_engine e1(r());
    // std::uniform_int_distribution<int> uniform_dist(0, 5000);
    ...
    ASSERT_TRUE(found);
    bool not_found = merkle_tree_builder.check_membership(1, initial_value, leaf_index + 1, sibling_path, root);
