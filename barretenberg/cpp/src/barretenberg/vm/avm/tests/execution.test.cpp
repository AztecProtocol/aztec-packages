...
TEST_F(AvmExecutionTests, opGetContractInstanceOpcodes)
{
    ...
    const auto execution_hints = ExecutionHints().with_contract_instance_hints({ { address, { address, exists, 2, deployer, class_id, init_hash, public_keys_hints } } });

    std::string bytecode_hex = to_hex(OpCode::GETCONTRACTINSTANCE) + // opcode GETCONTRACTINSTANCE
                               "00"                                  // Indirect flag
                               + to_hex(static_cast<uint8_t>(ContractInstanceMember::DEPLOYER)) + // member enum
                               "00000001"                            // address offset
                               "00000010"                            // dst offset
                               "00000011"                            // exists offset
    ...
    EXPECT_THAT(instructions.at(1),
                AllOf(Field(&Instruction::op_code, OpCode::GETCONTRACTINSTANCE),
                      Field(&Instruction::operands,
                            ElementsAre(VariantWith<uint8_t>(2),
                                        VariantWith<uint8_t>(static_cast<uint8_t>(ContractInstanceMember::DEPLOYER)),
                                        VariantWith<uint16_t>(1),
                                        VariantWith<uint16_t>(10),
                                        VariantWith<uint16_t>(11)))));
    EXPECT_THAT(instructions.at(2),
                AllOf(Field(&Instruction::op_code, OpCode::GETCONTRACTINSTANCE),
                      Field(&Instruction::operands,
                            ElementsAre(VariantWith<uint8_t>(2),
                                        VariantWith<uint8_t>(static_cast<uint8_t>(ContractInstanceMember::CLASS_ID)),
                                        VariantWith<uint16_t>(1),
                                        VariantWith<uint16_t>(12),
                                        VariantWith<uint16_t>(13)))));

    EXPECT_THAT(instructions.at(3),
                AllOf(Field(&Instruction::op_code, OpCode::GETCONTRACTINSTANCE),
                      Field(&Instruction::operands,
                            ElementsAre(VariantWith<uint8_t>(2),
                                        VariantWith<uint8_t>(static_cast<uint8_t>(ContractInstanceMember::INIT_HASH)),
                                        VariantWith<uint16_t>(1),
                                        VariantWith<uint16_t>(14),
                                        VariantWith<uint16_t>(15)))));
