...
template <typename FF_> class memImpl {
  public:
    ...
    {
        const auto alias_mem_SEL_DIRECT_MEM_OP_A =
            ((new_term.mem_sel_op_a + new_term.mem_sel_op_poseidon_read_a) + new_term.mem_sel_op_poseidon_write_a);
        const auto alias_mem_SEL_DIRECT_MEM_OP_B =
            ((new_term.mem_sel_op_b + new_term.mem_sel_op_poseidon_read_b) + new_term.mem_sel_op_poseidon_write_b);
        const auto alias_mem_SEL_DIRECT_MEM_OP_C =
            ((new_term.mem_sel_op_c + new_term.mem_sel_op_poseidon_read_c) + new_term.mem_sel_op_poseidon_write_c);
        const auto alias_mem_SEL_DIRECT_MEM_OP_D =
            ((new_term.mem_sel_op_d + new_term.mem_sel_op_poseidon_read_d) + new_term.mem_sel_op_poseidon_write_d);
        const auto alias_mem_NUM_SUB_CLK = FF(12);
        const auto alias_mem_IND_OP = (((new_term.mem_sel_resolve_ind_addr_a + new_term.mem_sel_resolve_ind_addr_b) +
                                        new_term.mem_sel_resolve_ind_addr_c) +
                                       new_term.mem_sel_resolve_ind_addr_d);
        const auto alias_mem_SUB_CLK =
            (new_term.mem_sel_mem *
             ((((new_term.mem_sel_resolve_ind_addr_b + alias_mem_SEL_DIRECT_MEM_OP_B) +
                (FF(2) * (new_term.mem_sel_resolve_ind_addr_c + alias_mem_SEL_DIRECT_MEM_OP_C))) +
               (FF(3) * (new_term.mem_sel_resolve_ind_addr_d + alias_mem_SEL_DIRECT_MEM_OP_D))) +
              (FF(4) * ((FF(1) - alias_mem_IND_OP) + new_term.mem_rw))));
        const auto alias_mem_DIFF =
        ...
        }
        {
            ...
            auto tmp = (new_term.mem_sel_mem - ((((((((alias_mem_SEL_DIRECT_MEM_OP_A + alias_mem_SEL_DIRECT_MEM_OP_B) +
                                                      alias_mem_SEL_DIRECT_MEM_OP_C) +
                                                     alias_mem_SEL_DIRECT_MEM_OP_D) +
        ...
        }
        {
            ...
            auto tmp = (new_term.mem_tsp - ((alias_mem_NUM_SUB_CLK * new_term.mem_clk) + alias_mem_SUB_CLK));
            tmp *= scaling_factor;
        ...
        }
        {
            ...
            auto tmp =
                (new_term.mem_sel_rng_chk *
                 (((alias_mem_DIFF - (new_term.mem_diff_hi * FF(4294967296UL))) - (new_term.mem_diff_mid * FF(65536))) -
