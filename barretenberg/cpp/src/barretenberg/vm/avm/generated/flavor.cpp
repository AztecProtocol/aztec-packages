...
AvmFlavor::AllConstRefValues::AllConstRefValues(
    ...
    , perm_main_slice_inv(il[731])
    , perm_main_mem_a_inv(il[732])
    , perm_main_mem_b_inv(il[733])
    , perm_main_mem_c_inv(il[734])
    , perm_main_mem_d_inv(il[735])
    , perm_main_mem_ind_addr_a_inv(il[736])
    , perm_main_mem_ind_addr_b_inv(il[737])
    , perm_main_mem_ind_addr_c_inv(il[738])
    , perm_main_mem_ind_addr_d_inv(il[739])
    , lookup_rng_chk_pow_2_inv(il[740])
    , lookup_rng_chk_diff_inv(il[741])
    , lookup_rng_chk_0_inv(il[742])
    , lookup_rng_chk_1_inv(il[743])
    , lookup_rng_chk_2_inv(il[744])
    , lookup_rng_chk_3_inv(il[745])
    , lookup_rng_chk_4_inv(il[746])
    , lookup_rng_chk_5_inv(il[747])
    , lookup_rng_chk_6_inv(il[748])
    , lookup_rng_chk_7_inv(il[749])
    , lookup_mem_rng_chk_0_inv(il[750])
    , lookup_mem_rng_chk_1_inv(il[751])
    , lookup_mem_rng_chk_2_inv(il[752])
    ...
    , lookup_l2_gas_rng_chk_0_inv(il[758])
    , lookup_l2_gas_rng_chk_1_inv(il[759])
    , lookup_da_gas_rng_chk_0_inv(il[760])
    , lookup_da_gas_rng_chk_1_inv(il[761])
    ...
    , main_internal_return_ptr_shift(il[783])
    , main_l2_gas_remaining_shift(il[784])
    , main_pc_shift(il[785])
    , main_sel_execution_end_shift(il[786])
    , main_sel_execution_row_shift(il[787])
    , mem_glob_addr_shift(il[788])
    , mem_rw_shift(il[789])
    , mem_sel_mem_shift(il[790])
    , mem_tag_shift(il[791])
    , mem_tsp_shift(il[792])
    , mem_val_shift(il[793])
    , merkle_tree_leaf_index_shift(il[794])
    , merkle_tree_leaf_value_shift(il[795])
    , merkle_tree_path_len_shift(il[796])
    , poseidon2_full_a_0_shift(il[797])
    , poseidon2_full_a_1_shift(il[798])
    , poseidon2_full_a_2_shift(il[799])
    , poseidon2_full_a_3_shift(il[800])
    , poseidon2_full_execute_poseidon_perm_shift(il[801])
    , poseidon2_full_input_0_shift(il[802])
    , poseidon2_full_input_1_shift(il[803])
    , poseidon2_full_input_2_shift(il[804])
    , poseidon2_full_num_perm_rounds_rem_shift(il[805])
    , poseidon2_full_sel_poseidon_shift(il[806])
    , poseidon2_full_start_poseidon_shift(il[807])
    , slice_addr_shift(il[808])
    , slice_clk_shift(il[809])
    , slice_cnt_shift(il[810])
    , slice_col_offset_shift(il[811])
    , slice_sel_cd_cpy_shift(il[812])
    , slice_sel_mem_active_shift(il[813])
    , slice_sel_return_shift(il[814])
    , slice_sel_start_shift(il[815])
    , slice_space_id_shift(il[816])
{}
