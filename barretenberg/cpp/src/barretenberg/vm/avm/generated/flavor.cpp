...
AvmFlavor::AllConstRefValues::AllConstRefValues(
    ...
    , main_sel_op_static_call(il[229])
    ...
    , main_sel_returndata(il[242])
    , main_sel_rng_16(il[243])
    , main_sel_rng_8(il[244])
    , main_sel_slice_gadget(il[245])
    , main_side_effect_counter(il[246])
    , main_sload_write_offset(il[247])
    , main_space_id(il[248])
    , main_sstore_write_offset(il[249])
    , main_tag_err(il[250])
    , main_w_in_tag(il[251])
    , mem_addr(il[252])
    , mem_clk(il[253])
    , mem_diff(il[254])
    , mem_glob_addr(il[255])
    , mem_last(il[256])
    , mem_lastAccess(il[257])
    , mem_one_min_inv(il[258])
    , mem_r_in_tag(il[259])
    , mem_rw(il[260])
    , mem_sel_mem(il[261])
    , mem_sel_mov_ia_to_ic(il[262])
    , mem_sel_mov_ib_to_ic(il[263])
    , mem_sel_op_a(il[264])
    , mem_sel_op_b(il[265])
    , mem_sel_op_c(il[266])
    , mem_sel_op_cmov(il[267])
    , mem_sel_op_d(il[268])
    , mem_sel_op_poseidon_read_a(il[269])
    , mem_sel_op_poseidon_read_b(il[270])
    , mem_sel_op_poseidon_read_c(il[271])
    , mem_sel_op_poseidon_read_d(il[272])
    , mem_sel_op_poseidon_write_a(il[273])
    , mem_sel_op_poseidon_write_b(il[274])
    , mem_sel_op_poseidon_write_c(il[275])
    , mem_sel_op_poseidon_write_d(il[276])
    , mem_sel_op_slice(il[277])
    , mem_sel_resolve_ind_addr_a(il[278])
    , mem_sel_resolve_ind_addr_b(il[279])
    , mem_sel_resolve_ind_addr_c(il[280])
    , mem_sel_resolve_ind_addr_d(il[281])
    , mem_sel_rng_chk(il[282])
    , mem_skip_check_tag(il[283])
    , mem_space_id(il[284])
    , mem_tag(il[285])
    , mem_tag_err(il[286])
    , mem_tsp(il[287])
    ...
    , mem_w_in_tag(il[289])
    , pedersen_clk(il[290])
    , pedersen_input(il[291])
    , pedersen_output(il[292])
    , pedersen_sel_pedersen(il[293])
    ...
    , perm_pos_mem_read_a_inv(il[654])
    , perm_pos_mem_read_b_inv(il[655])
    , perm_pos_mem_read_c_inv(il[656])
    , perm_pos_mem_read_d_inv(il[657])
    , perm_pos_mem_write_a_inv(il[658])
    , perm_pos_mem_write_b_inv(il[659])
    , perm_pos_mem_write_c_inv(il[660])
    , perm_pos_mem_write_d_inv(il[661])
    , perm_slice_mem_inv(il[662])
    , perm_main_alu_inv(il[663])
    , perm_main_bin_inv(il[664])
    , perm_main_conv_inv(il[665])
    , perm_main_pos2_perm_inv(il[666])
    , perm_main_pedersen_inv(il[667])
    , perm_main_slice_inv(il[668])
    , perm_main_mem_a_inv(il[669])
    , perm_main_mem_b_inv(il[670])
    , perm_main_mem_c_inv(il[671])
    , perm_main_mem_d_inv(il[672])
    , perm_main_mem_ind_addr_a_inv(il[673])
    , perm_main_mem_ind_addr_b_inv(il[674])
    , perm_main_mem_ind_addr_c_inv(il[675])
    , perm_main_mem_ind_addr_d_inv(il[676])
    , lookup_rng_chk_pow_2_inv(il[677])
    , lookup_rng_chk_diff_inv(il[678])
    , lookup_rng_chk_0_inv(il[679])
    , lookup_rng_chk_1_inv(il[680])
    , lookup_rng_chk_2_inv(il[681])
    , lookup_rng_chk_3_inv(il[682])
    , lookup_rng_chk_4_inv(il[683])
    , lookup_rng_chk_5_inv(il[684])
    , lookup_rng_chk_6_inv(il[685])
    , lookup_rng_chk_7_inv(il[686])
    , lookup_pow_2_0_inv(il[687])
    , lookup_pow_2_1_inv(il[688])
    , lookup_byte_lengths_inv(il[689])
    , lookup_byte_operations_inv(il[690])
    , lookup_opcode_gas_inv(il[691])
    , kernel_output_lookup_inv(il[692])
    , lookup_into_kernel_inv(il[693])
    , lookup_cd_value_inv(il[694])
    , lookup_ret_value_inv(il[695])
    , incl_main_tag_err_inv(il[696])
    , incl_mem_tag_err_inv(il[697])
    , binary_acc_ia_shift(il[698])
    , binary_acc_ib_shift(il[699])
    , binary_acc_ic_shift(il[700])
    , binary_mem_tag_ctr_shift(il[701])
    , binary_op_id_shift(il[702])
    , cmp_a_hi_shift(il[703])
    , cmp_a_lo_shift(il[704])
    , cmp_b_hi_shift(il[705])
    , cmp_b_lo_shift(il[706])
    , cmp_cmp_rng_ctr_shift(il[707])
    , cmp_op_gt_shift(il[708])
    , cmp_p_sub_a_hi_shift(il[709])
    , cmp_p_sub_a_lo_shift(il[710])
    , cmp_p_sub_b_hi_shift(il[711])
    , cmp_p_sub_b_lo_shift(il[712])
    , cmp_sel_rng_chk_shift(il[713])
    , main_da_gas_remaining_shift(il[714])
    , main_emit_l2_to_l1_msg_write_offset_shift(il[715])
    , main_emit_note_hash_write_offset_shift(il[716])
    , main_emit_nullifier_write_offset_shift(il[717])
    , main_emit_unencrypted_log_write_offset_shift(il[718])
    , main_internal_return_ptr_shift(il[719])
    , main_l1_to_l2_msg_exists_write_offset_shift(il[720])
    , main_l2_gas_remaining_shift(il[721])
    , main_note_hash_exist_write_offset_shift(il[722])
    , main_nullifier_exists_write_offset_shift(il[723])
    , main_nullifier_non_exists_write_offset_shift(il[724])
    , main_pc_shift(il[725])
    , main_sel_execution_row_shift(il[726])
    , main_sload_write_offset_shift(il[727])
    , main_sstore_write_offset_shift(il[728])
    , mem_glob_addr_shift(il[729])
    , mem_rw_shift(il[730])
    , mem_sel_mem_shift(il[731])
    , mem_tag_shift(il[732])
    , mem_tsp_shift(il[733])
    , mem_val_shift(il[734])
    , slice_addr_shift(il[735])
    , slice_clk_shift(il[736])
    , slice_cnt_shift(il[737])
    , slice_col_offset_shift(il[738])
    , slice_sel_cd_cpy_shift(il[739])
    , slice_sel_mem_active_shift(il[740])
    , slice_sel_return_shift(il[741])
    , slice_sel_start_shift(il[742])
