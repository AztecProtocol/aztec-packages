...
FF AvmBytecodeTraceBuilder::compute_address_from_instance(ContractInstanceHint contract_instance)
{
    ...
    // Double check noir needs to hash infinity flag...
    std::vector<FF> public_keys_hash_fields = contract_instance.public_keys.to_fields();
    std::vector<FF> public_key_hash_vec{ PUBLIC_KEYS_HASH };
    ...
    return poseidon2::hash({ CONTRACT_ADDRESS_V1, partial_address, public_keys_hash });
}
