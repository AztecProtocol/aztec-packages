    ...
    : transcript(transcript)
{
    ...
    polynomials.w_non_shifted_accumulator = accumulator_claim.non_shifted_polynomial.share();
    polynomials.w_shifted_accumulator = accumulator_claim.shifted_polynomial.shifted();
    polynomials.w_non_shifted_instance = instance_claim.non_shifted_polynomial.share();
    ...
    std::vector<FF> instance_evaluations = { instance_claim.non_shifted_evaluation, instance_claim.shifted_evaluation };
    key = std::make_shared<MultilinearBatchingProvingKey>(std::move(polynomials),
                                                          std::move(accumulator_claim.challenge),
                                                          std::move(instance_claim.challenge),
                                                          ...
                                                          accumulator_claim.shifted_polynomial.share(),
                                                          instance_claim.shifted_polynomial.share());
...
void MultilinearBatchingProver::compute_new_claim()
{
    ...
    // two polys being accumulated, one can be larger
    // ideally accum into larger
    ...
    new_claim.non_shifted_polynomial = new_non_shifted_polynomial.share();
    new_claim.shifted_polynomial = new_shifted_polynomial.share();
