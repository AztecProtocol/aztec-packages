...
template <typename Flavor> class MegaTranscriptTests : public ::testing::Test {
  public:
    ...
    static TranscriptManifest construct_mega_honk_manifest()
    {
        ...
        size_t frs_per_Fr = FrCodec::template calc_num_fields<FF>();
        size_t frs_per_G = FrCodec::template calc_num_fields<Commitment>();
...
TYPED_TEST(MegaTranscriptTests, VerifierManifestConsistency)
{
    ...
    [[maybe_unused]] auto verifier_output = verifier.template verify_proof<DefaultIO>(proof);
