...
//  * the contribution from the present circuit only. For each j, we have the relationship T_i = T_{i-1} +
//  right_shift(t_i,
...
//         C_T_current[idx] = transcript->template receive_from_prover<Commitment>("T_CURRENT_" + std::to_string(idx +
//         1));
//     }

//     FF kappa = transcript->template get_challenge<FF>("kappa");

//     // Receive transcript poly evaluations and add corresponding univariate opening claims {(\kappa, p(\kappa),
//     [p(X)]} std::array<FF, Flavor::NUM_WIRES> T_prev_evals; std::array<FF, Flavor::NUM_WIRES> t_shift_evals;
