...
using Flavors = testing::Types<UltraFlavor, UltraKeccakFlavor>;
TYPED_TEST_SUITE(UltraHonkTests, Flavors);

// TODO(https://github.com/AztecProtocol/barretenberg/issues/1075): Make the tests run on UltraKeccakFlavor as
// well so we have a means of checking issues without having to run the Solidity verifier contract.
template <typename Flavor> class UltraHonkTests : public ::testing::Test {
  public:
    ...
    static std::vector<uint32_t> add_variables(auto& circuit_builder, std::vector<bb::fr> variables)
    {
    ...
    static void prove_and_verify(auto& circuit_builder, bool expected_result)
    {
    ...
    static void ensure_non_zero(auto& polynomial)
    {
...
{
    using ProverInstance = ProverInstance_<TypeParam>;
    using Prover = UltraProver_<TypeParam>;
    ...
    for (auto& poly : polynomials.get_selectors()) {
        TestFixture::ensure_non_zero(poly);
    }
...
TYPED_TEST(UltraHonkTests, StructuredTrace)
{
    using ProverInstance = ProverInstance_<TypeParam>;
    using Prover = UltraProver_<TypeParam>;
    using VerificationKey = typename TypeParam::VerificationKey;
    using Verifier = UltraVerifier_<TypeParam>;
