...
template <typename Flavor> class UltraHonkTests : public ::testing::Test {
  public:
    ...
    // TODO(https://github.com/AztecProtocol/barretenberg/issues/1075): Make the tests run on UltraKeccakFlavor as well
    // so we have a means of checking issues without having to run the Solidity verifier contract.
    ...
    TYPED_TEST(UltraHonkTests, create_gates_from_plookup_accumulators)
    {
        ...
        const fr input_lo =
            static_cast<uint256_t>(input_value).slice(0, plookup::fixed_base::table::BITS_PER_LO_SCALAR);
            ...
            for (size_t i = 0; i < num_tables; ++i) {
                ...
                TYPED_TEST(UltraHonkTests, LookupFailure)
                {
                    ...
                    // Failure mode 1: bad read counts/tags
                    {
                        ...
                        // Erroneously update the read counts/tags at an arbitrary index
                        // Note: updating only one or the other may not cause failure due to the design of the relation
                        // algebra. For

                        // example, the inverse is only computed if read tags is non-zero, otherwise the inverse at the
                        // row in question will be zero. So if read counts is incremented at some arbitrary index but
                        // read tags is not, the inverse will be 0 and the erroneous read_counts value will get
                ...
                TYPED_TEST(UltraHonkTests, test_no_lookup_proof)
                {
                    ...
                    for (size_t i = 0; i < 16; ++i) {
                        for (size_t j = 0; j < 16; ++j) {
                            ...
                            uint32_t add_idx = circuit_builder.add_variable(fr(left) + fr(right) +
                                                                            circuit_builder.get_variable(result_idx));
                ...
                TYPED_TEST(UltraHonkTests, sort_with_edges_gate)
                {
                    ...
                    {
                        auto circuit_builder = UltraCircuitBuilder();
                        auto idx = TestFixture::add_variables(circuit_builder, { 1,  2,  5,  6,  7,  10, 11, 13, 16, 17,
                                                                                 20, 22, 22, 25, 26, 29, 29, 32, 32, 33,
                                                                                 35, 38, 39, 39, 42, 42, 43, 45 });
                ...
                TYPED_TEST(UltraHonkTests, range_constraint)
                {
                    ...
                    }
                    {
                        auto circuit_builder = UltraCircuitBuilder();
                        auto indices =
                            TestFixture::add_variables(circuit_builder, { 1,  2,  3,  4,  5,  6,  10, 8,  15, 11,
                                                                          32, 21, 42, 79, 16, 10, 3,  26, 19, 51 });
                        for (size_t i = 0; i < indices.size(); i++) {
                    ...
                    }
                    {
                        auto circuit_builder = UltraCircuitBuilder();
                        auto indices =
                            TestFixture::add_variables(circuit_builder, { 1,  2,  3,  80, 5,  6,  29, 8,  15, 11,
                                                                          32, 21, 42, 79, 16, 10, 3,  26, 13, 14 });
                        for (size_t i = 0; i < indices.size(); i++) {
                    ...
                    }
                    {
                        auto circuit_builder = UltraCircuitBuilder();
                        auto indices =
                            TestFixture::add_variables(circuit_builder, { 1,  0,  3,  80, 5,  6,  29, 8,  15, 11,
                                                                          32, 21, 42, 79, 16, 10, 3,  26, 13, 14 });
                        for (size_t i = 0; i < indices.size(); i++) {
                            circuit_builder.create_new_range_constraint(indices[i], 79);
                        }
                ...
                TYPED_TEST(UltraHonkTests, composed_range_constraint)
                {
                    ...
                    circuit_builder.create_add_gate(
                        { a_idx, circuit_builder.zero_idx, circuit_builder.zero_idx, 1, 0, 0, -fr(e) });
                ...
                TYPED_TEST(UltraHonkTests, rom)
                {
                    ...
                    uint32_t rom_values[8]{
                        circuit_builder.add_variable(fr::random_element()),
                        circuit_builder.add_variable(fr::random_element()),
                        circuit_builder.add_variable(fr::random_element()),
                        circuit_builder.add_variable(fr::random_element()),
                        circuit_builder.add_variable(fr::random_element()),
                    ...
                    const auto d_value = circuit_builder.get_variable(a_idx) + circuit_builder.get_variable(b_idx) +
                ...
                TYPED_TEST(UltraHonkTests, ram)
                {
                    ...
                    circuit_builder.write_RAM_array(
                        ram_id, circuit_builder.add_variable(4), circuit_builder.add_variable(500));
