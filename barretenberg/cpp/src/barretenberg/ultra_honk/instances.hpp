...
template <IsUltraOrMegaHonk Flavor_, size_t NUM_ = 2> struct ProverInstances_ {
  public:
    ...
    using ArrayType = std::array<std::shared_ptr<ProverInstance>, NUM_>;
    ...
    std::shared_ptr<ProverInstance> const& operator[](size_t idx) const { return _data[idx]; }
    ...
    ProverInstances_(std::vector<std::shared_ptr<ProverInstance>> data)
    {
    ...
    template <size_t LENGTH, size_t skip_count = 0> auto row_to_univariates(size_t row_idx) const
    {
        ...
        for (size_t inst_idx = 0; auto& get_all : prover_polynomials_views) {
    ...
    typename Flavor::template ProverUnivariates<2> row_to_short_univariates(size_t row_idx) const
    {
        ...
        // Set the size corresponding to the number of rows in the execution trace
        // Iterate over the prover polynomials' views corresponding to each prover instance
        for (size_t inst_idx = 0; const auto& view : prover_polynomials_views) {
            // Iterate over all columns in the trace execution of a prover instance and extract their value at row_idx.
    ...
    size_t get_max_dyadic_size() const
    {
        return std::ranges::max(
            _data | std::views::transform([](const auto& inst) { return inst != nullptr ? inst->dyadic_size() : 0; }));
    }

  private:
    // Returns a vector containing pointer views to the prover polynomials corresponding to each prover instance.
...
template <IsUltraOrMegaHonk Flavor_, size_t NUM_ = 2> struct VerifierInstances_ {
    static_assert(NUM_ > 1, "Must have at least two verifier instances.");
    using Flavor = Flavor_;
  ...
  public:
    ...
    VerifierInstances_(const std::vector<std::shared_ptr<VerifierInstance>>& data)
    {
        BB_ASSERT_EQ(data.size(), NUM);
        for (size_t idx = 0; idx < data.size(); idx++) {
            _data[idx] = std::move(data[idx]);
    ...
    std::vector<std::vector<Commitment>> get_precomputed_commitments() const
    {
        ...
        for (size_t idx = 0; auto& commitment_at_idx : result) {
            for (auto [elt, inst] : zip_view(commitment_at_idx, _data)) {
                elt = inst->vk->get_all()[idx];
            }
            idx++;
        }
        return result;
    ...
    std::vector<std::vector<Commitment>> get_witness_commitments() const
    {
        ...
        for (size_t idx = 0; auto& commitment_at_idx : result) {
            for (auto [elt, inst] : zip_view(commitment_at_idx, _data)) {
                elt = inst->witness_commitments.get_all()[idx];
            }
            idx++;
        }
        return result;
    ...
    std::vector<std::vector<FF>> get_alphas() const
    {
        ...
        for (size_t idx = 0; auto& alpha_at_idx : result) {
            for (auto [elt, inst] : zip_view(alpha_at_idx, _data)) {
                elt = inst->alphas[idx];
            }
            idx++;
        }
    ...
    std::vector<std::vector<FF>> get_relation_parameters() const
    {
        ...
        for (size_t idx = 0; auto& params_at_idx : result) {
            for (auto [elt, inst] : zip_view(params_at_idx, _data)) {
                elt = inst->relation_parameters.get_to_fold()[idx];
            }
            idx++;
        }
        return result;
