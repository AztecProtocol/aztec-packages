    ...
    TranscriptManifest construct_ultra_honk_manifest(const size_t& log_n)
    {
        ...
        size_t data_types_per_Frs = [] {
            if constexpr (IsKeccakFlavor<Flavor>) {
                return U256Codec::template calc_num_fields<FF>();
            } else {
                return FrCodec::template calc_num_fields<FF>();
            }
        }();
        size_t data_types_per_G = [] {
            if constexpr (IsKeccakFlavor<Flavor>) {
                return U256Codec::template calc_num_fields<Commitment>();
            } else {
                return FrCodec::template calc_num_fields<Commitment>();
            }
        }();
