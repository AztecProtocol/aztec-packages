...
class MultiHonkTests : public ::testing::Test {
  public:
    ...
    using Prover = MultiHonkProver_<MultiMegaFlavor>;
    using Verifier = MultiHonkVerifier_<MultiMegaFlavor>;
     ...
     */
    static TranscriptManifest construct_multi_honk_manifest()
    {
...
TEST_F(MultiHonkTests, ProverManifestConsistency)
{
    ...
    auto manifest_expected = construct_multi_honk_manifest();
 ...
 */
TEST_F(MultiHonkTests, VerifierManifestConsistency)
{
    Builder builder;
    generate_test_circuit(builder);
 ...
 */
TEST_F(MultiHonkTests, CircuitValidWithStandardMega)
{
    Builder builder;
    generate_test_circuit(builder);
 ...
 */
TEST_F(MultiHonkTests, FullProveAndVerify)
{
    Builder builder;
    generate_test_circuit(builder);
 ...
 */
TEST_F(MultiHonkTests, FullProveAndVerifyZK)
{
    using ZKFlavor = MultiMegaZKFlavor;
    using ZKProverInstance = ProverInstance_<ZKFlavor>;
    using ZKProver = MultiHonkProver_<ZKFlavor>;
    using ZKVerifier = MultiHonkVerifier_<ZKFlavor>;
 ...
 */
TEST_F(MultiHonkTests, InterleavedEvalAndCommitmentRecovery)
{
 ...
 */
TEST_F(MultiHonkTests, CommitInterleavedHeterogeneousStartIndex)
{
    constexpr size_t BATCH_SIZE = Flavor::INTERLEAVING_BATCH_SIZE; // 4
...
template <typename Params> class MultiMegaRecursiveTests : public testing::Test {
    ...
    using InnerProver = MultiHonkProver_<InnerFlavor>;
    using InnerProverInstance = ProverInstance_<InnerFlavor>;
    using InnerFF = typename InnerFlavor::FF;
    using InnerCommitment = typename InnerFlavor::Commitment;
    using InnerVerifier = MultiHonkVerifier_<InnerFlavor, bb::DefaultIO>;
    ...
    // Recursive verifier uses MultiHonkVerifier_ (not UltraVerifier_)
    using RecursiveVerifier = MultiHonkVerifier_<RecursiveFlavor, IO>;
