    ...
    , modulus(slv->s.mkInteger(slv->modulus))
{
    if (!isconst) {
        ...
        // cvc5::Term halfp = slv->s.mkTerm(cvc5::Kind::INTS_DIVISION, {
        //     this->modulus,
        //     slv->s.mkInteger("2")
        // });
        // cvc5::Term lhalfp = slv->s.mkTerm(cvc5::Kind::NEG, {halfp});
        // cvc5::Term ge = slv->s.mkTerm(cvc5::Kind::GEQ, { this->term, lhalfp});
        // cvc5::Term lt = slv->s.mkTerm(cvc5::Kind::LT, { this->term, halfp });
        ...
        // info(strvalue);
        // if (strvalue[0] == '-') {
        //     this->term = slv->s.mkTerm(cvc5::Kind::ADD, { slv->s.mkInteger(strvalue), this->modulus });
        // } else {
        //     this->term = slv->s.mkInteger(strvalue);
        // }
    }
