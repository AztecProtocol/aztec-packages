    ...
    const bb::RamTranscript& ram_array)
{
        ...
        size_t index, size_t block_idx, auto& blk)
    {
        ...
        std::vector<uint32_t> first_row_variables;
        std::vector<uint32_t> second_row_variables;
        auto w1 = blk.w_l()[index]; // get opcode of operation, because function get_ecc_op_idx returns type uint32_t
                                    // and it adds as w1
        ...
        for (size_t blk_idx = 0; blk_idx < block_data.size() - 1; blk_idx++) {
            ...
            for (size_t gate_idx = 0; gate_idx < block_data[blk_idx].size(); gate_idx++) {
                ...
                if constexpr (std::is_same_v<CircuitBuilder, bb::MegaCircuitBuilder>) {
                    ...
                    auto eccop_gate_variables =
                        get_eccop_part_connected_component(gate_idx, blk_idx, block_data[blk_idx]);
                    if (connect_variables) {
    ...
    template <typename FF, typename CircuitBuilder>
    void StaticAnalyzer_<FF, CircuitBuilder>::connect_all_variables_in_vector(
    ...
    template <typename FF, typename CircuitBuilder>
    inline void StaticAnalyzer_<FF, CircuitBuilder>::remove_unnecessary_sha256_plookup_variables(
    ...
    template <typename FF, typename CircuitBuilder>
    void StaticAnalyzer_<FF, CircuitBuilder>::print_variables_gate_counts()
    {
