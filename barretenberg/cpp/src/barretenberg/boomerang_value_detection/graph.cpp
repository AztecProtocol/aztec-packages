 ...
 */
template <typename FF, typename CircuitBuilder>
inline std::vector<uint32_t> StaticAnalyzer_<FF, CircuitBuilder>::get_auxiliary_gate_connected_component(size_t index,
                                                                                                         size_t blk_idx,
                                                                                                         auto& block)
{
    std::vector<uint32_t> gate_variables;
...
{
    size_t block_index = find_block_index(circuit_builder.blocks.aux);
    ...
    for (const auto& record : rom_array.records) {
        ...
        auto q_1 = circuit_builder.blocks.aux.q_1()[gate_index];
        auto q_2 = circuit_builder.blocks.aux.q_2()[gate_index];
        auto q_3 = circuit_builder.blocks.aux.q_3()[gate_index];
        auto q_4 = circuit_builder.blocks.aux.q_4()[gate_index];
        auto q_m = circuit_builder.blocks.aux.q_m()[gate_index];
        auto q_arith = circuit_builder.blocks.aux.q_arith()[gate_index];
        auto q_c = circuit_builder.blocks.aux.q_c()[gate_index];
...
{
    auto block_data = circuit_builder.blocks.get();
    size_t blk_idx = find_block_index(circuit_builder.blocks.aux);
