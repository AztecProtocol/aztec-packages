...
{
    uint32_t real_variable_index = this->to_real(variable_index);
...
std::unordered_set<uint32_t> StaticAnalyzer_<FF, CircuitBuilder>::get_variables_in_one_gate()
{
    ...
    for (auto& pair : range_lists) {
        for (auto& elem : pair.second.variable_indices) {
            ...
            uint32_t real_variable_index = this->to_real(elem);
            if (variables_gate_counts[real_variable_index] == 1 && is_not_constant_variable) {
                decompose_variables.insert(real_variable_index);
            }
        }
    }
