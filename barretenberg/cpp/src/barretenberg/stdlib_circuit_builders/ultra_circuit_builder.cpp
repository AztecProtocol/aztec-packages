 ...
 *
 * However, if the previous gate is an ECC add/dbl such that (x3, y3) of the previous gate equals (x1, y1) of the
 * current gate, we can chain them together by simply setting the selector values of the previous gate to q_ecc = 1 and
 * q_1 = sign_coefficient (which in the relation translates to q_sign). We take advantage of this frequently when
...
void UltraCircuitBuilder_<ExecutionTrace>::create_ecc_add_gate(const ecc_add_gate_<FF>& in)
{
    ...
    // Determine whether we can fuse this addition operation into the previous gate
 ...
 * However, if the previous gate is an ECC add/dbl such that (x3, y3) of the previous gate equals (x1, y1) of the
 * current gate, we can chain them together by simply setting the selector values of the previous gate to q_ecc = 1 and
 * q_m = 1 (which in the relation translates to q_is_double = 1). We take advantage of this frequently when performing
 * chained additions/doubling operations.
 *
...
void UltraCircuitBuilder_<ExecutionTrace>::create_ecc_dbl_gate(const ecc_dbl_gate_<FF>& in)
{
    ...
    // Determine whether we can fuse this doubling operation into the previous gate
    bool can_fuse_into_previous_gate =
        block.size() > 0 &&                       /* a previous gate exists in the block */
