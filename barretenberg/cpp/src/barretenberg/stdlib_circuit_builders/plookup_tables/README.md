...
For each $n$-bit slice, we create a lookup table that maps every possible pair of $n$-bit inputs to their XOR result. In this case, we require a 6-bit table and a 2-bit table, with $2^{12} = 4096$ entries and $2^4 = 16$ entries, respectively. This type of table is referred to as a `BasicTable`. To compute the full 32-bit XOR result, we use a `MultiTable` which manages num-slices many `BasicTable`'s. In this example, slices $s_1$ through $s_5$ use the `BasicTable` for 6-bit XOR operations, and slice $s_0$ uses a `BasicTable` for 2-bit XOR operations. The `MultiTable` stores the following meta-data:
...
$$
\textsf{s}_{\text{decimal}} = s_{0} \cdot 2^{0} + s_{1} \cdot 2^{6} + s_{2} \cdot 2^{12} + s_{3} \cdot 2^{18} + s_{4} \cdot 2^{24} + s_{5} \cdot 2^{30}
$$
...
Note that the first coefficient is always 1 (i.e., $a_0 = 1$), which simplifies the computation of the remaining coefficients. In general, for a multi-table with coefficients $(a_0, a_1, \dots, a_{l-1})$, the step sizes are:

$$
\texttt{column\_step\_sizes}: \quad \left(1, \frac{a_1}{a_0}, \frac{a_2}{a_1}, \dots, \frac{a_{l-1}}{a_{l-2}}\right)
$$

The step sizes are used during the lookup process to incrementally compute accumulated values from the table lookups, avoiding the need for additional multiplication gates in the circuit. They represent the multiplicative factor needed to scale each slice to its correct position in the final reconstructed value.
