...
static constexpr size_t MAX_FIELDVM_BYTES_TO_CONSUME = 2048;
// Fuzzer input structure (total 4096 bytes)
struct FuzzerInput {
    uint8_t fieldvm_bytes_to_consume;                   // How many bytes FieldVM should consume (1-1536)
    uint8_t fieldvm_data[MAX_FIELDVM_BYTES_TO_CONSUME]; // FieldVM instructions for scalar computation (at beginning)
};

// Structure to store operation details for later printing
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* Data, size_t Size)
{
    ...
    if (Size < sizeof(FuzzerInput) + sizeof(SingleOp)) {
        return 0; // Invalid input size
    }

    const FuzzerInput* input = reinterpret_cast<const FuzzerInput*>(Data);

    // Validate input parameters
    size_t num_operations = (Size - sizeof(FuzzerInput)) / sizeof(SingleOp);
    if (num_operations == 0) {
        return 0;
    }
    ...
    try {
        ...
        size_t max_bytes_to_consume = std::min(static_cast<size_t>(16 * num_operations), MAX_FIELDVM_BYTES_TO_CONSUME);
        field_vm.run(input->fieldvm_data, max_bytes_to_consume);
        ...
        points.push_back(G1::point_at_infinity);
        ...
        // Process operations
        auto operations_start = reinterpret_cast<const SingleOp*>(Data + sizeof(FuzzerInput));
        for (size_t i = 0; i < num_operations; ++i) {
            const auto& op = operations_start[i];
        ...
        if (!result) {
            ...
            std::cout << "  operations: ";
            for (size_t i = 0; i < num_operations; ++i) {
                std::cout << static_cast<int>(operations_start[i].op_type) << " ";
            }
            std::cout << std::endl;
            std::cout << "  generator_indices: ";
            for (size_t i = 0; i < num_operations; ++i) {
                std::cout << static_cast<int>(operations_start[i].generator_index) << " ";
            }
            std::cout << std::endl;
            std::cout << "  FieldVM bytes to consume: " << static_cast<int>(input->fieldvm_bytes_to_consume)
                      << std::endl;
        ...
        std::cout << "Input parameters:" << std::endl;
        std::cout << "  num_operations: " << num_operations << std::endl;
        std::cout << "  num_generators: " << NUM_GENERATORS << " (fixed)" << std::endl;
        // If an exception occurs, it might indicate a bug
        // We don't want to crash the fuzzer, so we return 0
        assert(false);
        return 0;
    } catch (...) {
