    ...
    class ProverPolynomials : public AllEntities<Polynomial> {
      public:
        ...
        ProverPolynomials(const CircuitBuilder& builder)
        {
            ...
            lagrange_first.set(0, 1);
            lagrange_second.set(1, 1);
            lagrange_last.set(lagrange_last.size() - 1, 1);
            for (size_t i = 0; i < point_table_read_counts[0].size(); ++i) {
                ...
                lookup_read_counts_0.set(i + 1, point_table_read_counts[0][i]);
                lookup_read_counts_1.set(i + 1, point_table_read_counts[1][i]);
            }

            // compute polynomials for transcript columns
            parallel_for_range(transcript_rows.size(), [&](size_t start, size_t end) {
                for (size_t i = start; i < end; i++) {
                    transcript_accumulator_empty.set(i, transcript_rows[i].accumulator_empty);
                    transcript_add.set(i, transcript_rows[i].q_add);
                    transcript_mul.set(i, transcript_rows[i].q_mul);
                    transcript_eq.set(i, transcript_rows[i].q_eq);
                    transcript_reset_accumulator.set(i, transcript_rows[i].q_reset_accumulator);
                    transcript_msm_transition.set(i, transcript_rows[i].msm_transition);
                    transcript_pc.set(i, transcript_rows[i].pc);
                    transcript_msm_count.set(i, transcript_rows[i].msm_count);
                    transcript_Px.set(i, transcript_rows[i].base_x);
                    transcript_Py.set(i, transcript_rows[i].base_y);
                    transcript_z1.set(i, transcript_rows[i].z1);
                    transcript_z2.set(i, transcript_rows[i].z2);
                    transcript_z1zero.set(i, transcript_rows[i].z1_zero);
                    transcript_z2zero.set(i, transcript_rows[i].z2_zero);
                    transcript_op.set(i, transcript_rows[i].opcode);
                    transcript_accumulator_x.set(i, transcript_rows[i].accumulator_x);
                    transcript_accumulator_y.set(i, transcript_rows[i].accumulator_y);
                    transcript_msm_x.set(i, transcript_rows[i].msm_output_x);
                    transcript_msm_y.set(i, transcript_rows[i].msm_output_y);
                    transcript_base_infinity.set(i, transcript_rows[i].base_infinity);
                    transcript_base_x_inverse.set(i, transcript_rows[i].base_x_inverse);
                    transcript_base_y_inverse.set(i, transcript_rows[i].base_y_inverse);
                    transcript_add_x_equal.set(i, transcript_rows[i].transcript_add_x_equal);
                    transcript_add_y_equal.set(i, transcript_rows[i].transcript_add_y_equal);
                    transcript_add_lambda.set(i, transcript_rows[i].transcript_add_lambda);
                    transcript_msm_intermediate_x.set(i, transcript_rows[i].transcript_msm_intermediate_x);
                    transcript_msm_intermediate_y.set(i, transcript_rows[i].transcript_msm_intermediate_y);
                    transcript_msm_infinity.set(i, transcript_rows[i].transcript_msm_infinity);
                    transcript_msm_x_inverse.set(i, transcript_rows[i].transcript_msm_x_inverse);
                    transcript_msm_count_zero_at_transition.set(i, transcript_rows[i].msm_count_zero_at_transition);
                    transcript_msm_count_at_transition_inverse.set(i,
            ...
            if (transcript_rows[transcript_rows.size() - 1].accumulator_empty) {
                for (size_t i = transcript_rows.size(); i < dyadic_num_rows; ++i) {
                    transcript_accumulator_empty.set(i, 1);
                }
            }
            // in addition, unless the accumulator is reset, it contains the value from the previous row so this
            // must be propagated
            for (size_t i = transcript_rows.size(); i < dyadic_num_rows; ++i) {
                transcript_accumulator_x.set(i, transcript_accumulator_x[i - 1]);
                transcript_accumulator_y.set(i, transcript_accumulator_y[i - 1]);
            }
            ...
            parallel_for_range(point_table_rows.size(), [&](size_t start, size_t end) {
                for (size_t i = start; i < end; i++) {
                    ...
                    // precompute_select = 1)
                    precompute_select.set(i, (i != 0) ? 1 : 0);
                    precompute_pc.set(i, point_table_rows[i].pc);
                    precompute_point_transition.set(i, static_cast<uint64_t>(point_table_rows[i].point_transition));
                    precompute_round.set(i, point_table_rows[i].round);
                    precompute_scalar_sum.set(i, point_table_rows[i].scalar_sum);
                    precompute_s1hi.set(i, point_table_rows[i].s1);
                    precompute_s1lo.set(i, point_table_rows[i].s2);
                    precompute_s2hi.set(i, point_table_rows[i].s3);
                    precompute_s2lo.set(i, point_table_rows[i].s4);
                    precompute_s3hi.set(i, point_table_rows[i].s5);
                    precompute_s3lo.set(i, point_table_rows[i].s6);
                    precompute_s4hi.set(i, point_table_rows[i].s7);
                    precompute_s4lo.set(i, point_table_rows[i].s8);
                    ...
                    // into WNAF form
                    precompute_skew.set(i, point_table_rows[i].skew ? 7 : 0);
                    precompute_dx.set(i, point_table_rows[i].precompute_double.x);
                    precompute_dy.set(i, point_table_rows[i].precompute_double.y);
                    precompute_tx.set(i, point_table_rows[i].precompute_accumulator.x);
                    precompute_ty.set(i, point_table_rows[i].precompute_accumulator.y);
                }
            });
            ...
            parallel_for_range(msm_rows.size(), [&](size_t start, size_t end) {
                for (size_t i = start; i < end; i++) {
                    msm_transition.set(i, static_cast<int>(msm_rows[i].msm_transition));
                    msm_add.set(i, static_cast<int>(msm_rows[i].q_add));
                    msm_double.set(i, static_cast<int>(msm_rows[i].q_double));
                    msm_skew.set(i, static_cast<int>(msm_rows[i].q_skew));
                    msm_accumulator_x.set(i, msm_rows[i].accumulator_x);
                    msm_accumulator_y.set(i, msm_rows[i].accumulator_y);
                    msm_pc.set(i, msm_rows[i].pc);
                    msm_size_of_msm.set(i, msm_rows[i].msm_size);
                    msm_count.set(i, msm_rows[i].msm_count);
                    msm_round.set(i, msm_rows[i].msm_round);
                    msm_add1.set(i, static_cast<int>(msm_rows[i].add_state[0].add));
                    msm_add2.set(i, static_cast<int>(msm_rows[i].add_state[1].add));
                    msm_add3.set(i, static_cast<int>(msm_rows[i].add_state[2].add));
                    msm_add4.set(i, static_cast<int>(msm_rows[i].add_state[3].add));
                    msm_x1.set(i, msm_rows[i].add_state[0].point.x);
                    msm_y1.set(i, msm_rows[i].add_state[0].point.y);
                    msm_x2.set(i, msm_rows[i].add_state[1].point.x);
                    msm_y2.set(i, msm_rows[i].add_state[1].point.y);
                    msm_x3.set(i, msm_rows[i].add_state[2].point.x);
                    msm_y3.set(i, msm_rows[i].add_state[2].point.y);
                    msm_x4.set(i, msm_rows[i].add_state[3].point.x);
                    msm_y4.set(i, msm_rows[i].add_state[3].point.y);
                    msm_collision_x1.set(i, msm_rows[i].add_state[0].collision_inverse);
                    msm_collision_x2.set(i, msm_rows[i].add_state[1].collision_inverse);
                    msm_collision_x3.set(i, msm_rows[i].add_state[2].collision_inverse);
                    msm_collision_x4.set(i, msm_rows[i].add_state[3].collision_inverse);
                    msm_lambda1.set(i, msm_rows[i].add_state[0].lambda);
                    msm_lambda2.set(i, msm_rows[i].add_state[1].lambda);
                    msm_lambda3.set(i, msm_rows[i].add_state[2].lambda);
                    msm_lambda4.set(i, msm_rows[i].add_state[3].lambda);
                    msm_slice1.set(i, msm_rows[i].add_state[0].slice);
                    msm_slice2.set(i, msm_rows[i].add_state[1].slice);
                    msm_slice3.set(i, msm_rows[i].add_state[2].slice);
                    msm_slice4.set(i, msm_rows[i].add_state[3].slice);
                }
            });
            this->set_shifted();
        }
