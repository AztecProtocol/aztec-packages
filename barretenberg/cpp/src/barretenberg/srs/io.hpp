...
template <typename Curve> class IO {
    ...
    static void read_manifest(std::string const& filename, Manifest& manifest)
    {
        ...
        if (!file) {
            ...
            throw_or_abort(format("Only read ", read, " bytes from manifest file ", filename, " but expected ", sizeof(Manifest), "."));
        }
        file.close();
        ...
        char* buffer, size_t& size, std::string const& filename, size_t offset = 0, size_t amount = 0)
    {
        ...
        if (!file) {
            ptrdiff_t read = file.gcount();
            throw_or_abort(format("Only read ", read, " bytes from transcript file ", filename, " but expected ", size, "."));
        }
