...
class MemGrumpkinCrsFactory : public CrsFactory<curve::Grumpkin> {
  public:
    ...
    std::shared_ptr<VerifierCrs<curve::Grumpkin>> get_verifier_crs(size_t degree = 0) override;

  private:
    std::shared_ptr<ProverCrs<curve::Grumpkin>> prover_crs_;
    std::shared_ptr<VerifierCrs<curve::Grumpkin>> verifier_crs_;
};
