...
static void check_bn254_consistency(const fs::path& crs_download_path, size_t num_points, bool allow_download)
{
    ...
    // prover CRS
    auto f_prover = file_crs.get_prover_crs(num_points);
    auto m_prover = mem_crs.get_prover_crs(num_points);
...
static void check_grumpkin_consistency(const fs::path& crs_download_path, size_t num_points, bool allow_download)
{
    NativeGrumpkinCrsFactory file_crs(crs_download_path, allow_download);

    // read G1
    std::vector<Grumpkin::AffineElement> points(num_points);
    auto buf = read_file(bb::srs::bb_crs_path() / "grumpkin_g1.dat", num_points * sizeof(Grumpkin::AffineElement));
    for (size_t i = 0; i < num_points; ++i) {
        points[i] = from_buffer<Grumpkin::AffineElement>(buf, i * sizeof(Grumpkin::AffineElement));
    }
...
TEST(reference_string, bn254_consistency)
{
...
TEST(reference_string, grumpkin_consistency)
{
