    ...
    function verify(bytes calldata, bytes32[] calldata) public override returns (bool) {
        ...
        assembly {
           ...
           let shifted_scalar := 0
           {
            ...
            loadVk()
            {
                // DYNAMIC VK INJECTION POINT: The following mstore lines should be generated dynamically
                // by output_vk_sol_ultra_honk_opt and inserted here.
                // <DYNAMIC_VK>
            }
...
inline std::string get_honk_solidity_verifier(auto const& verification_key)
{
    ...
    output_vk_sol_ultra_honk_opt(stream, verification_key, false);
    std::string contract = HONK_CONTRACT_SOURCE;
    std::string vk_code = stream.str();
    size_t pos = contract.find("<INJECT_VERIFICATION_KEY>");
    if (pos != std::string::npos) {
        contract.replace(pos, std::string("<INJECT_VERIFICATION_KEY>").length(), vk_code);
    }
    return contract;
}
