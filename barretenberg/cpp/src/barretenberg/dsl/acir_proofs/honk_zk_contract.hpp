...
library ZKTranscriptLib {
    ...
    function generateEtaChallenge(Honk.ZKProof memory proof, bytes32[] calldata publicInputs, uint256 circuitSize, uint256 publicInputsSize)
        internal
        pure
 ...
 contract HonkVerifier is IVerifier {
    ...
    function verify(bytes calldata proof, bytes32[] calldata publicInputs) public view override returns (bool verified) {
      // Check the received proof is the expected size where each field element is 32 bytes
        ...
        ZKTranscript memory t = ZKTranscriptLib.generateTranscript(p, publicInputs, vk.circuitSize, NUMBER_OF_PUBLIC_INPUTS, 1);
