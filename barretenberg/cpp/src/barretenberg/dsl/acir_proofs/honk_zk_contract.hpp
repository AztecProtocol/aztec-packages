...
library ZKTranscriptLib {
        ...
        returns (Fr eta, Fr etaTwo, Fr etaThree, Fr previousChallenge)
    {
        ...
        for (uint256 i = 0; i < publicInputsSize - PAIRING_POINT_ACCUMULATOR_SIZE; i++) {
            round0[3 + i] = bytes32(publicInputs[i]);
        }
        for (uint256 i = 0; i < PAIRING_POINT_ACCUMULATOR_SIZE; i++) {
            round0[3 + publicInputsSize - PAIRING_POINT_ACCUMULATOR_SIZE + i] = FrLib.toBytes32(proof.pairingPointObject[i]);
        }
 ...
 contract HonkVerifier is IVerifier {
    ...
    function verify(bytes calldata proof, bytes32[] calldata publicInputs) public view override returns (bool verified) {
      // Check the received proof is the expected size where each field element is 32 bytes
        ...
        if (publicInputs.length != vk.publicInputsSize - PAIRING_POINT_ACCUMULATOR_SIZE || publicInputs.length != NUMBER_OF_PUBLIC_INPUTS - PAIRING_POINT_ACCUMULATOR_SIZE) {
    ...
    function computePublicInputDelta(bytes32[] memory publicInputs, Fr[PAIRING_POINT_ACCUMULATOR_SIZE] memory pairingPointObject, Fr beta, Fr gamma, uint256 offset)
        internal
        view
        returns (Fr publicInputDelta)
    {
        ...
        {
            for (uint256 i = 0; i < NUMBER_OF_PUBLIC_INPUTS - PAIRING_POINT_ACCUMULATOR_SIZE; i++) {
