#include "recursion_constraint_honk.hpp"
    ...
    bool has_valid_witness_assignments)
{
    ...
    static_cast<void>(nested_aggregation_indices);
    ...
    //         // Create a copy constraint between our dummy field and the witness index provided by
    //         RecursionConstraint.
    ...
    //         // In the case of no valid witness assignment, this makes sure that the RecursionConstraint witness
    //         indices
    ...
    //         bb::fr dummy_field = has_valid_witness_assignments ? builder.get_variable(proof_field_idx) :
    //         dummy_proof[i]; builder.assert_equal(builder.add_variable(dummy_field), proof_field_idx);
    //     }
    //     for (size_t i = 0; i < input.key.size(); ++i) {
    //         const auto key_field_idx = input.key[i];
    //         bb::fr dummy_field = has_valid_witness_assignments ? builder.get_variable(key_field_idx) :
    //         dummy_key[i]; builder.assert_equal(builder.add_variable(dummy_field), key_field_idx);
    //     }
    // }
    ...
    if (!inner_aggregation_indices_all_zero) {
        ...
        // aggregate it into nested_aggregation_object
    ...
    // deserialize the vkey from field elements
...
//  */
// std::vector<bb::fr> export_key_in_recursion_format(std::shared_ptr<plonk::verification_key> const& vkey)
// {
...
//             descriptor.source == PolynomialSource::PERMUTATION) {
