    ...
    const PredicateTestCase& mode = PredicateTestCase::ConstantTrue)
{
    ...
    if constexpr (std::is_same_v<RecursiveFlavor, bb::UltraRecursiveFlavor_<UltraCircuitBuilder>>) {
        ...
        return std::make_tuple(786327, 0);
    } else {
...
inline constexpr size_t CHONK_RECURSION_GATES = 1523014;
...
inline constexpr size_t INIT_KERNEL_GATE_COUNT = 15558;
...
inline constexpr size_t INNER_KERNEL_GATE_COUNT_HN = 36014;
inline constexpr size_t INNER_KERNEL_ECC_ROWS = 1766 + MSM_ROWS_OFFSET;
inline constexpr size_t INNER_KERNEL_ULTRA_OPS = 189;

// Tail kernel gate counts (verifies HN_TAIL proof)
inline constexpr size_t TAIL_KERNEL_GATE_COUNT = 19995;
inline constexpr size_t TAIL_KERNEL_ECC_ROWS = 947 + MSM_ROWS_OFFSET;
inline constexpr size_t TAIL_KERNEL_ULTRA_OPS = 101;

// Hiding kernel gate counts (verifies HN_FINAL proof)
inline constexpr size_t HIDING_KERNEL_GATE_COUNT = 21957;
inline constexpr size_t HIDING_KERNEL_ECC_ROWS = 1440 + MSM_ROWS_OFFSET;
inline constexpr size_t HIDING_KERNEL_ULTRA_OPS = 132;
