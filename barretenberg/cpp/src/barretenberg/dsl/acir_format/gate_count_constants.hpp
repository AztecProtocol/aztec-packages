    ...
    const PredicateTestCase& mode = PredicateTestCase::ConstantTrue)
{
    ...
    if constexpr (std::is_same_v<RecursiveFlavor, bb::UltraRecursiveFlavor_<UltraCircuitBuilder>>) {
        ...
        return std::make_tuple(772252, 0);
    } else {
...
inline constexpr size_t HIDING_KERNEL_GATE_COUNT = 75515;
inline constexpr size_t HIDING_KERNEL_ECC_ROWS = 5037 + MSM_ROWS_OFFSET;
inline constexpr size_t HIDING_KERNEL_ULTRA_OPS = 340;
