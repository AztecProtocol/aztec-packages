...
class AcirHonkRecursionConstraint : public ::testing::Test {
  public:
    ...
    Builder create_outer_circuit(std::vector<Builder>& inner_circuits)
    {
        ...
        std::array<uint32_t, HonkRecursionConstraint::AGGREGATION_OBJECT_SIZE> output_aggregation_object;
        ...
        for (auto& inner_circuit : inner_circuits) {
            ...
            std::vector<fr> inner_public_input_values(proof_witnesses.begin(),
                                                      proof_witnesses.begin() +
                                                          static_cast<std::ptrdiff_t>(num_inner_public_inputs));

            // We want to make sure that we do not remove the nested aggregation object in the case of the proof we want
            // to recursively verify contains a recursive proof itself. We are safe to keep all the inner public inputs
            ...
            const std::vector<bb::fr> key_witnesses = export_honk_key_in_recursion_format(inner_verifier.key);
