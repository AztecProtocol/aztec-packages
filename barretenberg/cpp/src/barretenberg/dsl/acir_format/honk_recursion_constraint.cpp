    ...
    requires(IsRecursiveFlavor<Flavor> && IsUltraHonk<typename Flavor::NativeFlavor>)
{
    ...
    if (!input.predicate.is_constant) {
        ...
        for (uint32_t i = 0; i < proof_fields.size(); ++i) {
            ...
            // proof generated by construct_honk_proof_for_simple_circuit or another one. We manually unset the
            // free_witness_tag to ensure the automatic tooling doesn't catch this as a free witness
            mock_proof_witness.unset_free_witness_tag();
            auto valid_proof =
                field_ct<Builder>::conditional_assign(predicate_witness, proof_fields[i], mock_proof_witness);
        ...
        vk_hash = field_ct<Builder>::conditional_assign(predicate_witness, vk_hash, mock_vk_hash);
        proof_fields = result;
