    ...
    requires(IsRecursiveFlavor<Flavor> && IsUltraHonk<typename Flavor::NativeFlavor>)
{
    ...
    if (!has_valid_witness_assignments) {
        ...
        // Create another mock proof and VK to conditionally select between the two depending on the predicate
        uint32_t pub_inputs_offset = Flavor::NativeFlavor::has_zero_row ? 1 : 0;

        // Generate mock honk vk
        auto honk_vk = create_mock_honk_vk<typename Flavor::NativeFlavor, IO>(
            1 << Flavor::VIRTUAL_LOG_N, pub_inputs_offset, input.public_inputs.size());
        place_holder_vk_fields = honk_vk->to_field_elements();
        place_holder_proof = create_mock_honk_proof<typename Flavor::NativeFlavor, IO>(input.public_inputs.size());
        mock_vk_hash = honk_vk->hash();
    } else {
        // If we have an actual witness, the mock proof and VK should be an actual verifiable honk proof and honk vk
        auto [proof, mock_vk] =
            construct_honk_proof_for_simple_circuit<typename Flavor::NativeFlavor>(input.public_inputs.size());
        place_holder_proof = proof;
        place_holder_vk_fields = mock_vk->to_field_elements();
        mock_vk_hash = mock_vk->hash();
    }
    ...
    if (!input.predicate.is_constant) {
        ...
        for (uint32_t i = 0; i < proof_fields.size(); ++i) {
            ...
            // an ad hoc correct proof instead of the real proof. Hence, it is not a soundness issue if we use the
            // specific ad hoc proof generated by construct_honk_proof_for_simple_circuit or another one. We
            // manually unset the free_witness_tag to ensure the automatic tooling doesn't catch this as a free
            // witness
        ...
        for (uint32_t i = 0; i < key_fields.size(); ++i) {
            field_ct<Builder> mock_vk_witness =
        ...
        vk_hash = field_ct<Builder>::conditional_assign(
            predicate_witness, vk_hash, field_ct<Builder>::from_witness(&builder, mock_vk_hash.get_value()));
        proof_fields = result;
        key_fields = result_vk;
    }
