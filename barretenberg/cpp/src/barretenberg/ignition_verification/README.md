...
Confirms that the 100.8M G1 points in the 20 sealed transcript files form a valid power-of-tau sequence: `[G1, τ·G1, τ²·G1, …, τ^(N-1)·G1]`.
...
**Also checks:** The sealed G2 point (τ·G2) matches the value hardcoded in `bn254_crs_data.hpp`.
...
| File | Purpose |
|---|---|
| `main.cpp` | CLI entry point, orchestrates the three checks |
...
| `*.test.cpp` | Unit tests (synthetic SRS round-trips, corruption detection, deserialization against known byte sequences) |
