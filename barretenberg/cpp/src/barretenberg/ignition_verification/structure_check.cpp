                         ...
                         std::function<void(size_t, size_t)> progress_callback)
{
    ...
    for (size_t chunk = 0; chunk < total_chunks; ++chunk) {
        ...
        R_accum += Curve::Element(G1::batch_mul(std::span<const G1>(points).subspan(0, n), std::span<Fr>(scalars)));

        // L += Σ r_i · points[i+1] for i in [0, n-1]
        L_accum += Curve::Element(G1::batch_mul(std::span<const G1>(points).subspan(1, n), std::span<Fr>(scalars)));
