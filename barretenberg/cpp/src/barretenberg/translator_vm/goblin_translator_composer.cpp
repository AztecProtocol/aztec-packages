...
void GoblinTranslatorComposer::construct_wire_polynomials_base_goblin_translator(const CircuitBuilder& circuit_builder)
{
    // Populate the wire polynomials with values from conventional wires
    // WORKTODO: leave note on moving these vectors instead
    for (auto [poly, wire] : zip_view(proving_key->get_wires(), circuit_builder.wires)) {
        std::copy(wire.begin(), wire.end(), poly.begin());
    }
}
 ...
 */
void GoblinTranslatorComposer::compute_witness(const CircuitBuilder& circuit_builder)
{
    if (computed_witness) {
        return;
    }
    ...
    construct_wire_polynomials_base_goblin_translator(circuit_builder);
