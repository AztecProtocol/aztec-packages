    ...
    template <typename Params> class Transcript_ : public BaseTranscript<Params> {
      public:
        ...
        void serialize_full_transcript()
        {
            ...
            Base::template serialize_to_buffer<Commitment>(w_l_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(w_r_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(w_o_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(lookup_read_counts_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(lookup_read_tags_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(w_4_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(lookup_inverses_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(z_perm_comm, proof_data);
            for (size_t i = 0; i < CONST_PROOF_SIZE_LOG_N; ++i) {
                Base::template serialize_to_buffer<bb::Univariate<FF, BATCHED_RELATION_PARTIAL_LENGTH>>(sumcheck_univariates[i], proof_data);
            }
            Base::template serialize_to_buffer<std::array<FF, NUM_ALL_ENTITIES>>(sumcheck_evaluations, proof_data);
            for (size_t i = 0; i < CONST_PROOF_SIZE_LOG_N - 1; ++i) {
                Base::template serialize_to_buffer<Commitment>(gemini_fold_comms[i], proof_data);
            }
            for (size_t i = 0; i < CONST_PROOF_SIZE_LOG_N; ++i) {
                Base::template serialize_to_buffer<FF>(gemini_fold_evals[i], proof_data);
            }
            Base::template serialize_to_buffer<Commitment>(shplonk_q_comm, proof_data);
            Base::template serialize_to_buffer<Commitment>(kzg_w_comm, proof_data);
