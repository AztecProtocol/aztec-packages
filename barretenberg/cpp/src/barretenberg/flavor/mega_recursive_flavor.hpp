...
/**
 * @brief Recursive counterpart to DualMegaFlavor (BS=2 interleaved commitments).
 */
template <typename BuilderType> class DualMegaRecursiveFlavor_ : public MegaRecursiveFlavor_<BuilderType> {
  public:
    using CircuitBuilder = BuilderType;
    using Curve = stdlib::bn254<CircuitBuilder>;
    using PCS = KZG<Curve>;
    ...
    using FF = typename Curve::ScalarField;
    using Commitment = typename Curve::Element;
    using NativeFlavor = DualMegaFlavor;
    using Codec = stdlib::StdlibCodec<FF>;
    using Transcript = StdlibTranscript<CircuitBuilder>;
