...
TEST(BaseTranscript, TwoProversTwoFields)
{
    ...
    Transcript verifier_transcript_1{ prover_transcript.export_proof() };
    ...
    // state initializes to zero
    EXPECT_STATE(verifier_transcript_1, /*start*/ 0, /*written*/ 0, /*read*/ 0);
    Fr received_a = verifier_transcript_1.receive_from_prover<Fr>("a");
    // receiving is reading bytes input and writing them to an internal proof_data buffer
    EXPECT_STATE(verifier_transcript_1, /*start*/ 0, /*written*/ 32, /*read*/ 32);
    ...
    Transcript verifier_transcript_2{ prover_transcript.export_proof() };
    EXPECT_STATE(prover_transcript, /*start*/ 64, /*written*/ 0, /*read*/ 0);
    EXPECT_STATE(verifier_transcript_2, /*start*/ 0, /*written*/ 0, /*read*/ 0);
    Fq received_b = verifier_transcript_2.receive_from_prover<Fq>("b");
    EXPECT_STATE(verifier_transcript_2, 0, 32, 32);
