...
// ============================================================================
// Gate Count Tests for Stdlib Deserialization Operations
// ============================================================================

/**
 * @brief Measure gate counts for scalar (fr) deserialization
 * @details Should be nearly zero - just witness creation
 */
TYPED_TEST(TranscriptTests, GateCount_ScalarDeserialization)
{
    if constexpr (!TestFixture::IsStdlib) {
        GTEST_SKIP() << "Gate counting only applies to stdlib transcripts";
    }
    ...
    size_t gates_before = this->get_num_gates();

    typename TestFixture::Transcript verifier;
    verifier.load_proof(this->export_proof(prover));
    auto received = verifier.template receive_from_prover<FF>("scalar");

    size_t gates_after = this->get_num_gates();
    size_t gates_used = gates_after - gates_before;

    info("Scalar deserialization gate count: ", gates_used);

    // Scalar deserialization should be very cheap (mostly witness creation)
    EXPECT_LT(gates_used, 10);

    this->check_circuit();
}

/**
 * @brief Measure gate counts for BN254 commitment deserialization
 * @details Includes bigfield reconstruction + point-at-infinity check + on-curve validation
 */
TYPED_TEST(TranscriptTests, GateCount_BN254CommitmentDeserialization)
{
    if constexpr (!TestFixture::IsStdlib) {
        GTEST_SKIP() << "Gate counting only applies to stdlib transcripts";
    }
    ...
    info("BN254 commitment deserialization gate count: ", gates_used);

    // BN254 commitment deserialization includes:
    // - Bigfield reconstruction (2 limbs -> bigfield x, y)
    // - Point at infinity check
    // - On-curve validation
    // This is more expensive than scalar
    EXPECT_GT(gates_used, 10);
    EXPECT_LT(gates_used, 1000); // Upper bound sanity check

    this->check_circuit();
}

/**
 * @brief Measure gate counts for Grumpkin commitment deserialization
 * @details Includes point-at-infinity check + on-curve validation (cheaper than BN254)
 */
TYPED_TEST(TranscriptTests, GateCount_GrumpkinCommitmentDeserialization)
{
    if constexpr (!TestFixture::IsStdlib) {
        GTEST_SKIP() << "Gate counting only applies to stdlib transcripts";
    }
    ...
    info("Grumpkin commitment deserialization gate count: ", gates_used);

    // Grumpkin is cheaper than BN254 (native field, no bigfield)
    // but still includes point-at-infinity and on-curve checks
    EXPECT_GT(gates_used, 5);
    EXPECT_LT(gates_used, 500); // Should be cheaper than BN254
 ...
 * @details Should scale linearly with array size
 */
TYPED_TEST(TranscriptTests, GateCount_ArrayDeserialization)
{
    if constexpr (!TestFixture::IsStdlib) {
        GTEST_SKIP() << "Gate counting only applies to stdlib transcripts";
    }
    ...
    info("Array[", SIZE, "] deserialization gate count: ", gates_used);

    // Array deserialization should scale with size (mostly witness creation)
    EXPECT_LT(gates_used, SIZE * 10);
 ...
 * @details Should be similar to array deserialization
 */
TYPED_TEST(TranscriptTests, GateCount_UnivariateDeserialization)
{
    if constexpr (!TestFixture::IsStdlib) {
        GTEST_SKIP() << "Gate counting only applies to stdlib transcripts";
    }
    ...
    info("Univariate[", LENGTH, "] deserialization gate count: ", gates_used);

    // Similar to array
    EXPECT_LT(gates_used, LENGTH * 10);
