...
// HONK recursion constraint validation — Phase 3 cursor-chain orchestrator.
//
// Baseline: UltraRecursiveFlavor_<UltraCircuitBuilder>, DefaultIO, constant-true predicate.
...
namespace HonkRecursionValidation {
...
// Legacy squeeze totals kept for ROLLUP / HONK_ZK cross-references. Baseline HONK now has a single
// Output-stage recursion_separator squeeze; do not use these to slice Phase 3 cursors.
...
// Legacy name: eta squeeze no longer anchors HONK; ignore argument and use Phase 2/3 oink start.
inline ArithBoundaries compute_arith_boundaries(size_t = 0)
{
    return compute_arith_boundaries_from_oink_start();
}

struct HonkRecursionValidationResult {
    bool is_valid = false;
...
HonkRecursionValidationResult validate_honk_recursion(CircuitBuilder& builder,
...
{
    HonkRecursionValidationResult result;
    ...
    // Coverage from Phase 2 primitive_start / block 0 for nnf+memory+poseidon (Oink owns p2 from 0).
