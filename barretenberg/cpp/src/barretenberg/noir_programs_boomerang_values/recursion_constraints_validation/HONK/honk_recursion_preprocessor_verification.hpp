...
namespace HonkRecursionValidation::Preprocessor {

// gate_challenges only (padding_indicator_array deleted). Promoted Phase 3 cursor dump.
...
struct PreprocessorValidationResult {
    ...
    bool squeeze_count_ok = true; // legacy; squeeze slicing removed for baseline HONK
};
                                                   ...
                                                   const Oink::OinkValidationResult& previous)
{
    ...
    result.nnf_end = previous.nnf_end; // no nnf in preprocessor
...
// Legacy overload for ROLLUP / older call sites that still pass squeeze vectors.
template <typename FF, typename CircuitBuilder>
PreprocessorValidationResult validate_preprocessor(CircuitBuilder& builder,
                                                   cdg::StaticAnalyzer_<FF, CircuitBuilder>& analyzer,
                                                   const Oink::OinkValidationResult& previous,
