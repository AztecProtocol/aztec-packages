...
TEST_F(BoomerangConstraintsTests, DetectCorruptedBooleanGate_qArith)
{
    ...
    ASSERT_TRUE(found_gate) << "Could not find boolean gate to corrupt";
    // Note: CircuitChecker won't catch q_arith=0 corruption because it disables the gate entirely
    // (0 * anything = 0 is trivially satisfied)
...
TEST_F(BoomerangConstraintsTests, DetectCorruptedBooleanGate_q1)
{
    ...
    ASSERT_TRUE(found_gate) << "Could not find boolean gate to corrupt";
    // Note: CircuitChecker won't catch q_1=0 corruption with witness=0 because
    // the corrupted gate (w_l*w_r = 0) is still satisfied when w_l=w_r=0
...
TEST_F(BoomerangConstraintsTests, DetectCorruptedXorConstraint_LookupTableSelector)
{
    ...
    // Note: CircuitChecker won't catch q_lookup=0 corruption because it only validates
    // arithmetic relations, not lookup selector configuration
