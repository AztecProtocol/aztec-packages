...
// BB_TLS: thread_local on native, plain static on wasm. WASM thread_local for
// non-trivial types has bad init/teardown semantics inside wasi-threads workers,
 ...
 *
 * Optimised for callers that dispatch parallel_for many times per operation
 * (hundreds of times per proof in the MSM): per-call overhead must be cheap
 * on heterogeneous P/E hosts and on WASM, so the hot path avoids mutexes
 * and condition variables entirely.
 */
...
// parallel_for again would race on its master's pool state, so nested calls fall back
// to serial execution on the calling thread.
