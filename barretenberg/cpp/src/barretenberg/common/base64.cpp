...
static unsigned int pos_of_char(const unsigned char chr)
{
    ...
    if (chr >= 'A' && chr <= 'Z')
        ...
        //
        throw_or_abort("Input is not valid base64-encoded data.");
}
...
template <typename String> static std::string decode(String encoded_string, bool remove_linebreaks)
{
    ...
    while (pos < length_of_string) {
        ...
        size_t pos_of_char_1 = pos_of_char(static_cast<unsigned char>(encoded_string[pos + 1]));

        //
        // Emit the first output byte that is produced in each chunk:
        //
        ret.push_back(static_cast<std::string::value_type>(
            ((pos_of_char(static_cast<unsigned char>(encoded_string[pos + 0])) << 2) + ((pos_of_char_1 & 0x30) >> 4))));

        if ((pos + 2 < length_of_string) && // Check for data that is not padded with equal signs (which is
                                            // allowed by RFC 2045)
            ...
            //
            unsigned int pos_of_char_2 = pos_of_char(static_cast<unsigned char>(encoded_string[pos + 2]));
            ret.push_back(
            ...
            if ((pos + 3 < length_of_string) && encoded_string[pos + 3] != '=' && encoded_string[pos + 3] != '.') {
                ...
                //
                ret.push_back(static_cast<std::string::value_type>(
                    ((pos_of_char_2 & 0x03) << 6) + pos_of_char(static_cast<unsigned char>(encoded_string[pos + 3]))));
            }
        }
