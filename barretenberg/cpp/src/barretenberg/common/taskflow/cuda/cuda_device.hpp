...
{
    int threads = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&threads, cudaDevAttrMaxThreadsPerBlock, d),
                  "failed to query the maximum threads per block on device ",
                  d)
    return threads;
}
...
{
    int dim = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&dim, cudaDevAttrMaxBlockDimX, d),
                  "failed to query the maximum x-dimension per block on device ",
                  d)
    return dim;
}
...
{
    int dim = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&dim, cudaDevAttrMaxBlockDimY, d),
                  "failed to query the maximum y-dimension per block on device ",
                  d)
    return dim;
}
...
{
    int dim = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&dim, cudaDevAttrMaxBlockDimZ, d),
                  "failed to query the maximum z-dimension per block on device ",
                  d)
    return dim;
}
...
{
    int num = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&num, cudaDevAttrMaxSharedMemoryPerBlock, d),
                  "failed to query the maximum shared memory per block on device ",
                  d)
    return num;
}
...
inline size_t cuda_get_device_warp_size(int d)
{
    int num = 0;
    TF_CHECK_CUDA(
        cudaDeviceGetAttribute(&num, cudaDevAttrWarpSize, d), "failed to query the warp size per block on device ", d)
    return num;
}
...
{
    int num = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&num, cudaDevAttrComputeCapabilityMajor, d),
                  "failed to query the major number of compute capability of device ",
                  d)
    return num;
}
...
{
    int num = 0;
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&num, cudaDevAttrComputeCapabilityMinor, d),
                  "failed to query the minor number of compute capability of device ",
                  d)
    return num;
}
...
inline bool cuda_get_device_unified_addressing(int d)
{
    ...
    TF_CHECK_CUDA(cudaDeviceGetAttribute(&num, cudaDevAttrUnifiedAddressing, d),
                  "failed to query unified addressing status on device ",
                  d)
    return num;
}
