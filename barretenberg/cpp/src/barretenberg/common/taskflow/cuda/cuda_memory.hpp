...
inline size_t cuda_get_free_mem(int d)
{
    ...
    size_t free, total;
    TF_CHECK_CUDA(cudaMemGetInfo(&free, &total), "failed to get mem info on device ", d);
    return free;
}
...
the device memory.
*/
template <typename T> T* cuda_malloc_device(size_t N, int d)
{
    cudaScopedDevice ctx(d);
    T* ptr{ nullptr };
    TF_CHECK_CUDA(cudaMalloc(&ptr, N * sizeof(T)), "failed to allocate memory (", N * sizeof(T), "bytes) on device ", d)
    return ptr;
}
...
with the caller.
*/
template <typename T> T* cuda_malloc_device(size_t N)
{
    T* ptr{ nullptr };
    TF_CHECK_CUDA(cudaMalloc(&ptr, N * sizeof(T)), "failed to allocate memory (", N * sizeof(T), "bytes)")
    return ptr;
}
...
*/
template <typename T> T* cuda_malloc_shared(size_t N)
{
    T* ptr{ nullptr };
    TF_CHECK_CUDA(cudaMallocManaged(&ptr, N * sizeof(T)), "failed to allocate shared memory (", N * sizeof(T), "bytes)")
    return ptr;
}
...
*/
inline void cuda_memcpy_async(cudaStream_t stream, void* dst, const void* src, size_t count)
{
    TF_CHECK_CUDA(cudaMemcpyAsync(dst, src, count, cudaMemcpyDefault, stream), "failed to perform cudaMemcpyAsync");
}
...
*/
inline void cuda_memset_async(cudaStream_t stream, void* devPtr, int value, size_t count)
{
    TF_CHECK_CUDA(cudaMemsetAsync(devPtr, value, count, stream), "failed to perform cudaMemsetAsync");
}
...
@private
*/
template <typename T> struct cudaSharedMemory {
...
@private
*/
template <> struct cudaSharedMemory<int> {
    __device__ int* get()
    {
...
@private
*/
template <> struct cudaSharedMemory<unsigned int> {
...
@private
*/
template <> struct cudaSharedMemory<char> {
    __device__ char* get()
    {
...
@private
*/
template <> struct cudaSharedMemory<unsigned char> {
...
@private
*/
template <> struct cudaSharedMemory<short> {
...
@private
*/
template <> struct cudaSharedMemory<unsigned short> {
...
@private
*/
template <> struct cudaSharedMemory<long> {
    __device__ long* get()
    {
...
@private
*/
template <> struct cudaSharedMemory<unsigned long> {
...
@private
*/
template <> struct cudaSharedMemory<bool> {
    __device__ bool* get()
    {
...
@private
*/
template <> struct cudaSharedMemory<float> {
...
@private
*/
template <> struct cudaSharedMemory<double> {
...
template <typename T> class cudaDeviceAllocator {
  ...
  public:
    ...
    */
    ...
    {
        void* ptr = NULL;
        TF_CHECK_CUDA(cudaMalloc(&ptr, n * sizeof(T)), "failed to allocate ", n, " elements (", n * sizeof(T), "bytes)")
    ...
    */
    ...
    through @c a2.
    */
    template <typename U> bool operator==(const cudaDeviceAllocator<U>&) const noexcept { return true; }
    ...
    through @c a2.
    */
    template <typename U> bool operator!=(const cudaDeviceAllocator<U>&) const noexcept { return false; }
};
  ...
  public:
    ...
    */
    pointer allocate(size_type n, const void* = 0)
    {
        void* ptr{ nullptr };
        TF_CHECK_CUDA(
            cudaMallocManaged(&ptr, n * sizeof(T)), "failed to allocate ", n, " elements (", n * sizeof(T), "bytes)")
        return static_cast<pointer>(ptr);
    }
    ...
    */
    ...
    through @c a2.
    */
    template <typename U> bool operator==(const cudaUSMAllocator<U>&) const noexcept { return true; }
    ...
    through @c a2.
    */
    template <typename U> bool operator!=(const cudaUSMAllocator<U>&) const noexcept { return false; }
};
...
@private
*/
template <typename T> class cudaDeviceVector {

  public:
    cudaDeviceVector() = default;

    cudaDeviceVector(size_t N)
        : _N{ N }
    {
        if (N) {
            TF_CHECK_CUDA(
                cudaMalloc(&_data, N * sizeof(T)), "failed to allocate device memory (", N * sizeof(T), " bytes)");
        }
    }
