...
template <unsigned i, unsigned count, bool valid = (i < count)> struct cudaIterate {
    template <typename F> __device__ static void eval(F f)
    {
        f(i);
...
template <unsigned i, unsigned count> struct cudaIterate<i, count, false> {
    template <typename F> __device__ static void eval(F) {}
};

template <unsigned begin, unsigned end, typename F> __device__ void cuda_iterate(F f)
{
    cudaIterate<begin, end>::eval(f);
}

template <unsigned count, typename F> __device__ void cuda_iterate(F f)
{
    cuda_iterate<0, count>(f);
}

template <unsigned count, typename T> __device__ T reduce(const T (&x)[count])
{
    T y;
...
template <unsigned count, typename T> __device__ void fill(T (&x)[count], T val)
{
    cuda_iterate<count>([&](auto i) { x[i] = val; });
}

// Invoke unconditionally.
template <unsigned nt, unsigned vt, typename F> __device__ void cuda_strided_iterate(F f, unsigned tid)
{
...
template <unsigned vt, typename F> __device__ void cuda_thread_iterate(F f, unsigned tid)
{
...
template <typename T, unsigned size> struct cudaArray {
    T data[size];
...
template <typename T> struct cudaArray<T, 0> {
    __device__ T operator[](unsigned) const { return T(); }
    __device__ T& operator[](unsigned) { return *(T*)nullptr; }
};

template <typename T, typename V, unsigned size> struct cudaKVArray {
...
__device__ auto cuda_mem_to_reg_strided(I mem, unsigned tid, unsigned count)
{
    ...
    cudaArray<T, vt> x;
    cuda_strided_iterate<nt, vt, vt0>([&](auto i, auto j) { x[i] = mem[j]; }, tid, count);
    return x;
}

template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename T, typename it_t>
__device__ void cuda_reg_to_mem_strided(cudaArray<T, vt> x, unsigned tid, unsigned count, it_t mem)
{

    cuda_strided_iterate<nt, vt, vt0>([=](auto i, auto j) { mem[j] = x[i]; }, tid, count);
}

template <unsigned nt, unsigned vt, unsigned vt0 = vt, typename I, typename O>
__device__ auto cuda_transform_mem_to_reg_strided(I mem, unsigned tid, unsigned count, O op)
{
    using T = std::invoke_result_t<O, typename std::iterator_traits<I>::value_type>;
    cudaArray<T, vt> x;
    cuda_strided_iterate<nt, vt, vt0>([&](auto i, auto j) { x[i] = op(mem[j]); }, tid, count);
    return x;
}
...
__device__ auto cuda_shared_to_reg_thread(const T (&shared)[shared_size], unsigned tid, bool sync = true)
{
    ...
    cudaArray<T, vt> x;
    cuda_thread_iterate<vt>([&](auto i, auto j) { x[i] = shared[j]; }, tid);
...
__device__ void cuda_reg_to_shared_strided(cudaArray<T, vt> x, unsigned tid, T (&shared)[shared_size], bool sync = true)
{
    ...
    cuda_strided_iterate<nt, vt>([&](auto i, auto j) { shared[j] = x[i]; }, tid);
...
__device__ auto cuda_reg_to_mem_thread(
    cudaArray<T, vt> x, unsigned tid, unsigned count, it_t mem, T (&shared)[shared_size])
{
...
struct cudaLoadStoreIterator : std::iterator_traits<const T*> {
    ...
    __device__ assign_t operator[](I index) const { return assign_t{ load, store, base + index }; }

    __device__ assign_t operator*() const { return assign_t{ load, store, base }; }
...
template <typename T, typename I = unsigned, typename L> auto cuda_make_load_iterator(L load, I base = 0)
{
    return cuda_make_load_store_iterator<T>(load, cudaEmpty(), base);
}

template <typename T, typename I = unsigned, typename S> auto cuda_make_store_iterator(S store, I base = 0)
{
...
template <typename T> __device__ void cuda_swap(T& a, T& b)
{
    auto c = a;
    a = b;
    b = c;
...
template <typename F, typename... args_t> __global__ void cuda_kernel(F f, args_t... args)
{
...
template <class T> struct cuda_plus {
    __device__ T operator()(T a, T b) const { return a + b; }
};

template <class T> struct cuda_minus {
    __device__ T operator()(T a, T b) const { return a - b; }
};

template <class T> struct cuda_multiplies {
    __device__ T operator()(T a, T b) const { return a * b; }
};

template <class T> struct cuda_maximum {
    __device__ T operator()(T a, T b) const { return a > b ? a : b; }
};

template <class T> struct cuda_minimum {
    __device__ T operator()(T a, T b) const { return a < b ? a : b; }
};

template <class T> struct cuda_less {
    __device__ T operator()(T a, T b) const { return a < b; }
};

template <class T> struct cuda_greater {
    __device__ T operator()(T a, T b) const { return a > b; }
};
