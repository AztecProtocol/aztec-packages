...
*/
enum class cudaMergeBoundType { LOWER, UPPER };

/** @private */
template <typename T, unsigned N> struct cudaMergePair {
    cudaArray<T, N> keys;
...
struct cudaMergeRange {
    ...
    __device__ cudaRange a_range() const { return cudaRange{ a_begin, a_end }; }
    __device__ cudaRange b_range() const { return cudaRange{ b_begin, b_end }; }

    __device__ cudaMergeRange to_local() const { return cudaMergeRange{ 0, a_count(), a_count(), total() }; }
    ...
    __device__ cudaMergeRange partition(unsigned mp0, unsigned diag0, unsigned mp1, unsigned diag1) const
    {
        return cudaMergeRange{ a_begin + mp0, a_begin + mp1, b_begin + diag0 - mp0, b_begin + diag1 - mp1 };
    }

    __device__ bool a_valid() const { return a_begin < a_end; }

    __device__ bool b_valid() const { return b_begin < b_end; }
};

/** @private */
template <cudaMergeBoundType bounds = cudaMergeBoundType::LOWER,
          typename a_keys_it,
          typename b_keys_it,
          typename comp_t>
__device__ auto cuda_merge_path(
    a_keys_it a_keys, unsigned a_count, b_keys_it b_keys, unsigned b_count, unsigned diag, comp_t comp)
{
    ...
    while (beg < end) {
        ...
        bool pred = (cudaMergeBoundType::UPPER == bounds) ? comp(a_key, b_key) : !comp(b_key, a_key);
...
__device__ auto cuda_merge_path(keys_it keys, cudaMergeRange range, unsigned diag, comp_t comp)
{
    ...
    return cuda_merge_path<bounds>(
        keys + range.a_begin, range.a_count(), keys + range.b_begin, range.b_count(), diag, comp);
}
...
__device__ bool cuda_merge_predicate(T a_key, T b_key, cudaMergeRange range, comp_t comp)
{
    ...
    if (range_check && !range.a_valid()) {
        ...
        p = (cudaMergeBoundType::UPPER == bounds) ? comp(a_key, b_key) : !comp(b_key, a_key);
    }
    return p;
}
...
inline __device__ auto cuda_compute_merge_range(
    unsigned a_count, unsigned b_count, unsigned partition, unsigned spacing, unsigned mp0, unsigned mp1)
{
...
load_two_streams_reg(a_it a, unsigned a_count, b_it b, unsigned b_count, unsigned tid)
{
    ...
    cudaArray<T, vt> x;
    cuda_strided_iterate<nt, vt>(
        [&](auto i, auto index) { x[i] = (index < a_count) ? a[index] : b[index]; }, tid, a_count + b_count);
    return x;
}

/** @private */
template <unsigned nt, unsigned vt, typename A, typename B, typename T, unsigned S>
__device__ void cuda_load_two_streams_shared(
    A a, unsigned a_count, B b, unsigned b_count, unsigned tid, T (&shared)[S], bool sync = true)
{
...
/** @private */
template <unsigned nt, unsigned vt, typename T>
__device__ auto cuda_gather_two_streams_strided(
    const T* a, unsigned a_count, const T* b, unsigned b_count, cudaArray<unsigned, vt> indices, unsigned tid)
{
...
cuda_gather_two_streams_strided(
    a_it a, unsigned a_count, b_it b, unsigned b_count, cudaArray<unsigned, vt> indices, unsigned tid)
{

    b -= a_count;
    cudaArray<T, vt> x;
    cuda_strided_iterate<nt, vt>(
        [&](auto i, auto j) { x[i] = (indices[i] < a_count) ? a[indices[i]] : b[indices[i]]; }, tid, a_count + b_count);
...
__device__ void cuda_transfer_two_streams_strided(
    a_it a, unsigned a_count, b_it b, unsigned b_count, cudaArray<unsigned, vt> indices, unsigned tid, c_it c)
{
    ...
    a_it a, b_it b, cudaMergeRange range_mem, unsigned tid, comp_t comp, T (&keys_shared)[S])
{
    ...
    cuda_load_two_streams_shared<nt, vt>(
        a + range_mem.a_begin, range_mem.a_count(), b + range_mem.b_begin, range_mem.b_count(), tid, keys_shared, true);
    ...
    auto merged = cuda_serial_merge<bounds, vt>(keys_shared, range_local.partition(mp, diag), comp);
...
void cuda_merge_path_partitions(
    P&& p, a_keys_it a, unsigned a_count, b_keys_it b, unsigned b_count, unsigned spacing, comp_t comp, unsigned* buf)
{
                     ...
                     void* ptr)
{
    ...
    cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid, auto bid) {
        ...
        if (has_values) {
            ...
            cuda_transfer_two_streams_strided<E::nt>(a_vals + range.a_begin,
                                                     range.a_count(),
                                                     b_vals + range.b_begin,
                                                     range.b_count(),
                                                     indices,
                                                     tid,
                                                     c_vals + dest_offset);
