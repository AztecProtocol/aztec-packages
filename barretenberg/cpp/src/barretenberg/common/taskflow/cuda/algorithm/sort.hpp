...
__device__ inline auto cuda_compute_merge_sort_frame(unsigned partition, unsigned coop, unsigned spacing)
{
    ...
    return cudaMergeRange{ a_begin, a_begin + size, b_begin, b_begin + size };
}

/** @private */
__device__ inline auto cuda_compute_merge_sort_range(unsigned count,
                                                     unsigned partition,
                                                     unsigned coop,
                                                     unsigned spacing)
{
    ...
    return cudaMergeRange{ frame.a_begin, min(count, frame.a_end), min(count, frame.b_begin), min(count, frame.b_end) };
}

/** @private */
__device__ inline auto cuda_compute_merge_sort_range(
    unsigned count, unsigned partition, unsigned coop, unsigned spacing, unsigned mp0, unsigned mp1)
{
...
/** @private */
template <unsigned nt, unsigned vt, typename K, typename V> struct cudaBlockSort {
    ...
    __device__ auto merge_pass(
        cudaKVArray<K, V, vt> x, unsigned tid, unsigned count, unsigned pass, C comp, Storage& storage) const
    {
...
void merge_sort_loop(P&& p, K_it keys_input, V_it vals_input, unsigned count, C comp, void* buf)
{
    ...
    cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid, auto bid) {
        ...
        if (has_values) {
            unsorted.vals =
                cuda_mem_to_reg_thread<E::nt, E::vt>(vals_input + tile.begin, tid, tile.count(), shared.vals);
        }
    ...
    for (unsigned pass = 0; pass < R; ++pass) {
        ...
        cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid, auto bid) {
            ...
            if (has_values) {
                ...
                cuda_transfer_two_streams_strided<E::nt>(vals_input + range.a_begin,
                                                         range.a_count(),
                                                         vals_input + range.b_begin,
                                                         range.b_count(),
                                                         indices,
                                                         tid,
                                                         vals_output + tile.begin);
            }
        });
...
*/
template <typename P, typename K, typename V = cudaEmpty> unsigned cuda_sort_buffer_size(unsigned count)
{
    ...
    return R ? (count * sizeof(K) + (has_values ? count * sizeof(V) : 0) + (B + 1) * sizeof(unsigned)) : 0;
}
