...
template <unsigned nt, typename T> struct cudaBlockReduce {
    ...
    template <typename op_t> __device__ T operator()(unsigned, T, Storage&, unsigned, op_t, bool = true) const;
};
...
__device__ T
cudaBlockReduce<nt, T>::operator()(unsigned tid, T x, Storage& storage, unsigned count, op_t op, bool ret) const
{
...
void cuda_transform_reduce(P&& p, I first, I last, T* res, O bop, U uop, void* buf)
{
    ...
    // reduction loop
    detail::cuda_reduce_loop(
        p, cuda_make_load_iterator<T>([=] __device__(auto i) { return uop(*(first + i)); }), count, res, bop, buf);
}
