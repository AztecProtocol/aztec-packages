...
/** @private */
enum class cudaScanType : int { EXCLUSIVE = 1, INCLUSIVE };

/** @private */
template <typename T, unsigned vt = 0, bool is_array = (vt > 0)> struct cudaScanResult {
    T scan;
    T reduction;
};

/** @private */
template <typename T, unsigned vt> struct cudaScanResult<T, vt, true> {
    cudaArray<T, vt> scan;
    T reduction;
};
...
/** @private */
template <unsigned nt, typename T> struct cudaBlockScan {
    ...
    union storage_t {
        T data[2 * nt];
        struct {
            T threads[nt], warps[num_warps];
        };
    };

    // standard scan
    template <typename op_t>
    __device__ cudaScanResult<T> operator()(unsigned tid,
                                            T x,
                                            storage_t& storage,
                                            unsigned count = nt,
                                            op_t op = op_t(),
                                            T init = T(),
                                            cudaScanType type = cudaScanType::EXCLUSIVE) const;

    // vectorized scan. accepts multiple values per thread and adds in
    // optional global carry-in
    template <unsigned vt, typename op_t>
    __device__ cudaScanResult<T, vt> operator()(unsigned tid,
                                                cudaArray<T, vt> x,
                                                storage_t& storage,
                                                T carry_in = T(),
...
__device__ cudaScanResult<T> cudaBlockScan<nt, T>::operator()(
    unsigned tid, T x, storage_t& storage, unsigned count, op_t op, T init, cudaScanType type) const
{
    ...
    result.scan = (tid < count) ? (cudaScanType::INCLUSIVE == type ? x : (tid ? storage.data[first + tid - 1] : init))
                                : result.reduction;
    __syncthreads();
...
template <unsigned nt, typename T>
template <unsigned vt, typename op_t>
__device__ cudaScanResult<T, vt> cudaBlockScan<nt, T>::operator()(unsigned tid,
                                                                  cudaArray<T, vt> x,
                                                                  storage_t& storage,
                                                                  T carry_in,
                                                                  bool use_carry_in,
                                                                  ...
                                                                  op_t op,
                                                                  T init,
                                                                  cudaScanType type) const
{

    // Start with an inclusive scan of the in-range elements.
    if (count >= nt * vt) {
        cuda_iterate<vt>([&](auto i) { x[i] = i ? op(x[i], x[i - 1]) : x[i]; });
    } else {
        cuda_iterate<vt>([&](auto i) {
            auto index = vt * tid + i;
            x[i] = i ? ((index < count) ? op(x[i], x[i - 1]) : x[i - 1]) : (x[i] = (index < count) ? x[i] : init);
        });
    }
...
void cuda_single_pass_scan(P&& p, cudaScanType scan_type, I input, unsigned count, O output, C op
    ...
    if (B > cudaScanRecursionThreshold) {
        ...
        cuda_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__(auto tid, auto bid) {
            ...
            cuda_strided_iterate<E::nt, E::vt>(
                [&](auto i, auto j) { scalar = i ? op(scalar, x[i]) : x[0]; }, tid, tile.count());
...
void cuda_transform_inclusive_scan(P&& p, I first, I last, O output, C bop, U uop, void* buf)
{
    ...
    // launch the scan loop
    detail::cuda_scan_loop(p,
                           detail::cudaScanType::INCLUSIVE,
...
void cuda_exclusive_scan(P&& p, I first, I last, O output, C op, void* buf)
{
    ...
    // launch the scan loop
    detail::cuda_scan_loop(p, detail::cudaScanType::EXCLUSIVE, first, count, output, op, buf);
}
