...
class cudaFlow {
  ...
  public:
    ...
    */
    ...
    (e.g., @c cudaMalloc).
    */
    template <typename C> cudaTask host(C&& callable);
    ...
    */
    template <typename T,
              std::enable_if_t<is_pod_v<T> && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4), void>* = nullptr>
    ...
    */
    ...
    @endcode
    */
    template <typename C> cudaTask capture(C&& callable);
...
inline cudaTask cudaFlow::noop()
{
    ...
    auto node = _cfg.emplace_back(_cfg, std::in_place_type_t<cudaFlowNode::Empty>{});

    TF_CHECK_CUDA(cudaGraphAddEmptyNode(&node->_native_handle, _cfg._native_handle, nullptr, 0),
...
// Function: host
template <typename C> cudaTask cudaFlow::host(C&& c)
{

    auto node = _cfg.emplace_back(_cfg, std::in_place_type_t<cudaFlowNode::Host>{}, std::forward<C>(c));
    ...
    TF_CHECK_CUDA(cudaGraphAddHostNode(&node->_native_handle, _cfg._native_handle, nullptr, 0, &p),
...
// Function: kernel
template <typename F, typename... ArgsT> cudaTask cudaFlow::kernel(dim3 g, dim3 b, size_t s, F f, ArgsT... args)
{

    auto node = _cfg.emplace_back(_cfg, std::in_place_type_t<cudaFlowNode::Kernel>{}, (void*)f);
    ...
    TF_CHECK_CUDA(cudaGraphAddKernelNode(&node->_native_handle, _cfg._native_handle, nullptr, 0, &p),
...
cudaTask cudaFlow::zero(T* dst, size_t count)
{
    ...
    auto node = _cfg.emplace_back(_cfg, std::in_place_type_t<cudaFlowNode::Memset>{});

    auto p = cuda_get_zero_parms(dst, count);

    TF_CHECK_CUDA(cudaGraphAddMemsetNode(&node->_native_handle, _cfg._native_handle, nullptr, 0, &p),
...
cudaTask cudaFlow::copy(T* tgt, const T* src, size_t num)
{
    ...
    auto node = _cfg.emplace_back(_cfg, std::in_place_type_t<cudaFlowNode::Memcpy>{});

    auto p = cuda_get_copy_parms(tgt, src, num);

    TF_CHECK_CUDA(cudaGraphAddMemcpyNode(&node->_native_handle, _cfg._native_handle, nullptr, 0, &p),
...
// Function: host
template <typename C> void cudaFlow::host(cudaTask task, C&& c)
{
...
void cudaFlow::kernel(cudaTask task, dim3 g, dim3 b, size_t s, F f, ArgsT... args)
{
    ...
    TF_CHECK_CUDA(cudaGraphExecKernelNodeSetParams(_exe, task._node->_native_handle, &p),
                  "failed to update kernel parameters on ",
                  task);
}
...
void cudaFlow::copy(cudaTask task, T* tgt, const T* src, size_t num)
{
    ...
    TF_CHECK_CUDA(cudaGraphExecMemcpyNodeSetParams(_exe, task._node->_native_handle, &p),
                  "failed to update memcpy parameters on ",
                  task);
}

// Function: update memcpy parameters
inline void cudaFlow::memcpy(cudaTask task, void* tgt, const void* src, size_t bytes)
{
...
inline void cudaFlow::memset(cudaTask task, void* dst, int ch, size_t count)
{
    ...
    TF_CHECK_CUDA(cudaGraphExecMemsetNodeSetParams(_exe, task._node->_native_handle, &p),
                  "failed to update memset parameters on ",
                  task);
}
...
// Function: capture
template <typename C> void cudaFlow::capture(cudaTask task, C c)
{
    ...
    TF_CHECK_CUDA(
        cudaGraphExecChildGraphNodeSetParams(_exe, task._node->_native_handle, node_handle->cfg._native_handle),
...
// Function: capture
template <typename C> cudaTask cudaFlow::capture(C&& c)
{

    // insert a subflow node
    auto node = _cfg.emplace_back(_cfg, std::in_place_type_t<cudaFlowNode::Subflow>{});
    ...
    TF_CHECK_CUDA(cudaGraphAddChildGraphNode(
                      &node->_native_handle, _cfg._native_handle, nullptr, 0, node_handle->cfg._native_handle),
