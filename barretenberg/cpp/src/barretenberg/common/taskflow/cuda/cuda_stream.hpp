...
class cudaStream :
  ...
  public:
    ...
    */
    void synchronize() const { TF_CHECK_CUDA(cudaStreamSynchronize(object), "failed to synchronize a CUDA stream"); }
    ...
    */
    ...
    {
        TF_CHECK_CUDA(
            cudaStreamBeginCapture(object, m), "failed to begin capture on stream ", object, " with thread mode ", m);
    }
...
*/
class cudaEvent : public cudaObject<cudaEvent_t, cudaEventCreator, cudaEventDeleter> {
  ...
  public:
    ...
    */
    explicit cudaEvent(unsigned int flag)
        : cudaObject(cudaEventCreator{}(flag))
    {}
};
