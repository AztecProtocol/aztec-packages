...
*/
inline std::vector<std::pair<cudaGraphNode_t, cudaGraphNode_t>> cuda_graph_get_edges(cudaGraph_t graph)
{
...
inline cudaGraphNodeType cuda_get_graph_node_type(cudaGraphNode_t node)
{
    ...
    TF_CHECK_CUDA(cudaGraphNodeGetType(node, &type), "failed to get native graph node type");
    return type;
}
...
*/
template <typename T> void cuda_dump_graph(T& os, cudaGraph_t g)
{
    ...
    while (stack.empty() == false) {
        ...
        for (auto& node : nodes) {
            ...
            if (type == cudaGraphNodeTypeGraph) {
                ...
                os << 'p' << node << "[label=\"" << cuda_graph_node_type_to_string(type) << "\"];\n";
            }
        }
...
*/
class cudaGraph : public cudaObject<cudaGraph_t, cudaGraphCreator, cudaGraphDeleter> {
  ...
  public:
    ...
    */
    explicit cudaGraph(cudaGraph_t native)
        : cudaObject(native)
    {}
...
*/
class cudaGraphExec : public cudaObject<cudaGraphExec_t, cudaGraphExecCreator, cudaGraphExecDeleter> {
  ...
  public:
    ...
    */
    explicit cudaGraphExec(cudaGraphExec_t exec)
        : cudaObject(exec)
    {}
    ...
    */
    void instantiate(cudaGraph_t graph)
    {
        ...
        TF_CHECK_CUDA(cudaGraphInstantiate(&object, graph, nullptr, nullptr, 0),
...
class cudaFlowGraph {
  ...
  public:
    ...
    template <typename... ArgsT> cudaFlowNode* emplace_back(ArgsT&&...);
...
class cudaFlowNode {
    ...
    struct Kernel {
        ...
        template <typename F> Kernel(F&& f);
    ...
    using handle_t = std::variant<Empty, Host, Memset, Memcpy, Kernel, Subflow, Capture>;
  ...
  public:
    ...
    template <typename... ArgsT> cudaFlowNode(cudaFlowGraph&, ArgsT&&...);
...
template <typename C>
cudaFlowNode::Host::Host(C&& c)
    : func{ std::forward<C>(c) }
{}
...
inline void cudaFlowNode::_precede(cudaFlowNode* v)
{
    ...
    if (_handle.index() != cudaFlowNode::CAPTURE) {
        TF_CHECK_CUDA(cudaGraphAddDependencies(_cfg._native_handle, &_native_handle, &v->_native_handle, 1),
...
template <typename... ArgsT> cudaFlowNode* cudaFlowGraph::emplace_back(ArgsT&&... args)
{
