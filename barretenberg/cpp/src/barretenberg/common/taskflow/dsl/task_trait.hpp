...
template <typename TASK, typename CONTEXT> struct TaskCb {
    using TaskType = TASK;
    void build(FlowBuilder& build, const CONTEXT& context) { task_ = build.emplace(TaskType{ context }()); }
...
// a task self
template <typename TASK> struct TaskTrait<TASK, std::enable_if_t<std::is_base_of<TaskSignature, TASK>::value>> {
