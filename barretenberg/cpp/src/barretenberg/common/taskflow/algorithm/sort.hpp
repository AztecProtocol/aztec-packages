...
template <typename I> constexpr size_t parallel_sort_cutoff()
{
    ...
    if constexpr (std::is_same_v<value_type, std::string>) {
        ...
        if constexpr (object_size < 16)
            return 4096;
        else if constexpr (object_size < 32)
            return 2048;
        else if constexpr (object_size < 64)
            return 1024;
        else if constexpr (object_size < 128)
            return 768;
        else if constexpr (object_size < 256)
            return 512;
        else if constexpr (object_size < 512)
            return 256;
        else
            return 128;
    }
...
template <typename T, size_t cacheline_size = 64> inline T* align_cacheline(T* p)
{
...
inline void swap_offsets(
    Iter first, Iter last, unsigned char* offsets_l, unsigned char* offsets_r, size_t num, bool use_swaps)
{
...
template <typename RandItr, typename Compare> void insertion_sort(RandItr begin, RandItr end, Compare comp)
{
...
// in [begin, end).
template <typename RandItr, typename Compare> void unguarded_insertion_sort(RandItr begin, RandItr end, Compare comp)
{
...
template <typename RandItr, typename Compare> bool partial_insertion_sort(RandItr begin, RandItr end, Compare comp)
{
...
template <typename Iter, typename Compare> std::pair<Iter, bool> partition_right(Iter begin, Iter end, Compare comp)
{
...
template <typename RandItr, typename Compare> RandItr partition_left(RandItr begin, RandItr end, Compare comp)
{
...
void parallel_pdqsort(tf::Runtime& rt, Iter begin, Iter end, Compare comp, int bad_allowed, bool leftmost = true)
{
    ...
    while (true) {
        ...
        const auto pair = Branchless ? partition_right_branchless(begin, end, comp) : partition_right(begin, end, comp);
        ...
        rt.silent_async([&rt, begin, pivot_pos, comp, bad_allowed, leftmost]() mutable {
...
// 3-way quick sort
template <typename RandItr, typename C> void parallel_3wqsort(tf::Runtime& rt, RandItr first, RandItr last, C compare)
{
...
template <typename B, typename E, typename C> TF_FORCE_INLINE auto make_sort_task(B b, E e, C cmp)
{
...
template <typename B, typename E> TF_FORCE_INLINE auto make_sort_task(B beg, E end)
{
...
// Function: sort
template <typename B, typename E, typename C> Task FlowBuilder::sort(B beg, E end, C cmp)
{
    return emplace(make_sort_task(beg, end, cmp));
}

// Function: sort
template <typename B, typename E> Task FlowBuilder::sort(B beg, E end)
{
