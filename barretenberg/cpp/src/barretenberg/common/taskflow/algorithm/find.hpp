...
bool find_if_loop(
    std::atomic<size_t>& offset, Iterator& beg, size_t& prev_e, size_t curr_b, size_t curr_e, Predicate predicate)
{
    // early prune
    ...
    return [=, &result](Runtime& rt) mutable {
        ...
        if constexpr (part.type() == PartitionerType::STATIC) {
            ...
            for (size_t w = 0, curr_b = 0; w < W && curr_b < N; ++w, curr_b += chunk_size) {
                ...
                launch_loop(W, w, rt, part, [N, W, curr_b, chunk_size, beg, &predicate, &offset, &part]() mutable {
                    part.loop_until(
                        N, W, curr_b, chunk_size, [&, prev_e = size_t{ 0 }](size_t part_b, size_t part_e) mutable {
                            return detail::find_if_loop(offset, beg, prev_e, part_b, part_e, predicate);
                        });
                });
            }
        ...
        // dynamic partitioner
            ...
            launch_loop(N, W, rt, next, part, [N, W, beg, &predicate, &offset, &next, &part]() mutable {
                part.loop_until(N, W, next, [&, prev_e = size_t{ 0 }](size_t curr_b, size_t curr_e) mutable {
...
auto make_find_if_not_task(B first, E last, T& result, UOP predicate, P part = P())
{
    ...
    return [=, &result](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= part.chunk_size()) {
            launch_loop(part, [&]() { result = std::find_if_not(beg, end, predicate); });
            return;
        }
        ...
        if constexpr (part.type() == PartitionerType::STATIC) {
            ...
            for (size_t w = 0, curr_b = 0; w < W && curr_b < N; ++w, curr_b += chunk_size) {
                ...
                launch_loop(W, w, rt, part, [N, W, curr_b, chunk_size, beg, &predicate, &offset, &part]() mutable {
                    part.loop_until(
                        N, W, curr_b, chunk_size, [&, prev_e = size_t{ 0 }](size_t part_b, size_t part_e) mutable {
                            return detail::find_if_not_loop(offset, beg, prev_e, part_b, part_e, predicate);
                        });
                });
            }
...
auto make_min_element_task(B first, E last, T& result, C comp, P part = P())
{
    ...
    return [=, &result](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= part.chunk_size()) {
            launch_loop(part, [&]() { result = std::min_element(beg, end, comp); });
            return;
        }
        ...
        if constexpr (part.type() == PartitionerType::STATIC) {
            ...
            for (size_t w = 0, curr_b = 0; w < W && curr_b < N; ++w, curr_b += chunk_size) {
                ...
                launch_loop(W, w, rt, part, [beg, curr_b, N, W, chunk_size, &comp, &mutex, &result, &part]() mutable {
        ...
        // dynamic partitioner
            ...
            std::atomic<size_t> next(0);
            launch_loop(N, W, rt, next, part, [beg, N, W, &next, &comp, &mutex, &result, &part]() mutable {
                // pre-reduce
                ...
                // loop reduce
                part.loop(N, W, next, [&, prev_e = s0 + 2](size_t part_b, size_t part_e) mutable {
                    std::advance(beg, part_b - prev_e);
                    for (size_t x = part_b; x < part_e; x++, beg++) {
                        if (comp(*beg, *smallest)) {
                            smallest = beg;
...
auto make_max_element_task(B first, E last, T& result, C comp, P part = P())
{
    ...
    return [=, &result](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= part.chunk_size()) {
            launch_loop(part, [&]() { result = std::max_element(beg, end, comp); });
            return;
        }
