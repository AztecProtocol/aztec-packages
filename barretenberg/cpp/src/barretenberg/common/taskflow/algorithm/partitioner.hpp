...
*/
template <typename C = DefaultClosureWrapper> class PartitionerBase : public IsPartitioner {
  ...
  public:
    ...
    */
    template <typename F> void closure_wrapper(F&& fn) { _closure_wrapper = std::forward<F>(fn); }
...
@endcode
*/
template <typename C = DefaultClosureWrapper> class GuidedPartitioner : public PartitionerBase<C> {
  ...
  public:
        ...
        while (curr_b < N) {
            ...
            // coarse-grained
                ...
                if (next.compare_exchange_strong(
                        curr_b, curr_e, std::memory_order_relaxed, std::memory_order_relaxed)) {
                    func(curr_b, curr_e);
    ...
    */
    ...
    void loop_until(size_t N, size_t W, std::atomic<size_t>& next, F&& func) const
    {
...
@endcode
*/
template <typename C = DefaultClosureWrapper> class DynamicPartitioner : public PartitionerBase<C> {
  ...
  public:
    ...
    */
    template <typename F, std::enable_if_t<std::is_invocable_r_v<bool, F, size_t, size_t>, void>* = nullptr>
    void loop_until(size_t N, size_t, std::atomic<size_t>& next, F&& func) const
    {
...
@endcode
*/
template <typename C = DefaultClosureWrapper> class StaticPartitioner : public PartitionerBase<C> {
  ...
  public:
    ...
    */
    template <typename F, std::enable_if_t<std::is_invocable_r_v<bool, F, size_t, size_t>, void>* = nullptr>
    void loop_until(size_t N, size_t W, size_t curr_b, size_t chunk_size, F&& func)
    {
        size_t stride = W * chunk_size;
        while (curr_b < N) {
            size_t curr_e = std::min(curr_b + chunk_size, N);
...
@endcode
*/
template <typename C = DefaultClosureWrapper> class RandomPartitioner : public PartitionerBase<C> {
...
*/
template <typename P> inline constexpr bool is_partitioner_v = std::is_base_of<IsPartitioner, P>::value;
