        ...
        if constexpr (part.type() == PartitionerType::STATIC) {
            ...
            for (size_t w = 0, curr_b = 0; w < W && curr_b < N; ++w, curr_b += chunk_size) {
                ...
                launch_loop(W, w, rt, part, [=, &part]() mutable {
                    part.loop(
                        N, W, curr_b, chunk_size, [&, prev_e = size_t{ 0 }](size_t part_b, size_t part_e) mutable {
...
{
    return emplace(make_transform_task(first1, last1, d_first, c, part));
}
...
{
    return emplace(make_transform_task(first1, last1, first2, d_first, c, part));
}
