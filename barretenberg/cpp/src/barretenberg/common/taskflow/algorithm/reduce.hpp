    ...
    return [=, &r = init](Runtime& rt) mutable {
        ...
        if constexpr (part.type() == PartitionerType::STATIC) {
            ...
            for (size_t w = 0, curr_b = 0; w < W && curr_b < N; ++w, curr_b += chunk_size) {
                ...
                launch_loop(W, w, rt, part, [=, &bop, &mtx, &r, &part]() mutable {
                    ...
                    // loop reduce
                    part.loop(N, W, curr_b, chunk_size, [&, prev_e = curr_b + 2](size_t part_b, size_t part_e) mutable {
                        if (part_b > prev_e) {
        ...
        // dynamic partitioner
            ...
            launch_loop(N, W, rt, next, part, [=, &bop, &mtx, &next, &r, &part]() mutable {
                ...
                // loop reduce
                part.loop(N, W, next, [&, prev_e = s0 + 2](size_t curr_b, size_t curr_e) mutable {
...
template <typename B, typename E, typename T, typename BOP, typename UOP, typename P = DefaultPartitioner>
            ...
            for (size_t w = 0, curr_b = 0; w < W && curr_b < N; ++w, curr_b += chunk_size) {
                ...
                launch_loop(W, w, rt, part, [=, &bop, &uop, &mtx, &r, &part]() mutable {
                    ...
                    part.loop(N,
                              ...
                              curr_b,
                              chunk_size,
                              [&, prev_e = curr_b + (chunk_size == 1 ? 1 : 2)](size_t part_b, size_t part_e) mutable {
                                  if (part_b > prev_e) {
                                      std::advance(beg, part_b - prev_e);
                                  } else {
                                      part_b = prev_e;
...
auto make_transform_reduce_task(B1 b1, E1 e1, B2 b2, T& init, BOP_R bop_r, BOP_T bop_t, P part = P())
{
