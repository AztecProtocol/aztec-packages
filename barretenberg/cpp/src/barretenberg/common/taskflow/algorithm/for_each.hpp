...
auto make_for_each_task(B b, E e, C c, P part = P())
{
    ...
    return [=](Runtime& rt) mutable {
        ...
        if (W <= 1 || N <= part.chunk_size()) {
            launch_loop(part, [&]() { std::for_each(beg, end, c); });
            return;
        }
...
// Function: for_each
template <typename B, typename E, typename C, typename P> Task FlowBuilder::for_each(B beg, E end, C c, P part)
{
    return emplace(make_for_each_task(beg, end, c, part));
}
...
{
    return emplace(make_for_each_index_task(beg, end, inc, c, part));
}
