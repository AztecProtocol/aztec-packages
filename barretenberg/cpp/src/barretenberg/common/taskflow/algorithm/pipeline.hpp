...
class DeferredPipeflow {
    ...
    template <typename... Ps> friend class Pipeline;

    template <typename P> friend class ScalablePipeline;
...
class Pipeflow {
    ...
    template <typename... Ps> friend class DataPipeline;
  ...
  public:
    ...
    */
    size_t pipe() const { return _pipe; }

    /**
    @brief queries the token identifier
    */
    size_t token() const { return _token; }
    ...
    */
    size_t num_deferrals() const { return _num_deferrals; }
...
*/
template <typename C = std::function<void(tf::Pipeflow&)>> class Pipe {
  ...
  public:
    ...
    */
    PipeType type() const { return _type; }
    ...
    */
    void type(PipeType type) { _type = type; }
    ...
    */
    template <typename U> void callable(U&& callable) { _callable = std::forward<U>(callable); }
...
template <typename... Ps> class Pipeline {
  ...
  private:
    ...
    template <size_t... I> auto _gen_meta(std::tuple<Ps...>&&, std::index_sequence<I...>);
...
// Function: num_lines
template <typename... Ps> size_t Pipeline<Ps...>::num_lines() const noexcept
{
    return _pipeflows.size();
}

// Function: num_pipes
template <typename... Ps> constexpr size_t Pipeline<Ps...>::num_pipes() const noexcept
{
    return sizeof...(Ps);
}

// Function: num_tokens
template <typename... Ps> size_t Pipeline<Ps...>::num_tokens() const noexcept
{
    return _num_tokens;
}

// Function: graph
template <typename... Ps> Graph& Pipeline<Ps...>::graph()
{
    return _graph;
}

// Function: reset
template <typename... Ps> void Pipeline<Ps...>::reset()
{
...
// Procedure: _on_pipe
template <typename... Ps> void Pipeline<Ps...>::_on_pipe(Pipeflow& pf, Runtime& rt)
{
    visit_tuple(
        [&](auto&& pipe) {
...
template <typename... Ps> void Pipeline<Ps...>::_check_dependents(Pipeflow& pf)
{
...
template <typename... Ps> void Pipeline<Ps...>::_construct_deferred_tokens(Pipeflow& pf)
{
    ...
    // auto res = _deferred_tokens.emplace(
    _deferred_tokens.emplace(std::piecewise_construct,
...
template <typename... Ps> void Pipeline<Ps...>::_resolve_token_dependencies(Pipeflow& pf)
{
...
// Procedure: _build
template <typename... Ps> void Pipeline<Ps...>::_build()
{
...
// move assignment operator
template <typename P> ScalablePipeline<P>& ScalablePipeline<P>::operator=(ScalablePipeline&& rhs)
{
    _num_tokens = rhs._num_tokens;
    _pipes = std::move(rhs._pipes);
    _pipeflows = std::move(rhs._pipeflows);
...
// Function: num_lines
template <typename P> size_t ScalablePipeline<P>::num_lines() const noexcept
{
    return _pipeflows.size();
}

// Function: num_pipes
template <typename P> size_t ScalablePipeline<P>::num_pipes() const noexcept
{
    return _pipes.size();
}

// Function: num_tokens
template <typename P> size_t ScalablePipeline<P>::num_tokens() const noexcept
{
    return _num_tokens;
}

// Function: graph
template <typename P> Graph& ScalablePipeline<P>::graph()
{
    return _graph;
}

// Function: _line
template <typename P> typename ScalablePipeline<P>::Line& ScalablePipeline<P>::_line(size_t l, size_t p)
{
    return _lines[l * num_pipes() + p];
}

template <typename P> void ScalablePipeline<P>::reset(size_t num_lines, P first, P last)
{
...
// Function: reset
template <typename P> void ScalablePipeline<P>::reset(P first, P last)
{
...
// Function: reset
template <typename P> void ScalablePipeline<P>::reset()
{
...
// Procedure: _on_pipe
template <typename P> void ScalablePipeline<P>::_on_pipe(Pipeflow& pf, Runtime& rt)
{
...
template <typename P> void ScalablePipeline<P>::_check_dependents(Pipeflow& pf)
{
    ++pf._num_deferrals;
...
// Procedure: _construct_deferred_tokens
// Construct a data structure for a deferred token
template <typename P> void ScalablePipeline<P>::_construct_deferred_tokens(Pipeflow& pf)
{
...
// Procedure: _resolve_token_dependencies
// Resolve dependencies for tokens that defer to current token
template <typename P> void ScalablePipeline<P>::_resolve_token_dependencies(Pipeflow& pf)
{
...
// Procedure: _build
template <typename P> void ScalablePipeline<P>::_build()
{
    ...
    for (size_t l = 0; l < num_lines(); l++) {
        ...
        _tasks[l + 1] =
            fb.emplace([this, l](tf::Runtime& rt) mutable {
                  auto pf = &_pipeflows[l];

              pipeline:

                  _line(pf->_line, pf->_pipe)
                      .join_counter.store(static_cast<size_t>(_pipes[pf->_pipe]->type()), std::memory_order_relaxed);
                  ...
                  if (_pipes[c_f]->type() == PipeType::SERIAL &&
                      _line(n_l, c_f).join_counter.fetch_sub(1, std::memory_order_acq_rel) == 1) {
                      retval[n++] = 1;
                  }
