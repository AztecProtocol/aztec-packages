               ...
               size_t chunk_size)
{
    ...
    rt.executor().corun_until([&counter]() { return counter.load(std::memory_order_acquire) == 0; });
...
auto make_inclusive_scan_task(B first, E last, D d_first, BOP bop, P part = P())
{
    ...
    return [=](Runtime& rt) mutable {
        ...
        if (W <= 1 || N <= 2) {
            launch_loop(part, [&]() { std::inclusive_scan(s_beg, s_end, d_beg, bop); });
            return;
        }
...
auto make_inclusive_scan_task(B first, E last, D d_first, BOP bop, T init, P part = P())
{
    ...
    return [=](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= 2) {
            launch_loop(part, [&]() { std::inclusive_scan(s_beg, s_end, d_beg, bop, init); });
            return;
        }
...
template <typename B,
          ...
          typename P = DefaultPartitioner,
          std::enable_if_t<is_partitioner_v<std::decay_t<P>>, void>* = nullptr>
auto make_transform_inclusive_scan_task(B first, E last, D d_first, BOP bop, UOP uop, P part = P())
{
    ...
    return [=](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= 2) {
            launch_loop(part, [&]() { std::transform_inclusive_scan(s_beg, s_end, d_beg, bop, uop); });
            return;
        }
...
template <typename B,
          ...
          typename P = DefaultPartitioner,
          std::enable_if_t<!is_partitioner_v<std::decay_t<T>>, void>* = nullptr>
auto make_transform_inclusive_scan_task(B first, E last, D d_first, BOP bop, UOP uop, T init, P part = P())
{
    ...
    return [=](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= 2) {
            launch_loop(part, [&]() { std::transform_inclusive_scan(s_beg, s_end, d_beg, bop, uop, init); });
            return;
        }
...
auto make_exclusive_scan_task(B first, E last, D d_first, T init, BOP bop, P part = P())
{
    ...
    return [=](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= 2) {
            launch_loop(part, [&]() { std::exclusive_scan(s_beg, s_end, d_beg, init, bop); });
            return;
        }
...
auto make_transform_exclusive_scan_task(B first, E last, D d_first, T init, BOP bop, UOP uop, P part = P())
{
    ...
    return [=](Runtime& rt) mutable {
        ...
        // only myself - no need to spawn another graph
        if (W <= 1 || N <= 2) {
            launch_loop(part, [&]() { std::transform_exclusive_scan(s_beg, s_end, d_beg, init, bop, uop); });
            return;
        }
...
template <typename B,
          ...
          typename P,
          std::enable_if_t<is_partitioner_v<std::decay_t<P>>, void>*>
Task FlowBuilder::transform_inclusive_scan(B first, E last, D d_first, BOP bop, UOP uop, P part)
{
    return emplace(make_transform_inclusive_scan_task(first, last, d_first, bop, uop, part));
}
...
template <typename B,
          ...
          typename P,
          std::enable_if_t<!is_partitioner_v<std::decay_t<T>>, void>*>
Task FlowBuilder::transform_inclusive_scan(B first, E last, D d_first, BOP bop, UOP uop, T init, P part)
{
    return emplace(make_transform_inclusive_scan_task(first, last, d_first, bop, uop, init, part));
}
...
{
    return emplace(make_exclusive_scan_task(first, last, d_first, init, bop, part));
}
...
Task FlowBuilder::transform_exclusive_scan(B first, E last, D d_first, T init, BOP bop, UOP uop, P part)
{
    return emplace(make_transform_exclusive_scan_task(first, last, d_first, init, bop, uop, part));
}
