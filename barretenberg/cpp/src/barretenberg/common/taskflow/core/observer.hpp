...
struct Segment {
    ...
    template <typename Archiver> auto save(Archiver& ar) const { return ar(name, type, beg, end); }

    template <typename Archiver> auto load(Archiver& ar) { return ar(name, type, beg, end); }

    Segment() = default;

    Segment(const std::string& n, TaskType t, observer_stamp_t b, observer_stamp_t e)
        : name{ n }
        , type{ t }
        , beg{ b }
        , end{ e }
    ...
    auto span() const { return end - beg; }
};
...
struct Timeline {
    ...
    template <typename Archiver> auto save(Archiver& ar) const { return ar(uid, origin, segments); }

    template <typename Archiver> auto load(Archiver& ar) { return ar(uid, origin, segments); }
};
...
struct ProfileData {
    ...
    template <typename Archiver> auto save(Archiver& ar) const { return ar(timelines); }

    template <typename Archiver> auto load(Archiver& ar) { return ar(timelines); }
};
...
class ChromeObserver : public ObserverInterface {
    ...
    struct Segment {
        ...
        Segment(const std::string& n, observer_stamp_t b, observer_stamp_t e);
    };
    ...
    for (size_t w = first; w < _timeline.segments.size(); w++) {
        ...
        for (size_t i = 0; i < _timeline.segments[w].size(); i++) {
            ...
            os << "\"ph\":\"X\","
               ...
               << ',' << "\"dur\":"
               << duration_cast<microseconds>(_timeline.segments[w][i].end - _timeline.segments[w][i].beg).count();
...
{
    return std::accumulate(_timeline.segments.begin(),
                           _timeline.segments.end(),
                           size_t{ 0 },
...
inline void TFProfObserver::Summary::dump_tsum(std::ostream& os) const
{
    ...
    os << std::setw(type_w) << "-Task-" << std::setw(count_w + 2) << "Count" << std::setw(time_w + 2) << "Time (us)"
       << std::setw(avg_w + 2) << "Avg (us)" << std::setw(min_w + 2) << "Min (us)" << std::setw(max_w + 2) << "Max (us)"
       << '\n';
    ...
    for (size_t i = 0; i < TASK_TYPES.size(); i++) {
        ...
        os << std::setw(type_w) << to_string(TASK_TYPES[i]) << std::setw(count_w + 2) << tsum[i].count
           << std::setw(time_w + 2) << tsum[i].total_span << std::setw(avg_w + 2) << std::to_string(tsum[i].avg_span())
           << std::setw(min_w + 2) << tsum[i].min_span << std::setw(max_w + 2) << tsum[i].max_span << '\n';
    }
}
...
inline void TFProfObserver::Summary::dump_wsum(std::ostream& os) const
{
    ...
    os << std::setw(w_w) << "-Worker-" << std::setw(l_w + 2) << "Level" << std::setw(t_w) << "Task"
       << std::setw(c_w + 2) << "Count" << std::setw(d_w + 2) << "Time (us)" << std::setw(avg_w + 2) << "Avg (us)"
       << std::setw(min_w + 2) << "Min (us)" << std::setw(max_w + 2) << "Max (us)" << '\n';
    ...
    for (const auto& ws : wsum) {
        ...
        os << std::setw(w_w) << ws.id << std::setw(l_w + 2) << ws.level;
        ...
        for (size_t i = 0; i < TASK_TYPES.size(); i++) {
            ...
            os << to_string(TASK_TYPES[i]) << std::setw(c_w + 2) << ws.tsum[i].count << std::setw(d_w + 2)
               << ws.tsum[i].total_span << std::setw(avg_w + 2) << std::to_string(ws.tsum[i].avg_span())
               << std::setw(min_w + 2) << ws.tsum[i].min_span << std::setw(max_w + 2) << ws.tsum[i].max_span << '\n';
        }

        // per-worker summary
        os << std::setw(w_w + l_w + t_w + c_w + 4) << ws.count << std::setw(d_w + 2) << ws.total_span
           << std::setw(avg_w + 2) << std::to_string(ws.avg_span()) << std::setw(min_w + 2) << ws.min_span
...
inline void TFProfObserver::on_exit(WorkerView wv, TaskView tv)
{
    ...
    _timeline.segments[w][_stacks[w].size()].emplace_back(tv.name(), tv.type(), beg, observer_stamp_t::clock::now());
}
    ...
    for (size_t w = first; w < _timeline.segments.size(); w++) {
        for (size_t l = 0; l < _timeline.segments[w].size(); l++) {
            ...
            for (size_t i = 0; i < _timeline.segments[w][l].size(); ++i) {
                ...
                // span
                os << "{\"span\":[" << duration_cast<microseconds>(s.beg - _timeline.origin).count() << ","
...
end_of_summary:
    ...
    os << "==Observer " << _timeline.uid << ": " << num_workers() << " workers completed " << num_tasks()
       << " tasks in " << view << " us\n";
