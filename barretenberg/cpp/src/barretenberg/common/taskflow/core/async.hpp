...
// Function: async
template <typename P, typename F> auto Executor::async(P&& params, F&& f)
{
...
// Function: async
template <typename F> auto Executor::async(F&& f)
{
...
template <typename P, typename F> void Executor::silent_async(P&& params, F&& f)
{
...
// Function: silent_async
template <typename F> void Executor::silent_async(F&& f)
{
...
{
    return silent_dependent_async(DefaultTaskParams{}, std::forward<F>(func), std::forward<Tasks>(tasks)...);
}
...
tf::AsyncTask Executor::silent_dependent_async(P&& params, F&& func, Tasks&&... tasks)
{
    ...
    AsyncTask task(node_pool.animate(std::forward<P>(params),
                                     nullptr,
                                     nullptr,
                                     num_dependents,
...
tf::AsyncTask Executor::silent_dependent_async(P&& params, F&& func, I first, I last)
{
...
add_successor:
    ...
    if (state.compare_exchange_weak(
            target, Node::AsyncState::LOCKED, std::memory_order_acq_rel, std::memory_order_acquire)) {
...
inline void Executor::_tear_down_dependent_async(Worker& worker, Node* node)
{
    ...
    while (!handle->state.compare_exchange_weak(
        target, Node::AsyncState::FINISHED, std::memory_order_acq_rel, std::memory_order_relaxed)) {
