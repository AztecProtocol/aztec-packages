...
<a href="https://www.di.ens.fr/~zappa/readings/ppopp13.pdf">Correct and Efficient Work-Stealing for Weak Memory
Models</a>, and extends it to include priority.
...
*/
template <typename T, unsigned TF_MAX_PRIORITY = static_cast<unsigned>(TaskPriority::MAX)> class TaskQueue {
    ...
    struct Array {
        ...
        void push(int64_t i, T o) noexcept { S[i & M].store(o, std::memory_order_relaxed); }

        T pop(int64_t i) noexcept { return S[i & M].load(std::memory_order_relaxed); }
...
// Constructor
template <typename T, unsigned TF_MAX_PRIORITY> TaskQueue<T, TF_MAX_PRIORITY>::TaskQueue(int64_t c)
{
    assert(c && (!(c & (c - 1))));
...
// Destructor
template <typename T, unsigned TF_MAX_PRIORITY> TaskQueue<T, TF_MAX_PRIORITY>::~TaskQueue()
{
    unroll<0, TF_MAX_PRIORITY, 1>([&](auto p) {
...
// Function: empty
template <typename T, unsigned TF_MAX_PRIORITY> bool TaskQueue<T, TF_MAX_PRIORITY>::empty() const noexcept
{
...
// Function: empty
template <typename T, unsigned TF_MAX_PRIORITY> bool TaskQueue<T, TF_MAX_PRIORITY>::empty(unsigned p) const noexcept
{
...
// Function: size
template <typename T, unsigned TF_MAX_PRIORITY> size_t TaskQueue<T, TF_MAX_PRIORITY>::size() const noexcept
{
    size_t s = 0;
...
// Function: size
template <typename T, unsigned TF_MAX_PRIORITY> size_t TaskQueue<T, TF_MAX_PRIORITY>::size(unsigned p) const noexcept
{
    int64_t b = _bottom[p].data.load(std::memory_order_relaxed);
    int64_t t = _top[p].data.load(std::memory_order_relaxed);
...
// Function: pop
template <typename T, unsigned TF_MAX_PRIORITY> T TaskQueue<T, TF_MAX_PRIORITY>::pop()
{
    for (unsigned i = 0; i < TF_MAX_PRIORITY; i++) {
        if (auto t = pop(i); t) {
            return t;
...
// Function: pop
template <typename T, unsigned TF_MAX_PRIORITY> TF_FORCE_INLINE T TaskQueue<T, TF_MAX_PRIORITY>::pop(unsigned p)
{
...
// Function: steal
template <typename T, unsigned TF_MAX_PRIORITY> T TaskQueue<T, TF_MAX_PRIORITY>::steal()
{
    for (unsigned i = 0; i < TF_MAX_PRIORITY; i++) {
        if (auto t = steal(i); t) {
            return t;
...
// Function: steal
template <typename T, unsigned TF_MAX_PRIORITY> T TaskQueue<T, TF_MAX_PRIORITY>::steal(unsigned p)
{
...
// Function: capacity
template <typename T, unsigned TF_MAX_PRIORITY> int64_t TaskQueue<T, TF_MAX_PRIORITY>::capacity() const noexcept
{
    int64_t s = 0;
    unroll<0, TF_MAX_PRIORITY, 1>([&](auto i) { s = i ? capacity(i) + s : capacity(i); });
    return s;
}
...
template <typename T, unsigned TF_MAX_PRIORITY>
TF_NO_INLINE typename TaskQueue<T, TF_MAX_PRIORITY>::Array* TaskQueue<T, TF_MAX_PRIORITY>::resize_array(Array* a,
                                                                                                        unsigned p,
                                                                                                        std::int64_t b,
                                                                                                        std::int64_t t)
{
