...
class Graph {
  ...
  private:
    ...
    @private
    */
    template <typename... ArgsT> Node* _emplace_back(ArgsT&&...);
};
...
class Runtime {
  ...
  public:
    ...
    */
    template <typename P, typename F> auto async(P&& params, F&& f);
  ...
  protected:
    ...
    */
    ...
    @private
    */
    template <typename P, typename F> auto _async(Worker& w, P&& params, F&& f);

    /**
    @private
    */
    template <typename P, typename F> void _silent_async(Worker& w, P&& params, F&& f);
};
...
constexpr bool is_task_params_v =
    std::is_same_v<std::decay_t<P>, TaskParams> || std::is_same_v<std::decay_t<P>, DefaultTaskParams> ||
...
class Node {
    ...
    enum class AsyncState : int { UNFINISHED = 0, LOCKED = 1, FINISHED = 2 };
    ...
    struct Condition {
        ...
        template <typename C> Condition(C&&);
    ...
    struct MultiCondition {
        ...
        template <typename C> MultiCondition(C&&);
    ...
    struct DependentAsync {
        ...
        template <typename C> DependentAsync(C&&);
    ...
    using handle_t = std::variant<Placeholder,    // placeholder
  ...
  public:
    ...
    template <typename... Args> Node(const std::string&, unsigned, Topology*, Node*, size_t, Args&&...);

    template <typename... Args> Node(const std::string&, Topology*, Node*, size_t, Args&&...);

    template <typename... Args> Node(const TaskParams&, Topology*, Node*, size_t, Args&&...);

    template <typename... Args> Node(const DefaultTaskParams&, Topology*, Node*, size_t, Args&&...);
...
// Constructor
template <typename C>
Node::MultiCondition::MultiCondition(C&& c)
    : work{ std::forward<C>(c) }
{}
...
// Constructor
template <typename T>
inline Node::Module::Module(T& obj)
    : graph{ obj.graph() }
{}
...
Node::Node(
    const std::string& name, unsigned priority, Topology* topology, Node* parent, size_t join_counter, Args&&... args)
    : _name{ name }
    , _priority{ priority }
    , _topology{ topology }
    , _parent{ parent }
...
// Constructor
template <typename... Args>
Node::Node(const std::string& name, Topology* topology, Node* parent, size_t join_counter, Args&&... args)
    : _name{ name }
    , _topology{ topology }
    , _parent{ parent }
    , _join_counter{ join_counter }
...
// Constructor
template <typename... Args>
Node::Node(const TaskParams& params, Topology* topology, Node* parent, size_t join_counter, Args&&... args)
    : _name{ params.name }
...
// Constructor
template <typename... Args>
Node::Node(const DefaultTaskParams&, Topology* topology, Node* parent, size_t join_counter, Args&&... args)
    : _topology{ topology }
    , _parent{ parent }
    , _join_counter{ join_counter }
    , _handle{ std::forward<Args>(args)... }
...
*/
struct NodeDeleter {
    void operator()(Node* ptr) { node_pool.recycle(ptr); }
};
...
@private
*/
template <typename... ArgsT> Node* Graph::_emplace_back(ArgsT&&... args)
{
