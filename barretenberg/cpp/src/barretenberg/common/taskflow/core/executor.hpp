...
class Executor {
  ...
  public:
    ...
    */
    template <typename C> tf::Future<void> run(Taskflow& taskflow, C&& callable);
    ...
    */
    template <typename C> tf::Future<void> run_n(Taskflow& taskflow, size_t N, C&& callable);
    ...
    */
    template <typename P> tf::Future<void> run_until(Taskflow& taskflow, P&& pred);
    ...
    */
    template <typename P, typename C> tf::Future<void> run_until(Taskflow& taskflow, P&& pred, C&& callable);
    ...
    */
    template <typename Observer, typename... ArgsT> std::shared_ptr<Observer> make_observer(ArgsT&&... args);
    ...
    */
    ...
    This member function is thread-safe.
    */
    template <typename P, typename F> auto async(P&& params, F&& func);
  ...
  private:
    ...
    template <typename P> void _corun_until(Worker&, P&&);
};
...
inline void Executor::_spawn(size_t N)
{
    ...
    for (size_t id = 0; id < N; ++id) {
        ...
        _wids.emplace(
            std::piecewise_construct, std::forward_as_tuple(_threads[id].get_id()), std::forward_as_tuple(id));
#endif
    }
...
template <typename P> void Executor::_corun_until(Worker& w, P&& stop_predicate)
{
...
template <typename Observer, typename... ArgsT> std::shared_ptr<Observer> Executor::make_observer(ArgsT&&... args)
{
...
template <typename Observer> void Executor::remove_observer(std::shared_ptr<Observer> ptr)
{
...
inline void Executor::_process_exception(Worker&, Node* node)
{
    ...
    else if (auto tpg = node->_topology;
             tpg && ((tpg->_state.fetch_or(flag, std::memory_order_relaxed) & Topology::EXCEPTION) == 0)) {
...
// Function: run
template <typename C> tf::Future<void> Executor::run(Taskflow& f, C&& c)
{
...
// Function: run_n
template <typename C> tf::Future<void> Executor::run_n(Taskflow& f, size_t repeat, C&& c)
{
...
// Function: run_until
template <typename P> tf::Future<void> Executor::run_until(Taskflow& f, P&& pred)
{
...
// Function: run_until
template <typename P, typename C> tf::Future<void> Executor::run_until(Taskflow& f, P&& p, C&& c)
{
...
// Function: run_until
template <typename P, typename C> tf::Future<void> Executor::run_until(Taskflow&& f, P&& pred, C&& c)
{
...
// Function: corun
template <typename T> void Executor::corun(T& target)
{
...
template <typename P> void Executor::corun_until(P&& predicate)
{
...
inline void Executor::_set_up_graph(Graph& g, Node* parent, Topology* tpg, int state, SmallVector<Node*>& src)
{
...
// Procedure: corun
template <typename T> void Runtime::corun(T&& target)
{
...
template <typename P> void Runtime::corun_until(P&& predicate)
{
...
inline void Runtime::corun_all()
{
    _executor._corun_until(_worker,
                           [this]() -> bool { return _parent->_join_counter.load(std::memory_order_acquire) == 0; });
    _parent->_process_exception();
}
...
template <typename P, typename F> void Runtime::_silent_async(Worker& w, P&& params, F&& f)
{

    _parent->_join_counter.fetch_add(1, std::memory_order_relaxed);

    auto node = node_pool.animate(std::forward<P>(params),
                                  _parent->_topology,
                                  _parent,
                                  0,
...
template <typename F> void Runtime::silent_async(F&& f)
{
    _silent_async(*_executor._this_worker(), DefaultTaskParams{}, std::forward<F>(f));
}

// Function: silent_async
template <typename P, typename F> void Runtime::silent_async(P&& params, F&& f)
{
    _silent_async(*_executor._this_worker(), std::forward<P>(params), std::forward<F>(f));
}

// Function: silent_async_unchecked
template <typename F> void Runtime::silent_async_unchecked(F&& f)
{
    _silent_async(_worker, DefaultTaskParams{}, std::forward<F>(f));
}

// Function: silent_async_unchecked
template <typename P, typename F> void Runtime::silent_async_unchecked(P&& params, F&& f)
{
...
// Function: _async
template <typename P, typename F> auto Runtime::_async(Worker& w, P&& params, F&& f)
{
...
// Function: async
template <typename F> auto Runtime::async(F&& f)
{
    return _async(*_executor._this_worker(), DefaultTaskParams{}, std::forward<F>(f));
}

// Function: async
template <typename P, typename F> auto Runtime::async(P&& params, F&& f)
{
