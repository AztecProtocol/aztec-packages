...
// Constructor
inline AsyncTask::AsyncTask(Node* ptr)
    : _node{ ptr }
{
    _incref();
}
...
{
    if (_node) {
        std::get_if<Node::DependentAsync>(&(_node->_handle))->use_count.fetch_add(1, std::memory_order_relaxed);
    }
}
...
{
    if (_node &&
        std::get_if<Node::DependentAsync>(&(_node->_handle))->use_count.fetch_sub(1, std::memory_order_acq_rel) == 1) {
...
inline size_t AsyncTask::use_count() const
{
    return _node == nullptr
               ? size_t{ 0 }
               : std::get_if<Node::DependentAsync>(&(_node->_handle))->use_count.load(std::memory_order_relaxed);
}

// Function: is_done
inline bool AsyncTask::is_done() const
{
    return std::get_if<Node::DependentAsync>(&(_node->_handle))->state.load(std::memory_order_acquire) ==
