...
class syclFlow {
  ...
  public:
    ...
    */
    ...
    specified value.
    */
    template <typename T> syclTask fill(void* ptr, const T& pattern, size_t count);
    ...
    */
    ...
    @endcode
    */
    template <typename I, typename C> syclTask for_each(I first, I last, C&& callable);
    ...
    @endcode
    */
    template <typename I, typename C> syclTask for_each_index(I first, I last, I step, C&& callable);
    ...
    @endcode
    */
    template <typename I, typename C, typename... S> syclTask transform(I first, I last, C&& callable, S... srcs);
    ...
    @endcode
    */
    template <typename I, typename T, typename C> syclTask reduce(I first, I last, T* result, C&& op);
    ...
    *result = *first++;  // no initial values partitipcate in the loop
    ...
    @endcode
    */
    template <typename I, typename T, typename C> syclTask uninitialized_reduce(I first, I last, T* result, C&& op);
...
// Function: fill
template <typename T> syclTask syclFlow::fill(void* ptr, const T& pattern, size_t count)
{
...
{
    return on([f = std::forward<F>(func)](sycl::handler& h) { h.single_task(f); });
}
...
// Function: fill
template <typename T> void syclFlow::fill(syclTask task, void* ptr, const T& pattern, size_t count)
{
...
// Function: parallel_for
template <typename... ArgsT> void syclFlow::parallel_for(syclTask task, ArgsT&&... args)
{
...
Task FlowBuilder::emplace_on(C&& callable, Q&& q)
{
    auto n = _graph._emplace_back(
        ...
        [c = std::forward<C>(callable), queue = std::forward<Q>(q)](Executor& e, Node* p) mutable {
...
// FlowBuilder::emplace
template <typename C, std::enable_if_t<is_syclflow_task_v<C>, void>*> Task FlowBuilder::emplace(C&& callable)
{
