...
template <typename T> struct NoInit {
    ...
    constexpr NoInit(T value) noexcept
        : v{ value }
    {}
...
template <typename T> auto make_moc(T&& m)
{
...
// std::variant
// ----------------------------------------------------------------------------
template <typename T, typename> struct get_index;

template <size_t I, typename... Ts> struct get_index_impl {};
...
template <typename T, typename... Ts> struct get_index<T, std::variant<Ts...>> : get_index_impl<0, T, Ts...> {};

template <typename T, typename... Ts> constexpr auto get_index_v = get_index<T, Ts...>::value;
...
// STL-styled iterator
template <typename B, typename E> struct stateful_iterator {
...
template <typename B, typename E> using stateful_iterator_t = typename stateful_iterator<B, E>::type;

// raw integral index
template <typename B, typename E, typename S> struct stateful_index {
...
template <typename B, typename E, typename S> using stateful_index_t = typename stateful_index<B, E, S>::type;

// ----------------------------------------------------------------------------
// visit a tuple with a functor at runtime
// ----------------------------------------------------------------------------

template <typename Func, typename Tuple, size_t N = 0> void visit_tuple(Func func, Tuple& tup, size_t idx)
{
    if (N == idx) {
...
template <auto beg, auto end, auto step, bool valid = (beg < end)> struct Unroll {
...
template <auto beg, auto end, auto step> struct Unroll<beg, end, step, false> {
    template <typename F> static void eval(F) {}
};

template <auto beg, auto end, auto step, typename F> void unroll(F f)
{
...
template <typename T, typename... Ts> struct filter_duplicates {
    using type = T;
};

template <template <typename...> class C, typename... Ts, typename U, typename... Us>
struct filter_duplicates<C<Ts...>, U, Us...> : std::conditional_t<(std::is_same_v<U, Ts> || ...),
                                                                  filter_duplicates<C<Ts...>, Us...>,
                                                                  filter_duplicates<C<Ts..., U>, Us...>> {};

template <typename T> struct unique_variant;

template <typename... Ts> struct unique_variant<std::variant<Ts...>> : filter_duplicates<std::variant<>, Ts...> {};

template <typename T> using unique_variant_t = typename unique_variant<T>::type;
...
template <typename T> constexpr static bool is_std_compare_v = is_std_compare<T>::value;
...
template <typename T, typename... Ts> using all_same = all_true<std::is_same_v<T, Ts>...>;

template <typename T, typename... Ts> constexpr bool all_same_v = all_same<T, Ts...>::value;
