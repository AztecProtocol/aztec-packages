...
template <typename T, typename = void> class SmallVectorTemplateCommon : public SmallVectorBase {
  ...
  private:
    ...
    template <typename X> struct AlignedUnionType {
...
@private
*/
template <typename T> class SmallVectorImpl : public SmallVectorTemplateBase<T, IsPod<T>::value> {
...
*/
template <typename T, unsigned N = 2> class SmallVector : public SmallVectorImpl<T> {
...
template <typename T> inline void swap(tf::SmallVectorImpl<T>& LHS, tf::SmallVectorImpl<T>& RHS)
{
    LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N> inline void swap(tf::SmallVector<T, N>& LHS, tf::SmallVector<T, N>& RHS)
{
    LHS.swap(RHS);
}
} // namespace std
