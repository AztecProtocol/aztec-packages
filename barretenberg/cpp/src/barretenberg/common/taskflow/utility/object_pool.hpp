...
//
template <typename T, size_t S = 65536> class ObjectPool {
    ...
    static_assert(S && (!(S & (S - 1))), "block size S must be a power of two");
  ...
  public:
    ...
    */
    template <typename... ArgsT> T* animate(ArgsT&&... args);
  ...
  private:
    ...
    template <class P, class Q> constexpr size_t _offset_in_class(const Q P::* member) const;

    template <class P, class Q> constexpr P* _parent_class_of(Q*, const Q P::* member);

    template <class P, class Q> constexpr P* _parent_class_of(const Q*, const Q P::* member) const;
    ...
    template <typename C> void _for_each_block_safe(Blocklist*, C&&);

    template <typename C> void _for_each_block(Blocklist*, C&&);
};
...
// Destructor
template <typename T, size_t S> ObjectPool<T, S>::~ObjectPool()
{
...
template <typename T, size_t S> size_t ObjectPool<T, S>::num_bins_per_local_heap() const
{
    return B;
}

// Function: num_objects_per_bin
template <typename T, size_t S> size_t ObjectPool<T, S>::num_objects_per_bin() const
{
    return W;
}

// Function: num_objects_per_block
template <typename T, size_t S> size_t ObjectPool<T, S>::num_objects_per_block() const
{
    return M;
}

// Function: emptiness_threshold
template <typename T, size_t S> float ObjectPool<T, S>::emptiness_threshold() const
{
    return 1.0f / F;
}

// Function: num_global_heaps
template <typename T, size_t S> size_t ObjectPool<T, S>::num_global_heaps() const
{
    return 1;
}

// Function: num_lheaps
template <typename T, size_t S> size_t ObjectPool<T, S>::num_local_heaps() const
{
    return _lheaps.size();
}

// Function: num_heaps
template <typename T, size_t S> size_t ObjectPool<T, S>::num_heaps() const
{
    return _lheaps.size() + 1;
}

// Function: capacity
template <typename T, size_t S> size_t ObjectPool<T, S>::capacity() const
{
...
template <typename T, size_t S> size_t ObjectPool<T, S>::num_available_objects() const
{
...
template <typename T, size_t S> size_t ObjectPool<T, S>::num_allocated_objects() const
{
...
// Function: _bin
template <typename T, size_t S> size_t ObjectPool<T, S>::_bin(size_t u) const
{
    return u == M ? F : u / W;
}
...
// Function: _block_of
template <typename T, size_t S> constexpr typename ObjectPool<T, S>::Block* ObjectPool<T, S>::_block_of(Blocklist* list)
{
...
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_init_head(Blocklist* list)
{
    list->next = list;
    list->prev = list;
}
...
//
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_push_front(Blocklist* curr, Blocklist* head)
{
...
//
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_push_back(Blocklist* curr, Blocklist* head)
{
...
// the prev/next entries already!
//
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_del_impl(Blocklist* prev, Blocklist* next)
{
    next->prev = prev;
    prev->next = next;
}
...
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_del(Blocklist* entry)
{
...
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_replace(Blocklist* old, Blocklist* curr)
{
    curr->next = old->next;
    curr->next->prev = curr;
    curr->prev = old->prev;
...
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_move_front(Blocklist* list, Blocklist* head)
{
...
template <typename T, size_t S> void ObjectPool<T, S>::_blocklist_move_back(Blocklist* list, Blocklist* head)
{
    _blocklist_del_impl(list->prev, list->next);
...
template <typename T, size_t S> bool ObjectPool<T, S>::_blocklist_is_first(const Blocklist* list, const Blocklist* head)
{
...
// @list: the entry to test
// @head: the head of the list
template <typename T, size_t S> bool ObjectPool<T, S>::_blocklist_is_last(const Blocklist* list, const Blocklist* head)
{
...
template <typename T, size_t S> bool ObjectPool<T, S>::_blocklist_is_empty(const Blocklist* head)
{
...
// @head: the list to test.
template <typename T, size_t S> bool ObjectPool<T, S>::_blocklist_is_singular(const Blocklist* head)
{
    return !_blocklist_is_empty(head) && (head->next == head->prev);
}

// Procedure: _for_each_block
template <typename T, size_t S> template <typename C> void ObjectPool<T, S>::_for_each_block(Blocklist* head, C&& c)
{
    Blocklist* p;
...
template <typename T, size_t S> T* ObjectPool<T, S>::_allocate(Block* s)
{
    if (s->top == nullptr) {
...
template <typename T, size_t S> void ObjectPool<T, S>::_deallocate(Block* s, T* ptr)
{
...
// Function: allocate
template <typename T, size_t S> template <typename... ArgsT> T* ObjectPool<T, S>::animate(ArgsT&&... args)
{
...
// Function: destruct
template <typename T, size_t S> void ObjectPool<T, S>::recycle(T* mem)
{
...
// Function: _this_heap
template <typename T, size_t S> typename ObjectPool<T, S>::LocalHeap& ObjectPool<T, S>::_this_heap()
{
...
// Function: _next_pow2
template <typename T, size_t S> constexpr unsigned ObjectPool<T, S>::_next_pow2(unsigned n) const
{
    if (n == 0)
        return 1;
    n--;
