...
# Only produces static libraries (bb-external) — uses Android sysroot headers (downloaded automatically).
# Arg is preset name: zig-arm64-android or zig-x86_64-android
function build_android {
  set -eu
  preset=$1
  # Download Android sysroot if not present
...
function build {
  ...
  if semver check "$REF_NAME" && [[ "$(arch)" == "amd64" ]]; then
    ...
    if [ "$(arch)" == "amd64" ] && [ "$CI_FULL" -eq 1 ]; then
      bash scripts/download-android-sysroot.sh
      builds+=("build_cross arm64-macos true" build_smt_verification "build_android zig-arm64-android" "build_android zig-x86_64-android")
    fi
