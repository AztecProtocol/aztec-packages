...
function bench_cmds {
  ...
  echo "$prefix barretenberg/cpp/scripts/run_bench.sh native bb-micro-bench/native/chonk build/bin/client_ivc_bench ClientIVCBench/Full/5$"
  ...
  echo "$prefix barretenberg/cpp/scripts/run_bench.sh wasm bb-micro-bench/wasm/chonk build-wasm-threads/bin/client_ivc_bench ClientIVCBench/Full/5$"
  prefix="$hash:CPUS=1"
