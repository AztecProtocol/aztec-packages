...
function test_cmds {
  ...
  if [ "$(arch)" == "amd64" ] && [ "$CI" -eq 1 ]; then
    ...
    declare -A asan_tests=(
      ...
      ["chonk_tests"]="ChonkTests.BasicStructured"
...
function bench_cmds {
  ...
  prefix="$hash:CPUS=1"
  echo "$prefix barretenberg/cpp/scripts/run_bench.sh native bb-micro-bench/native/client_ivc_verify build/bin/chonk_bench VerificationOnly$"
}
