...
`;
  ...
  /** Generate the client wrapper — typed methods over generic IpcClient */
  generateClient(schema: CompiledSchema): string {
    ...
    const methods = schema.commands
      ...
      .map(c => {
        ...
        return `    pub fn ${methodName}(self: *${prefix}Client, cmd: types.${zigCmdName}) !types.${zigRespName} {
        const resp_name, const resp_payload = try self.ipc.call("${c.name}", try cmd.toPayload(alloc));
    ...
    return `//! AUTOGENERATED - DO NOT EDIT
//! ${prefix} IPC client — typed methods over generic IPC transport.
...
pub const ${prefix}Client = struct {
    ipc: ipc.IpcClient,

    pub fn connect(socket_path: []const u8) !${prefix}Client {
        return .{ .ipc = try ipc.IpcClient.connect(socket_path) };
    }

    pub fn close(self: *${prefix}Client) void {
        self.ipc.close();
    }

    pub fn shutdown(self: *${prefix}Client) !void {
        _ = try self.ipc.call("${prefix}Shutdown", Payload.mapPayload(alloc));
    }
