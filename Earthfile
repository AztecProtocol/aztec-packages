...
bootstrap:
    ...
    RUN git remote add origin https://github.com/aztecprotocol/aztec-packages &>/dev/null
    RUN git fetch --depth 1 origin $EARTHLY_GIT_HASH &>/dev/null
    RUN git checkout FETCH_HEAD &>/dev/null
