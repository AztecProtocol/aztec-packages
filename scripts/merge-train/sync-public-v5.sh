...
PUBLIC_BRANCH="${PUBLIC_BRANCH:-public-v5}"
UPSTREAM_REMOTE="${UPSTREAM_REMOTE:-upstream}"
UPSTREAM_REPO="${UPSTREAM_REPO:-https://github.com/AztecProtocol/aztec-packages.git}"
UPSTREAM_BRANCH="${UPSTREAM_BRANCH:-v5}"
DRY_RUN="${DRY_RUN:-0}"

PRESERVE_PATHS=(
  ".github/workflows/public-v5-pr-body.yml"
  ".github/workflows/public-v5-to-v5.yml"
  ".github/workflows/sync-upstream-v5.yml"
  "scripts/merge-train/merge-public-v5.sh"
  "scripts/merge-train/sync-public-v5.sh"
  "scripts/merge-train/update-public-v5-pr-body.sh"
)
...
if ! remote_branch_exists "$REMOTE" "$PUBLIC_BRANCH"; then
  echo "$REMOTE/$PUBLIC_BRANCH does not exist yet; skipping sync"
  exit 0
fi

configure_upstream_remote

if ! remote_branch_exists "$UPSTREAM_REMOTE" "$UPSTREAM_BRANCH"; then
  echo "$UPSTREAM_REMOTE/$UPSTREAM_BRANCH does not exist yet; skipping sync"
  exit 0
fi
...
new_commit=$(git commit-tree "$new_tree" \
  -p "$public_sha" \
  -p "$upstream_sha" \
  -m "chore: sync public-v5 with upstream v5")
