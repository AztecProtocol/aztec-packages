...
<available_subagents>
Specialist subagents under `.claude/agents/` are auto-discovered by Claude Code's `Agent` tool. Other coding agents that do not auto-load this directory (e.g. Codex) should treat each file as a domain-expert reference and read it on demand when the matching task type comes up.
...
<available_skills>
Skills are task-specific workflows. Claude Code's `Skill` tool auto-loads them by name; other agents should read the matching `SKILL.md` (or `*.md` for `commands/`) when the user asks for one of the named tasks and follow its instructions.
