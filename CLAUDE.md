...
<bumping_noir>
To bump the Noir compiler version (e.g. a request like "bump the noir compiler version to X"), run `noir/scripts/bump_noir_compiler.sh <ref>` from anywhere in the repo. `<ref>` is any git reference in noir-lang/noir — a release tag such as `v1.0.0-beta.23`, a nightly tag such as `nightly-2026-06-02`, a branch, or a commit. One invocation moves all three artifacts that must change together — the `noir/noir-repo` submodule pointer, `avm-transpiler/Cargo.lock` (targeted `cargo update -p` on the noir crates only), and `yarn-project/yarn.lock` — and stages them. Do not bump the submodule by hand; skipping the lockfile updates leaves the tree inconsistent and breaks the AVM transpiler and TS builds. The script stages but does not commit; review the diff, then commit as `chore: update Noir to <ref>`. A compiler bump can also change `noir-projects` formatter output — run the `noir-sync-update` skill's full checklist (notably `./bootstrap.sh format` in `noir-projects`) before finishing, or CI will fail.
</bumping_noir>
