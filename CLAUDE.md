...
This worktree (`zw/optimise-reduction`) works on the WebGPU MSM v2 bucket-reduction kernels (`barretenberg/ts/src/msm_webgpu/`). Benchmarking and profiling rules:
...
# Phone profiling
bash $LOCK phones acquire "zw/optimise-reduction"
bash $SK/profile_both.sh 5198 "http://localhost:5198/..." ./myresult
bash $LOCK phones release

# Mac-local benchmarking
bash $LOCK mac acquire "zw/optimise-reduction"
# ... run local bench ...
bash $LOCK mac release
```

Hold the lock only while a measurement is actually executing. Prepare everything first (edit code, regenerate shaders, decide the full probe list), then acquire → run the whole batch back-to-back → release. Never hold the lock across edit/think/regen cycles between runs; if iterating, release and re-acquire per batch. Target holds of a few minutes; ~30-minute holds starve sibling agents.
</msm_webgpu_benchmarking>

<phone_testing_safety>
Diagnostics expected to crash a phone's GPU process/driver (compile-bomb probes, watchdog-risk dispatches) require Zac's EXPLICIT permission BEFORE running — stop and ask first. If any phone run crashes a device or produces no output (timeout, missing results row), STOP all further runs immediately, report what happened, and wait for instructions. Do not retry, reboot, or continue the batch.
</phone_testing_safety>
