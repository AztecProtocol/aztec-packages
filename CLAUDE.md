...
<build_system>
Dependencies flow barretenberg → noir → l1-contracts → yarn-project. Drive full and dependency-spanning builds from the git root, via `./bootstrap.sh [target]` (the wrapper that runs toolchain checks then invokes `make`) or `make <target>` directly. `./bootstrap.sh` with no arg is `make fast` (everything needed for development); `./bootstrap.sh build yarn-project` (or `make yarn-project`) runs the full TS build chain after building bb, noir, and l1-contracts; `make bb-cpp-native` builds barretenberg C++ native only; `make noir` builds the Noir compiler; `make l1-contracts` builds the Solidity contracts via Foundry.

Component-level `./bootstrap.sh` scripts (`yarn-project/bootstrap.sh`, `barretenberg/ts/bootstrap.sh`, etc.) are NOT standalone — they assume every upstream component is already built and current. A clean checkout has no artifacts, so the first build on a branch must be a baseline `./bootstrap.sh` (or `make fast`) from the repo root. Changing only one component does not remove that prerequisite: the upstream artifacts it links/binds against still need to exist somewhere. Once the baseline is in place, prefer the root wrappers (`./bootstrap.sh build <target>` / `make <target>`) over running component bootstrap scripts directly — the Makefile encodes the dependency graph and only rebuilds what changed; hand-running component scripts skips the graph and silently picks up stale upstream artifacts.

For tight inner-loop iteration on a single component after the baseline build, the native tooling is often faster than going through bootstrap.sh: `cmake --build build --target <t>` in `barretenberg/cpp/`, `forge build` in `l1-contracts/`, and `yarn build` from inside `yarn-project/` (not the git root). When iterating across components in this style, rebuild in dependency order (bb → noir → l1-contracts → yarn-project) so downstream components see fresh upstream artifacts; these tools do not refresh upstream artifacts on your behalf either.
