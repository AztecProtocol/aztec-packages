...
| `upstream <n>... [--branch <name>]` | Builds a local preview branch in `labs/`'s repository with the named patches (in series order) applied under your own identity — what the drained result will look like. Nothing is pushed; the labs team drains the `.patch` files themselves. |
