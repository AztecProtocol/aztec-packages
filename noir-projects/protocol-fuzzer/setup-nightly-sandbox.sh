...
#   5. Compiles the side-effect contract inside the container (nargo + bb-avm)
#   6. Imports test accounts
#   7. Installs an aztec-wallet wrapper script so rash! calls are forwarded
#      into the container transparently
#
# After running this script, you can run the fuzzer directly:
#
...
#
#   RUST_LOG=debug cargo run -- --machine token --max-steps 5
#
#   SIDE_EFFECT_ARTIFACT_PATH=/tmp/side_effect_contract-SideEffect.json \
...
docker exec "$CONTAINER_NAME" bash -c '
    ...
    # Only copy packages that do NOT already exist to avoid overwriting
    ...
    for scope in node_modules/@*; do
        ...
        for pkg in "$scope"/*; do
            name=$(basename "$pkg")
            [ ! -e "/usr/src/yarn-project/node_modules/$scope_name/$name" ] && \
                cp -r "$pkg" "/usr/src/yarn-project/node_modules/$scope_name/"
        done
    done
'
...
mkdir -p "${NIGHTLY_BUILD_DIR}/contracts/test/side_effect_contract/src"
...
CONTRACT_DIR="${REPO_ROOT}/noir-projects/protocol-fuzzer/contracts/side_effect_contract"
if [ ! -f "${CONTRACT_DIR}/src/main.nr" ]; then
    die "Side-effect contract source not found at ${CONTRACT_DIR}/src/main.nr"
fi

cp "${CONTRACT_DIR}/src/main.nr" "${NIGHTLY_BUILD_DIR}/contracts/test/side_effect_contract/src/main.nr"
cp "${CONTRACT_DIR}/Nargo.toml" "${NIGHTLY_BUILD_DIR}/contracts/test/side_effect_contract/Nargo.toml"
cp "${REPO_ROOT}/noir-projects/protocol-fuzzer/contracts/Nargo.toml" "${NIGHTLY_BUILD_DIR}/Nargo.toml"
...
log "Compiling side-effect contract with nightly nargo..."
...
log "Running bb-avm aztec_process (transpile + VK generation)..."
...
log "Stripping __aztec_nr_internals__ prefix from function names..."
docker exec "$CONTAINER_NAME" bash -c '
...
# Copy to the standard container path used by SIDE_EFFECT_ARTIFACT_PATH
...
# Also copy artifact to host so it can be committed to git
docker cp "${CONTAINER_NAME}:/tmp/side_effect_contract-SideEffect.json" \
    "${REPO_ROOT}/noir-projects/protocol-fuzzer/contracts/target/side_effect_contract-SideEffect.json"
log "Artifact copied to noir-projects/protocol-fuzzer/contracts/target/side_effect_contract-SideEffect.json"
...
echo "  Artifact:   /tmp/side_effect_contract-SideEffect.json (container)"
echo ""
...
echo ""
echo "  # Token machine"
echo "  RUST_LOG=debug cargo run -- --machine token --max-steps 5"
echo ""
echo "  # Side-effect machine"
echo "  SIDE_EFFECT_ARTIFACT_PATH=/tmp/side_effect_contract-SideEffect.json \\"
echo "    RUST_LOG=debug cargo run -- --machine side-effect --max-steps 5"
echo ""
