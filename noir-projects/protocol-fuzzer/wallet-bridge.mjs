...
const DATA_DIR =
  process.env.WALLET_DATA_DIRECTORY || join(homedir(), ".aztec/wallet");
...
const { importTestAccounts } = await import(
  `${CLI}/cmds/import_test_accounts.js`
);
...
const handlers = {
  "/import-test-accounts": async ({ prove }) => {
    ...
    const { stdout } = await capturing((log) =>
      importTestAccounts(w, db, false, log),
    );
    const accounts = [...stdout.matchAll(/Address:\s+(0x[0-9a-fA-F]+)/g)].map(
      (m) => m[1],
    );
      ...
      if (hashMatch) {
        try {
          ...
          if (effect) {
            const d = effect.data;
            result.txEffects = {
              l2ToL1Msgs: d.l2ToL1Msgs
                .filter((m) => !m.isZero())
                .map((m) => m.toBigInt().toString()),
            };
          }
        } catch (err) {
