...
if ! $ci3/cache/download noir-projects-circuits-$CIRCUITS_HASH.tar.gz; then
  parallel --joblog circuits.log --line-buffer --tag --halt now,fail=1 {} ::: {noir-contracts,noir-protocol-circuits,mock-protocol-circuits}/bootstrap_circuits.sh
...
if ! $ci3/cache/download noir-projects-vks-$VKS_HASH.tar.gz; then
  parallel --joblog vks.log --line-buffer --tag --halt now,fail=1 {} ::: {noir-contracts,noir-protocol-circuits,mock-protocol-circuits}/bootstrap_vks.sh
