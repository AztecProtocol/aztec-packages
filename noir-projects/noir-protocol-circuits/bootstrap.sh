  ...
  if ! cache_download vk-$hash.tar.gz 1>&2; then
    ...
    function write_vk {
      if echo "$name" | grep -qE "${hiding_kernel_regex}"; then
        ...
        denoise "$BB write_vk --scheme client_ivc --verifier_type standalone_hiding --use-sumcheck-ivc -b - -o $outdir"
      elif echo "$name" | grep -qE "${ivc_regex}"; then
        denoise "$BB write_vk --scheme client_ivc --verifier_type standalone --use-sumcheck-ivc -b - -o $outdir"
    ...
    if echo "$name" | grep -qE "rollup_root"; then
      ...
      jq -r '.bytecode' $json_path | base64 -d | gunzip | $BB write_vk --scheme client_ivc --use-sumcheck-ivc --verifier_type ivc -b - -o $outdir
