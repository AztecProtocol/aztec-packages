...
function build {
  ...
  if [ -f pinned-build.tar.gz ]; then
    ...
    # The reset-circuit variant manifest is normally produced by generate_variants, which we
    # skip when consuming a pinned build. It is bundled inside the archive (see pin-build);
    # restore it to the project root where downstream codegen (noir-protocol-circuits-types)
    # expects it.
    if [ -f target/private_kernel_reset_dimensions.json ]; then
      mv target/private_kernel_reset_dimensions.json ./private_kernel_reset_dimensions.json
    fi
    return
  fi
...
function pin-build {
  ...
  # Bundle the reset-circuit variant manifest alongside the compiled artifacts. It lives
  # outside target/ and is regenerated by generate_variants, which is not run when a pinned
  # build is consumed, yet downstream codegen (noir-protocol-circuits-types) requires it.
  # build() restores it to the project root on extraction.
  if [ -f private_kernel_reset_dimensions.json ]; then
    cp private_kernel_reset_dimensions.json target/private_kernel_reset_dimensions.json
  fi
  tar czf pinned-build.tar.gz -C target .
  # Keep target/ clean of the manifest copy now that it is captured in the archive.
  rm -f target/private_kernel_reset_dimensions.json
