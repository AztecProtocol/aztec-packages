#!/usr/bin/env bash
# Precommit hook for formatting staged noir files in the fnd (protocol) projects.
...
staged_nr_files=$(git diff --cached --name-only --diff-filter=d | grep '^noir-projects/fnd/.*\.nr$' || true)
# Check for unstaged .nr files too
unstaged_nr_files=$(git diff --name-only --diff-filter=d | grep '^noir-projects/fnd/.*\.nr$' || true)
