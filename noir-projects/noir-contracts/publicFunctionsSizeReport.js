...
async function main() {
  ...
  for (var i = 0; i < artifactPaths.length; i++) {
    ...
    contractArtifact.functions.forEach(async (func) => {
      if (
        func.custom_attributes.includes("public") ||
        func.name === "public_dispatch"
      ) {
        if (func.brillig_names.length != 1) {
          console.log("Expected only a single Brillig function");
          return;
        }
        let func_with_contract_name =
          contractArtifact.name + "::" + func.brillig_names[0];
        let program_report = {
          package_name: "",
