...
std::array<typename Flavor::GroupElement, 2> UltraRecursiveVerifier_<Flavor>::verify_proof(const plonk::proof& proof)
{
    ...
    ASSERT(static_cast<uint64_t>(circuit_size.get_value()) == key->circuit_size);
    ASSERT(static_cast<uint64_t>(public_input_size.get_value()) == key->num_public_inputs);

    std::vector<FF> public_inputs;
    for (size_t i = 0; i < static_cast<uint64_t>(public_input_size.get_value()); ++i) {
    ...
    const FF public_input_delta = proof_system::honk::compute_public_input_delta<Flavor>(
        public_inputs, beta, gamma, circuit_size, static_cast<uint64_t>(pub_inputs_offset.get_value()));
    ...
    auto sumcheck = Sumcheck(static_cast<uint64_t>(circuit_size.get_value()));
