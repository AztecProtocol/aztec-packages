...
/**
 * @brief Check the consistency of GoblinTranslator Main Relation
 *
 * @details
 *
 */
TEST_F(GoblinTranslatorRelationConsistency, MainRelation)
{
    using Flavor = honk::flavor::GoblinTranslatorBasic;
    using FF = typename Flavor::FF;
    static constexpr size_t FULL_RELATION_LENGTH = 5;
    ...
    const auto relation_parameters = compute_mock_relation_parameters();
    auto run_test = [&relation_parameters](bool is_random_input) {
        size_t NUM_LIMB_BITS = 68;
        FF shift = FF(uint256_t(1) << NUM_LIMB_BITS);
        FF shiftx2 = FF(uint256_t(1) << (NUM_LIMB_BITS * 2));
        FF shiftx3 = FF(uint256_t(1) << (NUM_LIMB_BITS * 3));
        uint512_t MODULUS_U512 = uint512_t(curve::BN254::BaseField::modulus);
        uint512_t BINARY_BASIS_MODULUS = uint512_t(1) << (NUM_LIMB_BITS << 2);
        uint512_t NEGATIVE_PRIME_MODULUS = BINARY_BASIS_MODULUS - MODULUS_U512;
        std::array<FF, 5> NEGATIVE_MODULUS_LIMBS = {
        ...
        auto relation = GoblinTranslatorMainRelation<FF>();

        // Compute expected full length Univariates using straight forward expressions
        constexpr std::size_t NUM_SUBRELATIONS = std::tuple_size_v<decltype(relation)::RelationUnivariates>;
        auto expected_full_length_univariates = std::array<Univariate<FF, FULL_RELATION_LENGTH>, NUM_SUBRELATIONS>();
        auto& op = extended_edges.op;
        auto& p_x_low_limbs = extended_edges.p_x_low_limbs;
        auto& p_y_low_limbs = extended_edges.p_y_low_limbs;
        auto& p_x_high_limbs = extended_edges.p_x_high_limbs;
        auto& p_y_high_limbs = extended_edges.p_y_high_limbs;
        auto& accumulators_binary_limbs_0 = extended_edges.accumulators_binary_limbs_0;
        auto& accumulators_binary_limbs_1 = extended_edges.accumulators_binary_limbs_1;
        auto& accumulators_binary_limbs_2 = extended_edges.accumulators_binary_limbs_2;
        auto& accumulators_binary_limbs_3 = extended_edges.accumulators_binary_limbs_3;
        auto& z_lo_limbs = extended_edges.z_lo_limbs;
        auto& z_hi_limbs = extended_edges.z_hi_limbs;
        auto& quotient_lo_binary_limbs = extended_edges.quotient_lo_binary_limbs;
        auto& quotient_hi_binary_limbs = extended_edges.quotient_hi_binary_limbs;
        auto& p_x_low_limbs_shift = extended_edges.p_x_low_limbs_shift;
        auto& p_y_low_limbs_shift = extended_edges.p_y_low_limbs_shift;
        auto& p_x_high_limbs_shift = extended_edges.p_x_high_limbs_shift;
        auto& p_y_high_limbs_shift = extended_edges.p_y_high_limbs_shift;
        auto& accumulators_binary_limbs_0_shift = extended_edges.accumulators_binary_limbs_0_shift;
        auto& accumulators_binary_limbs_1_shift = extended_edges.accumulators_binary_limbs_1_shift;
        auto& accumulators_binary_limbs_2_shift = extended_edges.accumulators_binary_limbs_2_shift;
        auto& accumulators_binary_limbs_3_shift = extended_edges.accumulators_binary_limbs_3_shift;
        auto& z_lo_limbs_shift = extended_edges.z_lo_limbs_shift;
        auto& z_hi_limbs_shift = extended_edges.z_hi_limbs_shift;
        auto& quotient_lo_binary_limbs_shift = extended_edges.quotient_lo_binary_limbs_shift;
        auto& quotient_hi_binary_limbs_shift = extended_edges.quotient_hi_binary_limbs_shift;
        auto& relation_wide_limbs = extended_edges.relation_wide_limbs;
        auto& relation_wide_limbs_shift = extended_edges.relation_wide_limbs_shift;
        auto& lagrange_odd = extended_edges.lagrange_odd;

        expected_full_length_univariates[0] =
            (accumulators_binary_limbs_0_shift * relation_parameters.evaluation_input_x[0] + op +
             ...
             (accumulators_binary_limbs_1_shift * relation_parameters.evaluation_input_x[0] +
        ...
        expected_full_length_univariates[1] =
            (relation_wide_limbs + accumulators_binary_limbs_2_shift * relation_parameters.evaluation_input_x[0] +
             ...
             (accumulators_binary_limbs_3_shift * relation_parameters.evaluation_input_x[0] +
        ...
        expected_full_length_univariates[2] =
            (reconstructed_previous_accumulator * reconstructed_evaluation_input_x + op +
