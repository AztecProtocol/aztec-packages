...
// WORKTODO: This is essentially a duplicate of the BarycentricData class, with the constexpr-ness removed. This is
// necessary for use with stdlib types in the recursive verifier which are not constexpr. Need to find a way to at least
// share some of the functionality with the original class.
 ...
 */
template <class Fr, size_t domain_size, size_t num_evals> class BarycentricDataRecursive {
  public:
    static constexpr size_t big_domain_size = std::max(domain_size, num_evals);

    /**
     * Static methods for computing arrays of precomputable data used for barycentric extension and evaluation
     */
    ...
    static std::array<Fr, domain_size> construct_lagrange_denominators(auto& big_domain)
    {
    ...
    BarycentricDataRecursive()
        : big_domain(construct_big_domain())
        , lagrange_denominators(construct_lagrange_denominators(big_domain))
        , precomputed_denominator_inverses(construct_denominator_inverses(big_domain, lagrange_denominators))
        , full_numerator_values(construct_full_numerator_values(big_domain))
    {}
