...
constexpr std::pair<uint64_t, uint64_t> uint256_t::addc(const uint64_t a, const uint64_t b, const uint64_t carry_in)
{
    ...
    const auto carry_temp = static_cast<const uint64_t>(sum < a);
...
constexpr std::pair<uint64_t, uint64_t> uint256_t::sbb(const uint64_t a, const uint64_t b, const uint64_t borrow_in)
{
    ...
    const auto borrow_temp_1 = static_cast<const uint64_t>(t_1 > a);
    const uint64_t t_2 = t_1 - b;
    const auto borrow_temp_2 = static_cast<const uint64_t>(t_2 > t_1);
                                                       ...
                                                       const uint64_t carry_in)
{
    ...
    result.first += a;
    const auto overflow_c = static_cast<const uint64_t>(result.first < a);
    result.first += carry_in;
    const auto overflow_carry = static_cast<const uint64_t>(result.first < carry_in);
