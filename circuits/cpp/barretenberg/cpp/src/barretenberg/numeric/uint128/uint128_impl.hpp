...
constexpr std::pair<uint32_t, uint32_t> uint128_t::addc(const uint32_t a, const uint32_t b, const uint32_t carry_in)
{
    ...
    const auto carry_temp = static_cast<const uint32_t>(sum < a);
...
constexpr std::pair<uint32_t, uint32_t> uint128_t::sbb(const uint32_t a, const uint32_t b, const uint32_t borrow_in)
{
    ...
    const auto borrow_temp_1 = static_cast<const uint32_t>(t_1 > a);
    const uint32_t t_2 = t_1 - b;
    const auto borrow_temp_2 = static_cast<const uint32_t>(t_2 > t_1);
                                                       ...
                                                       const uint32_t carry_in)
{
    ...
    result.first += a;
    const auto overflow_c = static_cast<const uint32_t>(result.first < a);
    result.first += carry_in;
    const auto overflow_carry = static_cast<const uint32_t>(result.first < carry_in);
