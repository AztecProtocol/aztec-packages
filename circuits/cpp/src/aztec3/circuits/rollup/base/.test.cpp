...
TEST_F(base_rollup_tests, native_compute_membership_historical_blocks_tree_negative)
{
    ...
    auto blocks_tree = MerkleTree(blocks_store, HISTORICAL_BLOCKS_TREE_HEIGHT);
    ...
    std::array<NT::fr, HISTORICAL_BLOCKS_TREE_HEIGHT> sibling_path{};
    for (size_t i = 0; i < HISTORICAL_BLOCKS_TREE_HEIGHT; ++i) {
        sibling_path[i] = hash_path[i] + 1;
    }
    inputs.historical_blocks_tree_root_membership_witnesses[0] = {
        .leaf_index = 0,
