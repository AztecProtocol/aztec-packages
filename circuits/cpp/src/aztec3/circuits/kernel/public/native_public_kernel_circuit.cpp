...
// void contract_logic(DummyComposer& composer, PrivateInputs<NT> const& private_inputs, PublicInputs<NT>&
// public_inputs)
// {
...
//         composer.do_assert(is_array_empty(new_commitments) == true, "new_commitments must be empty for static
//         calls"); composer.do_assert(is_array_empty(new_nullifiers) == true, "new_nullifiers must be empty for static
//         calls");
//     }
...
//     // TODO: this logic might need to change to accommodate the weird edge 3 initial txs (the 'main' tx, the 'fee'
//     tx,
...
void validate_inputs(DummyComposer& composer, PublicKernelInputs<NT> const& public_kernel_inputs)
{
    ...
    const NT::boolean is_base_case = start.public_call_count == 0;
    ...
    if (is_base_case) {
        ...
        composer.do_assert(public_kernel_inputs.previous_kernel.vk->contains_recursive_proof == false,
        ...
        composer.do_assert(start_public_call_stack_length != 0,
                           "Cannot execute public kernel circuit with an empty public call stack");
    }
}
                                              ...
                                              PublicKernelInputs<NT> const& public_kernel_inputs)
{
    ...
    public_inputs.end.aggregation_object = public_kernel_inputs.previous_kernel.public_inputs.end.aggregation_object;
