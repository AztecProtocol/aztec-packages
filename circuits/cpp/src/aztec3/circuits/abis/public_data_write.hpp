...
template <typename NCT> struct PublicDataWrite {
    ...
    template <typename Builder> PublicDataWrite<CircuitTypes<Builder>> to_circuit_type(Builder& builder) const
    {
        ...
        PublicDataWrite<CircuitTypes<Builder>> write = {
            to_ct(leaf_index),
            to_ct(old_value),
            to_ct(new_value),
        };
    ...
    template <typename Builder> PublicDataWrite<NativeTypes> to_native_type() const
    {
        static_assert((std::is_same<CircuitTypes<Builder>, NCT>::value));

        auto to_nt = [&](auto& e) { return aztec3::utils::types::to_nt<Builder>(e); };

        PublicDataWrite<NativeTypes> write = {
            to_nt(leaf_index),
            to_nt(old_value),
            to_nt(new_value),
        };
