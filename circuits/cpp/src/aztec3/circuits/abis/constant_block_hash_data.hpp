...
template <typename NCT> struct ConstantBlockHashData {
    ...
    boolean operator==(ConstantBlockHashData<NCT> const& other) const
    {
    ...
    template <typename Builder> ConstantBlockHashData<CircuitTypes<Builder>> to_circuit_type(Builder& builder) const
    {
        ...
        ConstantBlockHashData<CircuitTypes<Builder>> data = {
    ...
    template <typename Builder> ConstantBlockHashData<NativeTypes> to_native_type() const
    {
        static_assert(std::is_same<CircuitTypes<Builder>, NCT>::value);
        auto to_native_type = [&]<typename T>(T& e) { return e.template to_native_type<Builder>(); };
        auto to_nt = [&](auto& e) { return aztec3::utils::types::to_nt<Builder>(e); };

        ConstantBlockHashData<NativeTypes> data = {
...
std::ostream& operator<<(std::ostream& os, ConstantBlockHashData<NCT> const& historic_tree_roots)
{
