...
template <typename NCT> struct OptionalPublicCircuitPublicInputs {
    typedef typename NCT::fr fr;
    typedef typename std::optional<fr> opt_fr;
    typedef typename NCT::boolean boolean;
    typedef typename NCT::address address;
    typedef typename std::optional<boolean> opt_boolean;
    typedef typename std::optional<address> opt_address;
    typedef typename std::optional<StateTransition<NCT>> opt_st;
    typedef typename std::optional<StateRead<NCT>> opt_sr;
    ...
    std::array<opt_st, STATE_TRANSITIONS_LENGTH> state_transitions{};
    std::array<opt_sr, STATE_READS_LENGTH> state_reads{};

    std::array<fr, PUBLIC_CALL_STACK_LENGTH> public_call_stack = zero_array<fr, PUBLIC_CALL_STACK_LENGTH>();
    std::array<fr, L1_MSG_STACK_LENGTH> l1_msg_stack = zero_array<fr, L1_MSG_STACK_LENGTH>();

    opt_fr historic_public_data_tree_root = 0;

    opt_address prover_address;

    boolean operator==(OptionalPublicCircuitPublicInputs<NCT> const& other) const
    {
        return call_context == other.call_context && args == other.args && return_values == other.return_values &&
               emitted_events == other.emitted_events && state_transitions == other.state_transitions &&
               state_reads == other.state_reads && public_call_stack == other.public_call_stack &&
    ...
    static OptionalPublicCircuitPublicInputs<NCT> create()
    {

        auto new_inputs = OptionalPublicCircuitPublicInputs<NCT>();
        ...
        new_inputs.state_transitions.fill(std::nullopt);
        new_inputs.state_reads.fill(std::nullopt);
        ...
        new_inputs.prover_address = std::nullopt;

        return new_inputs;
    };

    template <typename Composer>
    OptionalPublicCircuitPublicInputs<CircuitTypes<Composer>> to_circuit_type(Composer& composer) const
    {
        static_assert((std::is_same<NativeTypes, NCT>::value));
        ...
        OptionalPublicCircuitPublicInputs<CircuitTypes<Composer>> pis = {
            .call_context = to_circuit_type(call_context),
    ...
    PublicCircuitPublicInputs<NCT> remove_optionality() const
    {
        auto get_value = [&](auto& e) { return e.value(); };

        return PublicCircuitPublicInputs<NCT>{
            ...
            .state_transitions = map(state_transitions, get_value),
            .state_reads = map(state_reads, get_value),
            ...
            .prover_address = prover_address.value(),
        };
    }
}; // namespace aztec3::circuits::abis

template <typename NCT>
void read(uint8_t const*& it, OptionalPublicCircuitPublicInputs<NCT>& public_circuit_public_inputs)
{
    using serialize::read;

    OptionalPublicCircuitPublicInputs<NCT>& pis = public_circuit_public_inputs;
    read(it, pis.call_context);
    read(it, pis.args);
    read(it, pis.return_values);
    read(it, pis.emitted_events);
...
template <typename NCT>
void write(std::vector<uint8_t>& buf, OptionalPublicCircuitPublicInputs<NCT> const& public_circuit_public_inputs)
{
    using serialize::write;

    OptionalPublicCircuitPublicInputs<NCT> const& pis = public_circuit_public_inputs;
...
template <typename NCT>
std::ostream& operator<<(std::ostream& os, OptionalPublicCircuitPublicInputs<NCT> const& public_circuit_public_inputs)
