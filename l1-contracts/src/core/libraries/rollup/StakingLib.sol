...
library StakingLib {
  ...
  function deposit(address _attester, address _withdrawer, bool _stayOnLatestRollup) internal {
    require(
    ...
    store.entryQueue.enqueue(_attester, _withdrawer, _stayOnLatestRollup);
    ...
    for (uint256 i = 0; i < numToDequeue; i++) {
      ...
      (bool success, bytes memory data) = address(store.gse).call(
        abi.encodeWithSelector(
          IStakingCore.deposit.selector, args.attester, args.withdrawer, args.stayOnLatestRollup
        )
      );
      if (success) {
