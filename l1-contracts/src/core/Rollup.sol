...
contract Rollup is Decoder {
  ...
  function process(bytes memory _proof, bytes calldata _l2Block) external {
    (uint256 l2BlockNumber, bytes32 startStateHash, bytes32 endStateHash, bytes32 publicInputHash) =
      _decode(_l2Block);

    // @todo Proper genesis state. If the state is empty, we allow anything for now.
    if (rollupStateHash != bytes32(0) && rollupStateHash != startStateHash) {
      revert InvalidStateHash(rollupStateHash, startStateHash);
    }
