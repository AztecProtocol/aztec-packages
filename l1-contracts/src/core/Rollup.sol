...
contract Rollup is IRollup {
  ...
  function process(bytes memory _proof, bytes calldata _l2Block) external override(IRollup) {
    ...
    (bytes32 inHash, bytes32 outHash, bytes32[] memory l2ToL1Msgs, bytes32[] memory l1ToL2Msgs) =
  ...
  function _computePublicInputHash(
    ...
    return Hash.sha256ToField(bytes.concat(_header, _diffRoot, _l1ToL2MsgsHash));
  }
}
