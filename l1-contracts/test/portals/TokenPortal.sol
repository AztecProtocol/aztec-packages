...
contract TokenPortal {
  ...
  event DepositToAztecPublic(bytes32 to, uint256 amount, bytes32 privateContentHash, bytes32 key, uint256 index);

  event DepositToAztecPrivate(uint256 amount, bytes32 privateContentHash, bytes32 key, uint256 index);
  ...
  function depositToAztecPrivate(uint256 _amount, bytes32 _privateContentHash) external returns (bytes32, uint256) {
    ...
    // Hash the public message content to be reconstructed in the receiving contract - the signature below does not
    // correspond to a real function. It's just an identifier of an action.
    ...
    // Emit event
    emit DepositToAztecPrivate(_amount, _privateContentHash, key, index);
