...
contract ProposeLibHarness {
  ...
  function validateInboxConsumption(
    ...
    ProposeLib.validateInboxConsumption(_inbox, _inboxRollingHash, _bucketHint, _slotNumber, _parentTotalMsgCount);
  }
}
...
contract ProposeInboxConsumptionTest is Test {
  ...
  function testEmptyInbox() public {
    ...
    assertEq(inbox.getConsumedBucketSeq(), 0, "consumed pointer at genesis bucket");
  }
  ...
  function testConsumeUpToLatestBucket() public {
    ...
    rollup.validateInboxConsumption(inbox, endHash, 1, SLOT, 0);
    assertEq(inbox.getConsumedBucketSeq(), 1, "consumed pointer advanced");
  }
  ...
  function testBucketPastCutoffNeedNotBeConsumed() public {
    ...
    vm.warp(GENESIS_TIME + Slot.unwrap(SLOT) * SLOT_DURATION);
    rollup.validateInboxConsumption(inbox, bytes32(0), 0, SLOT, 0);
    assertEq(inbox.getConsumedBucketSeq(), 0, "nothing consumed");
  }
  ...
  function testCapEscape() public {
    ...
    rollup.validateInboxConsumption(inbox, endHash, 4, SLOT, 0);
    assertEq(inbox.getConsumedBucketSeq(), 4, "consumed pointer advanced");
  }
  ...
  function testConsumedPointerIsMonotonic() public {
    vm.warp(cutoff);
    _send(0);
    vm.roll(block.number + 1);
    vm.warp(cutoff + 10);
    ...
    vm.warp(GENESIS_TIME + Slot.unwrap(SLOT) * SLOT_DURATION);
    rollup.validateInboxConsumption(inbox, inbox.getBucket(2).rollingHash, 2, SLOT, 0);
    assertEq(inbox.getConsumedBucketSeq(), 2, "consumed pointer at bucket 2");

    // Marking an earlier position (e.g. after a prune rewound the pending chain) does not rewind the
    // pointer: it is a cache of the rollup's per-checkpoint records, not the source of truth.
    vm.prank(address(rollup));
    inbox.markBucketConsumed(1);
    assertEq(inbox.getConsumedBucketSeq(), 2, "consumed pointer did not rewind");
  }

  function testMarkBucketConsumedOnlyRollup() public {
    vm.warp(cutoff);
    _send(0);
  ...
  function testMarkBucketConsumedRejectsFutureBucket() public {
    vm.prank(address(rollup));
