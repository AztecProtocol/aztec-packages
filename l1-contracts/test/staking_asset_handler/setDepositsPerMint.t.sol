...
contract SetDepositsPerMintTest is StakingAssetHandlerBase {
  ...
  function test_WhenOwnerAddsValidators(uint256 _depositsPerMint) external {
    ...
    for (uint256 i = 0; i < _depositsPerMint; i++) {
      vm.expectEmit(true, true, true, true, address(stakingAssetHandler));
      emit IStakingAssetHandler.AddedToQueue(validators[i], validators[i]);
      vm.prank(caller);
      stakingAssetHandler.addValidator(validators[i], validators[i], realProof);
    ...
    vm.expectEmit(true, true, true, true, address(stakingAssetHandler));
    emit IStakingAssetHandler.AddedToQueue(address(0xbeefdeef), address(0xbeefdeef));
    vm.prank(caller);
    stakingAssetHandler.addValidator(address(0xbeefdeef), address(0xbeefdeef), realProof);
