...
contract SetupEpochTest is ValidatorSelectionTestBase {
    ...
    function test_GivenTheRollupIsInGenesisState() external setup(0) whenTheRollupIsInGenesisState {
        ...
        Timestamp nextEpochTimestamp = Timestamp.wrap(block.timestamp + TestConstants.AZTEC_EPOCH_DURATION * TestConstants.AZTEC_SLOT_DURATION);
        ...
        assertTrue(nextEpochSeed != sampleSeed, "Next epoch seed should be different from current epoch seed");
        ...
        uint256 epoch1SampleSeedInPast = IValidatorSelection(address(rollup)).getSampleSeedAt(nextEpochTimestamp);
        assertEq(epoch1SampleSeedInPast, nextEpochSeed, "Next epoch seed should be the same as the current epoch seed");

        // Call setupEpoch again, from the current epoch it should not change the sample seed
        rollup.setupEpoch();
        assertEq(IValidatorSelection(address(rollup)).getCurrentSampleSeed(), nextEpochSeed, "Sample seed should not change");

        // Seed for the next epoch should be set
        Timestamp nextEpochTimestamp2 = Timestamp.wrap(block.timestamp + TestConstants.AZTEC_EPOCH_DURATION * TestConstants.AZTEC_SLOT_DURATION);
        uint256 nextEpochSeedInPast = IValidatorSelection(address(rollup)).getSampleSeedAt(nextEpochTimestamp2);
        assertTrue(nextEpochSeedInPast != nextEpochSeed, "Next epoch seed should not be the same as the current epoch seed");
    }

    function test_WhenTheRollupIsNotInEpoch0ButIsInGenesisState() external setup(0) whenTheRollupIsInGenesisState {
        // it should set the sample seed to max
        // it should set the sample seed for the next epoch
        ...
        // Advance into the next epoch
        uint256 nextEpochTimestamp = block.timestamp + TestConstants.AZTEC_EPOCH_DURATION * TestConstants.AZTEC_SLOT_DURATION;
        vm.warp(nextEpochTimestamp);

        // Call setupEpoch, sample seed for initial epoch should be uint224.max
        rollup.setupEpoch();
        assertEq(IValidatorSelection(address(rollup)).getCurrentSampleSeed(), type(uint224).max, "Sample seed should be max");

        // Get sample seed for initial epoch
        uint256 initialEpochSeed = IValidatorSelection(address(rollup)).getSampleSeedAt(Timestamp.wrap(initialTimestamp));
    ...
    function test_GivenTheSeedHasBeenSampled() external setup(4) whenTheRollupIsNotInGenesisState {
        ...
        rollup.setupEpoch();
        uint256 initialEpochSeedAfterSetup = IValidatorSelection(address(rollup)).getSampleSeedAt(Timestamp.wrap(block.timestamp));
        ...
        for (uint256 i = 0; i < initialCommittee.length; i++) {
            assertEq(committeeAfterRepeatedSetup[i], initialCommittee[i], "Committee should have the same validators");
        }
        ...
        address[] memory committeeAfterAddingExtraValidators = IValidatorSelection(address(rollup)).getCurrentEpochCommittee();
        for (uint256 i = 0; i < committeeAfterAddingExtraValidators.length; i++) {
            assertEq(committeeAfterAddingExtraValidators[i], initialCommittee[i], "Committee should be the same");
        }
        ...
        address[] memory committeeAfterJumpingIntoFuture = IValidatorSelection(address(rollup)).getCommitteeAt(Timestamp.wrap(savedTimestamp));
        for (uint256 i = 0; i < committeeAfterJumpingIntoFuture.length; i++) {
            assertEq(committeeAfterJumpingIntoFuture[i], initialCommittee[i], "Committee should be the same");
        }
    }
        ...
        whenItHasBeenALongTimeSinceTheLastSampleSeedWasSet
    {
        ...
        uint256 nextEpochTimestamp2 = block.timestamp + TestConstants.AZTEC_EPOCH_DURATION * TestConstants.AZTEC_SLOT_DURATION;
