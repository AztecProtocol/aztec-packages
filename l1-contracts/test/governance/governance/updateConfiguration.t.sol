...
contract UpdateConfigurationTest is GovernanceBase {
    ...
    whenConfigurationIsInvalid
  {
    ...
    config.requiredYeaMargin =
      bound(_val, ConfigurationLib.REQUIRED_APPROVAL_MARGIN_UPPER + 1, type(uint256).max);
    vm.expectRevert(
      abi.encodeWithSelector(
        Errors.Governance__ConfigurationLib__RequiredApprovalMarginTooBig.selector
      )
    );
  ...
  function test_WhenRequiredApprovalMarginGeMinAndLeMax(uint256 _val)
    external
    whenCallerIsSelf
    whenConfigurationIsValid
  {
    ...
    uint256 val = bound(_val, 0, ConfigurationLib.REQUIRED_APPROVAL_MARGIN_UPPER);
