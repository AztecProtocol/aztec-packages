...
contract ApellaBase is TestBase {
  ...
  function setUp() public virtual {
...

    {
      ...
      proposals["call_asset"] = apella.getProposal(proposalIds["call_asset"]);
    }

    {
      ...
      proposals["upgrade"] = apella.getProposal(proposalIds["upgrade"]);
    }

    {
      ...
      proposals["revert"] = apella.getProposal(proposalIds["revert"]);
    }

    {
      ...
      proposals["empty"] = apella.getProposal(proposalIds["empty"]);
    }
  }
  ...
  function _stateRejected(bytes32 _proposalName) internal {
    ...
    assertTrue(apella.getProposalState(proposalId) == DataStructures.ProposalState.Rejected);
  }
  ...
  function _stateQueued(
    ...
    token.approve(address(apella), totalPower);
    apella.deposit(_voter, totalPower);
    vm.stopPrank();

    _stateActive(_proposalName);

    vm.startPrank(_voter);
    apella.vote(proposalId, yeas, true);
    apella.vote(proposalId, votesCast - yeas, false);
    vm.stopPrank();

    vm.warp(Timestamp.unwrap(proposal.activeThrough()) + 1);

    assertEq(
      apella.getProposalState(proposalId), DataStructures.ProposalState.Queued, "invalid state"
    );
  }
  ...
  function _stateExecutable(
    ...
    assertEq(
      apella.getProposalState(proposalId), DataStructures.ProposalState.Executable, "invalid state"
    );
  }
  ...
  function _stateExpired(
    ...
    assertEq(
      apella.getProposalState(proposalId), DataStructures.ProposalState.Expired, "invalid state"
    );
  }
