...
contract UsePowerTest is Test {
  ...
  function test_GivenPowerUsedPlusAmountGtPowerAt(
    ...
    uint256 timestamp =
      bound(_timestamp, block.timestamp + DEPOSIT_GRANULARITY_SECONDS, type(uint32).max);
    ...
    if (preAmount > 0) {
      delegationLib.usePower(
        _delegatee, _proposalId, Timestamp.wrap(timestamp - DEPOSIT_GRANULARITY_SECONDS), preAmount
      );
    }
    ...
    delegationLib.usePower(
      _delegatee, _proposalId, Timestamp.wrap(timestamp - DEPOSIT_GRANULARITY_SECONDS), amount
    );
  }
  ...
  function test_GivenPowerUsedPlusAmountLePowerAt(
    ...
    vm.record();
    delegationLib.usePower(
      _delegatee, _proposalIds[0], Timestamp.wrap(timestamp - DEPOSIT_GRANULARITY_SECONDS), amount
    );
    ...
    delegationLib.usePower(
      _delegatee, _proposalIds[1], Timestamp.wrap(timestamp - DEPOSIT_GRANULARITY_SECONDS), amount
    );
