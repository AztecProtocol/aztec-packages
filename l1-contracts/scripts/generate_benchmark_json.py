...
def load_config() -> Dict[str, int]:
    """Load benchmark configuration values from the test contract."""
    import subprocess
    ...
    try:
        ...
        print(f"Warning: Could not extract benchmark config: {e}")
        return {}
...
def generate_flat_benchmark_list(gas_results: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for config_name, config_data in gas_results.items():
        for function_name, gas_stats in config_data.items():
            ...
            benchmark_entry = {
                "name": f"{config_name}/{display_function_name}",
        ...
        if config and all(
            ...
            slot_duration = config.get("SLOT_DURATION", 36)
            epoch_duration = config.get("EPOCH_DURATION", 32)
            proofs_per_epoch = config.get("PROOFS_PER_EPOCH", 200) / 100
            ...
            benchmark_entry = {
                "name": f"{config_name}/gasPerSecond",
...
def save_benchmark_json(
    ...
    output_file: str = "bench-out/l1-gas.bench.json",
):
