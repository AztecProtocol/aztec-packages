...
2. **Ordered Emission**: Ensure that side effects (note hashes, nullifiers, logs) are emitted in the same order as they were created.
   - Refer to [Tail Private Kernel Circuit](./private-kernel-tail.md#verifying-ordered-arrays) for order enforcement.
...
The circuit undergoes the following validations for data within [`private_inputs`](#privateinputs).[`private_call`](#privatecall):
...
| Field                         | Type           | Description                                                             |
| ----------------------------- | -------------- | ----------------------------------------------------------------------- |
| `note_hash_range_hints`       | [`field`, `N`] | Indices of the next emitted private call requests for note hashes.      |
| `nullifier_range_hints`       | [`field`, `N`] | Indices of the next emitted private call requests for nullifiers.       |
| `unencrypted_log_range_hints` | [`field`, `N`] | Indices of the next emitted private call requests for unencrypted logs. |
| `encrypted_log_range_hints`   | [`field`, `N`] | Indices of the next emitted private call requests for encrypted logs.   |
| `encrypted_note_range_hints`  | [`field`, `N`] | Indices of the next emitted private call requests for encrypted notes.  |
...
| `note_hash_read_requests`                   | [`[ReadRequest; C]`](./private-function#readrequest)                                       | Requests to prove the note hashes being read exist.                                                                            |
| `nullifier_read_requests`                   | [`[ReadRequest; C]`](./private-function#readrequest)                                       | Requests to prove the nullifiers being read exist.                                                                             |
| `nullifier_key_validation_request_contexts` | [`[ParentSecretKeyValidationRequestContext; C]`](#parentsecretkeyvalidationrequestcontext) | Requests to validate nullifier keys.                                                                                           |
| `public_call_request_contexts`              | [`[PublicCallRequestContext; C]`](./public-kernel-tail.md#publiccallrequestcontext)        | Requests to call publics functions.                                                                                            |
| `private_call_request_stack`                | [`[PrivateCallRequestContext; C]`](#privatecallrequestcontext)                             | Requests to call private functions. Pushed to the stack in reverse order so that they will be executed in chronological order. |
