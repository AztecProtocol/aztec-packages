...
if ! git diff-index --quiet HEAD --; then
  echo "Warning: You have unstaged changes. Disabling S3 caching and local minio caching to not accidentally pollute cache (which uses git data)." >&2
...
# Create our .dockerignore from .gitignore
