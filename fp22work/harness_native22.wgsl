@group(0) @binding(0) var<storage, read> ina: array<u32>;
@group(0) @binding(1) var<storage, read> inb: array<u32>;
@group(0) @binding(2) var<storage, read_write> outc: array<u32>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    var a: array<u32, 8>;
    var b: array<u32, 8>;
    for (var i = 0u; i < 8u; i = i + 1u) { a[i] = ina[t*8u+i]; b[i] = inb[t*8u+i]; }
    let r = montgomery_product_22(a, b);
    for (var i = 0u; i < 8u; i = i + 1u) { outc[t*8u+i] = r[i]; }
}
