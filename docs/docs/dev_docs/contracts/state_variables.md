...
Aztec public state follows an account-based model. That is, each state occupies a leaf in an account-based merkle tree: the [_public state tree_](#). See [here](#) for more of the technical details.
...
Aztec private state follows a utxo-based model. That is, a private state's current value is represented as one or many [notes](#notes). Each note is stored as an individual leaf in a utxo-based merkle tree: the [_private state tree_](#).
...
There's a limit on the maxinum number of notes this function can return at a time. Check [here](#) and look for `MAX_READ_REQUESTS_PER_CALL` for the up-to-date number.
