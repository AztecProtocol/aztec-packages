...
This error happens when you try to read a pending note_hash that doesn't exist.

#### 2021 - PRIVATE_KERNEL\_\_UNRESOLVED_NON_TRANSIENT_READ_REQUEST

For a transient read request we skip merkle membership checks since pending note-hashes aren't inserted into the private data tree yet.
...
#### 3026 - PUBLIC_KERNEL\_\_NEW_NOTE_HASHES_PROHIBITED_IN_STATIC_CALL
...
- using invalid historic private data tree state (aka historic note-hashes tree)
