...
This is possible through a combination of private smart contracts and a rollup architecture.

## Private Smart Contracts

A smart contract on Aztec is a collection of functions, written as ZK-SNARK circuits. These circuits can have different modes of execution on the Aztec network:

1. **Private Functions** -- can read and write private state, read historic public state, consume or send messages to / from L1, and read Ethereum state. Can call other private functions in the same contract, or other contracts. They can call public functions.
...
Head to the overview of the [Foundational Concepts](../concepts/foundation/main) page to get into the details of the network architecture.

Jump to the [Quickstart](../dev_docs/getting_started/quickstart.md) to get started building on Aztec.
