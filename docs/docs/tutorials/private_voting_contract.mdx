...
import { Test } from "@site/src/components/Test/test.tsx";
...
<Test hidden>yes | npx aztec-app init -v zpedro/tutorials_refactor</Test>

```bash
yes | npx aztec-app init
```

<Test hidden>touch yarn.lock</Test>
...
<Test replaceLines="0" file="index.ts">
...
<Test replaceLines="+" file="index.ts">
...
<Test replaceLines="4" file="index.ts" hidden>

`ts const pxe = createPXEClient("http://aztec:8080") `
...
<Test file="src/main.nr" replaceLines="3,-1">
...
<Test file="src/main.nr" replaceLines="3,2">
...
<Test replaceLines="-1,-1" file="src/main.nr">
...
<Test replaceLines="2,3" file="src/main.nr">
