---
title: Memory Model
description: Understanding memory and type tags in the Aztec Virtual Machine
sidebar_position: 1
---
...
Some AVM opcodes (see the [Instruction Set](isa)) write to memory. Any opcode that writes to memory is responsible for properly tagging written cells and ensuring that written values fit in the associated range.
...
- An instruction operates on a _range_ of data (see [EMITUNENCRYPTEDLOG](isa#EMITUNENCRYPTEDLOG))
