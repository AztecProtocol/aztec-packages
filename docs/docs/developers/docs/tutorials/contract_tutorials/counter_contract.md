...
- `state_vars::Owned,`
  Brings in `Owned`, a wrapper for owner-based private storage that allows accessing state by owner address.

- `use balance_set::BalanceSet;`
  Imports `BalanceSet`, an abstraction that manages private integer-like state variables (like our counter), handling note creation and coin selection automatically.

## Declare storage

Add this below the imports. It declares the storage variables for our contract. We use `Owned<BalanceSet>` to store counter values for each owner's `AztecAddress`.
