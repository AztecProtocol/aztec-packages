...
<!-- NOTE: How is this different from getting started with sandbox - need to merge then delete this one!!! -->
...
The sandbox must be running for the update command to work. Make sure it is [installed and running](../aztec-cli/sandbox-reference.md).
...
Check the [Quickstart](../../getting_started_on_sandbox.md) for a refresher on how to send transactions using `aztec-wallet` or check the [reference here](../wallet-cli/cli_wallet_reference.md)
...
You can learn more about custom commands in the [sandbox reference](../aztec-cli/sandbox-reference.md).
