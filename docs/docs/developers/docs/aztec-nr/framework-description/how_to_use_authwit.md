...
fn execute_private_action(
    ...
    storage.values.at(from).sub(value).emit(from, MessageDelivery.CONSTRAINED_ONCHAIN);
    storage.values.at(to).add(value).emit(to, MessageDelivery.CONSTRAINED_ONCHAIN);
}
```
