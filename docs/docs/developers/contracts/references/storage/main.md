...
We are going to start with a blank project and fill in the token contract source code defined on Github [here](https://github.com/AztecProtocol/aztec-packages/blob/#include_aztec_version/noir-contracts/contracts/token_contract/src/main.nr), and explain what is being added as we go.
...
We will be going over the code in `main.nr` [here](https://github.com/AztecProtocol/aztec-packages/tree/#include_aztec_version/noir-contracts/contracts/token_contract/src). If you are following along and want to compile `main.nr` yourself, you need to add the other files in the directory as they contain imports that are used in `main.nr`.

:::

Just below the contract definition, add the following imports:

#include_code imports /noir-contracts/contracts/token_contract/src/main.nr rust

We are importing the Option type, items from the `value_note` library to help manage private value storage, note utilities, context (for managing private and public execution contexts), `state_vars` for helping manage state, `types` for data manipulation and `oracle` for help passing data from the private to public execution context. We also import the `auth` [library](https://github.com/AztecProtocol/aztec-packages/blob/#include_aztec_version/aztec-nr/aztec/src/auth.nr) to handle token authorizations from [Account Contracts](../../../../learn/concepts/accounts/main.md). Check out the Account Contract with AuthWitness [here](https://github.com/AztecProtocol/aztec-packages/blob/#include_aztec_version/noir-contracts/contracts/schnorr_single_key_account_contract/src/main.nr).

[SafeU120](https://github.com/AztecProtocol/aztec-packages/blob/#include_aztec_version/aztec-nr/safe-math/src/safe_u120.nr) is a library to do safe math operations on unsigned integers that protects against overflows and underflows.

For more detail on execution contexts, see [Contract Communication](../../../../learn/concepts/communication/main).

### Types files

We are also importing types from a `types.nr` file, which imports types from the `types` folder. You can view them [here](https://github.com/AztecProtocol/aztec-packages/tree/#include_aztec_version/noir-contracts/contracts/token_contract/src).
...
#include_code storage_struct /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code set_admin /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code set_minter /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code mint_public /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code shield /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code burn_public /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code unshield /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code burn /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code initialize /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code increase_public_balance /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code reduce_total_supply /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code admin /noir-contracts/contracts/token_contract/src/main.nr rust

#### `is_minter`

A getter function for checking the value of associated with a `minter` in the public `minters` mapping.

#include_code is_minter /noir-contracts/contracts/token_contract/src/main.nr rust

#### `total_supply`

A getter function for checking the token `total_supply`.

#include_code total_supply /noir-contracts/contracts/token_contract/src/main.nr rust
...
#include_code balance_of_public /noir-contracts/contracts/token_contract/src/main.nr rust
