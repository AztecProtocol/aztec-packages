...
A block might contain many transactions, each with complex proofs, but the final proof submitted to Ethereum is just a few kilobytes! Multiple provers can submit overlapping proofs, with rewards shared based on proof quality and consistency.

## Step 5: Settlement on Ethereum (Proven Chain)

This is where your transaction transitions from the pending chain (fast but reversible) to the proven chain, becoming truly final and unchangeable.
...
**Pending Chain** (fast confirmations):
