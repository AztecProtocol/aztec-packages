...
:::caution
This getting started guide uses **Devnet** rather than Alpha (Mainnet). Devnet has a **Sponsored FPC** (Fee Paying Contract) deployed, which covers transaction fees for you — making it much faster to get started without needing to bridge Fee Juice from L1. Note that the Devnet version (`4.0.0-devnet.2-patch.1`) may differ from the Alpha version (`4.2.0-aztecnr-rc.2`). For instructions on paying fees including Fee Juice bridging, see [How to Pay Fees](./docs/aztec-js/how_to_pay_fees#bridge-fee-juice-from-l1).
:::
...
```sh
VERSION=0.0.0 bash -i <(curl -sL https://install.aztec.network/0.0.0)
```

:::warning
The devnet is version dependent. It is currently running version `4.0.0-devnet.2-patch.1`. Maintain version consistency when interacting with the devnet to reduce errors.
:::

:::warning
The Sponsored FPC is **not** deployed on mainnet. The `SPONSORED_FPC_ADDRESS` and sponsored fee payment commands in this guide only apply to devnet. On mainnet, you must [bridge Fee Juice from L1](./docs/aztec-js/how_to_pay_fees#bridge-fee-juice-from-l1) to pay fees.
:::
