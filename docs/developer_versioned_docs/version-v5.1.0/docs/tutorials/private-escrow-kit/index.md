...
The Private Escrow Kit is a set of AI agent skills that scaffold a complete private escrow project on Aztec, including Noir contracts and a TypeScript SDK that are ready to build and adapt to your escrow. You describe the escrow you want in plain language, and the skill generates a working starting point built around the privacy patterns this page explains.
...
:::info Sources
The kit is delivered as the [aztec-private-escrow-skills](https://github.com/AztecProtocol/aztec-private-escrow-skills) repository. The skills target **Aztec v5.1.0**. The repository is the source of truth for the exact, current API; the code on this page is illustrative and may differ from the version you install.
:::
...
| Skill                     | What it does                                                                                                 |
| ------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `scaffold-escrow-project` | Creates the project (Noir sources, generated artifacts, the TypeScript SDK, and Bun localnet test setup)     |
...
4. **Build, test, deploy.** Compile the contract, add and run the project-specific localnet tests, and deploy.
...
- Aztec CLI `5.1.0` (see [AI Tooling](../../../ai_tooling.md) for installation and the recommended `aztec`-CLI-first workflow)
- [Bun](https://bun.sh)
...
Because there is no public bytecode to look up, you prove a secret escrow exists from its deployment and initialization effects (such as the initialization nullifier), not from a node lookup. Publish the class or instance only when public functions, public discovery, upgrades, or non-participant callers need it. Note that a contract with external public functions and a private initializer cannot skip class publication: the class must be registered onchain before initialization.
...
The manifest travels offchain and is never stored as contract state. The SDK's `EscrowManifest` class exposes `manifest.encrypt(recipientPublicKey)` and `EscrowManifest.decrypt(...)` for encrypted transport (ephemeral ECDH key exchange). On the receiving side, `manifest.register(wallet)` returns the typed escrow contract after registering the instance, the secret key, and the sender.
...
An escrow's lifecycle is **protocol state**, so it lives in the contract-owned `StateNote`. Authorized participants share one private view of where the order stands; the public sees nothing. The kit models lifecycle with up to five phases:
...
The broader kit design includes pluggable triggers, with zkTLS (via Primus) as the main example for pulling verified web facts onchain. In the current kit, proof-based release conditions are **design patterns with stubbed verifiers**, not a finished interface. The testing strategy calls for zkTLS, zkEmail, payment, and locker-code flows to start as skipped stubs that name the missing verifier and expected phase transition. Treat conditional release as something you design and implement on top of the lifecycle, and verify the integration end to end before relying on it.
:::
...
The scaffold is intentionally a contracts package plus a TypeScript SDK, with Bun test setup. There is no bundled API, CLI, or frontend; those belong in your application layer.
...
Deliver contract-owned notes and the escrow-addressed fill receipt with `MessageDelivery::onchain_constrained()`, so a malicious caller cannot brick future readers by withholding the replacement message. Deliver the caller's own `RoleAdded` recovery event with `MessageDelivery::onchain_unconstrained()`.
...
The scaffold includes Bun test setup for a local Aztec node, with `EmbeddedWallet` and the standard test accounts. Wallets are created ephemeral with proving off by default for speed:
...
The base template intentionally omits a project-specific `escrow.test.ts`. Generate or write that file for your escrow before running the test command below. With v5.1.0 prefunded localnet accounts, register them using `createSchnorrInitializerlessAccount`.

After adding the project-specific test, run localnet in one terminal and build and test the contracts package in another. `localnet` is a root script; `build`, `typecheck`, and `test` are contracts-package scripts:
...
Full localnet runs take minutes, so iterate with a targeted test (`bun test --preload ./ts/test/setup.ts --timeout 300000 ./ts/test/escrow.test.ts -t "atomic happy path"`) before one full pass. The suite should cover the security-critical paths:
...
- **Evolving API.** The skills target Aztec v5.1.0 and the code samples here are illustrative. Check the [repository](https://github.com/AztecProtocol/aztec-private-escrow-skills) for the current API.
...
- Build a token to escrow against: [Private Token Contract tutorial](../contract_tutorials/token_contract.md)
- Compare with the minimal single-owner custody contract: [Escrow standard](../../aztec-nr/standards/escrow.md)
- Drive contracts from TypeScript: [Aztec.js getting started](../js_tutorials/aztecjs-getting-started.md)
- Explore the kit: [aztec-private-escrow-skills](https://github.com/AztecProtocol/aztec-private-escrow-skills)
