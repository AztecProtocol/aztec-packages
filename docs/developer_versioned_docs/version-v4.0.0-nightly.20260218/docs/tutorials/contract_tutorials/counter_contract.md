...
This tutorial is compatible with the Aztec version `4.0.0-devnet.2-patch.0`. Install the correct version with `VERSION=latest bash -i <(curl -sL https://install.aztec.network/latest)`. Or if you'd like to use a different version, you can find the relevant tutorial by clicking the version dropdown at the top of the page.
