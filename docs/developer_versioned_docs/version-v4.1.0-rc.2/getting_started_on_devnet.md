...
:::caution
This getting started guide uses **Devnet** rather than Testnet. Devnet has a **Sponsored FPC** (Fee Paying Contract) deployed, which covers transaction fees for you — making it much faster to get started without needing to bridge Fee Juice from L1. Note that the Devnet version (`4.0.0-devnet.2-patch.1`) may differ from the Testnet version (`4.1.0-rc.2`). For testnet-specific instructions including Fee Juice bridging, see [How to Pay Fees](./docs/aztec-js/how_to_pay_fees#bridge-fee-juice-from-l1).
:::
...
:::warning
The Sponsored FPC is **not** deployed on testnet. The `SPONSORED_FPC_ADDRESS` and sponsored fee payment commands in this guide only apply to devnet. On testnet, you must [bridge Fee Juice from L1](./docs/aztec-js/how_to_pay_fees#bridge-fee-juice-from-l1) to pay fees.
:::
