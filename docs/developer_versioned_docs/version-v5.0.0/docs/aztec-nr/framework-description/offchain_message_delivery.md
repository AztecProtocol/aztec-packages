...
```
> <sup><sub><a href="https://github.com/AztecProtocol/aztec-packages/blob/v5.0.0/noir-projects/noir-contracts/contracts/app/token_contract/src/main.nr#L279-L300" target="_blank" rel="noopener noreferrer">Source code: noir-projects/noir-contracts/contracts/app/token_contract/src/main.nr#L279-L300</a></sub></sup>


Nothing else changes in the contract: the same notes and events are created, only their delivery mechanism differs.

Under the hood, `MessageDelivery::offchain()` encrypts the message without constraints and emits it via the [`deliver_offchain_message`](pathname:///aztec-nr-api/testnet/noir_aztec/messages/offchain_messages/fn.deliver_offchain_message) function. See the [`MessageDelivery`](pathname:///aztec-nr-api/testnet/noir_aztec/messages/message_delivery/struct.MessageDelivery) API reference for details on guarantees, costs, and privacy.
...
Once received, messages are managed by an inbox that the wallet syncs against (see [`receive`](pathname:///aztec-nr-api/testnet/noir_aztec/messages/processing/offchain/fn.receive) in the API reference):
...
- Browse the [`MessageDelivery`](pathname:///aztec-nr-api/testnet/noir_aztec/messages/message_delivery/struct.MessageDelivery) API reference
