...
This section describes the design pattern using the `private-fee-juice` package's `PrivateFPC` as an example. The contract stores an internal, note-based `BalanceSet` of Fee Juice per user. There is no constructor, no admin, and no public surface.
...
Two parties derive the same contract address if and only if they use the same compiled artifact and the same deployment salt. For any fully private FPC, using a common salt maximizes the shared privacy set. The community convention for the `private-fee-juice` `PrivateFPC` is `Fr.ZERO`.
...
## Example: pay fees with `PrivateFPC`
...
The `private-fee-juice` package ships detailed documentation for this design and its security properties:
...
- Browse the [`private-fee-juice`](https://github.com/alejoamiras/ecosystem-tooling/tree/main/packages/private-fee-juice) package for the Noir source, TypeScript SDK, and integration examples
