...
- Each meeting the minimum sequencer requirements (see [Sequencer Management](./sequencer_management.md))
...
Want to run multiple sequencer identities on a **single node** instead? See the [Advanced Keystore Patterns guide](../operation/keystore/advanced_patterns.md#multiple-sequencers)—that's a different use case from HA.
:::
