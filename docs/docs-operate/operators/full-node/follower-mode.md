...
State-dependent checks run against the follower's own replicated state, which trails the upstream by up to one polling interval. For state that only accumulates, like spent nullifiers, that staleness is harmless while the chain grows: it can only let through a transaction the upstream then rejects. Non-monotonic state can produce the opposite error during ordinary lag — a fee payer whose fee-juice balance was topped up in a block the follower has not replicated yet is briefly rejected for insufficient balance — and immediately after an upstream reorg even the nullifier check can misfire, rejecting a re-submitted transaction whose block was just pruned until the follower replicates the rollback. In all of these cases, retrying once the follower has caught up succeeds.
