...
# Handle both formats: full path relative to examples/ or just contract name
...
function get_contract_path {
  ...
  if [[ $input == *"/"* ]]; then
    ...
    contract_path=$(grep -oP "(?<=examples/)[^\"]+/$input" Nargo.toml)
...
function compile {
  ...
  contract_name=$(cat examples/$contract_path/src/main.nr | awk '/^contract / { print $2 } /^pub contract / { print $3 }')
...
function build {
  ...
  if [ "$#" -eq 0 ]; then
    rm -rf target
    mkdir -p $tmp_dir
    local contracts=$(grep -oP '(?<=examples/)[^"]+' Nargo.toml)
  else
    local contracts="$@"
  fi
  set +e
