...
# Check for changes in noir-projects/labs/aztec-nr (source for autogenerated aztec-nr docs)
if ! git diff --quiet "$COMPARE_REF" "$COMMIT_REF" -- ../noir-projects/labs/aztec-nr; then
  echo "Changes detected in noir-projects/labs/aztec-nr - proceeding with build"
  exit 1
fi
