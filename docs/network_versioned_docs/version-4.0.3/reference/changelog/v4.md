...
### Transaction collection config renamed

The environment variable for selecting the missing transactions collector implementation has been renamed:

**v3.0.0:**
```bash
--tx-collection-proposal-tx-collector-type <new|old>    ($TX_COLLECTION_PROPOSAL_TX_COLLECTOR_TYPE)
```

**v4.0.0:**
```bash
--tx-collection-missing-txs-collector-type <new|old>    ($TX_COLLECTION_MISSING_TXS_COLLECTOR_TYPE)
```

**Migration**: Update your configuration to use the new environment variable name. The default value (`new`) remains unchanged.
