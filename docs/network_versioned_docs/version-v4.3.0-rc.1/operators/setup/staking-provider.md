...
- Knowledge of running a sequencer node (see [Sequencer Setup Guide](./sequencer-setup.md))
...
- Aztec CLI v4.3.0-rc.1 or later installed:

```bash
VERSION=4.3.0-rc.1 bash -i <(curl -sL https://install.aztec.network/4.3.0-rc.1)
```

### Contract Addresses

For Staking Registry and GSE (Governance Staking Escrow) addresses, see the [Networks page](/networks#l1-contract-addresses).
...
Ensure your sequencer nodes are running and synced. See [Useful Commands](../sequencer-management/useful-commands.md) for commands to check sequencer status.
