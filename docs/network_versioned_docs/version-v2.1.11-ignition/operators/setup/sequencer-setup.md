...
For detailed instructions, advanced options, and complete examples, see the [Creating Sequencer Keystores guide](../keystore/creating-keystores.md).
...
This configuration includes only essential settings. The `--network mainnet` flag applies network-specific defaults—see the [CLI reference](../reference/cli-reference.md) for all available configuration options.
...
**→ [Register Your Sequencer (Self-Staking)](./registering-sequencer.md)**
...
**→ [Run as a Staking Provider](./staking-provider.md)**
...
- **[Register Your Sequencer](./registering-sequencer.md)** - Complete registration via staking dashboard
