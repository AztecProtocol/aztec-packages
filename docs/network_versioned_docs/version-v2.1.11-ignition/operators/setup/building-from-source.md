...
Check out the version tag you want to build. For example, to build version 2.1.11:
...
```bash
docker build -f release-image/Dockerfile --build-arg VERSION=2.1.11 -t aztec-local:2.1.11 .
```

:::tip
The tag `aztec-local:2.1.11` avoids conflicts with the official Docker Hub image and clearly indicates this is a locally-built version.
:::
...
aztec-local     2.1.11     abc123def456   2 minutes ago  2.5GB
```

### Verify Version

```bash
docker run --rm aztec-local:2.1.11 --version
```

Should display version 2.1.11.

### Test Basic Functionality

```bash
docker run --rm aztec-local:2.1.11 --help
```
...
- The version should match the git tag without the 'v' prefix (e.g., `2.1.11` not `v2.1.11`)
