...
Check out the version tag you want to build. For example, to build version 5.0.0-rc.2:

```bash
git checkout v5.0.0-rc.2
```
...
```bash
docker build -f release-image/Dockerfile --build-arg VERSION=5.0.0-rc.2 -t aztec-local:5.0.0-rc.2 .
```

:::tip
The tag `aztec-local:5.0.0-rc.2` avoids conflicts with the official Docker Hub image and clearly indicates this is a locally-built version.
:::
...
aztec-local     5.0.0-rc.2     abc123def456   2 minutes ago  2.5GB
```

### Verify Version

```bash
docker run --rm aztec-local:5.0.0-rc.2 --version
```

Should display version 5.0.0-rc.2.

### Test Basic Functionality

```bash
docker run --rm aztec-local:5.0.0-rc.2 --help
```
...
- The version should match the git tag without the 'v' prefix (e.g., `5.0.0-rc.2` not `v5.0.0-rc.2`)
