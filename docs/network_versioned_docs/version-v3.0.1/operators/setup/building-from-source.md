...
Check out the version tag you want to build. For example, to build version 3.0.1:
...
```bash
docker build -f release-image/Dockerfile --build-arg VERSION=3.0.1 -t aztec-local:3.0.1 .
```

:::tip
The tag `aztec-local:3.0.1` avoids conflicts with the official Docker Hub image and clearly indicates this is a locally-built version.
:::
...
aztec-local     3.0.1     abc123def456   2 minutes ago  2.5GB
```

### Verify Version

```bash
docker run --rm aztec-local:3.0.1 --version
```

Should display version 3.0.1.

### Test Basic Functionality

```bash
docker run --rm aztec-local:3.0.1 --help
```
...
- The version should match the git tag without the 'v' prefix (e.g., `3.0.1` not `v3.0.1`)
