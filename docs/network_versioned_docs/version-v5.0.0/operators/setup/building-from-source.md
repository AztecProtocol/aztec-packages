...
Check out the version tag you want to build. For example, to build version 5.0.0:
...
```bash
docker build -f release-image/Dockerfile --build-arg VERSION=5.0.0 -t aztec-local:5.0.0 .
```

:::tip
The tag `aztec-local:5.0.0` avoids conflicts with the official Docker Hub image and clearly indicates this is a locally-built version.
:::
...
aztec-local     5.0.0     abc123def456   2 minutes ago  2.5GB
```

### Verify Version

```bash
docker run --rm aztec-local:5.0.0 --version
```

Should display version 5.0.0.

### Test Basic Functionality

```bash
docker run --rm aztec-local:5.0.0 --help
```
...
- The version should match the git tag without the 'v' prefix (e.g., `5.0.0` not `v5.0.0`)
