...
This setup includes only essential settings. The `--network testnet` flag applies network-specific defaults—see the [CLI reference](../reference/cli-reference.md) for all available configuration options.
...
services:
  aztec-node:
    image: "aztecprotocol/aztec:5.0.0"
