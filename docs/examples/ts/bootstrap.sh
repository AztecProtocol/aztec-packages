...
validate_project() {
    ...
    (
        ...
        # Set package type to module for ESM support, and force typescript to
        # the monorepo's 5.x line across the whole dependency graph before the
        # first `yarn add`. The linked @aztec packages pull typescript
        # transitively; unpinned it now resolves to the 7.x native port, whose
        # package layout has no lib/_tsc.js, so yarn 4's builtin compat patch
        # fails to apply and the install aborts. A `resolutions` entry covers
        # transitive deps too, which the direct devDependency add below does not.
