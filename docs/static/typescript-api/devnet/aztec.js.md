...
```typescript
type DeployTxReceipt = TxReceipt & {
  contract: TContract;
...
```typescript
type PrivateEventFilter = EventFilterBase & {
  contractAddress: AztecAddress;
  scopes: AztecAddress[];
};
```
...
```typescript
type ProfileInteractionOptions = SimulateInteractionOptions & {
...
```typescript
type ProfileOptions = Omit<ProfileInteractionOptions, "fee"> & {
...
```typescript
type SendOptions = Omit<SendInteractionOptionsWithoutWait, "fee"> & {
  fee?: GasSettingsOption;
  wait?: W;
};
```
...
```typescript
type SimulateOptions = Omit<SimulateInteractionOptions, "fee"> & {
