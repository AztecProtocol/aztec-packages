...
```typescript
function createContractLogger(contractAddress: AztecAddress, getContractName: ContractNameResolver, options?: { instanceId?: string }) => Promise<Logger>
```
...
```
Displays debug logs collected during public function simulation, using the `contract_log::` prefixed logger format.
