...
- Saves files to `~/.aztec/v2.1.4/keystore/`
...
Wrote validator keystore to /Users/your-name/.aztec/v2.1.4/keystore/key1.json
Wrote staker output for 5 validator(s) to /Users/your-name/.aztec/v2.1.4/keystore/key1_staker_output.json
...
**Files created:**
- `~/.aztec/v2.1.4/keystore/key1.json` - Private keystore with all 5 validators
- `~/.aztec/v2.1.4/keystore/key1_staker_output.json` - Public keystore for staking dashboard
...
Open `~/.aztec/v2.1.4/keystore/key1.json` and add the `publisher` field to each validator object:
...
```bash
cast balance 0xE434A95e816991E66bF7052955FD699aEf8a286b --rpc-url https://ethereum-sepolia-rpc.publicnode.com
```
...
- Private keystore at `~/.aztec/v2.1.4/keystore/keyN.json`
- Public keystore at `~/.aztec/v2.1.4/keystore/keyN_staker_output.json`
...
- **Default directory**: `~/.aztec/v2.1.4/keystore/`
...
```bash
cat ~/.aztec/v2.1.4/keystore/key1.json | jq .
```

Check validator count:

```bash
jq '.validators | length' ~/.aztec/v2.1.4/keystore/key1.json
```

Verify BLS keys are present:

```bash
jq '.validators[0].attester.bls' ~/.aztec/v2.1.4/keystore/key1.json
```
...
jq -r '.validators[0].attester.eth' ~/.aztec/v2.1.4/keystore/key1.json
```
...
```bash
mkdir -p ~/.aztec/v2.1.4/keystore
chmod 755 ~/.aztec/v2.1.4/keystore
```
...
**Solutions:**
- Validate JSON syntax: `jq . ~/.aztec/v2.1.4/keystore/key1.json`
...
3. **Limit keystore access**
   ```bash
   chmod 600 ~/.aztec/v2.1.4/keystore/key1.json
   ```
...
| `--data-dir <path>` | Output directory | `~/.aztec/v2.1.4/keystore` |
