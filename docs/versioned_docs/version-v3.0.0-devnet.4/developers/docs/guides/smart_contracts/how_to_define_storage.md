...
```rust
assert(storage.authorized_users.at(context.msg_sender().unwrap()).read(), "caller is not authorized");
```
...
Like for public state, we define the struct to have context and a storage slot. You can view the implementation [here](https://github.com/AztecProtocol/aztec-packages/blob/v3.0.0-devnet.4/noir-projects/aztec-nr/aztec/src/state_vars/private_mutable.nr).
...
There's also a limit on the maximum number of notes that can be returned in one go. To find the current limit, refer to [this file (GitHub link)](https://github.com/AztecProtocol/aztec-packages/blob/v3.0.0-devnet.4/noir-projects/aztec-nr/aztec/src/note/constants.nr) and look for `MAX_NOTES_PER_PAGE`.
...
:::info
An example using a larger struct can be found in the [lending example](https://github.com/AztecProtocol/aztec-packages/tree/v3.0.0-devnet.4/noir-projects/noir-contracts/contracts/app/lending_contract)'s use of an [`Asset`](https://github.com/AztecProtocol/aztec-packages/tree/v3.0.0-devnet.4/noir-projects/noir-contracts/contracts/app/lending_contract/src/asset.nr).
:::
...
You can find the details of `PublicImmutable` in the implementation [here (GitHub link)](https://github.com/AztecProtocol/aztec-packages/blob/v3.0.0-devnet.4/noir-projects/aztec-nr/aztec/src/state_vars/public_immutable.nr).
