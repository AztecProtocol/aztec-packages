...
- Aztec toolchain installed (`aztec-up -v 3.0.0-nightly.20250924`)
...
```bash
yarn init
yarn add @aztec/aztec.js@v3.0.0-nightly.20250924 @aztec/accounts@v3.0.0-nightly.20250924
...
[dependencies]
aztec = { git = "https://github.com/AztecProtocol/aztec-packages/", tag = "v3.0.0-nightly.20250924", directory = "noir-projects/aztec-nr/aztec" }
```
...
For something like balances, you can use a simple library called `easy_private_state` which abstracts away a custom private Note. A Note is at the core of how private state works in Aztec and you can read about it [here](../../../aztec/concepts/storage/notes.md). For now, let's just import the library in `Nargo.toml`:

```toml
[dependencies]
easy_private_state = { git = "https://github.com/AztecProtocol/aztec-packages/", tag = "v3.0.0-nightly.20250924", directory = "noir-projects/aztec-nr/easy-private-state" }
```
...
- Explore [cross-chain communication](../../../aztec/concepts/communication/cross_chain_calls.md) to integrate with existing health systems
- Learn about [account abstraction](../../../aztec/concepts/accounts/index.md) for recovery mechanisms
