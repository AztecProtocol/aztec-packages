...
The sequencer proposes blocks and submits them to L1 along with attestations.
...
Validators read L1 state to validate blocks and participate in consensus.
...
**High frequency** (multiple calls per second):
- `eth_blockNumber` - Archiver polls for new blocks
- `eth_getTransactionReceipt` - Transaction monitoring

**Medium frequency** (multiple calls per minute):
- `eth_getLogs` - Event retrieval during sync
- `eth_call` - State reads for validation

**Low frequency** (occasional):
- `eth_sendRawTransaction` - Block proposals, proofs, slashing
- `eth_estimateGas` - Pre-transaction estimation
- `eth_getBalance` - Account monitoring

### Optimization strategies

1. **Batch calls**: Use multicall for reading multiple values
2. **Cache results**: Cache block numbers and timestamps when appropriate
3. **Parallel requests**: Use fallback transports for redundancy
4. **Polling intervals**: Configure polling intervals based on network conditions
5. **Block tags**: Use `latest`, `pending`, or specific block numbers strategically

## Infrastructure requirements

### For Archiver nodes

**Critical RPC methods**:
- `eth_blockNumber` (high reliability required)
- `eth_getLogs` (must support large block ranges)
- `eth_getBlockByNumber` (must return timestamps)

**Recommendations**:
- Use archive node or node with sufficient history
- Configure multiple RPC endpoints for fallback
- Ensure `eth_getLogs` supports your sync range

### For Sequencer/Prover nodes

**Critical RPC methods**:
- `eth_sendRawTransaction` (low latency required)
- `eth_getTransactionReceipt` (reliable delivery)
- `eth_estimateGas` (accurate estimates)
- `eth_getTransactionCount` (consistent nonces)

**Recommendations**:
- Use low-latency connection to RPC node
- Configure transaction timeout appropriately
- Monitor publisher account balances
- Use multiple publishers to distribute load

### For Validator nodes

**Critical RPC methods**:
- `eth_call` (accurate state reads)
- `eth_getBlockByNumber` (correct timestamps)
- `eth_getStorageAt` (direct storage access)

**Recommendations**:
- Use full node (archive not required)
- Ensure low latency for timely attestations
- Configure caching for frequently read state
...
- Learn about [Advanced Sequencer Setup](./advanced_sequencer_setup.md) for production configurations
- Check [Useful Commands](./reference/useful_commands.md) for monitoring tools
