...
This guide uses the **Docker Compose method**. This is the recommended approach for sequencer nodes.
:::
...
```bash
cast call [ROLLUP_ADDRESS] "getInfo(address)" [SEQUENCER_ADDRESS] --rpc-url [YOUR_RPC_URL]
```
...
```bash
cast call [ROLLUP_ADDRESS] "getAttesters()" --rpc-url [YOUR_RPC_URL]
```
