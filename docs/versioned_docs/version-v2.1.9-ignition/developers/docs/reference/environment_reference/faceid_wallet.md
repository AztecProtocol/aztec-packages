...
In this tutorial, we will use Apple Mac's Secure Enclave to store the private key, and use it in Aztec's [CLI Wallet](./cli_wallet_reference_autogen.md). This enables fully private, native, and seedless account abstraction!
...
Check [the reference](./cli_wallet_reference_autogen.md) for the whole set of commands, but these mean:
...
In this tutorial, we created an account with the Aztec's [CLI Wallet](./cli_wallet_reference_autogen.md), using the Apple Mac's Secure Enclave to store the private key.

You can use a multitude of authentication methods, for example with RSA you could use a passport as a recovery, or even as a signer in a multisig. All of this is based on the [account contract](https://github.com/AztecProtocol/aztec-packages/tree/v2.1.9/noir-projects/noir-contracts/contracts/account).
