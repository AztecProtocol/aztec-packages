...
- The `next` version docs should **not** be committed to the repo
- Versioned docs (e.g., `v1.0.0`) are automatically generated by CI when releases are cut

The generated docs are stored in `static/aztec-nr-api/<version>/` and linked from the [Aztec.nr API Reference](./docs/developers/docs/aztec-nr/api.mdx) page.

### Protocol Circuits API Documentation

The Protocol Circuits API reference is auto-generated from the `noir-projects/noir-protocol-circuits/` source code using `nargo doc`. This generates HTML documentation for the protocol's constraint system circuits, which is served from `static/protocol-circuits-api/`.

**Prerequisites:** You need `nargo` installed.
...
- When `noir-protocol-circuits` source code changes (new circuits, updated comments, etc.)
- The `next` version docs should **not** be committed to the repo
- Versioned docs (e.g., `v1.0.0`) are automatically generated by CI when releases are cut

The generated docs are stored in `static/protocol-circuits-api/<version>/` and provide API documentation for the protocol's private and public kernel circuits, rollup circuits, and other core protocol components. You can view the API reference [here](./docs/developers/docs/foundational-topics/advanced/circuits/protocol-circuits-api.mdx).
