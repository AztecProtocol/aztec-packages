...
function parseParameters(paramString) {
  ...
  return paramString.split(",").map((param) => {
    param = param
      .trim()
      .replace(/[\[:;,.]$/g, "")
      .replace(/^[\[:;,.]/g, ""); // Clean up start and end
...
function parseStruct(content) {
  ...
  while ((match = structRegex.exec(content)) !== null) {
    ...
    while (lineIndex >= 0) {
      ...
      if (
        line.startsWith("//") &&
        !line.includes("docs:start:") &&
        !line.includes("docs:end:")
      ) {
...
function parseFunctions(content) {
  ...
  while ((implMatch = implRegex.exec(content)) !== null) {
    ...
    while ((methodMatch = methodRegex.exec(implBlockContent)) !== null) {
      ...
      while (commentIndex >= 0) {
        const commentMatch = implBlockContent
          .substring(0, commentIndex)
          .match(/\/\/\s*(.*)\n\s*$/);
        if (
          commentMatch &&
          !commentMatch[1].includes("docs:start:") &&
          !commentMatch[1].includes("docs:end:")
        ) {
          description =
            commentMatch[1] + (description ? " " + description : "");
  ...
  while (
    (standaloneFunctionMatch = standaloneFunctionRegex.exec(content)) !== null
  ) {
    ...
    if (!functions.some((f) => f.name === name && f.isMethod)) {
      ...
      const descriptionMatch = content
        .substring(0, standaloneFunctionMatch.index)
        .match(/\/\/\s*(.*)\n\s*$/);
      if (descriptionMatch) {
        const precedingText = content.substring(0, descriptionMatch.index);
        if (
          !precedingText.includes("docs:start:") &&
    ...
    if (structInfo) {
      ...
      if (structInfo.fields.length > 0) {
        ...
        structInfo.fields.forEach((field) => {
          const cleanType = escapeHtml(
            field.type.replace(/[\[:;,]$/g, "").replace(/^[\[:;,]/g, "")
          );
      ...
      const methods = functions.filter(
        (f) => f.isMethod && f.structName === escapeHtml(structInfo.structName)
      );
      if (methods.length > 0) {
        markdown += `## Methods\n\n`;
...
let outputBaseDir = path.resolve(
  __dirname,
  "../../docs/build/reference/smart_contract_reference/aztec-nr"
);
