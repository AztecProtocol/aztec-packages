...
- Version config files are object-based JSON mapping release type to version string (e.g., `{"mainnet": "v4.2.0", "testnet": "v4.1.0", ...}`)
- Docusaurus-compatible array files (`developer_versions.json`, `network_versions.json`) are auto-generated from the config files
