...
1. Create a directory under `examples/ts/` with `index.ts`, `config.yaml`, `package.json`, `.yarnrc.yml`, and a `yarn.lock` generated by `yarn install`. See `examples/ts/README.md` for the package.json shape (relative `link:` paths for `@aztec/*` deps; pinned `tsx` + `typescript` shared across examples)
...
4. If a third-party dep range changes, regenerate every committed lockfile via `examples/bootstrap.sh refresh-ts-lockfiles`
