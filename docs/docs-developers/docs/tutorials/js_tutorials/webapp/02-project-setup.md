...
├── test-extension/      # Tutorial wallet extension (for devnet)
...
This tells the app where to find the Aztec node. For local development, this points to the default local network port. For devnet, the wallet extension manages the node connection.
