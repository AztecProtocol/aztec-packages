...
The tutorial includes a fully functional wallet extension in `test-extension/` that you can use on devnet. Unlike a mock extension, this wallet can:
...
Lets the user choose between "Local" (local network) and "Devnet". Calls `onNetworkChange` with the selected `NetworkType`, which determines which wallet mode the app uses.
