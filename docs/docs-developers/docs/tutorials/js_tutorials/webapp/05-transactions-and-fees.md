...
`SPONSORED_FPC_SALT` is a fixed constant so that the SponsoredFPC contract is deployed at a deterministic, well-known address across all networks (local network, devnet, and beyond). Your app can always compute where it lives without querying a registry.
