...
Before claiming, we need to mine a couple of L2 blocks. An L1-to-L2 message is not available the moment it is sent -- it becomes consumable only once an L2 block includes it. The local sandbox includes it as soon as it sees it; on a live network the proposer includes it in the next L2 block it builds after observing the L1 block carrying it. We use a helper that deploys throwaway contracts to force these blocks:
