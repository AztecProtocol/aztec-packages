...
Before claiming, we need to mine a couple of L2 blocks. An L1-to-L2 message is not available the moment it is sent -- it becomes consumable only once an L2 block includes it. The local sandbox includes it as soon as it sees it, and so does a live-network proposer by default; an operator can configure one to wait for the L1 block carrying it to gain a child first (`SEQ_INBOX_L1_CONFIRMATIONS=1`), which adds around 12 to 14 seconds. We use a helper that deploys throwaway contracts to force these blocks:
