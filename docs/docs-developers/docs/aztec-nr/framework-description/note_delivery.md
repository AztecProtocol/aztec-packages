...
When your wallet submits a transaction, it tells PXE which address to use as the sender for tags (typically the originating account). This sender address is then used along with the recipient address to compute a shared secret (via [Diffie-Hellman key exchange](https://www.geeksforgeeks.org/computer-networks/diffie-hellman-key-exchange-and-perfect-forward-secrecy/)), which generates the tag that allows recipients to efficiently find their notes. Contracts can override the sender at the log emission site by passing a `MessageDelivery` variant with a custom `SenderForTags`, e.g. `MessageDelivery::onchain_unconstrained_with_sender(SenderForTags::explicit(address))`.
