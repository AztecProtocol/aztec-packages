...
When your wallet submits a transaction, it tells the PXE which address to use as the sender for tags (typically the originating account). This sender address is then used along with the recipient address to compute a shared secret (via [Diffie-Hellman key exchange](https://www.geeksforgeeks.org/computer-networks/diffie-hellman-key-exchange-and-perfect-forward-secrecy/)), which generates the tag that allows recipients to efficiently find their notes. Contracts can call `set_sender_for_tags(addr)` to override this for their own call, but the override does not propagate to nested calls, siblings, or parents.
