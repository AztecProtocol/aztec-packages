...
- `getCheckpoint('proposed')` strictly targets the proposed-checkpoint store; returns `undefined` if no proposed entry exists (the proposed chain has not advanced past the confirmed tip).
...
**Throws on `'proposed'` + L1/attestations.** Proposed checkpoints have no L1 publish info or committee attestations (those data points only exist after L1 confirmation). The RPC now rejects requests that combine the two with `BadRequestError`:

```ts
// All three throw BadRequestError:
