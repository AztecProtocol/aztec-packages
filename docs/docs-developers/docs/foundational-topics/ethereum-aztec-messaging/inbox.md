...
A message is therefore eligible for the next block the proposer builds after its node observes the L1 block carrying it, and normally enters it, rather than waiting one to two checkpoints; only a message beyond the bucket end a checkpoint completes at waits for the following checkpoint.
...
- Node-side reorg recovery inherits a shortcut that trusts a locally stored message as final once its recorded L1 height is at or below the finalized block the node last recorded. A same-content reorg that re-mines a message higher is not noticed by the node when the Inbox position at the head still matches (only a replay refreshes recorded heights), so the message keeps its old height and the shortcut can later trust an unfinalized replacement. This is a known, deliberately deferred limitation of the node's recovery, not of the protocol.
