...
importance: 1
references: ["yarn-project/pxe/src/*", "yarn-project/key-store/src/*", "noir-projects/labs/aztec-nr/aztec/src/oracle/version.nr"]
---
...
The canonical version constants live in the PXE (`ORACLE_VERSION_MAJOR` / `ORACLE_VERSION_MINOR` in `yarn-project/pxe/src/oracle_version.ts`) and in Aztec.nr (`noir-projects/labs/aztec-nr/aztec/src/oracle/version.nr`). The two are kept in lockstep as part of each release.
