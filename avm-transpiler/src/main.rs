...
fn main() {
    ...
    let contract_json = fs::read_to_string(Path::new(in_contract_artifact_path)).expect("Unable to read file");
    let raw_json_obj: serde_json::Value = serde_json::from_str(&contract_json).expect("Unable to parse json");
    ...
    let contract: CompiledAcirContract = serde_json::from_str(&contract_json).expect("Unable to parse json");
    ...
    let transpiled_json = serde_json::to_string(&transpiled_contract).expect("Unable to serialize json");
